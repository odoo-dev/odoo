...
test("See records with custom pivot groups", async function () {
    ...
    await doMenuAction(cellMenuRegistry, ["pivot_see_records"], env);
    expect.verifySteps(["doAction"]);
    expect(doActionReceivedDomain).toEqual([["product_id", "in", [37, 41]]]);
    ...
    selectCell(model, "B3"); // Pivot value for "Group1" group and bar = false
    await doMenuAction(cellMenuRegistry, ["pivot_see_records"], env);
    expect.verifySteps(["doAction"]);
    ...
    await doMenuAction(cellMenuRegistry, ["pivot_see_records"], env);
    expect.verifySteps(["doAction"]);
    expect(doActionReceivedDomain).toEqual([["product_id", "not in", [37, 41]]]);
