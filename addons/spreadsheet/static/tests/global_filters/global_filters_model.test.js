...
test("SET_GLOBAL_FILTER_VALUE dispatched N times -> triggers N data source reloads", async function () {
    const { model } = await createSpreadsheetWithList({
...
test("SET_MANY_GLOBAL_FILTER_VALUE dispatched once -> triggers only 1 data source reload", async function () {
    const { model } = await createSpreadsheetWithList({
        mockRPC: function (_, { model: m, method }) {
            if (m === "partner" && method === "web_search_read") {
                expect.step("web_search_read");
