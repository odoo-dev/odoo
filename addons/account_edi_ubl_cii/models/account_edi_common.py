...
class FloatFmt(float):
    ...
    def __init__(self, value, min_dp=None, max_dp=None):
        if min_dp is None and max_dp is None:
            min_dp = 2
            max_dp = 2
        elif min_dp is None and max_dp is not None:
            min_dp = max_dp
        elif min_dp is not None and max_dp is None:
            ...
            while True:
                if max_dp == min_dp:
                    break
                max_dp_min_1 = max_dp - 1
                if math.isclose(round(value, max_dp), round(value, max_dp_min_1), abs_tol=1e-12):
                    max_dp = max_dp_min_1
                else:
                    break
