...
class AccountEdiCommon(models.AbstractModel):
        ...
        for allow_el in tree.iterfind(xpaths['root']):
            ...
            if reason_code == 'AEO':
                ...
                if tax:
                    ...
                    name = reason
                    tag = self._get_or_create_allowance_charge_tax_tag(record.company_id, reason, amount)
                    tax_tag_ids = [Command.set([tag.id])]
            else:
            ...
            line_vals.append([name, quantity, price_unit, tax_ids, tax_tag_ids])
        ...
        for charge in line_values.pop('charges'):
            ...
            if charge['reason_code'] == 'AEO':
                ...
                if tax:
                    ...
                    tag = self._get_or_create_allowance_charge_tax_tag(record.company_id, reason, amount)
                    if 'tax_tag_ids' not in line_values:
                        line_values['tax_tag_ids'] = [Command.set([tag.id])]
                    elif isinstance(line_values['tax_tag_ids'], list) and line_values['tax_tag_ids']:
                        line_values['tax_tag_ids'][0][2].append(tag.id)
    ...
    def _import_invoice_write_collected_values(self, collected_values):
        ...
        for base_line in base_lines:
            ...
            if base_line.get('tax_tag_ids'):
                create_values['tax_tag_ids'] = [Command.set(base_line['tax_tag_ids'].ids)]
            elif base_line['_create_values'].get('tax_tag_ids'):
                create_values['tax_tag_ids'] = base_line['_create_values']['tax_tag_ids']
