...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_lines(self, record, tree, xpath, document_type=False, tax_type=False, qty_factor=1):
        ...
        for line_tree in tree.iterfind(xpath):
            line_values = self.with_company(record.company_id).with_context(prefetched_products=all_products)._retrieve_invoice_line_vals(line_tree, document_type, qty_factor)
            line_values['tax_ids'], tax_logs = self.with_context(stored_taxes=all_taxes)._retrieve_taxes(record, line_values, tax_type)
