...
class AccountEdiUBL(models.AbstractModel):
    ...
    def _import_ubl_create_missing_customer(self, collected_values):
        ...
        if vat and customer._run_vat_test(vat, country, customer.is_company):
            customer.vat = vat
    ...
    def _import_ubl_invoice_line_add_price_unit_quantity_discount(self, collected_values):
        ...
        if (
            ...
            price_unit = subtotal
            quantity = 1.0
            discount_amount = subtotal - line_extension_amount + total_charges
    ...
    def _import_ubl_invoice_line_get_product_base_line_kwargs(self, collected_values):
        ...
        if name := collected_values.get('name'):
            base_line_kwargs['_to_write']['name'] = name
        if deferred_start_date := to_write.get('deferred_start_date'):
            base_line_kwargs['_to_write']['deferred_start_date'] = deferred_start_date
        if deferred_end_date := to_write.get('deferred_end_date'):
            base_line_kwargs['_to_write']['deferred_end_date'] = deferred_end_date
        return base_line_kwargs
    ...
    def _import_ubl_invoice_get_rounding_base_line_kwargs(self, collected_values):
        ...
        base_line_kwargs['_to_write']['name'] = _("Rounding")
        return base_line_kwargs
    ...
    def _import_ubl_invoice_get_allowance_charge_line_kwargs(self, collected_values):
        ...
        base_line_kwargs['_to_write']['name'] = reason
        return base_line_kwargs
        ...
        for line_collected_values in lines_collected_values:
            ...
            if uom_code:
                ...
                if matched_uom_xmlid:
                    if uom := self.env.ref(matched_uom_xmlid, raise_if_not_found=False):
    ...
    def _import_ubl_invoice_retrieve_accounts(self, collected_values):
        ...
        for line_collected_values in lines_collected_values:
            ...
            if not (predictive := account_values.get('invoice_predictive')):
                account = account_values['account'] = self.env['account.move.line']._predict_specific_account(
                    move=predictive['invoice'],
                    name=predictive['name'],
                    partner=predictive['partner'],
                )
                if account:
                    to_write['account_id'] = account.id
