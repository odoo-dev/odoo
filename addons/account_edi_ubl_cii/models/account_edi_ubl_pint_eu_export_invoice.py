...
class AccountEdiUblPintEuInvoice(models.AbstractModel):
    ...
    def _ubl_get_ubl_payment_means_node(self, vals):
        ...
        node['cac:PayeeFinancialAccount'] = self._ubl_get_ubl_payment_means_payee_financial_account_node(vals)
        return node

    def _get_ubl_legal_monetary_total_node(self, vals):
        # EXTENDS
        node = super()._get_ubl_legal_monetary_total_node(vals)
        invoice = vals['invoice']
        currency = vals['currency']
        node['cbc:PrepaidAmount']['_text'] = FloatFmt(invoice.amount_total - invoice.amount_residual, min_dp=currency.decimal_places)
        node['cbc:PayableAmount']['_text'] = FloatFmt(invoice.amount_residual, min_dp=currency.decimal_places)
        return node
