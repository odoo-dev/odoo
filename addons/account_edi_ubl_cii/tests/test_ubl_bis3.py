...
class TestUblBis3(AccountTestInvoicingCommon):
    ...
    def _assert_invoice_ubl_file(self, invoice, filename):
        ...
        # # Raw export.
        # raw_file_path_parent = f'addons/{self.test_module}/tests/raw_test_files'
        # raw_file_path = f'{raw_file_path_parent}/{filename}.xml'
        #
        # if not os.path.exists(raw_file_path_parent):
        #     os.makedirs(raw_file_path_parent)
        # tree = self.get_xml_tree_from_string(invoice.ubl_cii_xml_id.raw)
        # etree.indent(tree, space="\t", level=0)
        # ubl_content = etree.tostring(tree, pretty_print=True, xml_declaration=True, encoding='UTF-8')
        # with open(raw_file_path, 'wb+') as fd:
        #     fd.write(ubl_content)
        #
        # # Export with ___ignore___
        # if not os.path.exists(file_path):
        #     for node in tree.xpath("//*[local-name()='EmbeddedDocumentBinaryObject']"):
    ...
    def test_single_fixed_tax_price_excluded(self):
        ...
        invoice.action_post()
        self.env['account.move.send']._generate_and_send_invoices(invoice, sending_methods=['manual'])
        self._assert_invoice_ubl_file(invoice, 'test_single_fixed_tax_price_excluded')
    ...
    def test_multiple_fixed_taxes_price_excluded(self):
        ...
        invoice.action_post()
        self.env['account.move.send']._generate_and_send_invoices(invoice, sending_methods=['manual'])
        self._assert_invoice_ubl_file(invoice, 'test_multiple_fixed_taxes_price_excluded')
    ...
    def test_single_fixed_tax_price_excluded_and_discount(self):
        ...
        invoice.action_post()
        self.env['account.move.send']._generate_and_send_invoices(invoice, sending_methods=['manual'])
        self._assert_invoice_ubl_file(invoice, 'test_single_fixed_tax_price_excluded_and_discount')
