...
class TestAccountUblCii(AccountTestInvoicingCommon):
    ...
    def test_partner_ubl_cii_formats(self):
        def _get_ubl_cii_formats_info(self):
            return {
                'ubl_all_countries': {'on_peppol': True},
        ...
        with patch(
            ...
            self.assertEqual(Partner._get_ubl_cii_formats(), ['ubl_all_countries', 'peppol', 'cii'])
            ...
            self.assertEqual(partner_nz._get_suggested_invoice_edi_format(), 'peppol')
            self.assertEqual(partner_be._get_suggested_invoice_edi_format(), 'ubl_bis3')
            self.assertEqual(partner_au._get_suggested_invoice_edi_format(), 'cii')
            self.assertEqual(Partner._get_peppol_formats(), ['ubl_all_countries', 'peppol'])
