    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', '/') == '/':
                ...
                if vals.get('is_wave') and vals.get('picking_type_id'):
                    picking_type = self.env['stock.picking.type'].browse(vals.get('picking_type_id'))
                    sequence_prefix, sequence_number = (self.env['ir.sequence'].with_company(company_id).next_by_code('picking.wave') or '/').split('/')
