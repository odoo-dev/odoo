...
class TestPurchaseMrpFlow(AccountTestInvoicingCommon):
    ...
    def test_valuation_with_backorder(self):
        """Partially receiving a weight-based kit purchase order spreads the kit
        value over its components by cost share, and a component whose share of
        the received quantity is negligible in its own unit of measure is valued
        at zero."""
        ...
        # none). Only a fifth of each component's demand is received here, so
        # each valued component gets a fifth of its share (300); Cmp3's received
        # quantity rounds to zero in kilograms, so it is valued at zero.
