...
class AccountMove(models.Model):
    ...
    def _l10n_tr_nilvera_get_submitted_document_status(self, client):
        for invoice in self:
            response = client.request(
                "GET",
    ...
    def _l10n_tr_nilvera_einvoice_get_error_messages_from_response(self, response):
        ...
        if errors := response_json.get('Errors'):
            ...
            for error in errors:
                msg += "%s - %s: %s <br/>" % (error.get('Code'), error.get('Description'), error.get('Detail'))
