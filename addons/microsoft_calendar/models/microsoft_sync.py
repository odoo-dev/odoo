...
class MicrosoftCalendarSync(models.AbstractModel):
            ...
            for record in self:
                if record.need_sync_m and record.microsoft_id:
                    ...
                    if not vals.get('active', True) or (calendar_id and calendar_id != record._get_event_user_m().primary_calendar_id.id):
                        ...
                        if record.active:
                            # If the record was deleted from outlook because of a calendar change, we need to reset the sync fields
                            record.write({
                                'microsoft_id': False,
    ...
    def _microsoft_to_odoo_values(
        self, microsoft_event: MicrosoftEvent, default_reminders=(), default_values=None, with_ids=False, with_calendar_id=False
    ):
        """
