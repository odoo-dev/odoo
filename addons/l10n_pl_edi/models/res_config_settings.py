    ...
    def _set_l10n_pl_edi_certificate(self):
        for config in self:
            if config.l10n_pl_edi_certificate:
                ...
                config.l10n_pl_edi_register = False
    ...
    def _l10n_pl_edi_ksef_authenticate(self):
        ...
        if not vat or not vat.startswith('PL'):
            raise ValidationError(self.env._(
                "A polish VAT number must be set on company %(name).",
        ...
        mode = self.env['ir.config_parameter'].sudo().get_param('l10n_pl_edi_ksef.mode', 'test')  # 'prod'
        ksef_service = KsefApiService(
            self.company_id,
            mode,
