...
class AccountMove(models.Model):
    ...
    def _get_ksef_invoice_type(self):
        ...
        def is_downpayment_line(line):
            ...
            if hasattr(line, 'sale_line_ids'):
                return any(getattr(sl, 'is_downpayment', False) for sl in line.sale_line_ids)
            return False
        ...
        if self.move_type == 'out_invoice':
            has_dp_lines = any(is_downpayment_line(line) for line in self.invoice_line_ids)

            if has_dp_lines:
                # Check for Settlement (ROZ): Contains Regular lines AND Negative Downpayment lines
                has_regular_lines = any(not is_downpayment_line(line) for line in self.invoice_line_ids)
                has_deducted_dp = any(is_downpayment_line(line) and line.price_subtotal < 0 for line in self.invoice_line_ids)
    ...
    def _l10n_pl_ksef_get_xml_values(self):
        ...
        def get_amounts_from_tag(tax_tag_string):
            if 'OSS' in tax_tag_string:
                oss_tag = self.env['account.account.tag'].search([('name', '=', 'OSS'), ('applicability', '=', 'taxes')])
                return - sum(
    ...
    def _l10n_pl_ksef_render_xml(self):
        ...
        try:
            ...
            _logger.error("KSeF XML schema validation failed for invoice %s: %s", self.name, str(e))
            raise
    ...
    def _get_ksef_status_mapping(self):
        ...
        return {
            ...
            100: ('sent', _("KSeF Status: Invoice accepted for further processing (Code: 100).")),
            405: ('rejected', _("KSeF Status: Rejected (Code: 405). Processing canceled.")),
            410: ('rejected', _("KSeF Status: Rejected (Code: 410). Incorrect scope of permissions.")),
            415: ('rejected', _("KSeF Status: Rejected (Code: 415). It is not possible to send an invoice with an attachment.")),
            430: ('rejected', _("KSeF Status: Rejected (Code: 430). Invoice file verification error.")),
            435: ('rejected', _("KSeF Status: Rejected (Code: 435). File decryption error.")),
            440: ('rejected', _("KSeF Status: Rejected (Code: 440). Duplicate invoice.")),
            450: ('rejected', _("KSeF Status: Rejected (Code: 450). Invoice document semantics verification error.")),
    ...
    def action_update_invoice_status(self):
        ...
        if not self.l10n_pl_move_reference_number:
            raise UserError(_("This invoice does not have a KSeF Invoice Reference Number or Session ID. It may not have been sent yet."))

        company = self.company_id
        mode = self.env['ir.config_parameter'].sudo().get_param('l10n_pl_edi_ksef.mode') or 'prod'
        service = KsefApiService(
            company, mode, company.l10n_pl_ksef_session_id,
            company.l10n_pl_ksef_session_key, company.l10n_pl_ksef_session_iv
        )

        try:
            response = service.get_invoice_status(self.l10n_pl_move_reference_number, company.l10n_pl_ksef_session_id)
        ...
        status_description = status_info.get('description', _("No description provided."))
        ...
        if status_code in mapping:
            ...
            message = _("Received an unknown or unexpected status from KSeF (Code: %(code)s): %(description)s",
                        code=status_code,
    ...
    def action_get_invoice_UPO(self):
        ...
        if self.ksef_status != 'accepted':
            raise UserError(_("You can only download a UPO for an 'Accepted' invoice. Please update the status first."))

        mode = self.env['ir.config_parameter'].sudo().get_param('l10n_pl_edi_ksef.mode') or 'prod'
        service = KsefApiService(self.company_id, mode, self.company_id.l10n_pl_ksef_session_id, self.company_id.l10n_pl_ksef_session_key, self.company_id.l10n_pl_ksef_session_iv)

        try:
            upo_xml_content = service.get_invoice_upo(self.l10n_pl_move_reference_number, self.company_id.l10n_pl_ksef_session_id)
        except UserError as e:
        ...
        self.message_post(
            body=_("The KSeF Official Confirmation of Receipt (UPO) has been downloaded and attached."),
