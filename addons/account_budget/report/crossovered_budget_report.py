...
class BudgetReport(models.AbstractModel):
    ...
    def funct(self, object, form):
        ...
        result = []
        for budget in object:
            res = {}
            budget_ids = budget.crossovered_budget_line
            ...
            for analytic_account in analytic_accounts:
                ...
                result.append(res)
                lines = self.env['crossovered.budget.lines'].search([('id', 'in', budget_ids.ids), ('analytic_account_id', '=', analytic_account.id), ('date_to', '>=', form['date_from']), ('date_from', '<=', form['date_to'])])
    ...
    def render_html(self, data=None):
        ...
        followup_report = report._get_report_from_name('account_budget.report_crossoveredbudget')
        selected_records = self.env['crossovered.budget'].browse(data['ids'])
        docargs = {
            'doc_ids': self.ids,
        ...
        return report.render('account_budget.report_crossoveredbudget', docargs)
