...
class TestAnalyticToSaleToInvoice(SaleCommon):
    ...
    def setUpClass(cls):
        ...
        cls.services_sale_order.order_line[0].qty_delivered = 1
        cls.services_sale_order.order_line[1].qty_delivered = 1

        cls.services_sale_order._create_invoices()
    ...
    def test_upsale_lines_created_based_on_reinvoice_policy(self):
        ...
        self.assertEqual(
            len(
                self.services_sale_order.order_line.filtered(
                    lambda aal: aal.product_id == self.reinvoice_at_cost_product,
                ),
            ),
            2,
    ...
    def test_quantity_update_on_analytic_line_updates_upsale_lines(self):
        ...
        - Increasing quantity on an at-cost analytic line updates delivered qty.
        - Setting quantity to zero on a sales-price analytic line does not erase
            manually delivered quantities.
        """
        self.at_cost_aal.with_context(from_services_and_material=True).unit_amount = 3
        self.at_sale_price_aal.with_context(from_services_and_material=True).unit_amount = 0
    ...
    def test_analytic_lines_unlink_unsyncs_upsale_lines(self):
        ...
        extra_at_cost_aal_2 = self.env['account.analytic.line'].with_context(from_services_and_material=True).create({
            'name': 'Extra work At cost 2',
            'product_id': self.reinvoice_at_cost_product.id,
            'unit_amount': 2,
            'order_id': self.services_sale_order.id,
        })
        ...
        extra_at_cost_aal_2.so_line.qty_delivered = 3
        new_at_cost_upsale_order_line = extra_at_cost_aal.so_line
        new_at_cost_upsale_order_line_2 = extra_at_cost_aal_2.so_line
        at_cost_upsale_order_line = self.at_cost_aal.so_line

        extra_at_sale_price_aal.unlink()
        self.at_cost_aal.unlink()
        extra_at_cost_aal.unlink()
        extra_at_cost_aal_2.unlink()
        ...
        self.assertTrue(
            new_at_cost_upsale_order_line_2.exists(),
            "Upsale line with non zero ordered quantity should not be deleted for at cost analytic line.",
        )

        self.assertEqual(
            new_at_cost_upsale_order_line_2.qty_delivered,
            1,
            "Delivered quantity of upsale line with different delivered quantity that analytic line, should be decreased for at cost analytic line.",
        )
