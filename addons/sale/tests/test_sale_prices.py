...
class TestSalePrices(SaleCommon):
    ...
    def test_pricelist_minimal_qty(self):
        ...
        pricelist_rule = self._create_discount_pricelist_rule(min_quantity=4.0)
    ...
    def test_negative_discounts(self):
        """Test surcharges (or negative discounts)."""
        self.discount = -10
    ...
    def test_manual_price_prevents_recompute(self):
        ...
        self.assertEqual(so_line.price_unit_last_computed_vals['price_unit'], 0.0)
    ...
    def test_sale_tax_mapping(self):
        ...
        self.assertEqual(
            order.order_line.tax_ids.id,
            tax_b.id,
            "Wrong tax applied for specified product & pricelist",
        )

    def test_fiscalposition_application(self):
        """Test application of a fiscal position mapping price included to price included tax."""
        ...
        (product_tmpl_a, product_tmpl_b, product_tmpl_c, product_tmpl_d) = self.env[
            "product.template"
        ].create([
    ...
    def test_discount_and_untaxed_subtotal(self):
        """When adding a discount on a SO line, this test ensures that the untaxed amount to invoice
        is equal to the untaxed subtotal."""
    ...
    def test_show_discount(self):
        ...
        sale_order = self.env["sale.order"].create({
            "partner_id": self.partner.id,
            "order_line": [
                Command.create({"product_id": test_product_discount.id, "product_uom_qty": 1.0}),
                Command.create({"product_id": test_product_incl_discount.id, "product_uom_qty": 1}),
            ],
        })
    ...
    def test_combo_product_zero_base_price_distributes_evenly(self):
        ...
        combos = self.env["product.combo"].create([
            {"name": "G1", "combo_item_ids": [Command.create({"product_id": product_a.id})]},
    ...
    def test_so_included_tax_mapping(self):
        ...
        self.product.write({"lst_price": 121, "taxes_id": [Command.set(tax_a.ids)]})
