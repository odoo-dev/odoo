...
class TestTaxesDownPaymentSale(TestTaxCommonSale, TestTaxesDownPayment):
    ...
    def assert_sale_order_document_down_payment(
        ...
        # Full invoice.
        invoice = self.create_down_payment_invoice(sale_order, 'delivered', 0)
    ...
    def test_down_payment_invoice_multiple_taxes_and_accounts(self):
        ...
        for step, values in self._test_standard_flow_down_payment():
            if step == 'pre_init_sale_order':
                values['sale_order_values']['order_line'] = [
            ...
            elif step == 'post_init_sale_order':
                self.assertRecordValues(values['sale_order'], [{
                    'amount_untaxed': 14000.0,
                    'amount_tax': 2450.0,
                    'amount_total': 16450.0,
                }])
            elif step == 'pre_init_down_payment':
                # Create a down payment invoice of 30%.
                values['amount'] = 30
            elif step == 'post_init_down_payment':
                down_payment_label = f"Down Payment: {values['sale_order'].create_date.strftime('%m/%d/%Y')} (Draft)"
                self.assertRecordValues(values['sale_order'].order_line, [
                ...
                values['down_payment_invoice'].action_post()
                self.assertRecordValues(values['down_payment_invoice'], [{
                    'amount_untaxed': 4200.0,
                    'amount_tax': 735.0,
                    'amount_total': 4935.0,
                }])
                account_id = values['down_payment_invoice'].company_id.downpayment_account_id.id
                self.assertRecordValues(values['down_payment_invoice'].line_ids, [
            ...
            elif step == 'post_init_final_invoice':
                account_id = values['down_payment_invoice'].company_id.downpayment_account_id.id
                self.assertRecordValues(values['final_invoice'], [{
                    'amount_untaxed': values['sale_order'].amount_untaxed - values['down_payment_invoice'].amount_untaxed,
                    'amount_tax': values['sale_order'].amount_tax - values['down_payment_invoice'].amount_tax,
                    'amount_total': values['sale_order'].amount_total - values['down_payment_invoice'].amount_total,
                }])
                self.assertRecordValues(values['final_invoice'].line_ids, [
                    # Product lines:
    ...
    def test_down_payment_invoice_manual_removing_of_tax(self):
        ...
        for step, values in self._test_standard_flow_down_payment():
            if step == 'pre_init_sale_order':
                ...
                values['down_payment_invoice'].invoice_line_ids.tax_ids = [Command.clear()]
                values['down_payment_invoice'].action_post()
                self.assertRecordValues(values['down_payment_invoice'], [{
            ...
            elif step == 'pre_init_second_down_payment':
                # Second down payment of 20%.
                values['activate_second_down_payment'] = True
                values['amount'] = 20
            elif step == 'post_init_second_down_payment':
                values['second_down_payment_invoice'].action_post()
                self.assertRecordValues(values['second_down_payment_invoice'], [{
                ...
                self.assertRecordValues(values['final_invoice'], [{
                    'amount_untaxed': values['sale_order'].amount_untaxed - values['down_payment_invoice'].amount_untaxed - values['second_down_payment_invoice'].amount_untaxed,
                    'amount_tax': values['sale_order'].amount_tax - values['down_payment_invoice'].amount_tax - values['second_down_payment_invoice'].amount_tax,
                    'amount_total': values['sale_order'].amount_total - values['down_payment_invoice'].amount_total - values['second_down_payment_invoice'].amount_total,
                }])
                self.assertRecordValues(values['final_invoice'].line_ids, [
                    # Product line:
    ...
    def test_down_payment_invoice_foreign_currency_different_dates(self):
        ...
        for step, values in self._test_standard_flow_down_payment():
            if step == 'pre_init_sale_order':
                ...
                values['freeze_time'] = '2016-01-01'
                values['sale_order_values'].update({
            ...
            elif step == 'pre_init_final_invoice':
                values['freeze_time'] = '2017-01-01'
            elif step == 'post_init_final_invoice':
                self.assertRecordValues(values['final_invoice'], [{
                    'amount_untaxed': values['sale_order'].amount_untaxed - values['down_payment_invoice'].amount_untaxed,
                    'amount_tax': values['sale_order'].amount_tax - values['down_payment_invoice'].amount_tax,
    ...
    def test_down_payment_invoice_then_refunded_then_invoiced_again(self):
        ...
        for step, values in self._test_standard_flow_down_payment():
            if step == 'pre_init_sale_order':
                ...
                draft_down_payment_label = f"Down Payment: {values['sale_order'].create_date.strftime('%m/%d/%Y')} (Draft)"
                self.assertRecordValues(values['sale_order'].order_line, [
                ...
                # Post the down payment invoice.
                values['down_payment_invoice'].action_post()
                down_payment_dp_invoice_1 = f"Down Payment (ref: {values['down_payment_invoice'].name} on {values['down_payment_invoice'].invoice_date.strftime('%m/%d/%Y')})"
                self.assertRecordValues(values['sale_order'].order_line, [
                    {'name': 'line_1', 'tax_ids': tax_15.ids, 'price_subtotal': 1000.0},
                    {'name': "Down Payments", 'tax_ids': [], 'price_subtotal': 0.0},
                ...
                action_values = (
                    ...
                    .with_context(active_model='account.move', active_ids=values['down_payment_invoice'].ids)
                    .create({'journal_id': values['down_payment_invoice'].journal_id.id})
                    .reverse_moves()
                )
                ...
                # Post second down payment invoice
                values['second_down_payment_invoice'].action_post()
                down_payment_dp_invoice_2 = f"Down Payment (ref: {values['second_down_payment_invoice'].name} on {values['second_down_payment_invoice'].invoice_date.strftime('%m/%d/%Y')})"
                self.assertRecordValues(values['sale_order'].order_line, [
                    {'name': 'line_1', 'tax_ids': tax_15.ids, 'price_subtotal': 1000.0},
                    {'name': "Down Payments", 'tax_ids': [], 'price_subtotal': 0.0},
                    {'name': down_payment_dp_invoice_1, 'tax_ids': tax_15.ids, 'price_subtotal': 0.0},
                ...
                values['final_invoice'].action_post()
                self.assertRecordValues(values['final_invoice'], [{
                    'amount_untaxed': values['sale_order'].amount_untaxed - values['second_down_payment_invoice'].amount_untaxed,
                    'amount_tax': values['sale_order'].amount_tax - values['second_down_payment_invoice'].amount_tax,
                    'amount_total': values['sale_order'].amount_total - values['second_down_payment_invoice'].amount_total,
                }])

                # Credit note on the final invoice.
                action_values = (
                    self.env['account.move.reversal']
                    .with_context(active_model='account.move', active_ids=values['final_invoice'].ids)
                    .create({'journal_id': values['final_invoice'].journal_id.id})
                    .reverse_moves()
                )
                ...
                wizard = (
                    ...
                    .with_context(active_model=values['sale_order']._name, active_ids=values['sale_order'].ids)
                    .create({'advance_payment_method': 'delivered'})
                )
                action_values = wizard.create_invoices()
                new_final_invoice = self.env['account.move'].browse(action_values['res_id'])
                new_final_invoice.action_post()
                self.assertRecordValues(new_final_invoice, [{
                    'amount_untaxed': values['sale_order'].amount_untaxed - values['second_down_payment_invoice'].amount_untaxed,
                    'amount_tax': values['sale_order'].amount_tax - values['second_down_payment_invoice'].amount_tax,
                    'amount_total': values['sale_order'].amount_total - values['second_down_payment_invoice'].amount_total,
                }])
    ...
    def test_down_payment_100_first_then_0_final_invoice_round_per_line(self):
        ...
        for step, values in self._test_standard_flow_down_payment():
            if step == 'pre_init_sale_order':
                ...
                self.assert_invoice_tax_totals_summary(values['down_payment_invoice'], {
                    'same_tax_base': True,
                    'currency_id': other_currency.id,
                ...
                self.assert_invoice_tax_totals_summary(values['final_invoice'], {
                    'same_tax_base': True,
                    'currency_id': other_currency.id,
                    'company_currency_id': self.env.company.currency_id.id,
    ...
    def test_downpayment_invoice_lines_with_down_payment_account(self):
        for step, values in self._test_standard_flow_down_payment():
            if step == 'pre_init_sale_order':
                ...
                self.assertEqual(len(values['down_payment_invoice'].invoice_line_ids), 1)
                self.assertEqual(values['down_payment_invoice'].invoice_line_ids.account_id.id, self.company_data['company'].downpayment_account_id.id)
                # We should have half the amount of the Sale Order -> 840$
                self.assertEqual(values['down_payment_invoice'].amount_total, 840)
    ...
    def test_down_payment_with_global_discount(self):
        ...
        for step, values in self._test_standard_flow_down_payment():
            if step == 'pre_init_sale_order':
                ...
                self.apply_sale_order_discount(values['sale_order'], 'fixed', 990.00)
            elif step == 'pre_init_down_payment':
                ...
                so_base_lines = [
                    ...
                    for order_line in values['sale_order'].order_line
                ]
