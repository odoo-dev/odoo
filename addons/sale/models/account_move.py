...
class AccountMove(models.Model):
    ...
    def _link_analytic_lines_to_invoice(self):
        ...
        for invoice in invoices:
            invoice._get_analytic_lines_to_link().write({'reinvoice_move_id': invoice.id})

    def _get_analytic_lines_to_link(self):
    ...
    def _analytic_line_domain_get_invoiced_lines(self, so_lines):
        return Domain([
            ('so_line', 'in', so_lines.ids),
            '|', '|',
                ('reinvoice_move_id', '=', False),
                '&',
                    ('reinvoice_move_id.state', '=', 'cancel'),
                    ('reinvoice_move_id.payment_state', '!=', 'invoicing_legacy'),
                ('reinvoice_move_id.payment_state', '=', 'reversed')
        ])
