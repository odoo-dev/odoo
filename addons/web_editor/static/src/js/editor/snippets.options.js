...
var SnippetOption = Widget.extend({
     ...
     * @param {Object} value - the data name to customize
     * @param {jQuery} $opt - the related DOMElement option
     */
    setDataAttribute: function (previewMode, dataName, $opt) {
        let hasUserValue = false;
        const weInput = $opt[0];
        const inputValue = weInput.querySelector('input').value;
        const defaultValue = weInput.dataset.defaultValue;

        let value = defaultValue;
        if (inputValue) {
            const unit = weInput.dataset.unit;
            const v = parseFloat(inputValue.trim());
            if (!isNaN(v)) {
                hasUserValue = true;
                value = (v + unit);
            }
        }
        this.$target[0].dataset[dataName] = parseFloat(value);

        var toggleClass = weInput.dataset.toggleClass;
        if (toggleClass) {
    ...
    _select: function (previewMode, $opt) {
        ...
        methods.reverse().forEach(data => {
            ...
            for (const specialMethodName of ['setDataAttribute', 'setStyle']) {
                if (methodNames.includes(specialMethodName)) {
                    methodNames = [specialMethodName];
                    break;
                }
            }
    ...
    _setActive: function () {
        ...
        this.el.querySelectorAll('[data-set-data-attribute]').forEach(el => {
            ...
            if (!value || (value + unit) === el.dataset.defaultValue) {
                value = '';
            }
            inputEl.value = value;
        });
        ...
        this.el.querySelectorAll('[data-set-style]').forEach(el => {
            ...
            _.each($els, (el, i) => {
                ...
                if (values.length === 1 && Math.abs(values[0] - parseFloat(el.dataset.defaultValue)) < EPS) {
                    values.pop();
                }
    ...
    _onOptionInputKeydown: function (ev) {
        ...
        let value = parseFloat(input.value || input.parentNode.dataset.defaultValue);
        if (isNaN(value)) {
            return;
        }
     ...
     * @param {boolean} [valueEl]
     */
    buildSelectElement: function (selectEl, valueEl) {
...
registry.colorpicker = SnippetOption.extend({
    ...
    start: async function () {
        ...
        // Prepare the select element which will contain all the colorpicker UI
        const selectElData = document.createElement('we-select');
        const titleEl = this.el.querySelector('we-title');
        selectElData.setAttribute('string', titleEl.textContent);

        // Instanciate the color palette widget
        const options = {
            $target: this.$target,
        };
        ...
        await this.colorPalette.appendTo(selectElData);

        // Build the select element with a custom span to hold the color preview
        this.colorPreviewEl = document.createElement('span');
        const selectEl = this.buildSelectElement(selectElData, this.colorPreviewEl);
        selectEl.classList.add('o_we_so_color_palette');
...
registry.BackgroundPosition = SnippetOption.extend({
    ...
    start: function () {
        ...
        this.$target.on('snippet-option-change', () => {
            ...
            this._updateUI();
            // this.img is used to compute dragging speed, so needs to be updated
            this.img.src = this._getSrcFromCssValue();
        });

        // Rersize overlay content on window resized because background images change size,
        // and on carousel slide because they sometimes take up more space and move elements around them
        // TODO: remove slid event when carousel refactor is merged as carousels will have their correct height immediately.
        $(window).on('resize.bgposition slid.bs.carousel.bgposition', () => this._dimensionOverlay());
    },
    /**
     * @override
     */
    ...
    backgroundType: function (previewMode, value, $opt) {
        ...
        if (value === 'cover') {
            // background-size: cover; is set by the css when it doesn't have the repeat class
            this.$target.css('background-size', '');
        } else {
            this.$target.css('background-size', 'auto');
        }
    ...
    /**
     * Initializes the overlay, binds events to the buttons, inserts it in the manipulators.
     *
     * @private
     */
    _initOverlay: function () {
        this.$backgroundOverlay = $(qweb.render('web_editor.background_position_overlay'));
        this.$overlayContent = this.$backgroundOverlay.find('.o_overlay_content');
        this.$overlayBackground = this.$overlayContent.find('.o_overlay_background');

        this.$backgroundOverlay.find('.o_btn_apply').on('click', () => {
            this.$target.css('background-position', this.$bgDragger.css('background-position'));
            this._toggleOverlay(false);
        });
        this.$backgroundOverlay.find('.o_btn_discard').on('click', () => this._toggleOverlay(false));

        // Insert it in the manipulators container
     ...
     */
    _toggleOverlay: function (activate) {
        ...
        if (!activate) {
            ...
            this.$manipulators.removeClass('d-none');
        ...
        this.$manipulators = this.$overlay.parent().children(':not(.o_background_position_overlay)').addClass('d-none');
    ...
    _onDocumentClicked: function (ev) {
        if (!ev.target.closest('.o_background_position_overlay')) {
            this._toggleOverlay(false);
        }
    },
});
