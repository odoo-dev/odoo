        ...
        for sale_line in self:
            if sale_line.product_type or (sale_line.is_downpayment and sale_line.price_unit != 0):
                ...
                if sale_line.product_id.tracking != 'none':
                    ...
                    item['lot_qty_by_name'] = {line.lot_id.name: 0 for line in sale_line.move_ids.move_line_ids.filtered(lambda ml: ml.state == 'done')}
                    for line in sale_line.move_ids.move_line_ids.filtered(lambda ml: ml.state != 'done'):
                        item['lot_qty_by_name'][line.lot_id.name] = item['lot_qty_by_name'].get(line.lot_id.name, 0) + line.quantity
