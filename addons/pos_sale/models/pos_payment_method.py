...
class PosPaymentMethod(models.Model):
    ...
    def _get_sale_order_payments(self, session):
        """POS payments of ``session`` settled by this method.

        Restricted by `pos_sale_online_payment` to the ones actually backed by an
        accounting payment.
        """
        non_invoiced_orders = session._get_invoiced_and_non_invoiced_orders()[0]
        return non_invoiced_orders.payment_ids.filtered(
            lambda payment: payment.payment_method_id == self,
        )

    def _get_settled_account_payment(self, payment):
        """The accounting payment the customer already made for ``payment``.

        Provided by `pos_sale_online_payment`; without it nothing was collected in
        advance and there is no transfer to book.
        """
    ...
    def _create_sale_order_payment_line_transfer(self, session):
        ...
        for payment in self._get_sale_order_payments(session):
            account_payment = self._get_settled_account_payment(payment)
