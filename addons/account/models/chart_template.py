...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _instantiate_foreign_taxes(self, country, company):
        ...
        for tax_template in sorted(tax_data.values(), key=lambda x: any(rep_line.get('account_id') for _command, _id, rep_line in x.get('repartition_line_ids', []) if tax_template.get('tax_exigibility') == 'on_payment'), reverse=True):
            has_cash_basis = True
            ...
            account_id = [rep_line['account_id'] for _command, _id, rep_line in tax_template.get('repartition_line_ids', []) if rep_line.get('account_id')]
            if account_id:
