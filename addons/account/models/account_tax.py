...
class AccountTax(models.Model):
    ...
    @api.model
    def _get_batch_affect_base_tax_amount(self, batch, field_type='raw', in_foreign_currency=True):
    ...
    def _set_taxes_data_base_amount_distr(self, global_context, taxes_data_grouping_key, taxes_data):
        ...
        target_factors = []
        for tax_data in taxes_data:
            target_factors.append({'factor': tax_data['raw_base_amount'].value(), 'tax_data': tax_data})
            amounts.append(tax_data['base_amount'].value())
        ...
        for factor in result_factors:
            tax_data = factor['tax_data']
            tax_data['base_amount_distr'].set(factor['amount'])
    ...
    def _set_taxes_data_tax_amount_distr(self, global_context, taxes_data_grouping_key, taxes_data):
        ...
        target_factors = []
        for tax_data in taxes_data:
            target_factors.append({'factor': tax_data['raw_tax_amount'].value(), 'tax_data': tax_data})
