...
from odoo.addons.account.models.taxes_engine import Compute, sum_and_distribute_amounts_to
...
class AccountTax(models.Model):
    ...
    def _get_base_line_grouping_key(self, global_context, base_line):
        """ Get a grouping key about how base lines will be grouped together to compute their total amount excluded of tax.
    ...
    def _get_base_line_tax_data_grouping_key(self, global_context, base_line, tax):
        ...
        :param base_line:   A base line generated by '_prepare_base_line_for_taxes_computation'.
        :param tax:         A tax of the base line.
        :param company:     The company owning the base_line.
        :return:            A grouping key.
        """
        return {
    ...
    @api.model
    def _get_tax_line_grouping_key(self, tax_line, company):
    ...
    # -------------------------------------------------------------------------
    # TAXES ENGINE - common helpers - batch
    # -------------------------------------------------------------------------

    @api.model
    def _get_raw_tax_amount_currency(self, batch, tax_data, base):
        """ Set up 'raw_tax_amount_currency'. This code is common whatever the batch of taxes_data is
        price included or not.

        [!] Mirror of the same method in account_tax.js.
        PLZ KEEP BOTH METHODS CONSISTENT WITH EACH OTHERS.

        :param evaluation_context:  What you have access to compute the tax amount.
        :param base_untaxed:        The base amount without tax as a Variable instance.
        :param taxes_data:          A list of tax_data belonging to the same base_line and all sharing the
                                    same base.
        """
    ...
    @api.model
    def _get_price_excluded_raw_tax_amount_currency(self, batch, tax_data, base_untaxed):
        """ Set up 'raw_tax_amount_currency' for price excluded taxes.
        ...
        :param taxes_data:          A list of tax_data belonging to the same base_line and all sharing the
                                    same base.
        """
        if (amount := self._get_raw_tax_amount_currency(batch, tax_data, base_untaxed)) is not None:
            return amount
    ...
    @api.model
    def _get_price_included_raw_tax_amount_currency(self, batch, tax_data, base_taxed):
        """ Set up 'raw_tax_amount_currency' for price included taxes.

        [!] Mirror of the same method in account_tax.js.
        PLZ KEEP BOTH METHODS CONSISTENT WITH EACH OTHERS.

        :param evaluation_context:  What you have access to compute the tax amount.
        :param base_taxed:          The base amount tax included as a Variable instance.
        :param taxes_data:          A list of tax_data belonging to the same base_line and all sharing the
                                    same base.
        """
        if (amount := self._get_raw_tax_amount_currency(batch, tax_data, base_taxed)) is not None:
            return amount
    ...
    @api.model
    def _get_batch_affect_base_tax_amount_currency(self, batch, field_type='raw'):
        if field_type == 'rounded':
            field = 'tax_amount_currency'
        elif field_type == 'rounded_distr':
            field = 'tax_amount_currency_distr'
        else:  # elif field_type == 'raw':
            field = 'raw_tax_amount_currency'
        return sum(
    ...
    @api.model
    def _get_batch_tax_amount_currency(self, batch, field_type='raw'):
        if field_type == 'rounded':
            field = 'tax_amount_currency'
        elif field_type == 'rounded_distr':
            field = 'tax_amount_currency_distr'
    ...
    @api.model
    def _set_base_line_taxes_price_exclude_raw_tax_base_amount_currency(self, global_context, batch, apply_tax_on_rounded_base=True):
        currency = batch['_base_line']['currency_id']
        raw_base = batch['_evaluation_context']['raw_base']
        delta_base_untaxed = self._get_batch_affect_base_tax_amount_currency(
            batch,
            field_type='rounded_distr' if apply_tax_on_rounded_base else 'raw',
        )
        if apply_tax_on_rounded_base:
            base_untaxed = currency.round(raw_base) + delta_base_untaxed
        else:
            base_untaxed = raw_base + delta_base_untaxed

        raw_base_untaxed = raw_base + delta_base_untaxed
        for tax_data in batch['taxes_data']:
            tax_data['raw_tax_amount_currency'].set(self._get_price_excluded_raw_tax_amount_currency(batch, tax_data, base_untaxed))
            tax_data['raw_base_amount_currency'].set(raw_base_untaxed)

    @api.model
    def _set_base_line_taxes_price_include_raw_tax_base_amount_currency(self, global_context, batch, apply_tax_on_rounded_base=True):
        raw_base = batch['_evaluation_context']['raw_base']
        delta_base_taxed = self._get_batch_affect_base_tax_amount_currency(
            batch,
            field_type='rounded_distr' if apply_tax_on_rounded_base else 'raw',
        )
        raw_base_taxed = raw_base + delta_base_taxed
        for tax_data in batch['taxes_data']:
            tax_data['raw_tax_amount_currency'].set(self._get_price_included_raw_tax_amount_currency(batch, tax_data, raw_base_taxed))

        raw_base_untaxed = raw_base_taxed - self._get_batch_tax_amount_currency(
            batch,
            field_type='rounded_distr' if apply_tax_on_rounded_base else 'raw',
        )
        for tax_data in batch['taxes_data']:
    ...
    @api.model
    def _set_base_line_taxes_raw_tax_base_amount_currency(self, global_context, batch, apply_tax_on_rounded_base=True):
        if batch['price_include']:
            self._set_base_line_taxes_price_include_raw_tax_base_amount_currency(global_context, batch, apply_tax_on_rounded_base=apply_tax_on_rounded_base)
        else:
            self._set_base_line_taxes_price_exclude_raw_tax_base_amount_currency(global_context, batch, apply_tax_on_rounded_base=apply_tax_on_rounded_base)

    # -------------------------------------------------------------------------
    # TAXES ENGINE - common helpers - global_base_lines
    # -------------------------------------------------------------------------
    ...
    def _set_base_lines_total_excluded_currency_globally(self, global_context, base_lines_grouping_key, base_lines):
        ...
        for base_line in base_lines:
            tax_details = base_line['tax_details']
            raw_total_excluded_currency = tax_details['raw_total_excluded_currency'].value()
            target_factors.append({'factor': raw_total_excluded_currency, 'base_line': base_line})
            amounts.append(raw_total_excluded_currency)
        ...
        for factor in result_factors:
            tax_details = factor['base_line']['tax_details']
            tax_details['total_excluded_currency'].set(factor['amount'])

    @api.model
    def _set_base_lines_total_included_currency(self, global_context, base_lines_grouping_key, base_lines):
        for base_line in base_lines:
            tax_details = base_line['tax_details']
            tax_details['total_included_currency'].set(
                tax_details['total_excluded_currency'].value()
                + sum(tax_data['tax_amount_currency'].value() for tax_data in tax_details['taxes_data'])
            )

    @api.model
    def _set_base_lines_raw_total_included_currency(self, global_context, base_lines_grouping_key, base_lines):
        for base_line in base_lines:
            tax_details = base_line['tax_details']
            tax_details['raw_total_included_currency'].set(
    ...
    def _set_base_lines_raw_total_excluded(self, global_context, base_lines_grouping_key, base_lines):
        ...
        rate_factor = (1 / rate) if rate else 0.0
        for base_line in base_lines:
            tax_details = base_line['tax_details']
            batches = tax_details['_batches']
            first_batch = batches[0]
            ...
            if taxes_data:
                ...
                tax_details['raw_total_excluded'].set(tax_details['total_excluded_currency'].value() * rate_factor)
    ...
    def _set_base_lines_total_excluded_globally(self, global_context, base_lines_grouping_key, base_lines):
        ...
        for base_line in base_lines:
            tax_details = base_line['tax_details']
            raw_total_excluded = tax_details['raw_total_excluded'].value()
            target_factors.append({'factor': raw_total_excluded, 'base_line': base_line})
            amounts.append(raw_total_excluded)

        result_factors = sum_and_distribute_amounts_to(amounts, target_factors, rounding=company.currency_id.rounding)
        for factor in result_factors:
            tax_details = factor['base_line']['tax_details']
            tax_details['total_excluded'].set(factor['amount'])

    @api.model
    def _set_base_lines_total_included(self, global_context, base_lines_grouping_key, base_lines):
        for base_line in base_lines:
            tax_details = base_line['tax_details']
            tax_details['total_included'].set(
                tax_details['total_excluded'].value()
                + sum(tax_data['tax_amount'].value() for tax_data in tax_details['taxes_data'])
            )

    @api.model
    def _set_base_lines_raw_total_included(self, global_context, base_lines_grouping_key, base_lines):
        for base_line in base_lines:
            tax_details = base_line['tax_details']
            tax_details['raw_total_included'].set(
                tax_details['raw_total_excluded'].value()
                + sum(tax_data['raw_tax_amount'].value() for tax_data in tax_details['taxes_data'])
            )

    # -------------------------------------------------------------------------
    # TAXES ENGINE - common helpers - taxes_data
    # -------------------------------------------------------------------------
    ...
    def _set_taxes_data_base_amount_currency_per_line(self, global_context, taxes_data_grouping_key, taxes_data):
        ...
        for tax_data in taxes_data:
            ...
            tax_data['base_amount_currency'].set(
                ...
                + self._get_batch_affect_base_tax_amount_currency(batch, field_type='rounded')
                - (
                    self._get_batch_tax_amount_currency(batch, field_type='rounded')
                    if batch['price_include']
                    else 0.0
                )
            )
    ...
    def _set_taxes_data_base_amount_currency_globally(self, global_context, taxes_data_grouping_key, taxes_data):
        ...
        target_factors = []
        for tax_data in taxes_data:
            raw_base_amount_currency = tax_data['raw_base_amount_currency'].value()
            target_factors.append({'factor': raw_base_amount_currency, 'tax_data': tax_data})
            amounts.append(raw_base_amount_currency)

        result_factors = sum_and_distribute_amounts_to(amounts, target_factors, rounding=currency.rounding)
        for factor in result_factors:
            tax_data = factor['tax_data']
            tax_data['base_amount_currency'].set(factor['amount'])
    ...
    def _set_taxes_data_base_amount_currency_distr(self, global_context, taxes_data_grouping_key, taxes_data):
        ...
        target_factors = []
        for tax_data in taxes_data:
            target_factors.append({'factor': tax_data['raw_base_amount_currency'].value(), 'tax_data': tax_data})
            amounts.append(tax_data['base_amount_currency'].value())

        result_factors = sum_and_distribute_amounts_to(amounts, target_factors, rounding=currency.rounding, round_distr=False)
        for factor in result_factors:
            tax_data = factor['tax_data']
            tax_data['base_amount_currency_distr'].set(factor['amount'])
    ...
    def _set_taxes_data_tax_amount_currency_globally(self, global_context, taxes_data_grouping_key, taxes_data):
        ...
        target_factors = []
        for tax_data in taxes_data:
            raw_tax_amount_currency = tax_data['raw_tax_amount_currency'].value()
            target_factors.append({'factor': raw_tax_amount_currency, 'tax_data': tax_data})
            amounts.append(raw_tax_amount_currency)

        result_factors = sum_and_distribute_amounts_to(amounts, target_factors, rounding=currency.rounding)
        for factor in result_factors:
            tax_data = factor['tax_data']
            tax_data['tax_amount_currency'].set(factor['amount'])
    ...
    def _set_taxes_data_tax_amount_currency_distr(self, global_context, taxes_data_grouping_key, taxes_data):
        ...
        for tax_data in taxes_data:
            ...
            amounts.append(tax_data['tax_amount_currency'].value())

        result_factors = sum_and_distribute_amounts_to(amounts, target_factors, rounding=currency.rounding, round_distr=False)
        for factor in result_factors:
            tax_data = factor['tax_data']
            tax_data['tax_amount_currency_distr'].set(factor['amount'])

    @api.model
    def _set_taxes_data_raw_tax_amount(self, global_context, taxes_data_grouping_key, taxes_data):
        rate = taxes_data_grouping_key['rate']
        rate_factor = (1 / rate) if rate else 0.0
        for tax_data in taxes_data:
            tax_data['raw_tax_amount'].set(tax_data['tax_amount_currency_distr'].value() * rate_factor)

    @api.model
    def _set_taxes_data_raw_base_amount(self, global_context, taxes_data_grouping_key, taxes_data):
        rate = taxes_data_grouping_key['rate']
        rate_factor = (1 / rate) if rate else 0.0
        for tax_data in taxes_data:
            tax_data['raw_base_amount'].set(tax_data['base_amount_currency_distr'].value() * rate_factor)
    ...
    def _set_taxes_data_base_amount_globally(self, global_context, taxes_data_grouping_key, taxes_data):
        ...
        target_factors = []
        for tax_data in taxes_data:
            raw_base_amount = tax_data['raw_base_amount'].value()
            target_factors.append({'factor': raw_base_amount, 'tax_data': tax_data})
            amounts.append(raw_base_amount)

        result_factors = sum_and_distribute_amounts_to(amounts, target_factors, rounding=company.currency_id.rounding)
        for factor in result_factors:
            tax_data = factor['tax_data']
            tax_data['base_amount'].set(factor['amount'])
    ...
    def _set_taxes_data_tax_amount_globally(self, global_context, taxes_data_grouping_key, taxes_data):
        ...
        target_factors = []
        for tax_data in taxes_data:
            raw_tax_amount = tax_data['raw_tax_amount'].value()
            target_factors.append({'factor': raw_tax_amount, 'tax_data': tax_data})
            amounts.append(raw_tax_amount)

        result_factors = sum_and_distribute_amounts_to(amounts, target_factors, rounding=company.currency_id.rounding)
        for factor in result_factors:
            tax_data = factor['tax_data']
            tax_data['tax_amount'].set(factor['amount'])
    ...
    def _set_taxes_data_tax_amount_distr(self, global_context, taxes_data_grouping_key, taxes_data):
        ...
        for tax_data in taxes_data:
            target_factors.append({'factor': tax_data['raw_tax_amount_currency'].value(), 'tax_data': tax_data})
            amounts.append(tax_data['tax_amount'].value())

        result_factors = sum_and_distribute_amounts_to(amounts, target_factors, rounding=company.currency_id.rounding, round_distr=False)
        for factor in result_factors:
            tax_data = factor['tax_data']
            tax_data['tax_amount_distr'].set(factor['amount'])
    ...
    def _get_taxes_engine_functions_default(self, global_context):
        return {
            'base_line_batch_raw_tax_base_amount_currency': self._set_base_line_taxes_raw_tax_base_amount_currency,
            ...
            'base_lines_raw_total_included_currency': self._set_base_lines_raw_total_included_currency,
            'base_lines_total_included_currency': self._set_base_lines_total_included_currency,
            ...
            'base_lines_raw_total_included': self._set_base_lines_raw_total_included,
            'base_lines_total_included': self._set_base_lines_total_included,
            ...
            'taxes_raw_base_amount': self._set_taxes_data_raw_base_amount,
            'taxes_base_amount': self._set_taxes_data_base_amount_per_line,
            'taxes_raw_tax_amount': self._set_taxes_data_raw_tax_amount,
    ...
    @api.model
    def _set_base_line_taxes_raw_tax_base_amount_currency_no_round(self, global_context, batch):
        self._set_base_line_taxes_raw_tax_base_amount_currency(global_context, batch, apply_tax_on_rounded_base=False)

    @api.model
    def _get_taxes_engine_custom_functions_no_round(self, global_context):
        return {
            'base_line_batch_raw_tax_base_amount_currency': self._set_base_line_taxes_raw_tax_base_amount_currency_no_round,

            'base_lines_total_excluded_currency': None,
            'base_lines_total_included_currency': None,
            'base_lines_total_excluded': None,
            'base_lines_total_included': None,
    ...
    # -------------------------------------------------------------------------
    # TAXES ENGINE - Countries following rules of CEN EN16931.
    # -------------------------------------------------------------------------

    def _countries_using_cen_en16931_taxes_computation(self, global_context):
        # TODO: others countries from EAS
        return {'BE'}

    @api.model
    def _get_base_line_first_batch_price_include_cen_en16931(self, batch):
        def get_batch_reverse_dependencies(batch):
            if batch['reverse_dependencies']:
                return get_batch_reverse_dependencies(batch['reverse_dependencies'][0][1])
            return batch

        return get_batch_reverse_dependencies(batch)

    @api.model
    def _get_descending_affect_base_raw_tax_amount_cen_en16931(self, batch):
        raw_base = batch['_evaluation_context']['raw_base']

        def get_affect_base_raw_tax_amount(batch):
            affect_base_raw_tax_amount_currency = sum(
                sign * get_affect_base_raw_tax_amount(dep_batch)
                for sign, dep_batch in batch['dependencies']
                if sign < 0
            )
            return (
                -affect_base_raw_tax_amount_currency
                - (
                    sum(
                        self._get_price_included_raw_tax_amount_currency(batch, tax_data, raw_base + affect_base_raw_tax_amount_currency)
                        for tax_data in batch['taxes_data']
                    )
                    if batch['price_include']
                    else 0.0
        ...
        return get_affect_base_raw_tax_amount(batch)

    @api.model
    def _get_ascending_affect_base_raw_tax_amount_cen_en16931(self, batch, field_type='raw'):
        if field_type == 'rounded':
            field = 'tax_amount_currency'
        elif field_type == 'rounded_distr':
            field = 'tax_amount_currency_distr'
        ...
        return sum(
            sign * dep_tax_data[field].value()
            for sign, dep_batch in batch['reverse_dependencies']
            if dep_batch['price_include']
            for dep_tax_data in dep_batch['taxes_data']
        ) + sum(
            sign * dep_tax_data[field].value()
            for sign, dep_batch in batch['dependencies']
            if not dep_batch['price_include']
            for dep_tax_data in dep_batch['taxes_data']
        )

    @api.model
    def _set_base_line_batch_raw_tax_base_amount_currency_cen_en16931(self, global_context, batch):
        base_batch = self._get_base_line_first_batch_price_include_cen_en16931(batch)
        base_batch_tax_affect_base_dependencies = self._get_descending_affect_base_raw_tax_amount_cen_en16931(base_batch)
        tax_affect_base_reverse_dependencies = self._get_ascending_affect_base_raw_tax_amount_cen_en16931(batch, field_type='rounded_distr')

        currency = batch['_base_line']['currency_id']
        raw_base = batch['_evaluation_context']['raw_base'] + base_batch_tax_affect_base_dependencies
        raw_base_untaxed = raw_base + tax_affect_base_reverse_dependencies
        base_untaxed = currency.round(raw_base) + tax_affect_base_reverse_dependencies
    ...
    @api.model
    def _set_taxes_data_base_amount_currency_cen_en16931(self, global_context, taxes_data_grouping_key, taxes_data):
        for tax_data in taxes_data:
            batch = tax_data['_batch']
            base_batch = self._get_base_line_first_batch_price_include_cen_en16931(batch)
            base_batch_tax_affect_base_dependencies = self._get_descending_affect_base_raw_tax_amount_cen_en16931(base_batch)
            tax_affect_base_reverse_dependencies = self._get_ascending_affect_base_raw_tax_amount_cen_en16931(batch, field_type='rounded')

            currency = batch['_base_line']['currency_id']
            raw_base = batch['_evaluation_context']['raw_base'] + base_batch_tax_affect_base_dependencies
            tax_data['base_amount_currency'].set(currency.round(raw_base) + tax_affect_base_reverse_dependencies)

    @api.model
    def _get_taxes_engine_custom_functions_cen_en16931(self, global_context):
        return {
            'base_line_batch_raw_tax_base_amount_currency': self._set_base_line_batch_raw_tax_base_amount_currency_cen_en16931,

            'taxes_base_amount_currency': self._set_taxes_data_base_amount_currency_cen_en16931,
        }
    ...
    @api.model
    def _set_base_line_taxes_raw_tax_base_amount_currency_l10n_mx(self, global_context, batch):
        self._set_base_line_taxes_raw_tax_base_amount_currency(global_context, batch, apply_tax_on_rounded_base=False)

    @api.model
    def _get_taxes_engine_custom_functions_l10n_mx(self, global_context):
        return {
            'base_line_batch_raw_tax_base_amount_currency': self._set_base_line_taxes_raw_tax_base_amount_currency_l10n_mx,
    ...
    @api.model
    def _set_base_line_taxes_raw_tax_base_amount_currency_l10n_pt(self, global_context, batch):
        self._set_base_line_taxes_raw_tax_base_amount_currency(global_context, batch, apply_tax_on_rounded_base=False)
    ...
    def _set_base_lines_total_excluded_currency_globally_l10n_pt(self, global_context, base_lines_grouping_key, base_lines):
        ...
        for base_line in base_lines:
            tax_details = base_line['tax_details']
            raw_total_included_currency = tax_details['raw_total_included_currency'].value()
            target_factors.append({'factor': raw_total_included_currency, 'base_line': base_line})
            amounts.append(raw_total_included_currency)
    ...
    def _set_taxes_data_base_amount_currency_globally_l10n_pt(self, global_context, taxes_data_grouping_key, taxes_data):
        ...
        for tax_data in taxes_data:
            batch = tax_data['_batch']
            raw_total_amount_currency = (
                tax_data['raw_base_amount_currency'].value()
                + sum(tax_data['raw_tax_amount_currency'].value() for tax_data in batch['taxes_data'])
            )
            target_factors.append({'factor': raw_total_amount_currency, 'tax_data': tax_data})
            amounts.append(raw_total_amount_currency)
    ...
    def _get_taxes_engine_custom_functions_l10n_pt(self, global_context):
        return {
            'base_line_batch_raw_tax_base_amount_currency': self._set_base_line_taxes_raw_tax_base_amount_currency_l10n_pt,
    ...
    def _get_taxes_engine_custom_functions(self, global_context):
        ...
        elif country_code in self._countries_using_cen_en16931_taxes_computation(global_context):
            return self._get_taxes_engine_custom_functions_cen_en16931(global_context)
    ...
    @api.model
    def _init_taxes_engine_functions(self, global_context, default_functions):
        def set_lazy(items, keys, method, *args):
            def unpack():
                method(*args)

            for item in items:
                for key in keys:
                    item[key] = Compute(unpack)

        def set_zero(items, keys):
            for item in items:
                for key in keys:
                    item[key] = Compute(None)
                    item[key].set(0.0)
        ...
        for base_line in base_lines:
            for batch in base_line['tax_details']['_batches']:
                ...
                keys = ['raw_tax_amount_currency', 'raw_base_amount_currency']
                if method := default_functions['base_line_batch_raw_tax_base_amount_currency']:
                    set_lazy(items, keys, method, *args)
                else:
                    set_zero(items, keys)
        ...
        for taxes_data_grouping_key, taxes_data in taxes_data_per_grouping_key.items():
            ...
            for key, method in default_functions.items():
                if key.startswith('taxes_'):
                    keys = [key[6:]]
                    if method:
                        set_lazy(items, [key[6:]], method, *args)
                    else:
                        set_zero(items, keys)

    @api.model
    def _apply_tax_lines(self, global_context, base_lines, tax_lines):
        if not tax_lines:
            return
        ...
        for tax_line in tax_lines:
            grouping_key = self._get_tax_line_grouping_key(tax_line, company)
    ...
    @api.model
    def _apply_manual_tax_amounts(self, global_context, base_lines):
        company = global_context['company']
        for base_line in base_lines:
    ...
    def _add_tax_details(self, base_lines, company, tax_lines=None, rounding_method=None):
        ...
        self._init_taxes_engine_functions(global_context, default_functions)
        self._apply_manual_tax_amounts(global_context, base_lines)
        self._apply_tax_lines(global_context, base_lines, tax_lines)
    ...
    def _dispatch_taxes_into_new_base_lines(self, base_lines, company, exclude_function):
        ...
        while to_process:
            ...
            for remaining_tax_data in remaining_taxes_data:
                tax = remaining_tax_data['tax']

                if tax in tax_data_to_remove['taxes']:
                    ...
                    second_tax_data = new_remaining_taxes_data[1]

                    second_tax_details['taxes_data'].append(second_tax_data)
                    second_tax_details['raw_total_included_currency'] += second_tax_data['raw_tax_amount_currency']
                    second_tax_details['raw_total_included'] += second_tax_data['raw_tax_amount']
                    second_tax_details['total_included_currency'] += second_tax_data['tax_amount_currency']
                    second_tax_details['total_included'] += second_tax_data['tax_amount']
                else:
        ...
        self._fix_base_lines_tax_details_on_manual_tax_amounts(final_base_lines, company)
    ...
    def _turn_removed_taxes_into_new_base_lines(self, base_lines, company, grouping_function=None, aggregate_function=None):
        ...
        self._fix_base_lines_tax_details_on_manual_tax_amounts(reduced_base_lines, company)
