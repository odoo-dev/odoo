...
class AccountAccount(models.Model):
    ...
    def _search_new_account_code(self, start_code, cache=None):
        ...
        company_ids_to_check = (self.env.company.child_ids | self.env.company.parent_ids).ids

        def code_is_available(new_code):
            return (
                new_code not in cache
                and not self.sudo().search_count([('company_ids', 'in', company_ids_to_check), ('code', '=', new_code)], limit=1)
            )
    ...
    def _ensure_code_is_unique(self):
        """ Check account codes per companies. These are the checks:
            ...
            For each `account` in self,
            - for each `company` in `account.company_ids`, `account.with_company(company).code` must be set.
              This can be checked by checking that the code is set for each company in account.company_ids.root_id.

            - for each `company` in `account.company_ids`, there must not be any `other_account` that is:
                - accessible from a same company as `account`; and
                - has the same code as `account` in a company from which both are accessible.

              Noting that companies can access all the accounts of their parents, we can check this by ensuring that there
              is no `other_account` that belongs to a parent of `account.company_ids` or a child of `account.company_ids`,
              and that have the same code in the root company.

            For example, if account.company_ids == res.company(1), then, we check:
            - account.with_company(1).code != False
            - self.with_company(res.company(1).root_id).search([
                ('company_ids', 'in' <parents and children of res.company(1)>),
                ('code', '=', account.with_company(1).code),
              ]) must be empty.
        """
        ...
        for company, account_ids in account_ids_to_check_by_company.items():
            ...
            # Check 2.2: Check that there are no duplicates in database
                ...
                [
                    ('company_ids', 'in', (company.child_ids | company.parent_ids).ids),
