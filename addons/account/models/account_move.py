...
class AccountMove(models.Model):
    ...
    def _check_review_state_access(self, review_state):
        ...
        if not review_state and not is_user_able_to_review:
    ...
    def _update_product_qty_available(self, to_draft=False):
        ...
        is_stock_installed = 'stock' in installed_modules
        if is_stock_installed:
            return
        is_purchase_installed = 'purchase' in installed_modules
        qty_adjustments = defaultdict(float)
        product_company_map = {}
            ...
            for line in move.invoice_line_ids.filtered(lambda l: l.product_id.is_storable):
                ...
                if is_sale_move:
                    if move_type in ("out_invoice", "out_receipt"):
                        qty_adjustments[key] -= qty * reverse
                    else:
                        qty_adjustments[key] += qty * reverse
                ...
                product_company_map[key] = (product, company)

        for (product_id, company_id), qty_adj in qty_adjustments.items():
            product, company = product_company_map(product_id, company_id)
            if not float_is_zero(qty_adj, precision_rounding=product.uom_id.rounding):
                product.sudo().with_company(company).with_context(
                    skip_qty_available_update=True
                ).qty_available += qty_adj
    ...
    def _post(self, soft=True):
        ...
        to_post._update_product_qty_available(to_draft=False)
