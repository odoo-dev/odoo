...
class AccountMove(models.Model):
    ...
    def _recompute_cash_rounding_lines(self, lines_map, totals_map):
        ...
        :param lines_map:               The line_ids dispatched by type containing:
            ...
            * terms_lines:              The lines generated by the payment terms of the invoice.
            * rounding_lines:           The cash rounding lines of the invoice.
        :param totals_map: A dictionary having two keys:
            * total_balance:            Sum(balance) of each product / tax line.
            * total_amount_currency:    Sum(amount_currency) of each product / tax line.
        '''
        self.ensure_one()
        in_draft_mode = self != self._origin
        ...
        if self.invoice_cash_rounding_id:
            # Clear lines if the ware using a different cash rounding strategy.
            # Don't use a rounding line generated by an 'add_invoice_line' strategy if the strategy is now 'biggest_tax'.
            strategy = self.invoice_cash_rounding_id.strategy
            old_strategy = 'biggest_tax' if rounding_lines.tax_line_id else 'add_invoice_line'
            if rounding_lines and strategy != old_strategy:
                self.line_ids -= rounding_lines
                rounding_lines = self.env['account.move.line']

            if self.currency_id == self.company_id.currency_id:
                diff_balance = self.invoice_cash_rounding_id.compute_difference(self.company_id.currency_id, totals_map['total_balance'])
                diff_amount_currency = 0.0
                currency = False
            else:
                currency = self.currency_id
                diff_amount_currency = self.invoice_cash_rounding_id.compute_difference(self.currency_id, totals_map['total_amount_currency'])
            ...
            if strategy == 'biggest_tax':
                # Add the rounding in the biggest tax amount: The cash rounding line is added as a new tax line on the tax
                # having the biggest balance.
                biggest_tax_line = None
                for tax_line in lines_map['tax_lines']:
            ...
            elif strategy == 'add_invoice_line':
                # Add a line on the invoice for the rounding: The cash rounding line is added as a new invoice line.
                if rounding_lines:
                    rounding_lines.update({
                        'amount_currency': diff_amount_currency,
                        'debit': diff_balance > 0.0 and diff_balance or 0.0,
    ...
    @api.multi
    def _recompute_payment_terms_lines(self, lines_map, totals_map):
        ...
        available_term_lines_index = 0
        available_term_lines = lines_map['terms_lines'].sorted(lambda line: line.date_maturity or today)
        ...
        # payment term lines.
        if not lines_map['base_lines'] and not lines_map['tax_lines']:
        ...
        # Computation are done both in company's currency / foreign currency in order to ensure consistency even with
        ...
        for date_maturity, balance, amount_currency in to_compute:
            ...
            if available_term_lines_index < len(available_term_lines):
                candidate = available_term_lines[available_term_lines_index]
                available_term_lines_index += 1
                candidate.update({
    ...
    def _recompute_dynamic_lines(self, recompute_all_taxes=False):
        ...
        if self._is_invoice():
            # Compute totals.
            # /!\ This computation is not done into the first loop as we need to take into account the newly updated
            # tax lines.
            totals_map = {
                'total_balance': 0.0,
            ...
            self._recompute_payment_terms_lines(lines_map, totals_map)
    ...
    def _compute_amount(self):
        ...
        for move in self:
            ...
            for line in move.line_ids:
                ...
                # Untaxed amount.
                if move.type != 'entry' and not line.exclude_from_invoice_tab:
                    # Invoice.
                    total_untaxed += line.balance
                    total_untaxed_currency += line.amount_currency
                elif move.type == 'entry' and line.debit and not line.tax_line_id:
    ...
    def _is_invoice(self):
        self.ensure_one()
        return self.type in ('out_invoice', 'out_refund', 'in_invoice', 'in_refund', 'out_receipt', 'in_receipt')
    ...
    def post(self):
        for move in self:
            ...
            if move._is_invoice() and float_compare(move.amount_total, 0.0, precision_rounding=move.currency_id.rounding) < 0:
                raise UserError(_("You cannot validate an invoice with a negative total amount. You should create a credit note instead."))

            # Handle case when the invoice_date is not set. In that case, the invoice_date is set at today and then,
            # lines are recomputed accordingly.
            if not move.invoice_date and move._is_invoice():
...
class AccountMoveLine(models.Model):
    ...
    def _compute_tax_audit(self):
        ...
        for record in self:
            ...
            for tag in record.tag_ids:
                tag_amount = (tag.tax_negate and -1 or 1) * (record.journal_id.type == 'sale' and -1 or 1) * (record.move_id.type in ('in_refund', 'out_refund') and -1 or 1) * record.balance
        ...
        for vals in vals_list:
            ...
            if move._is_invoice():
                ...
                if any(vals.get(field) for field in ACCOUNTING_FIELDS):
                    ...
                    vals.update(self._get_computed_business_vals(
    ...
    def default_get(self, default_fields):
        ...
        if 'account_id' in default_fields \
            ...
            and self._context.get('default_type') in ('out_invoice', 'in_refund', 'out_receipt'):
            ...
            and (self._context.get('journal_id') or self._context.get('default_journal_id')) \
            and not values.get('account_id') \
            and self._context.get('default_type') in ('in_invoice', 'out_refund', 'in_receipt'):
            # Fill missing 'account_id'.
            journal = self.env['account.journal'].browse(self._context.get('default_journal_id') or self._context['journal_id'])
