...
class ProductProduct(models.Model):
    ...
    def _compute_forecasted_without_stock(self):
        ...
        target_state = self.env.context.get("read_transient_state", "draft")

        if not is_sale_installed:
            sale_domain = [
                ('move_id.state', '=', target_state),
                ('move_id.move_type', 'in', ('out_invoice', 'out_refund', 'out_receipt')),
                ('product_id', 'in', self.ids),
                ('product_id.is_storable', '=', True),
            ]
            sale_lines = self.env['account.move.line']._read_group(
                sale_domain,
                ['product_id', 'product_uom_id', 'move_id.move_type'],
                ['quantity:sum'],
            )
        ...
        if not is_purchase_installed:
            ...
            purchase_lines = self.env['account.move.line']._read_group(
                purchase_domain,
                ['product_id', 'product_uom_id', 'move_id.move_type'],
                ['quantity:sum'],
            )
