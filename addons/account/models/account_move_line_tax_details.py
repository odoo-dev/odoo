    ...
    def _get_query_tax_details_simplified(self, table_references, search_condition):
        ...
        return SQL('''
            ...
            tax_data AS (
                ...
                JOIN tax_lines account_move_line
                ...
                WHERE (
                    ...
                    OR sign(base_line.balance) = sign(account_move_line.balance * t.amount * account_move_line.factor_percent)
                ) AND (
                    NOT t.is_base_affected
                    OR NOT t.include_base_amount
                    OR base_line.applied_tax_ids[
                        ARRAY_LENGTH(base_line.applied_tax_ids, 1) - COALESCE(ARRAY_LENGTH(account_move_line.applied_tax_ids, 1), 0):ARRAY_LENGTH(base_line.applied_tax_ids, 1)
                    ] = ARRAY[account_move_line.tax_line_id] || account_move_line.applied_tax_ids
                ) AND (
