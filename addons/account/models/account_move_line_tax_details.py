...
class AccountMoveLine(models.Model):
    ...
    def _get_query_tax_details(self, table_references, search_condition, fallback=True) -> SQL:
        query = SQL(
            ...
            LEFT JOIN account_move_line_account_tax_rel tax_rel ON
                tax_rel.account_tax_id = COALESCE(account_move_line.group_tax_id, account_move_line.tax_line_id)
            LEFT JOIN account_move_line base_line ON
            ...
            WHERE
                ...
                AND json_data.value->>'tax_line_id' IS NOT NULL
                AND json_data.value->>'tax_repartition_line_id' IS NOT NULL
            ''',
