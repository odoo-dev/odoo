...
class AccountMoveLine(models.Model):
    ...
    def _get_query_tax_details(self, table_references, search_condition):
        ...
        Warning:
        This query does not support the `base_affected` tax mechanism. Use it only
        in contexts where no tax affects the base of another tax.
        """
        extra_aml_select_clause, extra_td_where_clause = self._get_tax_query_extra_clauses()
        self.env.cr.execute(SQL("""
            DROP TABLE IF EXISTS filtered_aml_tmp, base_lines_tmp, tax_lines_tmp;
        ...
        return SQL('''
            ...
            matched_tax_data_with_fallback AS (
                /*
            ...
            resolved_tax_data AS (
                SELECT
                    ...
                    tax_exigible
                FROM matched_tax_data_with_fallback
