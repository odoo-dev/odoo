...
class TestAuditTrail(AccountTestInvoicingCommon):
    ...
    def test_content(self):
        ...
        self._assert_contains_all(body, ["Draft", "Posted", "Status"])
        self._assert_contains_all(body, ["None", "MISC/2021/04/0001", "Number"])

        # 2. back to draft
        self.move.button_draft()
        body = self.get_trail(self.move)[0].body
        self._assert_contains_all(body, ["Posted", "Draft", "Status"])

        # 3. name change
        self.move.name = "nawak"
        body = self.get_trail(self.move)[0].body
        self._assert_contains_all(body, ["MISC/2021/04/0001", "nawak", "Number"])
