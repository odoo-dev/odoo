...
class AccountTestInvoicingCommon(ProductCommon):
    ...
    def setUp(self):
        ...
        # We patch globally the _pre_render_qweb_pdf to returns real PDFs instead of fake one when in test mode
        # as the current implementation returns html with PDF mime type which causes a lot of error in various tests.
        def _pre_render_qweb_pdf(self, report_ref, res_ids=None, data=None):
            if not res_ids:
                return {}, 'pdf'
            if isinstance(res_ids, int):
                res_ids = [res_ids]
            return {res_id: {'stream': io.BytesIO(base64.b64decode(PDF)), 'attachment': None}
                    for res_id in res_ids}, 'pdf'

        self.patch(self.env.registry['ir.actions.report'], "_pre_render_qweb_pdf", _pre_render_qweb_pdf)
