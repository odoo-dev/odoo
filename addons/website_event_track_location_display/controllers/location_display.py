...
class EventTrackLocationDisplayController(http.Controller):
    ...
    def _get_location_display_schedule(self, event, location_id, time_data=None):
        ...
        location_name = tracks[:1].location_id.sudo().name if tracks else event.name
    ...
    def _serialize_track(self, track, tz):
        ...
        return {
            ...
            'imageUrl': f'/web/image/event.track/{track.id}/image/256x256' if track.image else False,
            'tags': [
                {
                    'id': tag.id,
                    'name': tag.name,
    ...
    def _get_location_display_data(self, event_id, location_id):
        ...
        return {
            ...
            'nextTrack': self._serialize_track(schedule['next_track'], time_data['event_tz']),
            'upcomingTracks': [
