...
class ResPartner(models.Model):
    ...
    def check_vat(self):
        for partner in self:
            ...
            if partner.vat and not self.run_vat_test(partner.vat, country):
                partner_label = _("partner [%s]", partner.name)
                msg = partner.build_vat_error_message(country and country.code.lower() or None, partner.vat, partner_label)
                raise ValidationError(msg)

    @api.model
    def run_vat_test(self, vat_number, default_country):
        # TODO OCO DOC
        # Get company
    ...
    @api.model
    def build_vat_error_message(self, country_code, wrong_vat, record_label):
        if self.env.context.get('company_id'):
            company = self.env['res.company'].browse(self.env.context['company_id'])
        else:
            company = self.env.company
