...
DEMO_BILL_PATH = 'l10n_dk_edi/tools/demo_bill'
DEMO_ENC_KEY = 'l10n_dk_edi/tools/enc_key'
DEMO_PRIVATE_KEY = 'l10n_dk_edi/tools/private_key.pem'
...
def _mock_make_request(func, self, *args, **kwargs):
    ...
    def _mock_get_all_documents(user, args, kwargs):
        if not user.env['account.move'].search_count([
            ('l10n_dk_edi_message_uuid', '=', f'{user.company_id.id}_demo_vendor_bill')
        ]):
    ...
    def _mock_send_document(user, args, kwargs):
        ...
        get_messages_cron = user.env['ir.cron'].sudo().env.ref(
            'l10n_dk_edi.ir_cron_l10n_dk_edi_get_new_documents',
    ...
    return {
        ...
        'participant_status': lambda _user, _args, _kwargs: {'l10n_dk_edi_state': 'active'},
        'send_document': _mock_send_document,
    }[endpoint](self, args, kwargs)

def _mock_button_verify_partner_endpoint(func, self, *args, **kwargs):
    self.ensure_one()
    self.l10n_dk_edi_validity_last_check = fields.Date.today()
    self.l10n_dk_edi_is_endpoint_valid = True

def _mock_user_creation(func, self, *args, **kwargs):
    func(self, *args, **kwargs)
    self.write({
        'l10n_dk_ediproxy_state': 'active',
    })
    self.l10n_dk_edi_edi_user.write({
...
def _mock_deregister_participant(func, self, *args, **kwargs):
    ...
    demo_moves = self.env['account.move'].search([
        ...
        ('l10n_dk_edi_message_uuid', '=like', 'demo_%'),
    ])
    demo_moves.write({
        'l10n_dk_edi_message_uuid': None,
        'l10n_dk_edi_move_state': None,
    })
    ...
    self.env['account.move'].search([
        ('company_id', '=', self.company_id.id),
        ('l10n_dk_edi_message_uuid', '=', f'{self.company_id.id}_demo_vendor_bill'),
    ]).unlink()

    mode_constraint = self.env['ir.config_parameter'].get_param('l10n_dk_edi.mode_constraint')
    self.l10n_dk_edi_edi_user.unlink()
    self.l10n_dk_edi_proxy_state = 'not_registered'
    self.l10n_dk_edi_mode = mode_constraint
...
_demo_behaviour = {
    ...
    'button_l10n_dk_edi_check_partner_endpoint': _mock_button_verify_partner_endpoint,
    'button_create_l10n_dk_edi_proxy_user': _mock_user_creation,
    'button_deregister_l10n_dk_edi_participant': _mock_deregister_participant,
    'button_update_l10n_dk_edi_user_data': _mock_update_user_data,
}
...
def handle_demo(func, self, *args, **kwargs):
    ...
    def get_demo_mode_account_edi_proxy_client_user(self, args, kwargs):
        if self.id:
            return self.edi_mode == 'demo' and self.proxy_type == 'l10n_dk_edi'
        demo_param = self.env['ir.config_parameter'].get_param('l10n_dk_edi.edi.mode') == 'demo'
        if len(args) > 1 and 'proxy_type' in args[1]:
            return demo_param and args[1]['proxy_type'] == 'l10n_dk_edi'
        return demo_param

    def get_demo_mode_res_config_settings(self, args, kwargs):
        if self.account_peppol_edi_user:
            return self.l10n_dk_edi_user.edi_mode == 'demo'
        return self.l10n_dk_edi_mode == 'demo'

    def get_demo_mode_res_partner(self, args, kwargs):
        l10n_dk_edi_user = self.env.company.account_edi_proxy_client_ids.filtered(lambda user: user.proxy_type == 'l10n_dk_edi')
        if l10n_dk_edi_user:
            return l10n_dk_edi_user.edi_mode == 'demo'
        return False
