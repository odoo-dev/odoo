...
test("applyBestComboSuggestion converts matching lines into a combo for direct combos", async () => {
    const store = await setupSelfPosEnv();
    ...
    patchWithCleanup(comp.dialog, {
        add(component, props) {
            ...
            props.getPayload(props.potentialCombos.applicable[0]);
        },
    });

    await comp.applyBestComboSuggestion();
...
test("applyBestComboSuggestion splits repeated single-free combos into separate combo lines", async () => {
    const store = await setupSelfPosEnv();
    const combo1 = store.models["product.combo"].get(1);
    ...
    patchWithCleanup(comp.dialog, {
        add(component, props) {
            expect(component).toBe(ChooseComboPopup);
            props.getPayload(props.potentialCombos.applicable[1]);
        },
    });
...
test("applyBestComboSuggestion redirects to combo selection for upsell combos", async () => {
    const store = await setupSelfPosEnv();
    await store.addToCart(store.models["product.template"].get(8), 1);
    await store.addToCart(store.models["product.template"].get(10), 1);
    ...
    const comp = await mountWithCleanup(CartPage, {});
    expect(comp.showComboBtn).toBe(true);

    patchWithCleanup(comp.dialog, {
        add(component, props) {
            expect(component).toBe(ChooseComboPopup);
            props.getPayload(props.potentialCombos.upsell[0]);
        },
    });
    patchWithCleanup(comp.router, {
        navigate(route, params, options) {
            expect.step(`${route}:${params.id}:${options.redirctPage}`);
        },
    });

    expect(comp.bestComboSuggestion.product.id).toBe(comboProduct.id);
