...
test("test_preparation_categories_are_loaded: only preparation categories visible", async () => {
    ...
    await waitFor(".category_btn:contains('MOOL')");
    expect(".category_btn:contains('Miscellaneous')").toHaveCount(0);
    expect(".category_btn:contains('Combos')").toHaveCount(0);
    expect(".category_btn:contains('Test Category')").toHaveCount(0);
    expect(".category_btn:contains('Specials')").toHaveCount(0);
    expect(".category_btn:contains('Category 1')").toHaveCount(0);
    expect(".category_btn:contains('Category 2')").toHaveCount(0);
});
...
test("self_order_product_info: info popup shows product description", async () => {
    ...
    await contains(".o_self_product_box:contains('Product Info Test') .product_info_icon").click();
    await animationFrame();
    await waitFor('.modal-body:contains("Nice Product")');
});
...
test("self_combo_selector: Office Combo with attributes + 2 plain items", async () => {
    ...
    await Utils.clickBtn("Add to cart");
    await Utils.clickBtn("Checkout");
    await contains(".btn [data-icon='add']").click();
    await animationFrame();
...
test("self_multi_attribute_selector: multi-check attribute selection", async () => {
    ...
    expect("div h2:contains('Fabric')").toHaveCount(1);
    expect(".self_order_attribute_selection:has(button:count(2))").toHaveCount(2);
    expect(
        ".self_order_attribute_selection:has(button:contains('Leather')):has(button:contains('Custom'))"
    ).toHaveCount(1);
});
...
test("self_order_mobile_special_products_category: special products category hidden", async () => {
    ...
    await waitFor(".category_btn:contains('MOOL')");
    expect(".category_btn:contains('Specials')").toHaveCount(0);
    expect(".self_order_product_name:contains('Special 1')").toHaveCount(0);
});
...
test("self_order_mobile_no_access_token: no access token hides order button", async () => {
    ...
    await Utils.clickOrderNow();
    await Utils.selectLocation("Test-Takeout");
    expect(".btn:contains('Order')").toHaveCount(0);
});
