 ...
 * @param {} getDeps : function that returns a list with one item; this item should be true when the effect should be run and false when it should be stopped
...
function updateEntries(entriesOnScreen, entries) {
    ...
    const newEntriesIntersecting = entries.filter(
        (entry) => entry.isIntersecting && !entriesOnScreen.includes(entry)
    );
    return [
        ...entriesOnScreen.filter(
            (entry) => !entriesNoLongerIntersecting.some((e) => getName(e) === getName(entry))
        ),
        ...newEntriesIntersecting,
    ];
}
