...
from odoo.addons.hr_time.models.hr_time import get_employee_from_context
...
class HrTimeAllocation(models.Model):
    """ Allocation Requests Access specifications: similar to leave requests """
    _name = 'hr.time.allocation'
    ...
    def _default_work_entry_type_id(self):
        if self.env.user.has_group('hr_time.group_hr_time_user'):
    ...
    def _domain_employee_id(self):
        ...
        if not self.env.user.has_group('hr_time.group_hr_time_user'):
            domain += [
    ...
    accrual_plan_id = fields.Many2one('hr.time.accrual.plan', compute="_compute_accrual_plan_id",
    ...
    def _compute_is_officer(self):
        self.is_officer = self.env.user.has_group("hr_time.group_hr_time_user")
    ...
    def _get_future_leaves_on(self, accrual_date):
        ...
        fake_allocation = self.env['hr.time.allocation'].with_context(default_date_from=accrual_date).new(origin=self)
    ...
    def _get_next_states_by_state(self):
        ...
        is_officer = self.env.user.has_group('hr_time.group_hr_time_user')
    ...
    def onchange(self, values, field_names, fields_spec):
        ...
        if values and 'employee_id' in fields_spec and 'employee_id' not in self.env.context:
            ...
            self.with_context(employee_id=employee_id)
    ...
    def create(self, vals_list):
        ...
        allocations = super(HrTimeAllocation, self.with_context(mail_create_nosubscribe=True)).create(vals_list)
    ...
    def action_approve(self):
        ...
        allocation_to_approve = self.env['hr.time.allocation']
        allocation_to_validate = self.env['hr.time.allocation']
        for allocation in self:
    ...
    def _check_approval_update(self, state, raise_if_not_possible=True):
        ...
        if self.env.is_superuser():
            return True
        current_employee = self.env.user.employee_id
        is_administrator = self.env.user.has_group('hr_time.group_hr_time_manager')
        for allocation in self:
    ...
    def activity_update(self):
        to_clean, to_do, to_second_do = self.env['hr.time.allocation'], self.env['hr.time.allocation'], self.env['hr.time.allocation']
        activity_vals = []
        model_id = self.env['ir.model']._get_id('hr.time.allocation')
        confirm_activity = self.env.ref('hr_time.mail_act_leave_allocation_approval')
        approval_activity = self.env.ref('hr_time.mail_act_leave_allocation_second_approval')
        for allocation in self:
        ...
        if to_clean:
            to_clean.activity_unlink(['hr_time.mail_act_leave_allocation_approval'])
        if to_do:
            to_do.activity_feedback(['hr_time.mail_act_leave_allocation_approval', 'hr_time.mail_act_leave_allocation_second_approval'])
        if to_second_do:
            to_second_do.activity_feedback(['hr_time.mail_act_leave_allocation_approval'])
    ...
    def _track_log_get_default_subtype(self, track_init_values):
        if 'state' in track_init_values and self.state == 'validate':
            ...
            return allocation_notif_subtype_id or self.env.ref('hr_time.mt_leave_allocation')
    ...
    def message_subscribe(self, partner_ids=None, subtype_ids=None):
        ...
        if any(state in ['validate'] for state in self.mapped('state')):
            ...
            return super(HrTimeAllocation, self.sudo()).message_subscribe(partner_ids=partner_ids, subtype_ids=subtype_ids)
