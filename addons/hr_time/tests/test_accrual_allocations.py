...
class TestAccrualAllocations(TestHrHolidaysCommon):
    ...
    def setUpClass(cls):
        ...
        cls.accrual_plan_start1 = cls.env['hr.time.accrual.plan'].create({
        ...
        cls.accrual_plan_end1 = cls.env['hr.time.accrual.plan'].create({
    ...
    def test_check_gain(self):
        ...
        with freeze_time('2021-08-30'):
            ...
            accrual_plan_not_based_on_worked_time = self.env['hr.time.accrual.plan'].with_context(tracking_disable=True).create({
                'name': 'Accrual Plan For Test',
                'can_be_carryover': True,
                'level_ids': [(0, 0, {
                    'added_value_type': 'day',
            ...
            accrual_plan_based_on_worked_time = self.env['hr.time.accrual.plan'].with_context(tracking_disable=True).create({
                'is_based_on_worked_time': True,
                'can_be_carryover': True,
                'level_ids': [(0, 0, {
                    'added_value_type': 'day',
            ...
            allocation_not_worked_time = self.env['hr.time.allocation'].with_user(self.user_hrmanager_id).with_context(tracking_disable=True).create({
                'name': 'Accrual allocation for employee',
            ...
            allocation_worked_time = self.env['hr.time.allocation'].with_user(self.user_hrmanager_id).with_context(tracking_disable=True).create({
                'name': 'Accrual allocation for employee',
        ...
        with freeze_time(next_date):
            ...
            self.env['hr.time.allocation']._update_accrual()
            # Prorated
    ...
    def test_non_elligible_leaves(self):
        ...
        timeoff = self.env['hr.time'].create({
            'name': 'Paid Time Off',
            'employee_id': self.employee_emp.id,
    ...
    def test_elligible_leaves(self):
        ...
        timeoff_eligible = self.env['hr.time'].create({
            'name': 'Paid Time Off',
            'employee_id': self.employee_emp.id,
    ...
    def test_worked_leaves(self):
        ...
        remote_work = self.env['hr.time'].create({
            'name': 'Remote Work',
            'employee_id': self.employee_emp.id,
    ...
    def test_accrual_transition_immediately(self):
        with freeze_time("2017-12-05"):
            ...
            second_level = self.env['hr.time.accrual.level'].search([('accrual_plan_id', '=', accrual_plan.id), ('start_count', '=', 10)])
    ...
    def test_unused_accrual_lost(self):
        ...
        accrual_cron = self.env['ir.cron'].sudo().env.ref('hr_time.hr_time_allocation_cron_accrual')
    ...
    def test_accrual_creation_on_anterior_date(self):
        ...
        with freeze_time('2023-09-01'):
            accrual_allocation = self.env['hr.time.allocation'].new({
    ...
    def test_accrual_period_start_past_start_date(self):
        ...
        with freeze_time('2024-03-01'):
            with Form(self.env['hr.time.allocation'].with_user(self.user_hrmanager)) as f:
    ...
    def test_cancel_invalid_leaves_with_regular_and_accrual_allocations(self):
        ...
        with freeze_time('2024-05-06'):
            self.env['hr.time']._cancel_invalid_leaves()
    ...
    def test_accrual_leaves_cancel_cron(self):
        ...
        with freeze_time("2024-01-01"):
            ...
            excess_leave = self.env['hr.time'].create([{
                'employee_id': self.employee_emp.id,
                'work_entry_type_id': work_entry_type_no_negative.id,
                'request_date_from': '2024-01-05',
                'request_date_to': '2024-01-05',
            }])
            allowed_negative_leave = self.env['hr.time'].create([{
                'employee_id': self.employee_emp.id,
                'work_entry_type_id': work_entry_type_negative.id,
    ...
    def test_check_lastcall_change_regular_to_accrual(self):
        with freeze_time("2017-12-05"):
            ...
            allocation = self.env['hr.time.allocation'].with_context(tracking_disable=True).create({
                'name': 'Accrual allocation for employee',
                'employee_id': self.employee_emp.id,
                'work_entry_type_id': self.work_entry_type.id,
                'number_of_days': 10,
    ...
    def test_accrual_unused_accrual_reset_to_lost(self):
        ...
        plan = self.env["hr.time.accrual.level"].with_context(tracking_disable=True).create({
            "accrual_plan_id": accrual_plan.id,
        })
    ...
    def test_accrual_allocation_constraint_1(self):
        with self.assertRaises(ValidationError):
            self.env['hr.time.accrual.plan'].create({
                'name': 'Accrual Plan with no carryover',
                'accrued_gain_time': 'start',
                'carryover_date': 'year_start',
                'level_ids': [Command.create({
    ...
    def test_accrual_allocation_with_monthly_31st_milestone(self):
        ...
        with (freeze_time('2025-01-31')):
            allocation = self.env['hr.time.allocation'].new({
    ...
    def test_get_future_leaves_on(self):
        ...
        allocation_day = self.env['hr.time.allocation'].create({
            'name': 'Daily Accrual',
        ...
        allocation_hour = self.env['hr.time.allocation'].create({
