...
class GoogleGmailToken(models.Model):
    ...
    _sql_constraints = [
        ('unique_email_gmail_token', 'UNIQUE(email)', 'Only one token per account'),
    ]
    ...
    def _fetch_gmail_access_token_iap(self, refresh_token):
        ...
        if not response.ok:
            _logger.error('Can not contact IAP: %r.', response.text)
            raise UserError(_('Can not contact IAP.'))

        response = response.json()
        if 'error' in response:
            raise UserError(_('An error occurred: %r.', response['error']))
