...
class LunchSupplier(models.Model):
    ...
    def _sync_cron(self):
        for supplier in self:
            ...
            if ((lc and sendat_tz.date() <= fields.Datetime.context_timestamp(supplier, lc).date())
                or (not lc and sendat_tz <= fields.Datetime.context_timestamp(supplier, fields.Datetime.now()))):
    ...
    def write(self, values):
        ...
        cron_depends = {
            'name', 'active', 'send_by',
            'automatic_email_time', 'moment', 'tz',
        }
