...
class LunchAlert(models.Model):
    ...
    def _sync_cron(self):
        ...
        for alert in self:
            ...
            if ((lc and sendat_tz.date() <= fields.Datetime.context_timestamp(alert, lc).date())
                or (not lc and sendat_tz <= fields.Datetime.context_timestamp(alert, fields.Datetime.now()))):
    ...
    def write(self, values):
        ...
        cron_depends = {
            'name', 'active', 'mode', 'until',
            'notification_time', 'notification_moment', 'tz',
        }
        if set(values) & cron_depends:
            self._sync_cron()
