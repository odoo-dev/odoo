...
class LunchController(http.Controller):
    ...
    def _make_infos(self, user, **kwargs):
        ...
        is_manager = request.env.has_group('lunch.group_lunch_manager')
    ...
    def _check_user_impersonification(self, user_id=None):
        if (user_id and request.env.uid != user_id and not request.env.has_group('lunch.group_lunch_manager')):
