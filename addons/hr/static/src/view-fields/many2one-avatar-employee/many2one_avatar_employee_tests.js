/** @odoo-module alias=hr.viewFields.Many2OneAvatarEmployee.tests **/

import Many2OneAvatarEmployee from 'hr.viewFields.Many2OneAvatarEmployee';
...
QUnit.module('Many2OneAvatarEmployee', {}, function () {
QUnit.module('tests', {
    beforeEach() {
        beforeEach(this);
...
QUnit.test('many2one_avatar_employee widget in list view', async function (assert) {
    ...
    await afterNextRender(
        () => click(list.$('.o_data_cell:nth(0) .o_m2o_avatar')),
    );
    assert.verifySteps(
    ...
    await afterNextRender(
        () => click(list.$('.o_data_cell:nth(1) .o_m2o_avatar')),
    );
    assert.verifySteps(
    ...
    await afterNextRender(
        () => click(list.$('.o_data_cell:nth(2) .o_m2o_avatar')),
    );
    assert.verifySteps(
        [],
...
QUnit.test('many2one_avatar_employee widget in kanban view', async function (assert) {
    ...
    assert.strictEqual(
        ...
        '/web/image/hr.employee.public/7/image_128',
    );
    assert.strictEqual(
    ...
    assert.strictEqual(
        ...
        '/web/image/hr.employee.public/23/image_128',
    );
...
QUnit.test('many2one_avatar_employee: click on an employee not associated with a user', async function (assert) {
    ...
    assert.strictEqual(
        form.$('.o_field_widget[name=employee_id]').text().trim(),
        "Mario",
    );

    await click(form.$('.o_m2o_avatar'));
    assert.verifySteps([
        'read foo 1',
        'read hr.employee.public 11',
    ]);
