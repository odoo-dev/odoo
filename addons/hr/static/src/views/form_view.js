        ...
        if (record._config.resId) {
            ...
            const version_changes = Object.fromEntries(
                Object.entries(changes).filter(
                    ([key]) =>
                        record.fields[key].related?.includes("version_id") &&
                        !["contract_date_start", "contract_date_end"].includes(
                            record.fields[key].name
                        )
                )
            );
            ...
            if (
                Object.keys(version_changes).length > -1 &&
                record._values.version_id.id !== ver_ids.at(-1)
            ) {
                const version_changes_to_display = await this.orm.call(
                    "hr.employee",
                ...
                const proceed = await new Promise((resolve) => {
                    this.dialogService.add(
                        MultiVersionUpdateConfirmationDialog,
                        {
                            ...
                            change_multi: async () => {
                                const versions_to_update = ver_ids.slice(
                                    ver_ids.indexOf(record._values.version_id.id)
                                );
                                await this.orm.call("hr.version", "write", [
                                    versions_to_update.slice(1),
                                    version_changes,
                                ]);
                            ...
                            switch (result?.reason) {
                                ...
                                case "new_contract": {
                                    ...
                                    if (
                                        ...
                                        contractDateEnd < previousContractDateEnd
                                    ) {
                                    ...
                                    this.pendingNewContract = {
                                        ...
                                        contract_date_end: newContractDateEnd
                                            ? serializeDate(newContractDateEnd)
                                            : false,
                                ...
                                case "discard":
                                default: {
                                    changes.contract_date_end = previousContractDateEnd
                                        ? serializeDate(previousContractDateEnd)
                                        : false;
                                    break;
                                }
                            }
            ...
            record.model._updateConfig(
                record.config,
                {
                    context: Object.fromEntries(
                        Object.entries(record.config.context).filter(
                            ([key]) => key !== "multi_update_version_ids"
                        )
                    ),
                },
                { reload: false }
