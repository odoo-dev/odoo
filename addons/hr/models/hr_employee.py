...
class HrEmployeePrivate(models.Model):
    ...
    work_permit_expire = fields.Date('Work Permit Expiration Date', groups="hr.group_hr_user", tracking=True)
    work_permit = fields.Binary(string="Work Permit", groups="hr.group_hr_user", tracking=True)
    work_permit_activity_scheduled = fields.Boolean(default=False, groups="hr.group_hr_user")
    ...
    def _cron_check_work_permit_validity(self):
        ...
        nearly_expired_work_permits = self.search([('work_permit_activity_scheduled', "=", False), ('work_permit_expire', '<', outdated_days)])
        for employee in nearly_expired_work_permits.filtered(lambda employee: employee.parent_id):
            ...
            formated_date = format_date(employee.env, employee.work_permit_expire, date_format="dd MMMM y", lang_code=lang)
        ...
        nearly_expired_work_permits.write({'work_permit_activity_scheduled': True})
    ...
    def write(self, vals):
        ...
        if 'work_permit_expire' in vals:
            vals['work_permit_activity_scheduled'] = False
