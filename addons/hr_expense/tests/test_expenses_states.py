...
class TestExpensesStates(TestExpenseCommon, MailCase):
    ...
    def test_expense_next_activity(self):
        ...
        expenses_submitted = self.env['mail.mail'].search([('email_to', '=', self.expense_user_manager_2.email)])
        self.assertEqual(expenses_submitted.subject, "New expenses waiting for your approval")

    def test_expense_no_notification_on_state_change(self):
        """ Test no notification is sent to the manager when the state changes"""
        context_mail = {'tracking_disable': False}
        self.expenses_employee = self.expenses_employee.with_context(context_mail)
        self.expenses_employee.manager_id = self.expense_user_manager_2
        with self.mock_mail_gateway():
            # Submission is not tracked by a subtype
            self.expenses_employee.action_submit()
            self.expenses_employee.action_approve()
            self.flush_tracking()
        mail = self.env['mail.mail'].sudo().search(
            [('notified_partner_ids', 'in', self.expense_user_manager_2.partner_id.ids),
            ('subtype_id', '=', self.env.ref('hr_expense.mt_expense_approved').id)]
        )
