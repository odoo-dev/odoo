...
class HrEmployee(models.Model):
    ...
    def _group_hr_expense_user_domain(self):
        ...
        group_manager = self.env.ref('hr_expense.group_hr_expense_user', raise_if_not_found=False)
        return [
            '|', '&', ('groups_id', 'in', group.ids), ('id', 'parent_of', self.ids),
                 ('groups_id', 'in', group_manager.ids)
        ] if group else []
    ...
    def _compute_expense_manager(self):
        for employee in self:
            ...
            if (
                    manager
                    and manager.has_group('hr_expense.group_hr_expense_team_approver')
