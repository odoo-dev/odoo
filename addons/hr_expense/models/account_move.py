    ...
    def _creation_message(self):
        if self.expense_ids:
            if len(self.expense_ids) == 1:
                return _("Expense entry created from the expense: %(link)s", link=self.expense_ids._get_html_link())
            # Spaces are important
            links = ', '.join((_("%(expense_name)s", expense_name=expense._get_html_link()) for expense in self.expense_ids))
            return Markup(_("Expense entry created from the expenses: %(links)s", links=links))
