...
class L10nJpTotalAverageCostWizard(models.TransientModel):
    ...
    def _get_evaluation_batches(self, products, moves):
        ...
        orders themselves say what came out of what, which a bill of materials
        only approximates and a by-product is missing from entirely.
        """
        ...
        def depth(product):
            if product not in depths:
                if product in walked:
                    ...
                    # and core only forbids a loop through the components of a bill of
                    # materials, never one closed by a by-product
