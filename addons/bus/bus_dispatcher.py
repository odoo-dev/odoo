...
from .tools.misc import acquire_cursor, hashable
...
# How many seconds a worker holds a failed batch before requeueing it (prevent workers
# from retrying at full speed on an unreachable DB).
BATCH_RETRY_DELAY = 5
# How many seconds to sleep after an unexpected error within the worker/dispatcher loop.
ERROR_RETRY_DELAY = 25
...
class ChannelTopic:
    ...
    def _forward_notifications(notifications: list, websockets: Iterable[Websocket]):
        ...
        for websocket in websockets:
            ...
            # to `CLOSING/CLOSED` or ``OSError`` if the ``PollableQueue`` socket was
            # closed.
            with contextlib.suppress(InvalidStateException, OSError):
    ...
    def dispatch_notifications(self, notifications: list):
        ...
        for websocket, last_id in self._waiting_room_snapshot.items():
            catchup_from_idx = bisect_right(notifications, last_id, key=lambda n: n["id"])
        ...
        new_from_idx = bisect_right(notifications, self._last_fetched_id, key=lambda n: n["id"])
...
class BusDispatcher(threading.Thread):
    ...
    def _run_with_retry(self, fn: Callable[[], None], label: str):
        while not stop_event.is_set():
            try:
                ...
                stop_event.wait(ERROR_RETRY_DELAY)
            ...
            while not stop_event.is_set():
                ...
                while conn.notifies:
                    for channel in orjson.loads(conn.notifies.pop().payload):
                        if isinstance(channel, list):  # Protect against malformed channels.
                            channels_by_dbname[channel[0]].append(hashable(channel))
                        else:
                            _logger.warning("Unexpected payload", channel)
    ...
    def _worker_loop(self):
        while not stop_event.is_set():
            ...
            try:
                ...
                success = True
            except (InterfaceError, OperationalError, PoolError):
                # Temporary database/pool failure: retry quickly instead of falling back
                # to the long retry delay used for unexpected errors.
    ...
    def _release_claims(self, dbname: str, topics: set[ChannelTopic], *, rollback=False):
    ...
    def _fetch(self, cr, topics: set[ChannelTopic]):
    ...
    def _dispatch(self, topics: set[ChannelTopic], notifications_by_channel: dict[tuple, list]):
        if not notifications_by_channel:
            return
        for topic in topics:
            topic.dispatch_notifications(notifications_by_channel.get(topic._channel, []))

    def _kick_invalid_sessions(self, cr, topics: set[ChannelTopic]):
        ...
        for websocket in invalid:
            if websocket.state is ConnectionState.OPEN:
                with contextlib.suppress(OSError):  # Closed in the meantime.
