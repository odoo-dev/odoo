...
export class BusService {
    ...
    /** @private @type {"idle" | "started" | "stopped" | "unregistered"}*/
    _state = "idle";
    /**
     * @private
    ...
    constructor(env, services) {
        this._env = env;
        this._busParametersService = services["bus.parameters"];
    ...
    async _ensureWorkerInitialized() {
        if (!this._initializedResolvers) {
            ...
            this._workerService.send("BUS:INITIALIZE_CONNECTION", {
                websocketURL: `${this._busParametersService.serverURL.replace(
                    "http",
                    "ws"
                )}/websocket?version=${session.websocket_worker_version}`,
                db: session.db,
    ...
    subscribe(notificationType, callback) {
        ...
        const key = `${notificationType}_${callback}`;
        this._subscribeFnToWrapper.set(key, wrapper);
    ...
    unsubscribe(notificationType, callback) {
        const key = `${notificationType}_${callback}`;
        const wrapper = this._subscribeFnToWrapper.get(key);
        if (wrapper) {
            this._notificationBus.removeEventListener(notificationType, wrapper);
            this._subscribeFnToWrapper.delete(key);
        }
    }
}
