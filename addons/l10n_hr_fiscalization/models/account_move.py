...
class AccountMove(models.Model):
    ...
    def _l10n_hr_change_payment_method(self, new_payment_method, new_recipient_oib=None, change_oib=False):
        ...
        if self.l10n_hr_fiscalization_status != '0' or not self.l10n_hr_fiscalization_jir:
            return {
                'success': False,
                'error': _("Only fiscalized invoices can have their data changed.")
            }
        ...
        if fiscalization_date != today:
            return {
                'success': False,
                'error': _("Invoice data can only be changed on the same day the invoice was fiscalized.")
            }

        if new_payment_method == 'T' and change_oib and new_recipient_oib:
            return {
                'success': False,
                'error': _("Recipient OIB cannot be sent with payment method 'Transakcijski račun' (T).")
            }
        ...
        try:
            ...
            if response['success']:
                return {
                    ...
                    'details': _(response.get('errors', [])),
                    'invoice_details': response.get('invoice_details', {})
                }
            else:
                return {
                    'success': False,
                    'message': _("Fiscalization check failed: %s", response['error']),
                    'details': _(response.get('errors', [])),
                    'invoice_details': response.get('invoice_details', {})
                }
        except Exception as e:  # noqa: BLE001
