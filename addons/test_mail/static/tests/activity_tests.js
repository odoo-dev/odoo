...
QUnit.module("test_mail", {}, function () {
    ...
    QUnit.test("activity view: simple activity rendering", async function (assert) {
        ...
        assert.hasAttrValue(
            $th1.find(".o_kanban_counter_progress .progress-bar:first"),
            "data-bs-original-title",
            "1 Planned",
            "the counter progressbars should be correctly displayed"
        );
        assert.hasAttrValue(
            $th1.find(".o_kanban_counter_progress .progress-bar:nth-child(2)"),
            "data-bs-original-title",
            "1 Today",
            "the counter progressbars should be correctly displayed"
        );
        ...
        assert.hasAttrValue(
            $th2.find(".o_kanban_counter_progress .progress-bar:nth-child(3)"),
            "data-bs-original-title",
            "1 Overdue",
            "the counter progressbars should be correctly displayed"
        );
    ...
    QUnit.test("activity view: activity widget", async function (assert) {
        ...
        await testUtils.dom.click(
            document.querySelector(".today .o-activity-cell-closest-deadline")
        );
        assert.containsOnce(
            document.body,
        ...
        await testUtils.dom.click(
            document.querySelector(".overdue .o-activity-cell-closest-deadline")
        );
        assert.containsNone(
            document.body,
        ...
        await testUtils.dom.click(
            document.querySelector(".o-activity-list-popover-add-activity-button")
        );
        await testUtils.dom.click(
            document.querySelector(".overdue .o-activity-cell-closest-deadline")
        );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            assert.containsOnce(
                document.body,
                `tr[data-res-id=${mailTestActivityId1}] .o_m2o_avatar > img[data-src="/web/image/res.users/${resUsersId1}/avatar_128"]`,
    ...
    QUnit.test("Activity view: apply progressbar filter", async function (assert) {
        ...
        await testUtils.dom.click(
            document.querySelector(
                '.o_kanban_counter_progress .progress-bar[data-filter="planned"]'
            )
        );
        assert.containsOnce(
            document.querySelector(".o_activity_view thead"),
