...
QUnit.module("Views", (hooks) => {
  ...
  QUnit.test("simple bar chart rendering", async function (assert) {
    ...
    assert.strictEqual(graph.model.metaData.mode, "bar", "should be in bar chart mode by default");
  ...
  QUnit.test(
    ...
    async function (assert) {
      ...
      checkLabels(assert, graph, ["W5 2021, W1 2021", "W7 2021, W2 2021", "W3 2021", "W4 2021"]);
      checkDatasets(
        assert,
        graph,
        ["backgroundColor", "borderColor", "data", "label"],
      ...
      checkLegend(assert, graph, ["W5 2021, W1 2021", "W7 2021, W2 2021", "W3 2021", "W4 2021"]);
      checkTooltip(
        assert,
        graph,
        {
  ...
  QUnit.test("pie chart rendering (no data, several domains)", async function (assert) {
    ...
    checkTooltip(assert, graph, { lines: [{ label: "True group / xphone", value: "1.00" }] }, 0, 0);
    checkTooltip(
      assert,
      graph,
  ...
  QUnit.test("pie chart rendering (mix of positive and negative values)", async function (assert) {
    serverData.models.foo.records = [
      { bar: true, revenue: 2 },
      { bar: false, revenue: -3 },
    ];
  ...
  QUnit.skip("props modifications", async function (assert) {
    /** @todo find the right way to instantiate the graph view */
    ...
    Parent.template = xml`<View type="graph" t-props="state" t-ref="graph"/>`;
    ...
    assert.strictEqual(parent.graph.comp.model.metaData.mode, "line", "should be in 'line' mode");
    assert.strictEqual(parent.graph.comp.model.metaData.groupBy[0].toJSON(), "bar");
  ...
  QUnit.test("switching measure", async function (assert) {
    ...
    function checkMeasure(measure) {
      ...
      const item = findItem(graph, ".o_cp_bottom_left .o_dropdown ul .o_dropdown_item", measure);
