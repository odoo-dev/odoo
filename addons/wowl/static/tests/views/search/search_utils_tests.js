...
import { constructDateDomain } from "../../../src/views/search/search_utils";
import { patchDate } from "./search_model_tests";
QUnit.module("SearchUtils", function () {
  QUnit.module("Construct domain");
  QUnit.test("construct simple domain based on date field (no comparisonOptionId)", function (
    assert
  ) {
    assert.expect(4);
    const unpatchDate = patchDate(2020, 5, 1, 13, 0, 0);
    const referenceMoment = DateTime.utc();
    assert.deepEqual(constructDateDomain(referenceMoment, "date_field", "date", [], "ltr"), {
      domain: new Domain(`[]`),
      description: "",
    });
    assert.deepEqual(
    ...
    assert.deepEqual(
      constructDateDomain(referenceMoment, "date_field", "date", ["this_year"], "ltr"),
      {
        domain: new Domain(
  ...
  QUnit.test("construct simple domain based on datetime field (no comparisonOptionId)", function (
    assert
  ) {
    assert.expect(3);
    const unpatchDate = patchDate(2020, 5, 1, 13, 0, 0);
    ...
    assert.deepEqual(
      constructDateDomain(referenceMoment, "date_field", "datetime", ["this_year"], "ltr"),
      {
        domain: new Domain(
  ...
  QUnit.test("construct domain based on date field (no comparisonOptionId)", function (assert) {
    ...
    assert.deepEqual(
      constructDateDomain(
        ...
        "date_field",
        "date",
        ["this_year", "this_month", "antepenultimate_month"],
        "ltr"
      ),
      {
        domain: new Domain(
  ...
  //     });
  //     assert.deepEqual(
  //         constructDateDomain(referenceMoment, 'date_field', 'date', ['second_quarter', 'this_year'], "ltr"),
  //         {
  //             domain: new Domain(`["&", ["date_field", ">=", "2020-04-01"], ["date_field", "<=", "2020-06-30"]]`),
  ...
  //     });
  //     assert.deepEqual(
  //         constructDateDomain(referenceMoment, 'date_field', 'date', ['second_quarter', 'this_year'], "rtl"),
  //         {
  //             domain: new Domain(`["&", ["date_field", ">=", "2020-04-01"], ["date_field", "<=", "2020-06-30"]]`),
  ...
  //     testUtils.mock.unpatch(_t.database.db);
  // });
  // QUnit.skip("Moment.js localization does not affect formatted domain dates", async function (assert) {
  //     // assert.expect(1);
  //     // const unpatchDate = patchDate(2020, 5, 1, 13, 0, 0);
