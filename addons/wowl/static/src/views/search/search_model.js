...
export const SEARCH_MENU_TYPES_KEY = Symbol("searchMenuTypes");
export const ON_SAVE_PARAMS_KEY = Symbol("onSaveParams");
...
export function useSearch(params = {}) {
  ...
  const searchModel = new SearchModel(env, config);

  useBus(searchModel, "update", params.onUpdate || component.render);
...
export class SearchModel extends EventBus {
  constructor(env, config) {
    ...
    this.searchMenuTypes = new Set(
      config[SEARCH_MENU_TYPES_KEY] || ["filter", "groupBy", "favorite"]
    );
    this.onSaveParams = config[ON_SAVE_PARAMS_KEY];

    const { context, domain, dynamicFilters, model } = config;
