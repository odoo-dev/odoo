...
// ======= LEGACY =======
import { legacySetupProm } from "@wowl/legacy/legacy_setup";
...
export const barcodeService = {
  ...
  deploy(env) {
    ...
    hotkey.attachInterceptor(intercept);
    function intercept(infos) {
      if (!started) {
        return { intercepted: false };
      }

      // Clear any pending timeout as we intercepted a new hotkey
    ...
    function verifyBuffer() {
      ...
      if (match) {
        ...
        resolver({ intercepted: true });
      } else {
        resolver({ intercepted: false });
      }
    }
...
serviceRegistry.add(barcodeService.name, barcodeService);
