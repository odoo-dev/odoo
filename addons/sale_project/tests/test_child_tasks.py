...
class TestNestedTaskUpdate(TransactionCase):
    ...
    def test_associate_copied_task_to_copied_project(self):
        ...
        parent = self.env['project.task'].create({'name': 'parent task', 'project_template_id': project_tempalte.id})
        child = self.env['project.task'].create({'name': 'child task', 'parent_id': parent.id, 'project_template_id': project_tempalte.id})
        ...
        self.assertFalse(super_project.tasks & project_tempalte.task_ids)
