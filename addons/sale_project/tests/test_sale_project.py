...
class TestSaleProject(HttpCase, TestSaleProjectCommon):
    ...
    def test_task_global_project_service_takes_project_on_so(self):
        self.product_order_service2.project_id = False
        so = self.env['sale.order'].create({
            'partner_id': self.partner.id,
            'order_line': [
                Command.create({'product_id': self.product_order_service2.id}),
                Command.create({'product_id': self.product_order_service3.id}),
            ],
        })
        so_line_task_global_project = so.order_line.filtered(lambda sol: sol.product_id == self.product_order_service2)
        so_line_new_project = so.order_line.filtered(lambda sol: sol.product_id == self.product_order_service3)
        so.action_confirm()
        self.assertEqual(
            so.project_id,
            so_line_new_project.project_id,
            "The project of the SO should be set to the project that was generated by so_line_new_project at SO confirmation."
        )
        self.assertEqual(
            so.project_id,
            so_line_task_global_project.task_id.project_id,
            "The project of the task of so_line_task_global_project should be set to the project of the SO."
        )

    def test_task_global_project_service_no_project_on_so(self):
        self.product_order_service2.project_id = False
        so = self.env['sale.order'].create({
            'partner_id': self.partner.id,
    ...
    def test_so_project_set_on_confirm_in_batch(self):
        ...
        self.assertEqual(
            ...
            "The project of SO1 should be set to the project that was generated by its SOL at SO confirmation."
        )
        self.assertEqual(
            so2.project_id,
            so2.order_line.project_id,
            "The project of SO2 should be set to the project that was generated by its SOL at SO confirmation."
        )
