...
async function mountFilterValuesList({
    items,
    ...
    await mountWithCleanup(FilterValuesList, {
        props: {
            items,
            searchableParentRelations,
            onFilterChange,
...
test("basic text filter", async function () {
    ...
    await mountFilterValuesList({ items: [{ globalFilter, value: undefined }], model });
