...
class TestOnboardingTours(HttpCase):
    ...
    def setUp(self):
        ...
        # A configured instance always has an alias domain to send/receive
        # emails, a bare test database doesn't have one.
        if not company.alias_domain_id:
            company.alias_domain_id = self.env['mail.alias.domain'].create({
                'name': 'yourcompany.example.com',
            })
