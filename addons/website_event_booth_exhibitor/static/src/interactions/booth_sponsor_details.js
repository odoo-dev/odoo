...
export class BoothSponsorDetails extends Interaction {
    ...
    onClickContactDetails(ev) {
        ...
        const sponsorInfoEls = this.el.querySelector("label[for='sponsor_name'] > .mandatory_mark, label[for='sponsor_email'] > .mandatory_mark");
        for (const sponsorInfoEl of sponsorInfoEls) {
            sponsorInfoEl.classList.toggle("d-none", this.useContactDetails);
        }

        const contactInfoEls = this.el.querySelector("input[name='contact_name'], input[name='contact_email']");
