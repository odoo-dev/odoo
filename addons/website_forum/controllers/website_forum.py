...
class WebsiteForum(WebsiteProfile):
    ...
    def _get_forum_post_search_options(self, forum=None, tag=None, filters=None, my=None,
    ...
    def forum_post(self, forum, **post):
        ...
        if not user.email or not tools.single_email_re.match(user.email):
            slug = request.env['ir.http']._slug
            return request.redirect(f"/forum/{slug(forum)}/user/{request.session.uid}/edit?email_required=1")
    ...
    def _prepare_user_profile_values(self, user, **post):
        ...
        if not post.get('no_forum'):
            if post.get('forum'):
                ...
                values.update({
                    'edit_button_url_param': f'forum_id={post["forum_id"]}',
