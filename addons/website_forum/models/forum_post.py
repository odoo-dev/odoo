...
class ForumPost(models.Model):
    ...
    def _compute_is_follower(self):
        followed = self.sudo().search([
            ('id', 'in', self.ids),
            ('follower_ids', '=', self.env.user.partner_id.id),
        ])
    ...
    def _inverse_is_follower(self):
        for post in self:
            if post.is_follower:
                post.sudo().follower_ids |= self.env.user.partner_id
            else:
                post.sudo().follower_ids -= self.env.user.partner_id
    ...
    def _notify_state_update(self):
        for post in self:
            ...
            if post.state == 'active' and post.parent_id:
                ...
                partner_ids = (tag_partners | post.parent_id.follower_ids).ids
    ...
    @api.model
    def convert_comment_to_answer(self, comment):
