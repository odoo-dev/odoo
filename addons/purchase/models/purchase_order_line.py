...
class PurchaseOrderLine(models.Model):
    ...
    def _compute_display_name(self):
        if self.env.context.get('formatted_display_name') and self.env.context.get('display_order_name'):
            current_aml = self.env['account.move.line'].browse(self.env.context.get('purchase_matching_aml_id')).exists()
            for line in self:
                ...
                quantity = float_round(quantity, precision_rounding=line.uom_id.rounding)
