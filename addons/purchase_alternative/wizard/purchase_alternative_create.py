...
class PurchaseAlternativeCreate(models.TransientModel):
    ...
    copy_products = fields.Boolean(
        ...
        help="If this is checked, the order lines of the original PO will be copied.")
    ...
    def _compute_purchase_warn_msg(self):
        ...
        if not self.env.user.has_group('purchase.group_warning_purchase'):
            return
        # If partner has no warning, check its parent/company
        partner = self.partner_id.purchase_warn_msg and self.partner_id or self.partner_id.parent_id
