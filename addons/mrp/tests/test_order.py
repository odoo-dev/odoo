...
class TestMrpOrder(TestMrpCommon, MailCase):
    ...
    def test_end_date(self):
        ...
        self.flush_tracking()
        new_date_finished = datetime(2022, 6, 28, 18, 0)
        with self.mock_mail_gateway(), self.mock_mail_app():
            mo.write({'date_finished': new_date_finished})
            self.flush_tracking()
        self.assertMessageFields(self._new_msgs, {
            ...
            'tracking_values': [
                ('date_finished', 'datetime', datetime(2022, 6, 28, 8, 0), new_date_finished),
            ],
        })
    ...
    def test_replan_mo_after_updating_bom(self):
        ...
        self.assertRecordValues(mo.workorder_ids.sorted('sequence'), [
            {'date_start': datetime(2025, 10, 1, 8, 0)},
            {'date_start': datetime(2025, 10, 1, 9, 30)},
    ...
    def test_reset_to_draft_validated_mo(self):
        """The test assures resetting a validated basic MO to draft reverse its done moves peroperly
        and user is able to re-confirm and produce normally.
        """
        ...
        original_done_moves = (mo.move_raw_ids | mo.move_finished_ids).filtered(lambda m: m.state == 'done')

        mo.action_reset_to_draft()
        self.assertEqual(mo.state, 'draft')

        # components back in stock, finished product removed from stock.
        ...
        # move_raw_ids are back to draft
        self.assertTrue(all(m.state == 'draft' for m in mo.move_raw_ids))
        self.assertFalse(mo.move_raw_ids.filtered(lambda m: m.state == 'done'))
        # The original done moves are only detached from the MO, not deleted
        self.assertTrue(original_done_moves, "the original done moves still exist in the database for reports")
