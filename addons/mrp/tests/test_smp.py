...
class TestMrpSerialMassProduce(TestMrpCommon):
    ...
    def test_smp_produce_all(self):
        ...
        self.assertEqual(len(mo.all_production_ids), count)
        # Check generated serial numbers
        self.assertEqual(mo.all_production_ids.lot_producing_id.mapped('name'), ["sn#1", "sn#2", "sn#3", "sn#4", "sn#5"])
    ...
    def test_smp_produce_complex(self):
        ...
        self.assertEqual(mo.all_production_ids[0].move_raw_ids[1].move_lines_count, 1)
        self.assertEqual(mo.all_production_ids[1].move_raw_ids[1].move_lines_count, 1)
        # 3rd MO should have 2 move lines (2 different lots) for product_to_use_1
        self.assertEqual(mo.all_production_ids[2].move_raw_ids[1].move_lines_count, 2)

        # Verify mark as done

        mos = mo.all_production_ids
    ...
    def test_mass_produce_with_tracked_product(self):
        ...
        self.assertEqual(mo.state, "confirmed")
        # Each generated serial number should have its own mo
        self.assertEqual(len(mo.all_production_ids), 2)
        # Check generated serial numbers
        self.assertEqual(mo.all_production_ids.lot_producing_id.mapped('name'), ["sn#3", "sn#4"])
        #check byproduct quantity
        self.assertEqual(mo.all_production_ids.move_byproduct_ids.mapped('quantity'), [1, 1])
        # check the component quantity
        self.assertEqual(mo.all_production_ids.move_raw_ids.mapped('quantity'), [1, 1])
        # Mark the MOs as done
        mo.all_production_ids.move_raw_ids.picked = True
        mo.all_production_ids.button_mark_done()
        self.assertEqual(mo.all_production_ids.mapped('state'), ['done', 'done'])
    ...
    def test_mass_produce_with_tracked_product_2(self):
        ...
        # Each generated serial number should have its own mo
        self.assertEqual(mo.all_production_ids.lot_producing_id.mapped('name'), ["sn#5", "sn#6", "sn#7"])
        # check the component quantity
        self.assertRecordValues(mo.all_production_ids.move_raw_ids, [
        ...
        mo.all_production_ids.button_mark_done()
        self.assertEqual(mo.all_production_ids.mapped('state'), ['done', 'done', 'done'])
    ...
    def test_smp_produce_with_consumable_component(self):
        ...
        self.assertEqual(mo.state, "confirmed")
        # Each generated serial number should have its own mo
        self.assertEqual(len(mo.all_production_ids), 12)
