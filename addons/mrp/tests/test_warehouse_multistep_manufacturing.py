...
from odoo.tests import Form, Command
...
class TestMultistepManufacturingWarehouse(TestMrpCommon):
    ...
    def _check_location_and_routes(self):
        ...
        self.assertTrue(self.warehouse_1.manufacture_pull_id)
        self.assertTrue(self.warehouse_1.manufacture_pull_id.active, self.warehouse_1.manufacture_to_resupply)
        self.assertTrue(self.route_manufacture)
        # Check new routes created or not.
        self.assertTrue(self.warehouse_1.pbm_route_id)
        # Check location should be created and linked to warehouse.
        self.assertTrue(self.warehouse_1.pbm_loc_id)
        self.assertEqual(self.warehouse_1.pbm_loc_id.active, self.warehouse_1.manufacture_steps != 'mrp_one_step', "Input location must be de-active for single step only.")
    ...
    def test_00_create_warehouse(self):
        ...
        self.assertFalse(self.warehouse_1.pbm_route_id.rule_ids, 'only the update of global manufacture route should happen.')
        self.assertEqual(self.warehouse_1.manufacture_pull_id.location_dest_id.id, self.stock_location.id)
    ...
    def test_01_warehouse_twostep_manufacturing(self):
        ...
        self._check_location_and_routes()
        self.assertEqual(len(self.warehouse_1.pbm_route_id.rule_ids), 2)
        self.assertEqual(self.warehouse_1.manufacture_pull_id.location_dest_id.id, self.stock_location.id)
    ...
    def test_02_warehouse_twostep_manufacturing(self):
        ...
        self._check_location_and_routes()
        self.assertEqual(len(self.warehouse_1.pbm_route_id.rule_ids), 3)
        self.assertEqual(self.warehouse_1.manufacture_pull_id.location_dest_id.id, self.stock_location.id)
    ...
    def test_manufacturing_2_steps_sublocation(self):
        ...
        pre_1, pre_2 = self.env['stock.location'].create([{
            'name': name,
            'location_id': self.warehouse_1.pbm_loc_id.id,
            'usage': 'internal'
    ...
    def test_manufacturing_3_steps(self):
        ...
        self.assertEqual(move_raw_ids.picking_type_id, self.picking_type_manu)
        pbm_move = move_raw_ids.move_orig_ids
        self.assertEqual(len(pbm_move), 1)
        self.assertEqual(pbm_move.location_id, self.stock_location)
        self.assertEqual(pbm_move.location_dest_id, self.warehouse_1.pbm_loc_id)
        self.assertEqual(pbm_move.picking_type_id, self.warehouse_1.pbm_type_id)
        ...
        self.assertEqual(move_finished_ids.picking_type_id, self.picking_type_manu)
        production.button_mark_done()
        sam_move = move_finished_ids.move_dest_ids
        self.assertEqual(len(sam_move), 1)
        self.assertEqual(sam_move.location_id, self.warehouse_1.sam_loc_id)
        self.assertEqual(sam_move.location_dest_id, self.stock_location)
        self.assertEqual(sam_move.picking_type_id, self.warehouse_1.sam_type_id)
    ...
    def test_manufacturing_flow(self):
        ...
        with Form(self.warehouse_1) as warehouse:
            warehouse.manufacture_steps = 'pbm_sam'
        self.warehouse_1.flush_model()
        self.route_mto.active = True
        self.env['stock.quant']._update_available_quantity(self.raw_product, self.stock_location, 4.0)
        ...
        self.assertFalse(sum(self.env['stock.quant']._gather(self.raw_product, self.stock_location).mapped('quantity')))
        self.assertTrue(self.env['stock.quant']._gather(self.raw_product, self.warehouse_1.pbm_loc_id))
    ...
    def test_3_steps_and_byproduct(self):
        ...
        preprod_picking = pickings[0] if pickings[0].location_id == self.stock_location else pickings[1]
        self.assertEqual(preprod_picking.location_id, self.stock_location)
        self.assertEqual(preprod_picking.location_dest_id, self.warehouse_1.pbm_loc_id)
    ...
    def test_manufacturing_3_steps_trigger_reordering_rules(self):
        ...
        rr_form.location_id = self.stock_location
        rr_form.save()
    ...
    def test_update_component_qty(self):
        ...
        mo = self.env['mrp.production'].create({
            ...
            'bom_id': self.bom.id,
            'product_qty': 1,
            'location_src_id': self.warehouse_1.pbm_loc_id.id,
        })
        mo.action_confirm()
    ...
    def test_pick_components_uses_shipping_policy_from_picking_type(self):
        self.warehouse_1.manufacture_steps = "pbm"
        pick_components_type = self.warehouse_1.pbm_type_id
