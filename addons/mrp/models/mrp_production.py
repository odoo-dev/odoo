...
class MrpProduction(models.Model):
    ...
    def action_reset_to_draft(self):
        self._check_pre_reset_to_draft()
    ...
    def _check_pre_reset_to_draft(self):
        ...
        if not self.env.user.has_group('mrp.group_mrp_manager'):
            raise AccessError(_("Only manufacturing administrators can reset a manufacturing order to draft."))
    ...
    def _post_inventory(self, cancel_backorder=False):
        ...
        for order in self:
            ...
            for move in finish_moves:
                if move.product_id.tracking in ['lot', 'serial'] and (not move.lot_ids or move.lot_ids - order.lot_producing_ids):
