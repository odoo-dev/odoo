...
class MrpProduction(models.Model):
    ...
    def button_plan(self, as_soon_as_possible=True):
        ...
        for order in self:
            if order.is_planned or not order.workorder_ids:
        ...
        if skipped_order_count == len(self):
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'title': _('This MO cannot be planned because it has no Work Orders or it is already planned.'),
                    'type': 'warning',
                },
            }
        return True
    ...
    def action_split(self):
        ...
        if len(self) > 1:
            ...
            if self.state != 'draft' and (self.state != 'confirmed' or self.qty_producing > 0):
