...
class MrpBom(models.Model):
    ...
    def _compute_json_popover(self):
        ...
        for bom in self.filtered(lambda bom: bom.type != 'phantom' and bom._origin):
            ...
            component_info = max(bom_data.get('components'), key=lambda component: component.get('lead_time') or component.get('availability_delay') or component.get('manufacture_delay'), default={'lead_time': 0, 'availability_delay': 0, 'manufacture_delay': 0, 'name': ''})
            max_delay = component_info.get('lead_time') or component_info.get('availability_delay') or component_info.get('manufacture_delay')
