...
class MrpBom(models.Model):
    ...
    def _compute_json_popover(self):
        ...
        for bom in self.filtered(lambda bom: bom.type != 'phantom'):
            ...
            component_info = max(bom_data.get('components'), key=lambda component: component.get('lead_time') or component.get('availability_delay', 0) or component.get('manufacture_delay', 0), default={'availability_delay': 0, 'manufacture_delay': 0, 'name': ''})
            if any(b.days_to_prepare_mo != 0 for b in bom.bom_line_ids.child_bom_id):
                max_delay = component_info.get('lead_time')
            else:
            ...
            if not (component_info.get('name') and component_info.get('is_storable')):
                ...
                bom.json_popover = json.dumps({
                    ...
                    'bom_id': bom._origin.id if bom._origin else False,
