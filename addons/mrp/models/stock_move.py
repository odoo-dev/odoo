...
class StockMove(models.Model):
    ...
    def _unlink_if_draft_or_cancel(self):
        component_moves = self.filtered(
            lambda m: m.state != 'draft' and (m.raw_material_production_id or m.created_production_id))
        ...
        for move in component_moves:
            ...
            pickings = move.move_orig_ids.picking_id.filtered(
                lambda p: p.state != 'done' and p.move_ids - move.move_orig_ids)
            if pickings:
                documents = {(picking, picking.user_id): [{move: (parent_mo, (0, move.product_uom_qty))}] for picking in
                             pickings}
                parent_mo.with_context(exception_on_picking=True)._log_manufacture_exception(documents)
            child_mos = move.production_group_id.child_ids.production_ids.filtered(lambda p: p.product_id == move.product_id)
