...
class StockRule(models.Model):
    ...
    def _run_manufacture(self, procurements):
        ...
        for procurement, rule in procurements:
            ...
            if not mo or (bom and bom.enable_batch_size):
                procurement_qty = procurement.product_qty
                batch_size = bom.product_uom_id._compute_quantity(bom.batch_size, procurement.product_uom) if bom and bom.enable_batch_size else procurement_qty
                ...
                while procurement.product_uom.compare(procurement_qty, 0) > 0:
                    new_productions_values_by_company[procurement.company_id.id]['values'].append({
                        **vals,
                        'product_qty': procurement.product_uom._compute_quantity(batch_size, bom.product_uom_id) if bom else batch_size,
                    })
