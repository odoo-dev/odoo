...
class ProcurementGroup(models.Model):
    ...
    def _get_product_routes(self, product):
        ...
        manufacturing_routes = self.env['stock.rule'].search([('action', '=', 'manufacture'), ('picking_type_id.code', '=', 'mrp_operation')]).route_id
        if not any(bom.type == 'normal' for bom in product.bom_ids) and not any(route in product.route_ids for route in manufacturing_routes):
            route_ids -= manufacturing_routes
        return route_ids
