    ...
    def _run_manufacture(self, procurements):
        ...
        for procurement, rule in procurements:
            ...
            if not bom and self.env.context.get('from_orderpoint'):
                msg = _('There is no Bill of Materials to generate a Manufacturing Order for the product %s. Go to the product form and create a Bill of Materials for this product.', procurement.product_id.display_name)
...
class ProcurementGroup(models.Model):
    ...
    def _get_product_routes(self, product, valid_route_ids, warehouse_id):
        ...
        manufacturing_routes = self.env['stock.rule'].search([('action', '=', 'manufacture'), ('picking_type_id.code', '=', 'mrp_operation'), ('active', '=', True)]).route_id.ids
        if not any(bom.type == 'normal' for bom in product.bom_ids) and not any(route_id in product.route_ids.ids for route_id in manufacturing_routes):
            valid_route_ids -= set(manufacturing_routes)
