...
class ReportMrpReport_Mo_Overview(models.AbstractModel):
    ...
    def _get_report_data(self, production_id):
        ...
        initial_mo_cost = self._compute_cost_sums(components, operations)

        remaining_cost_share, byproducts = self._get_byproducts_data(production, initial_mo_cost, level=1, current_index='')
        summary = self._get_mo_summary(production, components, operations, initial_mo_cost, remaining_cost_share)
    ...
    def _get_cost_breakdown_data(self, production, extras, remaining_cost_share):
        ...
        breakdown_lines = [self._format_cost_breakdown_lines(0, production.product_id.display_name, production.product_id.uom_id.display_name,
                                                             (extras['total_mo_cost_components'] * remaining_cost_share) / production.product_uom_qty,
                                                             (extras['total_mo_cost_operations'] * remaining_cost_share) / production.product_uom_qty,
                                                             (extras['total_mo_cost'] * remaining_cost_share) / production.product_uom_qty)]
    ...
    def _get_mo_summary(self, production, components, operations, current_mo_cost, remaining_cost_share):
        ...
        return {
            ...
            'quantity_decorator': False,
            'quantity_consumed': 0.0,
            'quantity_consumed_decorator': False,
    ...
    def _get_duration_decorator(self, expected_duration, duration):
        if expected_duration is False or duration is False:
            return False
        return 'danger' if float_compare(duration, expected_duration, precision_rounding=0.01) > 0 else False

    def _get_quantity_decorator(self, expected_quantity, quantity, uom):
        if expected_quantity is False or quantity is False:
            return False
        return 'danger' if uom.compare(quantity, expected_quantity) > 0 else False

    def _get_consumed_quantity_decorator(self, production, move_raw):
        expected_quantity = move_raw.should_consume_qty if production.qty_producing else move_raw.product_uom_qty
        return self._get_quantity_decorator(expected_quantity, move_raw.quantity, move_raw.uom_id)
    ...
    def _get_operations_data(self, production, level=0, current_index=False):
        ...
        for index, workorder in enumerate(production.workorder_ids):
            ...
            duration = workorder.duration_expected if float_is_zero(wo_duration, precision_digits=2) else wo_duration
            duration_decorator = self._get_duration_decorator(workorder.duration_expected, wo_duration) if is_workorder_started else False
            ...
            operations.append({
                ...
                'quantity': duration,
                'quantity_decorator': duration_decorator,
                'uom_name': "",
        ...
        summary_duration_decorator = self._get_duration_decorator(total_expected_time, total_current_time) if self._is_production_started(production) else False

        return {
            'summary': {
                'index': f"{current_index}W",
                'quantity': total_current_time,
                'quantity_decorator': summary_duration_decorator,
    ...
    def _get_finished_operation_data(self, production, level=0, current_index=False):
        ...
        for index, workorder in enumerate(production.workorder_ids):
            ...
            duration_decorator = self._get_duration_decorator(workorder.duration_expected / 60, duration) if not estimate_cost else False
            operations.append({
                'level': level,
                'index': f"{current_index}W{index}",
                'model': workorder._name,
        ...
        summary_duration_decorator = self._get_duration_decorator(total_duration_expected / 60, total_duration)
        return {
            'summary': {
                'index': f"{current_index}W",
                'done': True,
    ...
    def _format_component_move(self, production, move_raw, replenishments, replenish_data, level, index):
        ...
        replenish_mo_cost = self._compute_cost_sums(replenishments)
        ...
        # When MO is done, show real cost in the MO Cost column
        if production.state == 'done':
            real_cost = currency.round(self._get_component_real_cost(move_raw, current_quantity if move_raw.picked else 0))
            mo_cost = real_cost
        # Quantity decorator: red if consumed quantity > expected quantity
        quantity_decorator = False
        if production.state == 'done':
            quantity_decorator = self._get_quantity_decorator(expected_quantity, current_quantity, move_raw.uom_id)
        component = {
            'level': level,
            'index': index,
            'id': product.id,
            ...
            'quantity_decorator': quantity_decorator,
            'uom': move_raw.uom_id,
            ...
            'quantity_consumed_decorator': self._get_consumed_quantity_decorator(production, move_raw),
    ...
    def _get_replenishment_lines(self, production, move_raw, replenish_data, level, current_index):
        ...
        for count, forecast_line in enumerate(current_lines):
            ...
            if doc_in._name == 'mrp.production':
                ...
                initial_mo_cost = self._compute_cost_sums(replenishment['components'], replenishment['operations'])
