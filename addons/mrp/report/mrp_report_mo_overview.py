...
class ReportMrpReport_Mo_Overview(models.AbstractModel):
    ...
    def _get_component_receipt(self, product, move, warehouse, replenishments, replenish_data):
        ...
        max_date = max([get(rep, 'date', True) for rep in replenishments_with_date], default=self.env.now.replace(hour=0, minute=0, second=0))
        ...
        if product.is_storable and production.state not in ('done', 'cancel')\
           and move_raw.uom_id.compare(missing_quantity, 0) > 0:
            ...
            if resupply_data:
                ...
                to_order_line['summary']['receipt'] = self._check_planned_start(production.date_start, self._format_receipt_date('estimated', self.env.now.replace(hour=0, minute=0, second=0) + timedelta(days=resupply_data['delay'])))
            else:
