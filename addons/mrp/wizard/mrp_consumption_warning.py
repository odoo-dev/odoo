...
class MrpConsumptionWarning(models.TransientModel):
    ...
    def action_set_qty(self):
        ...
        for production in self.mrp_production_ids:
            uom_to_show = self.env.context.get('uom_to_show', {})
            for line in self.mrp_consumption_warning_line_ids:
                if line.mrp_production_id != production:
                    continue
                uom_id = uom_to_show[str(line.product_id.id)]
                product_uom = self.env['uom.uom'].browse(uom_id)
                for move in production.move_raw_ids:
                    if line.product_id != move.product_id:
                        continue
                    qty_expected = product_uom._compute_quantity(line.product_expected_qty_uom, move.product_uom)
                ...
                if not float_is_zero(line.product_expected_qty_uom, precision_rounding=product_uom.rounding):
...
class MrpConsumptionWarningLine(models.TransientModel):
    ...
    product_uom_id = fields.Many2one('uom.uom', "Unit of Measure", readonly=True, store=False)
