...
patch(AttendeeCalendarCommonRenderer.prototype, {
    ...
    handleWorkLocationClick(target, date) {
        ...
        if (worklocationSet && !create) {
            if (!worklocations.id) {
                worklocations =
                    worklocations[location] && worklocations[location].find((wl) => wl.id == id);
            }
            if (worklocations) {
