...
_updateErrorDisplay: function(errors) {
    ...
    if (!errors.length) {
        this.$('.o_wetrack_proposal_error_section').addClass('d-none');
    } else { 
        this.$('.o_wetrack_proposal_error_section').removeClass('d-none');
    }
...
_onPartnerNameInput: function(ev) {
    ...
    this.$(".o_wetrack_contact_name_input").val(text).change();
},
