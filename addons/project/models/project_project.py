...
class ProjectProject(models.Model):
    ...
    def action_toggle_project_template_mode(self):
        ...
        return {
            ...
            "tag": "project_to_template_redirection_action",
        }
    ...
    def _get_template_from_project_undo_callbacks(self):
        ...
        callbacks = {}
        if self.active:
            self.with_context(skip_task_archive=True).write({'active': False})
    ...
    def action_create_template_from_project(self):
        ...
        template = self.env['project.project.template'].sudo().create(values)
        self.env['project.task'].search([('project_id', '=', self.id)]) \
            .write({'project_template_id': template.id})
        template._toggle_template_mode(True)
        config = {
            "tag": "project_template_show_notification",
            "params": {
                "record_id": template.id,
                "temp_res_model": "project.project.template",
            },
        }
