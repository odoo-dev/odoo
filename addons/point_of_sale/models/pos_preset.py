...
class PosPreset(models.Model):
    ...
    is_return = fields.Boolean(string='Return Mode', help="All quantity in the cart will be in negative. Ideal for return management.")
    color = fields.Integer(string='Color', default=lambda self: random.randint(0, 10))
    ...
    # Timing options
    use_timing = fields.Boolean(string='Manage Orders by time')
    ...
    interval_time = fields.Integer(string='Interval time (min)', default=20)
    ...
    def _load_pos_data_domain(self, data, config):
        return [('id', 'in', config.available_preset_ids.ids)]
    ...
    def action_open_linked_config(self):
        self.ensure_one()
        return {
            'name': _('Linked Point of Sale'),
            'view_mode': 'list',
            'res_model': 'pos.config',
            'type': 'ir.actions.act_window',
            'domain': [('available_preset_ids', 'in', self.id)]
        }
