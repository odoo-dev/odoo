...
class PosSalesDetailReport(models.AbstractModel):
    ...
    def _section_sales(self, options):
        ...
        report_currency = self._get_currency(options)
        qty = amount = 0.0
        for line in lines:
            ...
            amount += self._convert_to_report_currency(
                raw_amount, line.currency_id, report_currency,
                line.order_id.date_order, line.company_id,
            )
        return {
            'name': _('Sales'), 'level': 0,
            'foldability': 'expanded' if lines else 'static', 'style': 'bold',
            'qty': qty, 'amount_total': report_currency.round(amount),
        }
    ...
    def _section_sales_categories(self, unfold_context, options):
        ...
        for line in lines:
            ...
            raw_amount = self._get_product_total_amount(line)
            cat_data[key]['amount_total'] += self._convert_to_report_currency(
                raw_amount, line.currency_id, report_currency,
                line.order_id.date_order, line.company_id,
            )
        result = []
    ...
    def _section_payments(self, options):
        ...
        for payment in payments:
            ...
            amount += self._convert_to_report_currency(
                payment.amount, order_currency, report_currency,
                payment.pos_order_id.date_order, payment.pos_order_id.company_id,
            )
        return {
            'name': _('Payments'), 'level': 0,
            'foldability': 'collapsed' if payments else 'static', 'style': 'bold',
            'amount_total': report_currency.round(amount),
        }
        ...
        session_lines = [
            {
                ...
                'amount_total': report_currency.round(total),
            }
    ...
    def _section_discounts(self, options):
        ...
        report_currency = self._get_currency(options)
        disc_amount = 0.0
        for line in orders.lines.filtered(lambda l: l.discount > 0):
            raw_discount = line._get_discount_amount()
            disc_amount += self._convert_to_report_currency(
                raw_discount, line.currency_id, report_currency,
                line.order_id.date_order, line.company_id,
            )
        return {
            'name': _('Discounts'), 'level': 0,
            'foldability': 'static', 'style': 'bold',
            'count': len(disc_orders), 'amount': report_currency.round(disc_amount),
        }
    ...
    def _section_invoices(self, options):
        ...
        for order in invoiced:
            ...
            total_paid += self._convert_to_report_currency(
                order.amount_paid, order_currency, report_currency,
                order.date_order, order.company_id,
            )
        return {
            'name': _('Invoices'), 'level': 0,
            'foldability': 'collapsed' if invoiced else 'static', 'style': 'bold',
            'count': len(invoiced), 'amount_total': report_currency.round(total_paid),
        }
        ...
        for (session,) in groups:
            ...
            for inv in inv_list:
                session_total += self._convert_to_report_currency(
                    inv.get('total', 0.0), session.currency_id, report_currency,
                    session.start_at, session.company_id,
                )
            result.append({
                'record_id': session.id,
                'name': session.name, 'level': 1,
                'foldability': 'collapsed', 'style': 'bold',
                'count': len(inv_list), 'amount_total': report_currency.round(session_total),
            })
        return result
    ...
    def _section_session_control(self, options):
        ...
        currency = self._get_currency(options)
        total_paid = 0.0
        for order in orders:
            order_currency = order.currency_id
            total_paid += self._convert_to_report_currency(
                order.amount_paid, order_currency, currency,
                order.date_order, order.company_id,
            )
        return {
            ...
            for method in payments.mapped('payment_method_id'):
                ...
                if method.type == 'cash':
                    opening = self._convert_to_report_currency(
                        session.opening_balance or 0.0, session.currency_id, report_currency,
                        session.start_at, session.company_id,
                    )
                    closing = self._convert_to_report_currency(
                        session.closing_balance, session.currency_id, report_currency,
                        session.start_at, session.company_id,
                    )
                    ...
                    if acct_pays:
                        counted = report_currency.round(sum(
                            self._convert_to_report_currency(
                                p.amount_signed, p.currency_id, report_currency,
                                p.date, p.company_id,
                            ) for p in acct_pays
                        ))
                        ...
                        if move:
                            ...
                            move_amount = self._convert_to_report_currency(
                                move.amount_total, move.currency_id, report_currency,
                                move.date, move.company_id,
                            )
    ...
    def _build_cash_moves_cash(self, session, method):
        ...
        if session.opening_balance > 0:
            result.append({
                ...
                'foldability': 'static', 'style': 'normal',
                'amount_total': report_currency.round(self._convert_to_report_currency(
                    session.cash_register_balance_start, session.currency_id, report_currency,
                    session.start_at, session.company_id,
                )),
            })
        counter = 0
        for move in cash_moves:
            ...
            result.append({
                ...
                'foldability': 'static', 'style': 'normal',
                'amount_total': report_currency.round(self._convert_to_report_currency(
                    move.amount, move.currency_id, report_currency,
                    move.date, move.company_id,
                )),
            })
        return result
        ...
        if acct_pays:
            ...
            for p in pos_payments:
                ...
                expected += self._convert_to_report_currency(
                    p.amount, order_currency, report_currency,
                    p.pos_order_id.date_order, p.pos_order_id.company_id,
                )
    ...
    def _convert_to_report_currency(self, amount, from_currency, to_currency, date=False, company=False):
        """Convert an amount from one currency to another using the order date rate."""
        if not from_currency or not to_currency or from_currency == to_currency:
            return amount
        return from_currency._convert(amount, to_currency, company or self.env.company, date or fields.Date.today())
