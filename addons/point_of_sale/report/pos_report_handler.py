...
def report_section(*, id=None, parent=None, sequence=1, name=None, foldability="collapsed"):
    ...
    """
    if not id:
        msg = "report_section: 'id' is required."
...
class PosReportHandler(models.AbstractModel):
    ...
    def _get_currency(self, options=None):
        return self.env.company.currency_id

    def _get_sections_columns(self):
        return {}

    def _get_sections_data(self, options={}):
        ...
        for fn in self._get_section_methods():
            ...
            for line in normalized:
                line['columns'] = columns_by_section.get(fn._rs_id, [])
                should_expand = options.get('unfold_all', False) or line.get('foldability') == 'expanded'
                if should_expand:
                    line['lines'] = self._get_children_for_section(
                        fn._rs_id, line.get('record_id'), options,
                    )
                sections.append(line)
        return sections

    def _get_children_for_section(self, section_id, record_id=None, options={}):
        """Recursively fetch and expand all children of the given section."""
        children = self._get_unfold_lines(section_id, record_id, options)
        if not children:
            return []

        for child in children:
            should_expand = options.get('unfold_all', False) or child.get('foldability') == 'expanded'
            if should_expand:
                child['lines'] = self._get_children_for_section(
                    child.get('section_id'), child.get('record_id'), options,
                )

        return children

    def _get_unfold_lines(self, section_id, record_id=None, options={}):
    ...
    def _validate_sections(self):
        sections = {}
        for _, fn in inspect.getmembers(self.__class__, inspect.isfunction):
