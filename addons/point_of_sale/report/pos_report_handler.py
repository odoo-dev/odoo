...
class PosReportHandler(models.AbstractModel):
    ...
    def _get_sections_data(self, options=None):
        ...
        :param dict options: Report options; reads ``unfold_all`` flag from it.
        """
        sections = []
        unfold_all = (options or {}).get('unfold_all', False)
        columns_by_section = self._get_sections_columns()
        for fn in self._get_section_methods():
            ...
            for line in normalized:
                ...
                should_expand = unfold_all or line.get('foldability') == 'expanded'
                if should_expand:
    ...
    def _get_children_for_section(self, section_id, record_id=None, options=None, _visited=None):
        """Recursively fetch children for a section.

        Reads ``unfold_all`` from *options* to decide whether to recurse into
        children whose foldability is not ``'expanded'``.
        """
        if _visited is None:
            _visited = set()
        ...
        for child in children:
            should_expand = unfold_all or child.get('foldability') == 'expanded'
            if should_expand:
