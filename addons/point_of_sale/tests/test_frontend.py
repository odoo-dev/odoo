...
class TestPointOfSaleHttpCommon(AccountTestInvoicingHttpCommon):
    ...
    def setUpClass(cls):
        ...
        FP_POS_2M = cls.env['account.fiscal.position'].create({
            'name': "FP-POS-2M",
        })
        cls.env['account.tax'].create({'name': "DST", 'amount': 5, 'fiscal_position_ids': [Command.link(FP_POS_2M.id)], 'original_tax_ids': [Command.link(cls.src_tax.id)]})
        cls.env['account.tax'].create({'name': "DST2", 'amount': 10, 'fiscal_position_ids': [Command.link(FP_POS_2M.id)], 'original_tax_ids': [Command.link(cls.src_tax.id)]})
