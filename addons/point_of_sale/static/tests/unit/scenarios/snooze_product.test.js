...
test("snooze tracker marks product as unavailable then available", async () => {
...
test("ProductInfoPopup displays warning button state for snoozed product", async () => {
...
test("ProductInfoPopup displays available button state for non-snoozed product", async () => {
    const store = await setupPosEnv();
...
test("stop snooze confirmation dialog displays warning message", async () => {
    const store = await setupPosEnv();
    const now = DateTime.now();
    const endTime = now.plus({ hours: 1 });
