...
test("custom attribute alone displayed", async () => {
    ...
    expect(Object.keys(product.isConfigurable())).toHaveLength(2);
...
test("combo variant mix uses selected variant and attributes", async () => {
    const store = await setupPosEnv();
...
test("line configurators product", async () => {
    const store = await setupPosEnv();
    const order = store.addNewOrder();
...
test("line configurators combo", async () => {
    const store = await setupPosEnv();
    const order = store.addNewOrder();
    const configurableProduct = createConfigurableChair(store);
