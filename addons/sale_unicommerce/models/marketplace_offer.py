...
class MarketplaceOffer(models.Model):   
    ...
    # code = fields.Char(required=True, help="Marketplace code like 'shopify'")
    code = fields.Char( help="Marketplace code like 'shopify'")
    external_id = fields.Char("External Product ID", required=True, index=True)
    # external_variant_id = fields.Char("External Variant ID", required=True)
    external_variant_id = fields.Char("External Variant ID")
    # marketplace_id = fields.Many2one("crm.team", required=True, ondelete="cascade")
    ...
    raw_json = fields.Text("Raw JSON Data")
