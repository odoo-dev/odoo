...
class UnicommerceAccount(models.Model):
    ...
    def _call_uc_api(self, endpoint_key, method='POST', data=None, params=None):
        ...
        try:
            ...
            _logger.exception("Unexpected error while calling Unicommerce API: %s", url)
            raise UserError(_("An unexpected error occurred while calling Unicommerce API: %s") % str(e))
        ...
        try:
            ...
            for uc_product in uc_products:
                try:
                    ...
                    _logger.error(f"[sync_products] Failed to sync item {uc_product.get('skuCode')}: {e}")
            self.last_product_sync_datetime = fields.Datetime.now()
        except Exception as e:
            _logger.error(f"[sync_products] Failed to pull product list from Unicommerce: {e}")
            raise UserError(_("Failed to sync products from Unicommerce. Please check logs for more details."))
    ...
    def search_unicommerce_items(self):
        ...
        try:
            ...
            _logger.error(f"[search_unicommerce_items] API call failed: {e}")
            raise
    ...
    def get_unicommerce_item(self, product):
        ...
        try:
            ...
            _logger.error(f"[get_unicommerce_item] Failed to get item details for SKU {payload['code']}: {e}")
            raise    
    ...
    def create_item(self, uc_product_detail):
        ...
        if not internal_product:
            try:
                ...
                _logger.error(f"[] Failed to create product in Odoo: {e}")
        ...
        try:
            ...
            _logger.error(f"[] Failed to create/update marketplace offer: {e}")
    ...
    def upload_items_to_unicommerce(self, item_codes):
        try:
            ...
            _logger.info("[upload_items_to_unicommerce] Synced %d products with Unicommerce", len(payload.get('itemTypes', [])))
    
        except Exception as e:
            _logger.exception("[upload_items_to_unicommerce] Unexpected error occurred")
            raise UserError(_("Failed to push products to Unicommerce. Please check the logs."))
    ...
    def create_update_item(self, payload, update=False):
        try:
            ...
            _logger.error("[create_update_item] Failed to create/update items: %s", e)
            raise
    ...
    def create_update_category(self, product):
        try:
            ...
            _logger.error("[create_update_category] Error pushing category for product %s: %s", product.default_code, e)
            return False
        ...
        try:
            ...
            for uc_order in uc_orders or []:
                try:
                    ...
                    _logger.exception(f"[sync_orders] Failed to process order {uc_order.get('code')}: {e}")
                self.last_so_sync_datetime = fields.Datetime.now()                    
        except Exception as e:
            _logger.exception("[sync_orders] Failed to fetch sale orders")
            raise UserError(_("Failed to sync sale orders. Please check logs."))
    ...
    def search_sale_order(self):
        ...
        try:
            ...
            _logger.error(f"[search_sale_order] API call failed: {e}")
            raise        
    ...
    def get_sale_order(self, sku):
        ...
        try:
            ...
            _logger.error(f"[get_sale_order] Failed to get order details for code {code}: {e}")
            raise
    ...
    def create_order(self, uc_order_detail):
        try:
            ...
            if not order_lines_data:
                _logger.warning(f"[create_order] No valid order lines found for order {order_data.get('code')}")
                return
            ...
            _logger.info(f"[create_order] Sale order {sale_order.name} created for Unicommerce order {order_data.get('code')}")
    
        except Exception as e:
            _logger.exception(f"[create_order] Failed to create order for code {order_data.get('code')}")
            raise  
    ...
    def create_partner(self, name, email):
        try:
            ...
            _logger.info(f"[create_partner] Created new partner: {name} <{email}>")
            return partner
        except Exception as e:
            _logger.exception(f"[create_partner] Failed to create partner for {email}")
            raise
    ...
    def create_order_line(self, order_line_items):
        ...
        for item in order_line_items:
            ...
            if not product:
                _logger.warning(f"[create_order_line] Product with SKU {sku} not found in Odoo.")
                continue
            ...
            try:
                ...
                _logger.error(f"[create_order_line] Failed to build order line for product {sku}: {e}")
