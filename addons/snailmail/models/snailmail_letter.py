...
class SnailmailLetter(models.Model):
    ...
    def _get_failure_type(self, error):
        if error == 'CREDIT_ERROR':
            return 'snailmail_credit_error'
        if error == 'TRIAL_ERROR':
            return 'snailmail_trial_error'
        if error == 'NO_PRICE_AVAILABLE':
            return 'snailmail_no_price_available'
        if error == 'MISSING_REQUIRED_FIELDS':
            return 'snailmail_missing_required_fields'
        if error == 'FORMAT_ERROR':
            return 'snailmail_format_error'
        else:
            return 'snailmail_unknown_error'
    ...
    def _snailmail_print_invalid_address(self):
        ...
        self.message_id._notify_mail_notification_update()
