...
ThreadWidget.include({
    ...
    _openCreditErrorDialog: function (messageID, content) {
        ...
        new Dialog(this, {
            ...
            buttons: [{
                ...
                classes: 'btn-primary',
                close: true,
                click: _.bind(message.resendLetter, message),
            }, {
                text: _t("Cancel letter"),
                close: true,
                click: _.bind(message.cancelLetter, message),
            }, {
                text: _t("Close"),
                close: true,
            }],
     ...
     * @override
     */
    _onClickMessageNotificationError: function (ev) {
        const $target = $(ev.currentTarget);
        if ($target.data('message-type') === 'snailmail') {
            if ($target.data('failure-type') === 'snailmail_missing_required_fields') {
                this._onClickMissingRequiredFields(ev);
            }
            if ($target.data('failure-type') === 'snailmail_credit_error') {
                this._onClickCreditError(ev);
            }
            if ($target.data('failure-type') === 'snailmail_trial_error') {
                this._onClickTrialError(ev);
            }
            if ($target.data('failure-type') === 'snailmail_no_price_available') {
                this._onClickNoPriceAvailable(ev);
            }
            if ($target.data('failure-type') === 'snailmail_format_error') {
                this._onClickFormatError(ev);
            }
            if ($target.data('failure-type') === 'snailmail_unknown_error') {
