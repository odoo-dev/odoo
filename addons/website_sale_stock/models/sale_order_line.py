...
class SaleOrderLine(models.Model):
    ...
    def _get_max_available_qty(self):
        """Return the max quantity of a combo product. Ir is the max quantity of its selected combo
        item with the lowest max quantity. If none of the combo items has a max quantity, then the
        combo product also has no max quantity.
        """
        self.ensure_one()
