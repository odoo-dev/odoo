    ...
    def _get_additionnal_combination_info(self, product_or_template, quantity, uom, date, website):
        ...
        if product_or_template.type == 'combo':
            ...
            max_quantities = [
                ...
                if (max_quantity := combo._get_max_quantity(website, website.current_session_sale_order_id.sudo())) is not None
            ]
            if max_quantities:
        ...
        if product_or_template.is_product_variant:
            ...
            if not product_sudo.allow_out_of_stock_order:
                cart_quantity = product_sudo.uom_id._compute_quantity(
                    website.current_session_sale_order_id.sudo()._get_cart_qty(product_sudo.id),
                    to_unit=uom,
                )
