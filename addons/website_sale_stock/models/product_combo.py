...
class ProductCombo(models.Model):
    ...
    def _get_max_quantity(self, website, sale_order, **kwargs):
        """Return the max quantity of a combo. It is the max quantity of its combo item with the
        highest max quantity. If one of the combo items has no max quantity, then the combo also
        has no max quantity.
