...
class RegisterUserPajakIO(models.TransientModel):
    ...
    def action_register(self):
        ...
        # TODO: email and password stored in case users made a mistake of not proceeding with company registration on the spot
        res = response.json()
...
class RegisterCompanyPajakIO(models.TransientModel):
    ...
    def action_register(self):
        ...
        # if company registration is successful we want to also immediately create
        # IAP account in the IAP server and store the Client ID
        # account_id = self.env['iap.account'].get_account_id("l10n_id_pajakio_proxy")

        # TODO: in res.config.settings, we've defined "action_link_company_iap"
