...
class L10nSaEdiDocument(models.Model):
    ...
    res_model = fields.Selection([
        ...
        ('pos.order', "Simplified"), # todo move this to pos.override
    ], string="Invoice Type", readonly=True, required=True)
    res_id = fields.Many2oneReference("Resource", model_field="res_model", readonly=True, required=True)
    ...
    feedback = fields.Text("Errors/Warnings", translate=True)
    ...
    journal_id_id = fields.Many2one('account.journal', required=True)
    account_move_id = fields.Many2one('account.move', compute='_compute_resource_id')
    ...
    def _get_resources_grouped_by_model(self):
    ...
    def _l10n_sa_create_log(self, attachment=False):
        self.ensure_one()
        # Handle attachment as either a record, an ID, or False
        attachment_id = getattr(attachment, 'id', attachment) or self.attachment_id.id
        vals_list = [{
    ...
    def _l10n_sa_generate_zatca_template(self):
        ...
        if self.res_model == 'account.move':
            ...
            xml_content, errors = self.env['pos.edi.xml.ubl_21.zatca']._export_pos_order(record)
            if errors:
                return {
                    'error': _("Could not generate POS UBL content: %s", ", \n".join(errors)),
                    'blocking_level': 'error',
                }
            return self._l10n_sa_postprocess_zatca_template(xml_content)
    ...
    def _l10n_sa_apply_qr_code(self, xml_content):
        ...
        root = etree.fromstring(xml_content)
        record = self.env[self.res_model].browse(self.res_id)
        qr_code = record._get_l10n_sa_qr_code_str()
    ...
    def _l10n_sa_post_zatca_edi(self):  # no batch ensure that there is only one invoice
        ...
        if chain_head and chain_head != record and not chain_head._l10n_sa_is_in_chain():
            ...
            self.state = 'error'
            self.feedback = _("Error: This invoice is blocked due to %s. Please check it.", chain_head.name)
        ...
        if self.res_model == 'account.move':
            ...
            attachment_name = self.env['pos.edi.xml.ubl_21.zatca']._export_invoice_filename(record)
        ...
        if error := response_data.get('error'):
            ...
            if self.res_model == 'account.move':
                error_attachment = record._l10n_sa_log_results(submitted_xml, response_data, error=response_data.get('rejected'))
            elif self.res_model == 'pos.order' and response_data.get('rejected'):
                # For POS orders, save rejected XML as attachment
                xml_filename = attachment_name or f"{record.name}_rejected.xml"
                error_attachment = self.env['ir.attachment'].create({
                    'raw': submitted_xml,
                    'name': xml_filename,
                    'description': 'Rejected ZATCA Document - POS Order',
                    'res_id': record.id,
                    'res_model': self.res_model,
                    'type': 'binary',
            ...
            self.state = 'error'
            self.feedback = error
            self._l10n_sa_create_log(error_attachment)
            return
            # record.l10n_sa_edi_document_id._l10n_sa_create_log()
            # return {
            #     invoice: {
            #         **response_data,
        ...
        if self.res_model == 'account.move':
            record._l10n_sa_log_results(cleared_xml.encode(), response_data)
    ...
    def _l10n_sa_get_invoice_content_edi(self, invoice):
        ...
        doc = invoice.l10n_sa_edi_document_id.filtered(lambda d: d.state == 'sent')
