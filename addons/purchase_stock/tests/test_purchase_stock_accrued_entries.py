...
class TestAccruedPurchaseStock(PurchaseTestCommon):
    ...
    def test_purchase_stock_accruals_anglo_saxon_price_diff(self):
        ...
        def sort_lines(move):
            return move.line_ids.sorted(lambda l: (l.account_id.code, l.debit, l.credit))
        ...
        accrued_move = account_moves.filtered(lambda m: not m.reversed_entry_id)
        reversal_move = account_moves.filtered(lambda m: m.reversed_entry_id)

        # Accrued revenues entries.
        self.assertRecordValues(sort_lines(accrued_move), [
        ...
        self.assertRecordValues(sort_lines(reversal_move), [
