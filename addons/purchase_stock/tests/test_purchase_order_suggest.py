...
class TestPurchaseOrderSuggest(PurchaseTestCommon):
    ...
    def test_purchase_order_suggest_access_error_non_admin(self):
        ...
        self.env['stock.quant']._update_available_quantity(self.product_1, main_warehouse.lot_stock_id, 50)

        self._create_and_process_delivery_at_date(
            [(self.product_1, 10)], today - relativedelta(months=1), warehouse=main_warehouse  # 1 month ago
        )
        self._create_and_process_delivery_at_date(
            [(self.product_1, 10)], today - relativedelta(days=15), warehouse=main_warehouse  # 15 days ago (within last month)
        )

        self.env['stock.quant']._update_available_quantity(self.product_1, main_warehouse.lot_stock_id, -45)  # Leave only 5 units
