...
class ProcurementGroup(models.Model):
    ...
    def _get_product_routes(self, product):
        ...
        buy_routes = self.env['stock.rule'].search([('action', '=', 'buy'), ('picking_type_id.code', '=', 'incoming')]).route_id
        if not product.purchase_ok or not product.seller_ids and not any(route in product.route_ids for route in buy_routes):
            route_ids -= buy_routes
        return route_ids
