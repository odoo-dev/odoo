...
class StockWarehouse(models.Model):
    ...
    def _compute_buy_to_resupply(self):
        buy_warehouse_ids = self._find_or_create_global_route('purchase_stock.route_warehouse0_buy', _('Buy')).warehouse_ids.ids
    ...
    def _inverse_buy_to_resupply(self):
        ...
        for warehouse in self:
            if warehouse.buy_to_resupply:
                buy_route.sudo().warehouse_ids = [Command.link(warehouse.id)]
            else:
                buy_route.sudo().warehouse_ids = [Command.unlink(warehouse.id)]
