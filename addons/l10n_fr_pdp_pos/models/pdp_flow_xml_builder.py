...
class PdpFlow10XMLBuilder(models.AbstractModel):
    ...
    def _get_entry_line_price_unit(self, line):
        pos_entry_ids = self.env.context.get('l10n_fr_pdp_pos_entry_ids')
        is_pos_entry = (
            line.move_id.id in pos_entry_ids
            if pos_entry_ids is not None
            else line.move_id._l10n_fr_pdp_reports_pos_is_transaction_entry()
        )
        if is_pos_entry:
    ...
    def _get_tax_summary(
        ...
        # Reuse the batched lookup when the parent builder processes each entry line.
        builder = self.with_context(l10n_fr_pdp_pos_entry_ids=pos_entries.ids)
        return super(PdpFlow10XMLBuilder, builder)._get_tax_summary(
            move_lines,
            buyer=buyer,
            seller=seller,
