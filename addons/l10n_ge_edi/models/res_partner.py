...
class ResPartner(models.Model):
    ...
    def l10n_ge_edi_fetch_un_id(self):
        ...
        for company, partners in self.grouped("company_id").items():
            client = (company or self.env.company)._get_rsge_client()
            user_id = (company or self.env.company).sudo().l10n_ge_edi_user_id
            for partner in partners:
                if not partner.vat:
                    errors.append(
                        self.env._("%s: set a VAT/TIN first.", partner.display_name)
                    )
                    continue
                try:
                    partner.l10n_ge_edi_un_id = client.get_un_id_from_tin(
                        user_id=user_id, tin=partner.vat
                    )
                except RSgeError as error:
                    errors.append(
                        f"{partner.display_name}: {translate_rsge_error(self.env, error)}"
                    )
        if errors:
            message, notif_type = "\n".join(errors), "danger"
        else:
            message, notif_type = (
                self.env._("RS.ge Un Id fetched successfully!"),
                "success",
            )
