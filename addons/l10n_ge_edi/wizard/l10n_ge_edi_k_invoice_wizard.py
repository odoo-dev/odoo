...
class L10nGeEdiKInvoiceWizard(models.TransientModel):
    ...
    def _l10n_ge_edi_create_k_invoice_credit_note(self, k_type):
        ...
        try:
            k_id = client.save_k_invoice(
                user_id=user_id,
                invoice_id=int(original.l10n_ge_edi_invoice_id),
                k_type=int(k_type),
            )
        except RSgeError as error:
            raise UserError(translate_rsge_error(self.env, error)) from error

        new_move = original._reverse_moves(
            [{"l10n_ge_edi_invoice_id": str(k_id), "l10n_ge_edi_k_type": k_type}],
        )
        ...
        original.message_post(
            body=self.env._(
                "Corrective invoice created: %(new_move)s",
                new_move=new_move._get_html_link(),
            ),
        )
    ...
    def _l10n_ge_edi_create_k_invoice_type_1(self):
        ...
        self.move_id.l10n_ge_edi_refresh_status()
    ...
    def _l10n_ge_edi_create_k_invoice_type_3(self):
        ...
        if original.line_ids.filtered("reconciled"):
            raise UserError(
                self.env._(
        ...
        new_move = original.copy(
            {"l10n_ge_edi_invoice_id": str(k_id), "l10n_ge_edi_k_type": "3"},
        )

        new_lines = new_move.invoice_line_ids.filtered(
        ...
        original.l10n_ge_edi_refresh_status()
