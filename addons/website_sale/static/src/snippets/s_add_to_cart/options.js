...
options.registry.AddToCart = options.Class.extend({
    ...
    _updateButtonContent() {
        const action = this.$target[0].dataset.action;
        const buttonEl = this.$target[0].querySelector('.s_add_to_cart_button');

        this._buildButtonContent();

        const buttonTextEl = buttonEl.querySelector('text');
        const buttonIconEl = buttonEl.querySelector('i');

        const buttonContent = {
            add_to_cart: {text: _t("Add to Cart"), iconClass: 'fa fa-cart-plus'},
            buy_now: {text: _t("Buy Now"), iconClass: 'fa fa-credit-card'},
        };
        buttonTextEl.textContent = buttonContent[action].text;
        buttonIconEl.setAttribute('class', buttonContent[action].iconClass);
    },

    /**
     * This is used to rebuild the button's content in case it was deleted.
     * It creates an icon and a text tag with the right class
     */
    _buildButtonContent() {
        const buttonEl = this.$target[0].querySelector('.s_add_to_cart_button');
        ...
        if (!iconEl) buttonEl.append(document.createElement('i'));

        let buttonTextEl = buttonEl.querySelector('text');
        if (!buttonTextEl) {
            buttonTextEl = document.createElement('text');
            buttonTextEl.setAttribute('class', 's_add_to_cart_button_text ml-1');
