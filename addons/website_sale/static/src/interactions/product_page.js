...
export class ProductPage extends Interaction {
    ...
    onChangeAddQuantity(ev) {
        ...
        if (parent) {
            this._triggerVariantChange(parent);
        }
    }
    ...
    onChangeVariant(ev) {
        ...
        parent
            ...
            .forEach((el) =>
                el.checked ? el.setAttribute("checked", true) : el.removeAttribute("checked")
            );
        parent
            .querySelectorAll("select option")
            .forEach((el) =>
                el.selected ? el.setAttribute("selected", true) : el.removeAttribute("selected")
            );
    ...
    onChangeColorAttribute(ev) {
        ...
        parent
            ...
            .forEach((el) => el.classList.toggle("active", el.matches(":has(input:checked)")));
        ...
        if (variantContainer) {
            ...
            if (customInput && customInput.dataset.isCustom === "True") {
                ...
                if (
                    ...
                    customValueInput.dataset.customProductTemplateAttributeValueId =
                        attributeValueId;
    ...
    _checkExclusions(parent, combination) {
        const combinationDataJson = parent.querySelector("ul[data-attribute-exclusions]").dataset
            .attributeExclusions;
        ...
        if (combinationData.archived_combinations) {
            combinationData.archived_combinations.forEach((excludedCombination) => {
                const commonPtavs = excludedCombination.filter((ptav) =>
