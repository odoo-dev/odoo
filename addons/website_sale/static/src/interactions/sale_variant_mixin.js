...
export class VariantHandler extends Interaction {
    dynamicContent = {
        ".css_attribute_color input": { "t-on-change": this.onChangeColorAttribute },
        ".o_variant_pills": { "t-on-click": this.onClickVariantPills },
    }
    ...
    handleCustomValues(targetEl) {
        let variantContainerEl = undefined;
        let customInputEl = undefined;
        if (targetEl.matches("input[type='radio']:checked")) {
            variantContainerEl = targetEl.closest("ul").closest("li");
            customInputEl = targetEl;
        } else if (targetEl.matches("select")) {
            variantContainerEl = target.closest("li");
            customInputEl = targetEl.querySelector(`option[value="${target.value}"]`);
        }
        ...
        if (variantContainerEl) {
            if (customInputEl && customInputEl.dataset.is_custom === "True") {
                ...
                if (variantContainerEl.querySelector(".variant_custom_value")?.dataset.custom_product_template_attribute_value_id !== parseInt(attributeValueId)) {
                    variantContainerEl.querySelector(".variant_custom_value").remove();
                }
                if (!variantContainerEl.querySelector(".variant_custom_value")) {
                    const previousCustomValue = customInputEl.getAttribute("previous_custom_value");
                    const inputEl = document.createElement("input");
                    inputEl.type = "text"
                    inputEl.placeholder = attributeValueName
                    inputEl.classList.add("variant_custom_value form-control mt-2 custom_value_radio")
                    inputEl.dataset.custom_product_template_attribute_value_id = attributeValueId
                    inputEl.dataset.attribute_value_name = attributeValueName
                    if (previousCustomValue) {
                        inputEl.value = previousCustomValue;
                    }
                    variantContainerEl.appendChild(inputEl);
                }
            } else {
                variantContainerEl.querySelector(".variant_custom_value").remove();
            }
    ...
    onClickAddCartJSON(ev) {
        // TODO : Should the .prevent be put in the interaction that extend this one ?
        ev.preventDefault();
    ...
    */
     ...
     *
     * @private
     * @param {HTMLElement} parentEl the parent container to apply exclusions
     * @param {Array} combination the selected combination of product attribute values
 */
    checkExclusions(parentEl, combination) {
        const combinationData = parentEl.querySelector("ul[data-attribute_exclusions]").dataset.attribute_exclusions;
        ...
        for (const variantPillsEl of variantPillsEls) {
            ...
            variantPillsEl.setAttribute("title", variantPillsEl.dataset.value_name || "");
    ...
    disableInput(parentEl, attributeValueId, excludedBy, attributeNames, productName) {
        const inputEl = parentEl.querySelector(`option[value="${attributeValueId}"], input[value="${attributeValueId}]"`);
        ...
        if (excludedBy && attributeNames) {
            const targetEls = inputEl.matches("option") ? [inputEl] : [inputEl, inputEl.closest("label")];
            const excludedByData = [];
            for (const targetEl of targetEls) {
                if (targetEl.dataset.excludedBy) {
                    excludedByData.concat(JSON.parse(targetEl.dataset.excludedBy));
                }
            }
    ...
    onChangeCombination(ev, parentEl, combination) {
        ...
        const pricePerUomEl = parentEl.querySelector(".o_base_unit_price:first .oe_currency_value");
        if (pricePerUomEl) {
            if (isCombinationPossible && combination.base_unit_price != 0) {
                let pricePerUomAncestor = pricePerUomEl.closest(".o_base_unit_price_wrapper");
                while (pricePerUomAncestor) {
                    pricePerUomAncestor.classList.remove("d-none");
                    pricePerUomAncestor = pricePerUomEl.closest(".o_base_unit_price_wrapper");
                }
                pricePerUomEl.innerText = this.priceToString(combination.base_unit_price);
                parentEl.querySelector(".oe_custom_base_unit:first").innerText = combination.base_unit_name;
            } else {
                let pricePerUomAncestor = pricePerUomEl.closest(".o_base_unit_price_wrapper");
                while (pricePerUomAncestor) {
                    pricePerUomAncestor.classList.add("d-none");
                    pricePerUomAncestor = pricePerUomEl.closest(".o_base_unit_price_wrapper");
                }
            }
        }
        ...
        if ("product_tracking_info" in combination) {
            ...
            productEl.dataset.productTrackingInfo = combination["product_tracking_info"];
            productEl.dispatchEvent(new Event("view_item_event", combination["product_tracking_info"]));
        }
        ...
        if (combination.prevent_zero_price_sale) {
            ...
            productPriceEl.classList.classList.add("d-none");
            ...
            productUnavailableEl.classList.remove("d-none")
            productUnavailableEl.classList.add("d-flex");
        } else {
            ...
            productUnavailableEl.classList.remove("d-flex")
            productUnavailableEl.classList.add("d-none");
        }

        const priceEl = parentEl.querySelector(".oe_price:first .oe_currency_value");
        const defaultPriceEl = parentEl.querySelector(".oe_default_price:first .oe_currency_value");
        const comparePriceEl = parentEl.querySelector(".oe_compare_list_price")
        priceEl.innerText = this.priceToString(combination.price);
        defaultPriceEl.innerText = this.priceToString(combination.list_price);
    ...
    priceToString(price) {
        ...
        if (document.querySelectorAll(".decimal_precision")) {
            const decimalPrecisionEls = document.querySelector(".decimal_precision");
            const lastPrecisionEl = decimalPrecisionEls[decimalPrecisionEls.length - 1];
            precision = parseInt(lastPrecisionEl.dataset.precision);
        }
    ...
    */
    updateProductImage(container, displayImage, productId, productTemplateId) {
        ...
        const imageSrc = `/web/image/${model}/${modelId}/${this.productImageField ? this.productImageField : "image_1024"}`
    ...
    */
    onChangeColorAttribute(ev) {
        ...
        for (const cssAttributeColorEl of cssAttributeColorEls) {
            cssAttributeColorEl.classList.remove("active")
            if (cssAttributeColorEl.querySelector("input:checked")) {
                cssAttributeColorEl.classList.add("active")
            }
        }
    }
