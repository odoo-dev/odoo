...
class SaleOrder(models.Model):
    ...
    def _compute_abandoned_cart(self):
        for order in self:
            ...
            if order.website_id and order.state == 'draft' and order.date_order:
                ...
                # by default the expiration date is 1 hour if not specified on the website settings.
    ...
    @staticmethod
    def _search_is_unfulfilled(operator, value):
        if operator not in {'=', '!='}:
            return NotImplemented
    ...
    def _get_confirmation_template(self):
        """Override of `sale`, use the website specific order confirmation email template if set."""
        self.ensure_one()
    ...
    @staticmethod
    def _check_combo_quantities(line) -> bool:
    ...
    def _cart_recovery_email_send(self):
        """Send the cart recovery email on the current recordset, making sure that the portal token
        exists to avoid broken links, and marking the email as sent.
    ...
    @staticmethod
    def _get_period_domain(field, period_days, previous=False):
