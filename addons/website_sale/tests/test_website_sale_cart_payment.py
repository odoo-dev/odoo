...
class WebsiteSaleCartPayment(PaymentHttpCommon, WebsiteSaleCommon):
    ...
    def test_unpaid_orders_can_be_retrieved(self):
        ...
        for unpaid_order_tx_state in ('draft', 'cancel', 'error'):
            ...
            with mock_website_sale_request(website.env, website=website, sale_order_id=self.cart.id) as req:
                self.assertEqual(
                    req.cart,
                    self.cart,
    ...
    def test_paid_orders_cannot_be_retrieved(self):
        ...
        for paid_order_tx_state in ('pending', 'authorized', 'done'):
            ...
            with mock_website_sale_request(self.env, website=self.website, sale_order_id=self.cart.id) as req:
                self.assertFalse(
                    req.website._get_and_cache_current_order(),
