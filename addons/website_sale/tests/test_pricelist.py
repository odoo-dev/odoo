...
class TestWebsitePriceListAvailable(WebsiteSaleCommon):
    ...
    def test_property_product_pricelist_for_inactive_partner(self):
        ...
        # pricelists.
        # Real case if for public user. His `property_product_pricelist` need to be set as it is
        # passed through `_get_pl_partner_order` as the `website_pl` when searching for available
        # pricelists for active users.
...
class TestWebsitePriceListAvailableGeoIP(TestWebsitePriceListAvailable):
    ...
    def test_get_pricelist_available_geoip4(self):
        ...
        self.assertEqual(
            ...
            "Only pricelists for BE, accessible en website and selectable should be returned. It"
            " should also return the applied promo pl",
        )
...
class TestWebsitePriceListMultiCompany(TransactionCaseWithUserDemo):
    ...
    def test_property_product_pricelist_multi_company(self):
        ...
        self.env(user=self.user_demo)["product.pricelist"].browse(demo_pl.id).name  # noqa: B018
