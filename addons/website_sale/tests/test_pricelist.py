...
class TestWebsitePriceList(WebsiteSaleCommon):
    ...
    def setUpClass(cls):
        ...
        cls.benelux = cls.env['res.country.group'].create({
            'name': 'BeNeLux',
            'country_ids': [
                Command.set((cls.country_be + cls.env.ref('base.lu') + cls.env.ref('base.nl')).ids)
            ],
        })
        ...
        cls.args = {'show': False, 'current_pl': False}
    ...
    def _get_pricelist_available(self, show_visible=False):  # noqa: ARG002
        return self.get_pl(
    ...
    def test_pricelist_item_based_on_cost_for_templates(self):
        ...
        with self.mock_request(website_sale_current_pl=pricelist.id) as request:
            ...
            msg = (
                "Template has no variants, the price should be computed based on the template's "
                "cost."
            )
            ...
            msg = (
                "Template has variants, the price should be computed based on the 1st variant's "
                "cost."
            )
    ...
    def test_pricelist_anonymous_user(self):
        ...
        order_sudo = (
            ...
            .create({
                ...
                'order_line': [
                    Command.create({'name': self.product.name, 'product_id': self.product.id})
                ],
            })
        )
        ...
        with self.mock_request(
            website_sale_current_pl=list_benelux_2.id, website_sale_selected_pl_id=list_benelux_2.id
        ):
...
class TestWebsitePriceListAvailable(WebsiteSaleCommon):
    ...
    def setUpClass(cls):
        ...
        cls.backend_pl = Pricelist.create({'name': 'Backend Pricelist', 'website_id': False})
        ...
        cls.w1_pl = Pricelist.create({'name': 'Website 1 Pricelist', 'website_id': cls.website.id})
    ...
    def test_get_pricelist_available(self):
        ...
        self.assertEqual(
            pls,
            pls_to_return,
            "Every pricelist having the correct website_id set or (no website_id but a code or"
            " selectable) should be returned",
        )
        ...
        self.assertEqual(
            pls,
            pls_to_return,
            "Only selectable pricelists website compliant (website_id False or current website) "
            "should be returned",
        )

    def test_property_product_pricelist_for_inactive_partner(self):
        # `_get_partner_pricelist_multi` should consider inactive users when searching for
        # pricelists. Real case if for public user. His `property_product_pricelist` need to be set
        # as it is passed through `_get_pl_partner_order` as the `website_pl` when searching for
        # available pricelists for active users.
        public_partner = self.public_partner
        self.assertFalse(
            public_partner.active, "Ensure public partner is inactive (purpose of this test)"
        )
        pl = public_partner.property_product_pricelist
        self.assertEqual(
            len(pl), 1, "Inactive partner should still get a `property_product_pricelist`"
        )
...
class TestWebsitePriceListAvailableGeoIP(TestWebsitePriceListAvailable):
    def setUp(self):
        ...
        c_BENELUX = self.env['res.country.group'].create({
            ...
            'country_ids': [
                (
                    ...
                    0,
                    (
                        self.env.ref('base.be') + self.env.ref('base.lu') + self.env.ref('base.nl')
                    ).ids,
                )
            ],
        })
        ...
        (
            self.backend_pl + self.generic_pl_select + self.generic_pl_code + self.w1_pl_select
    ...
    def test_get_pricelist_available_geoip(self):
        ...
        self.assertEqual(
            pls,
            self.website1_be_pl,
            "Only pricelists for BE and accessible on website should be returned, and the partner"
            " pl",
        )
    ...
    def test_get_pricelist_available_geoip2(self):
        ...
        self.assertEqual(
            pls,
            self.website1_be_pl,
            "Only pricelists for BE and accessible on website should be returned as partner pl is "
            "not website compliant",
        )
    ...
    def test_get_pricelist_available_geoip3(self):
        ...
        self.assertEqual(
            pls,
            self.website1_be_pl,
            "Only pricelists for BE and accessible on website should be returned, but not the "
            "partner pricelist as it is website compliant but not GeoIP compliant.",
        )
    ...
    def test_get_pricelist_available_geoip4(self):
        ...
        self.assertEqual(
            ...
            "Only pricelists for BE, accessible en website and selectable should be returned."
            " It should also return the applied promo pl",
        )
...
class TestWebsitePriceListHttp(HttpCaseWithUserPortal):
    def test_get_pricelist_available_multi_company(self):
        ...
        self.assertEqual(
            r.status_code,
            200,
            "The page should not raise an access error because of reading pricelists from other "
            "companies",
        )
...
class TestWebsitePriceListMultiCompany(TransactionCaseWithUserDemo):
    ...
    def test_property_product_pricelist_multi_company(self):
        ...
        pricelist_name = self.env(user=self.user_demo)['product.pricelist'].browse(demo_pl.id).name
        self.assertIsNotNone(pricelist_name)
