    ...
    * address choice (/shop/checkout)
    ...
    def test_02_demo_address_and_company(self):
        ...
        with self.mock_request(user=self.demo_user, sale_order_id=so.id) as req:
            ...
            self.assertTrue(
                ...
                "Logged in user new shipping should not get the company of the sudo() neither the "
                "one from it's partner..",
            )
            self.assertEqual(
            ...
            self.assertEqual(
                ...
                self.company_b,
                "Logged in user edited billing (the partner itself) should not get its company"
                " modified.",
            )
            self.assertNotEqual(
    ...
    def test_03_public_user_address_and_company(self):
        ...
        with self.mock_request(sale_order_id=so.id) as req:
            ...
            self.assertEqual(
                new_partner.company_id,
                self.website.company_id,
                "Public user edited billing (the partner itself) should not get its company "
                "modified.",
            )
    ...
    def test_11_payment_term_when_address_change(self):
        ...
        with self.mock_request(user=self.portal_user) as req:
            ...
            self.assertFalse(
                so.payment_term_id,
                "The website default payment term should not be set on a sale order not coming "
                "from the website.",
            )
