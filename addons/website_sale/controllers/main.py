...
from odoo.tools import clean_context, lazy, str2bool
...
def handle_product_params_error(exc, product, category=None, **kwargs):
    ...
    if category and not (product and product.has_access('read')):
        return request.redirect(get_shop_path(category))
...
class WebsiteSale(payment_portal.PaymentPortal):
    ...
    def product_document(self, product_template, document_id):
        ...
        if not document or not document.active:
            return request.redirect(get_shop_path())
    ...
    def _prepare_product_values(self, product, category, **kwargs):
        ...
        if (last_attributes_search := request.session.get('attribute_values', [])):
            keep = QueryURL(
                get_shop_path(category),
                attribute_values=last_attributes_search
            )
        else:
            keep = QueryURL(get_shop_path(category))
            ...
            'itemListElement': [
                {
                    ...
                    'position': 1,
                    'name': 'All Products',
                    'item': f'{base_url}{get_shop_path()}',
                },
                {
                    '@type': 'ListItem',
                    'position': 2,
                    'name': category.name,
                    'item': f'{base_url}{get_shop_path(category)}',
                },
                {
                    '@type': 'ListItem',
                    'position': 3,
    ...
    def pricelist_change(self, pricelist, **post):
        ...
        return request.redirect(redirect_url or get_shop_path())
