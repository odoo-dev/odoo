...
class WebsiteSale(payment_portal.PaymentPortal):
    ...
    @staticmethod
    def sitemap_products(env, _rule, qs):
        website = env['website'].get_current_website()
        if website and website.ecommerce_access == 'logged_in' and not qs:
            # Make sure urls are not listed in sitemap when restriction is active
            # and no autocomplete query string is provided
    ...
    def _get_additional_shop_values(self, _values, **_kwargs):
        """Get additional values needed on the shop page.
    ...
    @route(
        ...
        handle_params_access_error=lambda _e, **_kwargs: NotFound.code,
    )
    def shop(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, tags='', **post):
        ...
        if filter_by_price_enabled:
            ...
            # ~4 times more efficient than a search for the cheapest and most expensive products.
    ...
    def add_product_media(
        ...
        """Add images and videos to product variants or templates, and link all of them to product.
        ...
        if type == 'image':  # Image case
            ...
            media_create_data = [
                Command.create({
                    # Images uploaded from url do not have any datas. This recovers them manually.
                    'name': image.name,
    ...
    def _include_country_and_state_in_address(self, address):
        """Fetch country_id and state_id and include the records in address.
        The object is included to simplify the comparison of addresses.
    ...
    def shop_payment_validate(self, sale_order_id=None, **_post):
        """Validate payment, called by the server when receiving an update for a transaction.
    ...
    def _prepare_shop_payment_confirmation_values(self, order):
        """Prepare payment confirmation page values.
    ...
    def order_2_return_dict(self, order):
        """Return the tracking_cart dict of the order for Google analytics basically defined
        to be inherited."""
        tracking_cart_dict = {
