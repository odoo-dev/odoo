...
class Website(main.Website):
    ...
    @route()
    def autocomplete(self, search_type=None, term=None, order=None, limit=5, offset=0, max_nb_chars=999, options=None):
        options = options or {}
        if 'display_currency' not in options:
            options['display_currency'] = request.website.currency_id
        return super().autocomplete(search_type, term, order, limit, offset, max_nb_chars, options)
