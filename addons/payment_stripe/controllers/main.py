...
class StripeController(http.Controller):
    ...
    def stripe_return_from_checkout(self, **data):
        ...
        _logger.info(f"received payment_intents response:\n{pprint.pformat(payment_intent)}")
    ...
    def stripe_return_from_validation(self, **data):
        ...
        _logger.info(f"received checkout/session response:\n{pprint.pformat(checkout_session)}")
    ...
    def _verify_webhook_signature(self, webhook_secret):
        ...
        if datetime.utcnow().timestamp() - event_timestamp > self.WEBHOOK_AGE_TOLERANCE:
            _logger.warning(f"ignored webhook event due to age tolerance: {event_timestamp}")
            return False
