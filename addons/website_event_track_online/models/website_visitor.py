...
class WebsiteVisitor(models.Model):
    ...
    event_track_ids = fields.Many2many(
        'event.track', string="Wishlisted Tracks",
        compute="_compute_event_tracks", compute_sudo=True,
        search="_search_event_track_ids",
        groups='event.group_event_user')
    event_track_count = fields.Integer(
        string="# Wishlisted",
        compute="_compute_event_tracks", compute_sudo=True,
        groups='event.group_event_user')
    event_track_wchildren_ids = fields.Many2many(
        'website.visitor', string="Visitor Wishlist (incl. duplicates)",
        compute="_compute_event_track_wchildren_ids", compute_sudo=True,
        search="_search_event_track_wchildren_ids",
        groups="event.group_event_user")
    ...
    def _compute_event_tracks(self):
        ...
        for visitor in self:
            visitor.event_track_ids = track_ids_map.get(visitor.id, [])
            visitor.event_track_count = len(track_ids_map.get(visitor.id, []))

    def _search_event_track_ids(self, operator, operand):
        if operator == "not in":
    ...
    @api.depends('parent_id', 'event_track_visitor_ids.track_id')
    def _compute_event_track_wchildren_ids(self):
        ...
        for visitor in self:
            ...
            visitor.event_track_wchildren_ids = visitor_track_ids + [track_id for track_id in parent_track_ids if track_id not in visitor_track_ids]

    def _search_event_track_wchildren_ids(self, operator, operand):
        """ Search visitors with terms on wishlisted tracks. E.g. [('event_track_wchildren_ids',
    ...
    def _link_to_visitor(self, target, unlink=False):
        ...
        return super(WebsiteVisitor, self)._link_to_visitor(target, unlink=unlink)
