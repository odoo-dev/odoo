...
class TestSubcontractingTracking(TransactionCase):
    ...
    def test_subcontracting_lot_tracked_product_no_extra_quants(self):
        """Ensure that lot-tracked products with a subcontracting route do not create extra quants at the
            subcontractor location. This test resupplies the components, creates a receipt picking from the
            subcontractor, processes the delivery of components, produces the finished product with lots,
            and validates the receipt, checking that the stock quantities are correctly updated."""
        resupply_sub_on_order_route = self.env['stock.route'].search([('name', '=', 'Resupply Subcontractor on Order')])
        self.comp1_sn.tracking = 'lot'
        self.comp1_sn.route_ids = [(4, resupply_sub_on_order_route.id)]
        self.comp2.route_ids = [(4, resupply_sub_on_order_route.id)]
        ...
        self.env['stock.quant']._update_available_quantity(self.comp1_sn, self.env.ref('stock.stock_location_stock'), 10, lot_id=comp1_sn_lot)
        self.env['stock.quant']._update_available_quantity(self.comp2, self.env.ref('stock.stock_location_stock'), 10)
