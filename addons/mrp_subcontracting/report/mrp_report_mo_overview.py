...
class ReportMrpReport_Mo_Overview(models.AbstractModel):
    ...
    def _get_components_data(self, production, replenish_data=False, level=0, current_index=False):
        ...
        if self._is_subcontracting(production) and production.state != 'done' and components:
            self_ctx = self.with_context(warehouse_location=True)
            warehouse_replenish_data = None
            for count, (component, move_raw) in enumerate(zip(components, production.move_raw_ids)):
                if not (
                    component['replenishments']
                    and move_raw.procure_method != 'make_to_stock'
                    and not self._is_dropshipped_component(production, move_raw)
                ):
                    continue
                ...
                component['replenishments'] += replenishments
        return components

    def _is_dropshipped_component(self, production, move_raw):
        rules = move_raw.product_id._get_rules_from_location(
            production.location_src_id, route_ids=move_raw.route_ids,
        )
        return any(rule.picking_type_id.code == 'dropship' for rule in rules)
    ...
    def _get_location_ids(self, production, replenish_data):
        if self._is_subcontracting(production):
            if self.env.context.get('warehouse_location'):
                return production.picking_ids.location_id.ids
