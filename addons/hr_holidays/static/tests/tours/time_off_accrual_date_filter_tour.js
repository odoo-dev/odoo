...
registry.category("web_tour.tours").add("time_off_accrual_date_filter_tour", {
    ...
    steps: () => [
        ...
        {
            content: "Check the days to make sure 31 is not available",
            trigger: ".o_select_menu_menu",
            run: function () {
                const menus = document.querySelectorAll(".o_select_menu_menu");
                const activeMenu = menus[menus.length - 1];
                const options = Array.from(activeMenu.querySelectorAll(".o_select_menu_item")).map(
                    (el) => el.innerText.trim()
                );
                if (!options.includes("31") || !options.includes("31")) {
                    throw new Error(
                        "Validation Failed: January should show days 30 and 31. Found: " +
                            options.join(", ")
                    );
                }
            },
        ...
        {
            content: "Check if the day snapped to 29",
            trigger: 'div[name="yearly_day"]',
            run: function () {
                const input = document.querySelector(
                    'div[name="yearly_day"] input.o_select_menu_input'
                );
                const currentDay = input ? input.value.trim() : "";

                console.log("Tour detected day value:", currentDay);

                if (currentDay === "31") {
                    throw new Error("Snap-back failed: Input still shows 31!");
                }
                if (currentDay != "29" && currentDay !== "") {
                    // If it's not 31 but not 29, it might have cleared entirely
                    console.log("Day was cleared or set to: " + currentDay);
                }
            },
        },
        {
        ...
        {
            content: "Check the days to make sure 30 and 31 are not available",
            trigger: ".o_select_menu_menu",
            run: function () {
                const menus = document.querySelectorAll(".o_select_menu_menu");
                const activeMenu = menus[menus.length - 1];
                ...
                if (options.includes("30") || options.includes("31")) {
                    throw new Error(
                        "Validation Failed: February should NOT show 30 or 31. Found: " +
                            options.join(", ")
                    );
                }
            },
        ...
        },
        {
            ...
            run: function () {
                ...
                if (options.includes("31")) {
                    throw new Error(
                        "Validation Failed: April should NOT show day 31. Found: " +
                            options.join(", ")
                    );
                }
            },
