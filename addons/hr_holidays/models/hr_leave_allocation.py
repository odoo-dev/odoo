...
class HolidaysAllocation(models.Model):
    ...
    employee_count = fields.Integer(string="Affected employees", compute='_compute_from_holiday_type',
                                    help="Display the number of employee impacted by this allocation", compute_sudo=True)
    ...
    def _compute_from_holiday_type(self):
        for allocation in self:
            ...
            allocation.employee_count = len(employees.ids)
    ...
    def create(self, values):
        ...
        if values.get('date_from') and fields.Date.from_string(values.get('date_from')) < fields.Date.context_today(self):
            raise ValidationError(_("The accrual start date must be today or after"))
