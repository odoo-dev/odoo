...
class PosOrderLine(models.Model):
    ...
    def _points_for_correction(self):
        ...
        for rule in self.coupon_id.program_id.rule_ids:
            order = self.order_id
            if order.is_refund:
                order = order.refunded_order_id
            amount_to_compare = order.amount_total if rule.minimum_amount_tax_mode == "incl" else (order.amount_total - order.amount_tax)
            qty_to_compare = sum(line.qty for line in order.lines.filtered(lambda l: not l.coupon_id) if rule.any_product or (line.product_id in rule.valid_product_ids))
