...
class PosOrder(models.Model):
    ...
    sell_loyalty_card_ids = fields.Many2many(
        comodel_name='loyalty.card', string="Sold Loyalty Cards",
        help="The coupons generated by this order line.")
    ...
    def _add_loyalty_history_lines(self, sell_loyalty_cards, loyalty_cards_to_update, loyalty_cards_to_update_before):
        print("\n\n\n sell_loyalty_cards", sell_loyalty_cards)
        print("\n\n\n loyalty_cards_to_update", loyalty_cards_to_update)
        print("\n\n\n loyalty_cards_to_update_before", loyalty_cards_to_update_before)
        loyalty_cards_to_update_dict = {item['coupon_id']: item for item in loyalty_cards_to_update}
        loyalty_cards_to_update_before_dict = {item['id']: item for item in loyalty_cards_to_update_before}
        history_lines_create_vals = []
        # breakpoint()
        for coupon in sell_loyalty_cards:
            order = self.browse(coupon['order']).exists()
            points = 0
            if not loyalty_cards_to_update_dict.get(coupon['id']):
                points = coupon['points']
            else:
                card_id = coupon['id']
                before = loyalty_cards_to_update_before_dict.get(card_id)
                if not before:
                    continue
                points = coupon['points'] - before['points']
    ...
    def sync_from_ui(self, orders):
        ...
        loyalty_cards_to_update_before = []
        sell_loyalty_cards = []
        card_fields = self._load_pos_data_fields(orders[0].get('config_id'))
        history_lines_create_vals = []
        # breakpoint()
        for order in orders:
            skip_deduction = order.get('is_refund', False)
            print("\n\n\n\n",order.get('sell_loyalty_card_ids', []))
            print("\n\n\n\n")
            for sell_loyalty_card in order.get('sell_loyalty_card_ids', []):
                if sell_loyalty_card[1]:
                    loyalty_card = self.env['loyalty.card'].browse(sell_loyalty_card[1]).read(card_fields, load=False)[0]
                    loyalty_cards_to_update_before.append({'order': order.get('access_token'), **loyalty_card})
                    loyalty_cards_to_update.append({'order': order.get('access_token'), 'coupon_id': sell_loyalty_card[1], **sell_loyalty_card[2]})
                sell_loyalty_card[2]['code'] = sell_loyalty_card[2]['code'] or self.env['loyalty.card']._generate_code()

        print("\n\n\n orders ",orders)
        data = super().sync_from_ui(orders)
        created_orders = self.browse([order['id'] for order in data['pos.order'] if order.get('id')]).exists()
        print("\n\n\n created_orders ",created_orders.read())
        for created_order in created_orders:
            for loyalty_card in created_order.sell_loyalty_card_ids:
                sell_loyalty_cards.append({'order': created_order.id, **loyalty_card.read(card_fields, load=False)[0]})
        self._add_loyalty_history_lines(sell_loyalty_cards, loyalty_cards_to_update, loyalty_cards_to_update_before)
        if not skip_deduction:
            sell_orders = created_orders.filtered(lambda o: not o.is_refund)
            history_lines_create_vals = []
            for line in sell_orders.lines.filtered('coupon_id'):
                ...
                point_to_correct = 0
                if coupon_id.program_type not in ('coupons', 'promo_code', 'gift_card', 'next_order_coupons'):
                    point_to_correct = line._points_for_correction()
                print("\n\n\n points",coupon_id.points, 'points_cost', line.points_cost, 'point_to_correct', point_to_correct )
                coupon_id.write({'points': coupon_id.points - line.points_cost - point_to_correct})
            self.env['loyalty.history'].create(history_lines_create_vals)
        return data
    ...
    #         for command in order.get('sell_loyalty_card_ids', []):
    #             vals = command[2]
    #             coupon_id = vals.get('coupon_id')

    #             if isinstance(coupon_id, int):
    #                 card = self.env['loyalty.card'].browse(coupon_id).read(card_fields, load=False)[0]
    #                 loyalty_cards_to_update_before.append({
    #                     'order': order['access_token'],
    #                     **card,
    #                 })
    #                 loyalty_cards_to_update.append({
    #                     'order': order['access_token'],
    #                     **vals,
    #                 })

    #             vals['code'] = vals['code'] or self.env['loyalty.card']._generate_code()

    #     data = super().sync_from_ui(orders)

    #     created_orders = self.browse([o['id'] for o in data['pos.order'] if o.get('id')]).exists()

    #     for order in created_orders:
    #         for card in order.sell_loyalty_card_ids:
    #             sell_loyalty_cards.append({
    #                 'order': order.id,
    #                 **card.read(card_fields, load=False)[0],
    #             })
    ...
    #         correction = line._points_for_correction() if coupon.program_type not in ('coupons', 'promo_code', 'gift_card', 'next_order_coupons') else 0
    #         coupon.points = coupon.points - line.points_cost - correction
