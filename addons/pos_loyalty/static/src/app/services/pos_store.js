...
patch(PosStore.prototype, {
    ...
    async preSyncAllOrders(orders) {
        ...
        for (const order of orders) {
            ...
            this.rewardProductByLineUuidCache[order.uuid] = order.loyalty_card_ids.map(
                (lc) => lc.uuid
            );
    ...
    async postSyncAllOrders(orders) {
        ...
        for (const order of orders) {
            ...
            if (this.rewardProductByLineUuidCache[orderUuid]) {
                const loyaltyCards = this.models["loyalty.card"].filter((lc) =>
                    this.rewardProductByLineUuidCache[orderUuid].includes(lc.uuid)
                );
    ...
    async _postProcessLoyalty(order) {
        ...
        for (const { coupon_id } of order._get_reward_lines()) {
            if (!coupon_id) {
                continue;
            }
            couponIds.add(coupon_id.id);
            programIds.add(coupon_id.program_id.id);
        }
        await Promise.all([
            this.data.searchRead("loyalty.card", [["id", "in", [...couponIds]]]),
            this.data.searchRead("loyalty.program", [["id", "in", [...programIds]]]),
        ]);
        order.new_coupon_info = order.loyalty_card_ids
            ...
            .map(({ program_id, expiration_date, code }) => ({
