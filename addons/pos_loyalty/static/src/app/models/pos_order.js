...
patch(PosOrder.prototype, {
    ...
    getLoyaltyPoints() {
        ...
        for (const pointChange of Object.values(this.uiState.couponPointChanges)) {
            ...
            const balance = loyaltyCard.points - won - this._getPointsCorrection(program);
            if (coupon_id !== 0) {
    ...
    processGiftCard(newGiftCardCode, points, expirationDate) {
        ...
        const loyaltyCard = this.models["loyalty.card"].find(
            (card) => card.code === newGiftCardCode
        );
        ...
        if (loyaltyCard) {
            ...
            this.sell_loyalty_card_ids = [["link", loyaltyCard]];
        ...
        for (const line of this.getOrderlines()) {
            if (
                ...
                if (line.reward_id.id == reward.id) {
                    console.log("line points cost", line.points_cost);
