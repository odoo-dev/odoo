...
class TestProgramWithCodeOperations(TestSaleCouponCommon):
    ...
    def test_program_usability(self):
        ...
        # But when you click, domain is false (default field value; empty string) so it won't
        # generate anything. This is even more weird because if you add something in the domain and
        # then delete it, you visually come back to the initial state except the domain became '[]'
    ...
    def test_program_basic_operation_coupon_code(self):
        # Test case: Generate a coupon for a customer, and add a reward then remove it
        # automatically.
    ...
    def test_on_next_order_reward_promotion_program(self):
        # The flow:
        # 1. Create a program `A` that gives a free `Product B` on next order if you buy a
        #    `product A`. This program should be code_needed with code `free_B_on_next_order`.
        # 2. Create a program `B` that gives 10% discount on next order automatically.
        # 3. Create a SO with a `third product` and recompute coupon, you SHOULD get a coupon (from
        #    program `B`) for your next order that will discount 10%.
        ...
        # 6. Verify you have 2 generated coupons and validate the SO (so the 2 generated coupons
        #    will be valid).
    ...
    def test_on_next_order_reward_promotion_program_with_requirements(self):
        ...
        with self.assertRaises(ValidationError):
            # It should error since we did not validate the previous SO, so the coupon is `reserved`
            # but not `new`.
            self._apply_promo_code(order_bis, order._get_reward_coupons()[0].code)
        order.action_confirm()
        # It should not error even if the SO does not have the requirements (700$ and 1 product A),
        # since these requirements where only used to generate the coupon that we are now applying.
        self._apply_promo_code(order_bis, order._get_reward_coupons()[0].code, no_reward_fail=False)
        self.assertEqual(
            len(order_bis.order_line), 2, "You should get 1 regular product_B and 1 free product_B"
        )
        order_bis._update_programs_and_rewards()
        self.assertEqual(
            len(order_bis.order_line),
            2,
            "Free product from a coupon generated from a promotion program on next order should not"
            " dissapear",
        )
