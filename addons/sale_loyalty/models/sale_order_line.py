...
class SaleOrderLine(models.Model):
    ...
    def create(self, vals_list):
        ...
        if pairs:
            self._rebuild_loyalty_history_usage(pairs)
        return res
    ...
    def _rebuild_loyalty_history_usage(self, pairs):
        """Recompute the consumption history for the given {(order, coupon),..} pairs."""
        loyalty_history = self.env["loyalty.history"].sudo()
        for order, coupon in pairs:
            if not coupon:
                continue
            usage = loyalty_history.search([
                ("card_id", "=", coupon.id),
            ...
            points_cost = sum(coupon_lines.mapped("points_cost"))
            if points_cost:
    ...
    def unlink(self):
        ...
        if pairs:
            self.env["sale.order.line"]._rebuild_loyalty_history_usage(pairs)
