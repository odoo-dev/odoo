...
export class BuilderRow extends Component {
    ...
    appendTooltip(ev) {
        if (!ev.target || !this.labelWrapperRef.el) {
            return;
        }
        const labelEl = ev.target.querySelector(".o_row_label");
        const isLabelTooLong = labelEl && labelEl.offsetWidth < labelEl.scrollWidth;
        if (isLabelTooLong) {
