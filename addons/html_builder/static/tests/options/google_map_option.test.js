...
describe("API key validation", () => {
     ...
     */
    const makeKey = (config = { billing: false, staticApi: false, placesApi: false, javascriptApi: false }) => JSON.stringify(config);
        ...
        patchWithCleanup(GoogleMapOptionPlugin.prototype, {
            ...
            fetchGoogleMap(key) {
                ...
                return {
                    ...
                    billing, staticApi, javascriptApi, placesApi,
                };
            },
            /**
    ...
    test("having an API key with billing, static API, javascript API and places API should not trigger the opening of the API key dialog", async () => {
        const key = makeKey({ billing: true, staticApi: true, javascriptApi: true, placesApi: true });
    ...
    test("having an API key with billing, static API and javascript API but no places API should trigger the opening of the API key dialog even though the response is 200", async () => {
        await keyShouldTriggerDialog({ billing: true, staticApi: true, javascriptApi: true, placesApi: false }, true);
    });
});
