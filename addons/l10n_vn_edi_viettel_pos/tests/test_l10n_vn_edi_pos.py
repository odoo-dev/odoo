...
class TestVNEDIPOS(TestVNEDI, TestPointOfSaleHttpCommon):
    ...
    def test_pos_sinvoice_auto_send_deferred_pdf(self):
        ...
        order = self._create_simple_order()
        cash_pm = self.session.payment_method_ids.filtered(lambda pm: pm.is_cash_count)[:1]
        order.write({
            'to_invoice': True,
            'state': 'paid',
        ...
        with patch('odoo.addons.l10n_vn_edi_viettel.models.sinvoice_service.SInvoiceService.create_invoice',
                   return_value=({'invoiceNo': 'K24TUT01', 'reservationCode': '123456'}, None)), \
             patch('odoo.addons.l10n_vn_edi_viettel.models.sinvoice_service.SInvoiceService.get_access_token',
                   return_value=({'access_token': '123', 'expires_in': '60'}, None)), \
             patch('odoo.addons.l10n_vn_edi_viettel.models.account_move.AccountMove._l10n_vn_edi_fetch_invoice_pdf_file_data',
                   return_value=({'name': 's.pdf', 'mimetype': 'application/pdf', 'raw': b'pdf', 'res_field': 'l10n_vn_edi_sinvoice_pdf_file'}, '')), \
             patch('odoo.addons.l10n_vn_edi_viettel.models.account_move.AccountMove._l10n_vn_edi_fetch_invoice_xml_file_data',
                   return_value=({'name': 's.xml', 'mimetype': 'application/xml', 'raw': b'xml', 'res_field': 'l10n_vn_edi_sinvoice_xml_file'}, '')):
            order._generate_order_invoice()

        invoice = order.account_move
        self.assertTrue(invoice, "Invoice should have been created")
        self.assertEqual(invoice.l10n_vn_edi_invoice_state, 'sent',
                         "SInvoice should be submitted during order validation even with deferred PDF")
