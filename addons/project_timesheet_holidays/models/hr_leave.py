...
class HrLeave(models.Model):
    ...
    def _generate_timesheets(self, ignored_resource_calendar_leaves=None):
        ...
        for leave in self:
            ...
            leave_version = employee.sudo()._get_versions_with_contract_overlap_with_period(leave.request_date_from, leave.request_date_to)[:1]
            if not leave_version:
                leave_version = employee.sudo()._get_version(leave.date_from.date())

            if leave_version.is_fully_flexible or not tz:
                continue

            employee_timezone = ZoneInfo(tz)

            if leave_version.is_flexible and (leave.work_entry_type_request_unit == 'hour' or leave.work_entry_type_request_unit == 'half_day' or leave.date_from.date() == leave.date_to.date()):
                ...
                if leave.work_entry_type_request_unit == 'hour':
                    ...
                    hours = leave_version.hours_per_day / 2
                else:  # Single-day leave
                    hours = leave_version.hours_per_day
                ...
                leave_from, leave_to = leave.date_from, leave.date_to
                if leave.work_entry_type_request_unit == 'day':
                    leave_from = datetime.combine(leave.request_date_from, time.min, tzinfo=employee_timezone)
                    leave_to = datetime.combine(leave.request_date_to, time.max, tzinfo=employee_timezone)
                work_hours_data = employee._list_work_time_per_day(
                    ...
                    calendar=leave_version.resource_calendar_id,
                )[employee.id]
