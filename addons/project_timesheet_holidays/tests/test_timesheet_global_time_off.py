...
class TestTimesheetGlobalTimeOff(common.TransactionCase):
    ...
    def test_timesheet_creation_and_deletion_for_time_off(self):
        ...
        # but not for part_time_employee
        leave_task = self.test_company.leave_timesheet_task_id

        self.assertFalse(leave_task.timesheet_ids.search([('employee_id', '=', self.part_time_employee.id)]).id)
        self.assertEqual(len(leave_task.timesheet_ids.search([('employee_id', '=', self.full_time_employee.id)]).ids), 5)
        self.assertEqual(len(leave_task.timesheet_ids.search([('employee_id', '=', self.full_time_employee_2.id)]).ids), 5)
