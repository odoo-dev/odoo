...
export class SurveySurveyActionHelper extends Component {
    ...
    async onStartFromScratchClick() {
        const action = await this.orm.call("survey.survey", "action_load_sample_custom", []);
        this.action.doAction(action);
    }

    async onTemplateClick(templateInfo) {
        const action = await this.orm.call("survey.survey", "action_load_survey_template_sample", [
