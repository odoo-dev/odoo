...
class TestOptionalHoliday(TestHrHolidaysCommon):
    ...
    def test_optional_holiday_hours_leave(self):
        with Form(self.env['hr.leave'].with_context(default_employee_id=self.employee_hrmanager_id), view="l10n_in_hr_holidays.hr_leave_view_form_inherit") as leave_form:
            leave_form.holiday_status_id = self.leave_type
            leave_form.l10n_in_optional_day_id = self.optional_holiday_1
