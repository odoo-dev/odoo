...
class HrVersion(models.Model):
    ...
    def _get_version_work_entries_values(self, date_start, date_stop):
        ...
        fr_versions = self.filtered(lambda c: c.company_id.country_id.code == 'FR' and c.resource_calendar_id != c.company_id.resource_calendar_id)
        if not fr_versions:
            return result
        ...
        all_leaves = self.env['hr.leave'].search_fetch([
            ...
            ('date_to', '>=', start_dt.astimezone(datetime.UTC).replace(tzinfo=None)),
        ...
        for version in fr_versions:
            ...
            for leave in leaves_per_employee[employee]:
                ...
                company_attendances = company_calendar._attendance_intervals_batch(
                    leave_start_dt, leave_end_dt_fr, resources_per_tz=resources_per_tz,
                )[resource.id]
                ...
                result += [{
                    'date_start': interval[0].astimezone(datetime.UTC).replace(tzinfo=None),
                    'date_stop': interval[1].astimezone(datetime.UTC).replace(tzinfo=None),
