    ...
    def _get_fr_date_from_to(self, date_from, date_to):
        ...
        if not self.request_unit_hours:
            ...
            def adjust_date_range(date_from, date_to, period, attendance_ids, employee_id):
                period_ids_from = attendance_ids.filtered(lambda a: a.day_period in period and int(a.dayofweek) == date_from.weekday())
                period_ids_to = attendance_ids.filtered(lambda a: a.day_period in period and int(a.dayofweek) == date_to.weekday())
                if period_ids_from:
                    min_hour = min(attendance.hour_from for attendance in period_ids_to)
                    date_from = self._to_utc(date_from, min_hour, employee_id)
                if period_ids_to:
                    max_hour = max(attendance.hour_to for attendance in period_ids_from)
            ...
            attendance_ids = self.company_id.resource_calendar_id.attendance_ids.filtered(lambda a: a.day_period != "lunch")
