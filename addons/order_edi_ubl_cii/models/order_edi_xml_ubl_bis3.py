...
class OrderEdiXmlUbl_Bis3(models.AbstractModel):
    ...
    def _import_fill_order(self, order, tree):
        ...
        discounts = [line.pop('discount', 0) for line in lines_vals]
        ...
        # Apply non-zero discounts
        for line, discount in zip(order.order_line, discounts):
            if discount:  # Only write if discount exists and isn’t 0
