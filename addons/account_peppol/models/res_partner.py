...
class ResPartner(models.Model):
    ...
    available_invoice_sending_methods = fields.Json(compute='_compute_available_invoice_sending_methods')
    available_invoice_edi_formats = fields.Json(compute='_compute_available_invoice_edi_formats')
    ...
    def _compute_available_invoice_sending_methods(self):
        methods = dict(self._fields['invoice_sending_method'].selection)
        if self.env.company.country_code not in PEPPOL_LIST:
            methods.pop('peppol')
        self.available_invoice_sending_methods = list(methods)

    @api.depends_context('company')
    @api.depends('invoice_sending_method')
    def _compute_available_invoice_edi_formats(self):
        for partner in self:
            if partner.invoice_sending_method == 'peppol':
                partner.available_invoice_edi_formats = self._get_peppol_formats()
            else:
                partner.available_invoice_edi_formats = list(dict(self._fields['invoice_edi_format'].selection))
