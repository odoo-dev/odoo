...
class TestPeppolMessage(TestAccountMoveSendCommon):
    ...
    def test_available_invoice_sending_methods(self):
        company_us = self.setup_other_company()['company']  # not a valid Peppol country
        self.assertTrue('peppol' in self.valid_partner.with_company(self.env.company).available_invoice_sending_methods)
        self.assertFalse('peppol' in self.valid_partner.with_company(company_us).available_invoice_sending_methods)

    def test_available_invoice_edi_formats(self):
        self.valid_partner.invoice_sending_method = 'peppol'
        self.assertFalse('facturx' in self.valid_partner.available_invoice_edi_formats)
        self.valid_partner.invoice_sending_method = 'email'
        self.assertTrue('facturx' in self.valid_partner.available_invoice_edi_formats)
