...
from odoo.addons.account.tools.partner_identifiers import IDENTIFIERS_METADATA, validate_participant_identifier
...
class PeppolRegistration(models.TransientModel):
    ...
    def _default_peppol_endpoint(self):
        ...
        preferred_identifiers = partner._peppol_get_possible_identifiers(enrich=True)
        if preferred_identifiers:
            preferred_identifier = preferred_identifiers[0]
            eas, _sep, endpoint = preferred_identifier.partition(':')
            return eas, endpoint
        preferred_eas = '0088'  # defaults to GLN
        preferred_eas_seq = 1000
        for _key, metadata in IDENTIFIERS_METADATA.items():
            if metadata.get('scheme') in REGISTRABLE_EAS and partner.country_code in metadata.get('countries', []) and metadata.get('sequence', 100) < preferred_eas_seq:
                preferred_eas = metadata.get('scheme')
                preferred_eas_seq = metadata.get('sequence', 100)
        return preferred_eas, None
