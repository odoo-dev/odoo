...
class PeppolRegistration(models.TransientModel):
    ...
    smp_registration = fields.Boolean(string='Register as a receiver', default=True)  # TODO remove in master
    bank_account_exists = fields.Boolean(compute='_compute_bank_account_exists')
    bank_account_id = fields.Many2one(
        comodel_name='res.partner.bank',
        string="Bank account",
        help="The bank account used for customer invoices.",
        domain=['|', ('company_id', '=', lambda self: self.env.company.id), ('company_id', '=', False)],
    )
        ...
        for wizard in self:
            ...
            if wizard.company_id.country_code == 'BE' and wizard.peppol_eas not in (False, '0208'):
                peppol_warnings['company_peppol_eas_warning'] = {
                    'message': _("The recommended identification for Belgium is BCE. "
                                 "The Endpoint should be the Company Registry number."),
                }
            wizard.peppol_warnings = peppol_warnings or False

    @api.depends('company_id')
    def _compute_bank_account_exists(self):
        for wizard in self:
            wizard.bank_account_exists = self.env['res.partner.bank'].search([
                '|',
    ...
    def button_register_peppol_participant(self):
        ...
        try:
            ...
            _logger.info(f"Peppol: Couldn't register participant as a receiver. \nError: {e}")
