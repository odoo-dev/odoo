...
EXCEPTION_CODE_MESSAGES_MAP: dict[int, Callable[[], str]] = {
    101: lambda: _('Something went wrong with your request'),
    102: lambda: _('Proxy error, please contact Odoo (missing "%s" - please make sure that it was loaded from the configuration panel)'),
    103: lambda: _('The document could not be validated.'),
    104: lambda: _('Could not find an XSD with which to validate the document with identifier "%s".'),
    ...
    203: lambda: _('The Service Metadata Publisher associated to this participant is not reachable. url: %s'),
...
EBMS_CODE_MESSAGES_MAP: dict[int, Callable[[], str]] = {
    ...
    50: lambda: _('The size of the data part in a fragment message is greater than %s'),
    51: lambda: _('More time than %s has passed since the first fragment was received but not all other fragments are received.'),
    52: lambda: _('Message properties were present in the fragment SOAP header that were not specified in %s'),
...
def get_exception_message(peppol_code, default_message=""):
    if peppol_code not in EXCEPTION_CODE_MESSAGES_MAP:
        return default_message
    return EXCEPTION_CODE_MESSAGES_MAP[peppol_code]()

def get_ebms_message(ebms_code, default_message=""):
    if ebms_code not in EBMS_CODE_MESSAGES_MAP:
        return default_message
    return EBMS_CODE_MESSAGES_MAP[ebms_code]()
