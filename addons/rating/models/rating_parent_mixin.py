...
class RatingParentMixin(models.AbstractModel):
    ...
    def _compute_rating_percentage_satisfaction(self):
        ...
        if self._rating_satisfaction_days:
            domain += [('write_date', '>=', fields.Datetime.to_string(self.env.now - timedelta(days=self._rating_satisfaction_days)))]
    ...
    def _search_rating_avg(self, operator, value):
        ...
        if self._rating_satisfaction_days:
            min_date = self.env.now - timedelta(days=self._rating_satisfaction_days)
