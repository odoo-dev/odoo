...
@tagged('-at_install', 'post_install', 'test_pricing_type')
class TestProjectPricingType(TestCommonSaleTimesheet):
    ...
    def test_pricing_type(self):
        """ Test the _compute_pricing_type when the user add a sales order item or some employee mappings in the project
            ...
            3) Set a customer and a SOL in the project and check if the pricing_type is equal to project_rate
        ...
        self.assertEqual(project.pricing_type, 'project_rate', 'The pricing type should be equal to project_rate since the project has a sales order.')
        ...
        # Even the project has no sales order item, since it has an employee rate, the pricing type must be equal to employee_rate.
        project.write({
            'sale_line_id': False,
        })
