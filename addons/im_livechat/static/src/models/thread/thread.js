...
const { FieldCommand, insert, link, noop, unlink } = require('mail/static/src/model/model_field_command.js');
...
registerClassPatchModel('mail.thread', 'im_livechat/static/src/models/thread/thread.js', {
    ...
    convertData(data) {
        ...
        if ('livechat_visitor' in data && data.livechat_visitor) {
            if (!data2.members) {
                data2.members = [];
            } else if(data2.members instanceof FieldCommand) {
                data2.members = [data2.members];
            }
