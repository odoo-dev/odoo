odoo.define("im_livechat.legacy.im_livechat.im_livechat", function (require) {
    ...
    var LivechatButton = Widget.extend({
        ...
        start: function () {
            ...
            if (this._history) {
                ...
                if (!autoPopupCookie || JSON.parse(autoPopupCookie)) {
                    this._autoPopupTimeout = setTimeout(this._openChat.bind(this), this._rule.auto_popup_timer * 1000);
                }
            }
        ...
        _sendMessage: function (message) {
            var self = this;
            return session.rpc("/mail/chat_post", { uuid: this._livechat.getUUID(), message_content: message.content }).then(function (messageId) {
                if (!messageId) {
                    self.displayNotification({
...
odoo.define("im_livechat.legacy.mail.model.AbstractThread", function (require) {
    ...
    var AbstractThread = Class.extend(Mixins.EventDispatcherMixin, {
        ...
        postMessage: function () {
            return this._postMessage.apply(this, arguments).then(this.trigger.bind(this, "message_posted"));
        },
        /**
    ...
    var ThreadTypingMixin = {
        ...
        getTypingMembersToText: function () {
            ...
            if (displayableTypingMembers.length === 0) {
                ...
                return _.str.sprintf(_t("%s and %s are typing..."), displayableTypingMembers[0].name, displayableTypingMembers[1].name);
            } else {
                return _.str.sprintf(_t("%s, %s and more are typing..."), displayableTypingMembers[0].name, displayableTypingMembers[1].name);
            }
        },
        /**
         ...
         */
        _onTypingMessageAdded: function (message) {
            var partnerID = message.hasAuthor() ? message.getAuthorID() : this._DEFAULT_TYPING_PARTNER_ID;
            this.unregisterTyping({ partnerID: partnerID });
        },
        /**
...
odoo.define("im_livechat.legacy.mail.model.AbstractMessage", function (require) {
    ...
    var AbstractMessage = Class.extend({
        ...
        hasNotificationsError() {
            return this.getNotifications().some((notif) => notif.notification_status === "exception" || notif.notification_status === "bounce");
        },
        /**
         ...
         */
        isEmpty: function () {
            return !this.hasTrackingValues() && !this.hasAttachments() && !this.getBody();
        },
        /**
...
odoo.define("im_livechat.legacy.mail.AbstractThreadWindow", function (require) {
    ...
    var AbstractThreadWindow = Widget.extend({
        ...
        renderHeader: function () {
            ...
            this.$header.html(QWeb.render("im_livechat.legacy.mail.AbstractThreadWindow.HeaderContent", options));
        },
        /**
        ...
        _onClickClose: function (ev) {
            ev.stopPropagation();
            ev.preventDefault();
            if (this.hasThread() && this._thread.getUnreadCounter() > 0 && !this.isFolded()) {
    ...
    var Timers = Class.extend({
        ...
        registerTimer: function (params) {
            ...
            if ("timeoutCallbackArguments" in params) {
                timerParams.onTimeout = this._timeoutCallback.bind.apply(this._timeoutCallback, [null].concat(params.timeoutCallbackArguments));
            } else {
...
odoo.define("im_livechat.legacy.mail.widget.Thread", function (require) {
    ...
    var ThreadWidget = Widget.extend({
            ...
            _.each(messages, function (message) {
                if (
                    ...
                    (prevMessage.isLinkedToDocumentThread() &&
                        ...
                        (prevMessage.getDocumentModel() !== message.getDocumentModel() || prevMessage.getDocumentID() !== message.getDocumentID()))
                ) {
        ...
        _onClickMessageReply: function (ev) {
            ...
            this.$(".o_thread_message").removeClass("o_thread_selected_message");
            this.$('.o_thread_message[data-message-id="' + this._selectedMessageID + '"]').addClass("o_thread_selected_message");
