/** @odoo-module alias=im_livechat.MockServer **/
...
MockServer.include({
    ...
    _mockMailChannel_ChannelGetLivechatVisitorInfo(ids) {
        ...
        if (members.length > 0 && members[0].id !== this.publicPartnerId) {
            ...
            const country = this._getRecords(
                'res.country',
                [['id', '=', members[0].country_id]],
            )[0];
            return {
    ...
    _mockMailChannelChannelInfo(ids, extra_info) {
        ...
        for (const channelInfo of channelInfos) {
            const channel = this._getRecords(
                'mail.channel',
                [['id', '=', channelInfo.id]],
            )[0];
            ...
            if (channel.channel_type === 'livechat') {
                ...
                if (channel.livechat_operator_id) {
                    const operator = this._getRecords(
                        'res.partner',
                ...
                channelInfo.livechat_visitor = this._mockMailChannel_ChannelGetLivechatVisitorInfo(
                    [channel.id],
                );
            }
        }
    ...
    _mockImLivechatChannel_getAvailableUsers(id) {
        const livechatChannel = this._getRecords(
            'im_livechat.channel',
            [['id', '=', id]],
        )[0];
        const users = this._getRecords(
            'res.users',
            [['id', 'in', livechatChannel.user_ids]],
        );
    ...
    _mockImLivechatChannel_getLivechatMailChannelVals(
        ...
        let visitor_user;
        if (user_id) {
            const visitor_user = this._getRecords(
                'res.users',
                [['id', '=', user_id]],
            )[0];
    ...
    _mockImLivechatChannel_openLivechatMailChannel(
        ...
        if (previous_operator_id) {
            const availableUsers = this._mockImLivechatChannel_getAvailableUsers(id);
            operator = availableUsers.find(
                user => user.partner_id === previous_operator_id,
            );
        }
        if (!operator) {
