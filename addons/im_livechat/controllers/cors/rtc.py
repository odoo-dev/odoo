...
class LivechatRtcController(RtcController):
    @route("/im_livechat/cors/rtc/channel/join_call", methods=["POST"], type="rpc", auth="public", cors="*")
    ...
    @route("/im_livechat/cors/rtc/channel/leave_call", methods=["POST"], type="rpc", auth="public", cors="*")
    def livechat_channel_call_leave(self, guest_token, channel_id):
        force_guest_env(guest_token)
        return self.channel_call_leave(channel_id)

    @route("/im_livechat/cors/rtc/session/update_and_broadcast", methods=["POST"], type="rpc", auth="public", cors="*")
    ...
    @route("/im_livechat/cors/rtc/session/notify_call_members", methods=["POST"], type="rpc", auth="public", cors="*")
    def livechat_session_call_notify(self, guest_token, peer_notifications):
        force_guest_env(guest_token)
        self.session_call_notify(peer_notifications)

    @route("/im_livechat/cors/channel/ping", methods=["POST"], type="rpc", auth="public", cors="*")
