...
class AccountEdiXmlUBLMyInvoisMY(models.AbstractModel):
    ...
    def _export_invoice_vals(self, invoice):
        ...
        if invoice.move_type == 'out_refund' and invoice.pos_order_ids:
            ...
            refunded_order = invoice.pos_order_ids[0].refunded_order_ids[0]
    ...
    def _export_invoice_constraints(self, invoice, vals):
        ...
        # Originally we block setting a E tax without giving the reason on the invoice, but with PoS we need to support tax-based reasons.
        # So the constrains need to be updated
        if f"myinvois_{invoice.id}_tax_exemption_required" in constraints:
            if not any(tax.l10n_my_tax_type == 'E' and not tax.l10n_my_tax_exemption_reason for tax in invoice.invoice_line_ids.tax_ids) or invoice.l10n_my_edi_exemption_reason:
                del constraints[f"myinvois_{invoice.id}_tax_exemption_required"]

        return constraints

    def _get_tax_unece_codes(self, invoice, tax):
        # OVERRIDE 'account_edi_ubl_cii'
        vals = super()._get_tax_unece_codes(invoice, tax)
        # We need to get the reason from the tax if set, otherwise we'll end up with some issues when refunding from the pos.
        if tax.l10n_my_tax_type == 'E':
            vals['tax_exemption_reason'] = tax.l10n_my_tax_exemption_reason or invoice.l10n_my_edi_exemption_reason
        return vals
