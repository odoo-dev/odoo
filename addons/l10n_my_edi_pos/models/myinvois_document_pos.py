...
class MyInvoisDocumentPoS(models.Model):
        ...
        for line in xml_vals['vals']['line_vals']:
            ...
            for classified_tax_category_val in item_vals['classified_tax_category_vals']:
                if classified_tax_category_val['tax_category_code'] == 'E' and not classified_tax_category_val['tax_exemption_reason']:
                    ...
                    builder._l10n_my_edi_make_validation_error(constrains, 'tax_exemption_required_on_tax', classified_tax_category_val['id'], classified_tax_category_val['percent'])
    ...
    def _add_customer_information(self, xml_vals):
        ...
        if not general_public_partner:
            raise UserError(self.env._("You must have a 'General Public' commercial partner with a VAT set to 'EI00000000010' in order to submit consolidated invoices."))
    ...
    def _add_line_information(self, xml_vals):
        ...
        for xml_line_information in xml_vals['xml_lines']:
            ...
            for grouping_key, tax_detail in xml_vals['lines_tax_information'].items():
    ...
    def _validate_taxes(self, orders):
        ...
        for tax in orders.lines.tax_ids:
            try:
                ...
                error_msg = self.env._("Tax '%s' is invalid: %s", tax.name, e.args[0])  # args[0] gives the error message
    ...
    def _prepare_aggregated_taxes(self, xml_vals):
        ...
        base_lines_aggregated_total_values = self.env['account.tax']._aggregate_base_lines_tax_details(base_lines, total_grouping_key_generator)
        total_values_per_grouping_key = self.env['account.tax']._aggregate_base_lines_aggregated_values(base_lines_aggregated_total_values)

        base_lines_aggregated_line_values = self.env['account.tax']._aggregate_base_lines_tax_details(base_lines, line_grouping_key_generator)
        line_values_per_grouping_key = self.env['account.tax']._aggregate_base_lines_aggregated_values(base_lines_aggregated_line_values)
        ...
        for grouping_key, tax_detail in total_values_per_grouping_key.items():
            total_taxes_vals['base_amount_currency'] += tax_detail['base_amount_currency']
            total_taxes_vals['base_amount'] += tax_detail['base_amount']
            total_taxes_vals['tax_amount_currency'] += tax_detail['tax_amount_currency']
            total_taxes_vals['tax_amount'] += tax_detail['tax_amount']
            total_taxes_vals['tax_details'][grouping_key] = tax_detail

        return (
            total_taxes_vals,
            line_values_per_grouping_key,
        )
    ...
    def _get_line_allowance_vals_list(self, line, tax_values_list=None):
        ...
        for grouping_key, tax_details in tax_values_list.items():
