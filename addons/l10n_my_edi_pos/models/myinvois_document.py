...
class MyInvoisDocument(models.Model):
    ...
    myinvois_issuance_date = fields.Date(
        ...
        compute='_compute_myinvois_issuance_date',
        store=True,
        copy=False,
    )
    # File fields
    ...
    # Related model field, to be used in later versions.
        ...
        for document in self.sorted(key=lambda d: (d.myinvois_issuance_date, d._origin.id)):
            ...
            if document_has_name:
                if not document._sequence_matches_date():
                    if document._get_last_sequence():
                        document.name = False
                        continue
                else:
                    if document_has_name or (not document_has_name and document._get_last_sequence()):
                        continue
            if document.myinvois_issuance_date and (not document_has_name or not document._sequence_matches_date()):
                document._set_next_sequence()

        self.filtered(lambda m: not m.name).name = '/'

    def _compute_myinvois_issuance_date(self):
        """ To extend to give it a value depending on the related record if needed. """
        self.myinvois_issuance_date = False
    ...
    def _compute_display_name(self):
        for document in self:
            document.display_name = document.name if document.name != '/' else _('Draft')
    ...
    def _unlink_check(self):
        for document in self:
            if document.myinvois_state in ["in_progress", "valid", "rejected"]:
                raise UserError(_('You cannot delete a document that is active on MyInvois.\nYou must cancel it first.'))
    ...
    def action_submit_to_myinvois(self):
        """ Submit the xml file(s) in self to the MyInvois API. """
    ...
    def _submit_to_myinvois(self):
        ...
        self.action_generate_xml_file(overwrite_file=False)
    ...
    def action_update_submission_status(self):
        """ Fetches the documents in self. """
        if len(self) == 1:
            self._myinvois_single_status_update()
        else:
            self._myinvois_submission_statuses_update()

    def action_generate_xml_file(self, overwrite_file=True):
        """ Generate the xml file representing this MyInvois Document.

        Optionally, can be set to not overwrite the existing file if any (used during submission).
        When overwriting, the previous file's name is updated to include an (old) tag to avoid confusion in the attachment list.

        :param: overwrite_file: If set, the existing file in myinvois_file_id will be overwritten.
        """
        new_documents_data = []
        for document in self:
            if document.myinvois_file_id and not overwrite_file:
                continue
            ...
            if errors:
                raise UserError(_("Error when generating the documents' files:\n\n- %(errors)s", errors='\n- '.join(errors)))
    ...
    def _myinvois_generate_xml_file(self):
        ...
        if not xml_vals:
            raise UserError(_("This consolidated invoice does not contain any relevant orders to send to MyInvois."))
        errors = [constraint for constraint in self._myinvois_export_document_constrains(xml_vals).values() if constraint]
        xml_content = self.env["ir.qweb"]._render(xml_vals["main_template"], xml_vals)
    ...
    def _myinvois_submit_documents(self, submissions_content):
        ...
        Only records in self having an xml_file_content in xml_contents will be sent.
        ...
        def _format_error_messages(errors_list):
            return self.env["account.move.send"]._format_error_html({"error_title": _("Error when sending the documents to the E-invoicing service."), "errors": errors_list})
            ...
            for batch in split_every(SUBMISSION_MAX_SIZE, records_to_send.ids, self.env['myinvois.document'].browse):
                ...
                if 'error' in batch_result:
                    ...
                    for document_result in batch_result['documents']:
                        ...
                        updated_values = {
                            'myinvois_external_uuid': document_result.get('uuid'),  # rejected documents do not have a uuid.
                        ...
                        if success:
                            ...
                            success_messages[record.id] = _('The document has been sent to MyInvois with uuid "%(uuid)s" and submission id "%(submission_id)s".\nValidation results will be available shortly.',
                            ...
                            error_messages[record.id] = _format_error_messages([self._l10n_my_edi_map_error(error) for error in document_result['errors']])
                            if self.invoice_ids:
    ...
    def _myinvois_get_submission_status(self):
        ...
        for proxy_user, records in self.grouped(lambda r: r._myinvois_get_proxy_user()).items():
            ...
            for submission_uid, submission_records in records.grouped('myinvois_submission_uid').items():
                ...
                if 'error' in result:
                    results[submission_uid]['error'] = self._l10n_my_edi_map_error(result['error'])
                else:
    ...
    def _myinvois_single_status_update(self):
        ...
        if 'validation_errors' in result:
            ...
            message = _('This document has been %(status)s for reason: %(reason)s', status=result['status'], reason=result['status_reason'])
    ...
    def _myinvois_submission_statuses_update(self, with_commit=True):
        ...
        for submission_uid, results in statuses.items():
            ...
            if results['error']:
                message = self.env["account.move.send"]._format_error_html({
                    "error_title": _("The status update failed with the following errors:"),
            ...
            for record, status in results['statuses'].items():
                ...
                if status.get('reason') or status['status'] == 'invalid':
                    if status.get('reason'):
                        message = _('The MyInvois platform returned a "%(status)s" status for this document for reason: %(reason)s', status=status['reason'], reason=status['reason'])
                    else:
                        message = _('The MyInvois platform returned an "%(status)s" status for this document.', status=status['reason'])
    ...
    def _myinvois_check_can_update_status(self):
        ...
        if time_difference >= datetime.timedelta(days=3):
            raise UserError(_('It has been more than 72h since the document validation, you can no longer cancel it.\n'
                              'Instead, you should issue a debit or credit note.'))
        if self.myinvois_state not in ['valid', 'rejected']:
            raise UserError(_('You can only change the state of a document in the valid or rejected states.'))
    ...
    def _myinvois_update_document(self, status, reason):
        ...
        for document in self:
            ...
            if 'error' in result:
                document._myinvois_log_message(message=self._l10n_my_edi_map_error(result['error']))
            else:
        ...
        if status in self._fields['myinvois_state'].get_values(self.env):
            successfully_updated_documents._myinvois_set_state(
                state=status,
                message=_('This document has been %(status)s for reason: %(reason)s', status=status, reason=reason),
            )
    ...
    def _l10n_my_edi_map_error(self, error):
        ...
        error_map = {
            ...
            'internal_server_error': self.env._('Server error; If the problem persists, please contact the Odoo support.'),
            # The proxy user credentials are either incorrect, or Odoo does not have the permission to invoice on their behalf.
            'invalid_tin': self.env._('Please make sure that your company TIN is correct, and that you gave Odoo sufficient permissions on the MyInvois platform.'),
            ...
            'rate_limit_exceeded': self.env._('The api request limit has been reached. Please wait until %(limit_reset_datetime)s to try again.',
                                     limit_reset_datetime=error.get('data')),  # Note, should be UTC. The TZ name is present in the formatted date.
            'hash_resubmitted': self.env._('This document has already been submitted and was deemed invalid.\n'
            ...
            'document_tin_not_found': self.env._('MyInvois could not match your TIN with the identification information you provided on the company.'),
            # This happens when the TIN of the supplier doesn't match with the TIN registered on the Proxy. Data contains the TIN.
            'document_tin_mismatch': self.env._("The TIN number of the supplier in the invoices does not match with the one provided at the time of registering for the e-invoice service.\n"
            ...
            'multiple_documents_id': self.env._('Multiple different supplier identification information were found in the invoices.\n'
            ...
            'multiple_documents_tin': self.env._('Multiple different supplier TIN were found in the invoices.\n'
            ...
            'update_incorrect_state': self.env._('You can only update the status of invoices in the valid state.'),
            'update_period_over': self.env._('It has been more than 72h since the invoice validation, you can no longer update it.\n'
                                    'Instead, you should issue or request a debit or credit note.'),
            'update_active_documents': self.env._('You cannot update this invoice, has it has been referenced by a debit or credit note.\n'
