...
class Session(http.Controller):
    ...
    @http.route('/web/session/get_session_info', type='rpc', auth='user', readonly=True)
    def get_session_info(self):
        # Crapy workaround for unupdatable Odoo Mobile App iOS (Thanks Apple :@)
        request.session.touch()
        return request.env['ir.http'].session_info()

    @http.route('/web/session/authenticate', type='rpc', auth="none")
    ...
    @http.route('/web/session/get_lang_list', type='rpc', auth="none")
    ...
    @http.route('/web/session/modules', type='rpc', auth='user', readonly=True)
    def modules(self):
        # return all installed modules. Web client is smart enough to not load a module twice
        return list(request.env.registry._init_modules)

    @http.route('/web/session/check', type='rpc', auth='user', readonly=True)
    def check(self):
        return  # ir.http@_authenticate does the job

    @http.route('/web/session/account', type='rpc', auth='user', readonly=True)
    def account(self):
    ...
    @http.route('/web/session/destroy', type='rpc', auth='user', readonly=True)
    def destroy(self):
