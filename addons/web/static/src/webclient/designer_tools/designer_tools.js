...
/**
 * Builds an element. Text always goes in through textContent, so a value read
 * off the stylesheet or typed into the editor can never be parsed as markup --
 * which is what every innerHTML on this page used to allow.
 *
 * Falsy children are dropped rather than appended: `append(null)` would insert
 * the string "null".
 */
function el(tag, { class: className, style, text, attrs } = {}, ...children) {
...
/** A table cell, the shape three of the four tables below are made of. */
const td = (text, className) => el("td", { class: className, text });

// [label, what to instantiate, selector of the node to measure]. Built rather
// than spelled as markup: these are the only fragments on the page that have
// to become real nodes, and parsing a string is how that used to be done.
const PARTS = [
    ["card", () => el("div", { class: "card" })],
    ["card-header", () => el("div", { class: "card" }, el("div", { class: "card-header" })), ".card-header"],
    ["dropdown-menu", () => el("div", { class: "dropdown-menu" })],
    ["dropdown-item", () => el("div", { class: "dropdown-menu" }, el("a", { class: "dropdown-item", attrs: { href: "#" } })), ".dropdown-item"],
    ["dropdown-header", () => el("div", { class: "dropdown-menu" }, el("h6", { class: "dropdown-header" })), ".dropdown-header"],
    ["popover", () => el("div", { class: "popover" })],
    ["tooltip-inner", () => el("div", { class: "tooltip" }, el("div", { class: "tooltip-inner" })), ".tooltip-inner"],
    ["form-control", () => el("input", { class: "form-control" })],
    ["form-check-input", () => el("input", { class: "form-check-input", attrs: { type: "checkbox" } })],
    ["btn-secondary", () => el("button", { class: "btn btn-secondary" })],
    ["kbd", () => el("kbd")],
];
...
function reportSheetFailure() {
    ...
    const anchor = el("a", { attrs: { target: "_blank" } });
    anchor.href = link ? link.href : "/web/assets/debug/web.assets_web.css";
    anchor.textContent = anchor.getAttribute("href");
    banner.append(
        el("b", { text: "The compiled stylesheet did not load." }),
        " Every value below is meaningless until it does — an unresolved " +
            "variable reads as black. Open ",
        anchor,
        " to see what the server answers."
    );
...
function render() {
    ...
    rampBody.replaceChildren();
    let previous = null;
    /** The swatch cell shared by both tables, tinted by the variable itself. */
    const chip = (name) => el("td", {}, el("span", { class: "chip", style: `background-color:var(--${name})` }));
    for (const name of RAMP) {
        const c = resolve(name, bodyBg);
        const l = lstar(c);
        const delta = previous === null ? null : Math.abs(l - lstar(previous));

        strip.append(el("div", { style: `background-color:var(--${name});color:${readable(c)}` },
            el("b", { text: name.replace(/^gray-/, "") }),
            el("span", { text: shown(c) }),
            el("span", { text: `L* ${num(c, l.toFixed(1))}` })
        ));

        rampBody.append(el("tr", {},
            chip(name),
            el("td", {}, el("code", { text: `--${name}` })),
            td(shown(c)),
            td(num(c, l.toFixed(1)), "num"),
            td(delta === null ? "—" : delta.toFixed(1), `num ${delta === null ? "" : cls(delta, 4, 7)}`.trim()),
            td(previous === null ? "—" : `${ratio(c, previous).toFixed(2)}:1`, "num")
        ));
        previous = c;
    }
    ...
    namedBody.replaceChildren();
    for (const name of NAMED) {
        const c = resolve(name, bodyBg);
        const r = ratio(c, bodyBg);
        namedBody.append(el("tr", {},
            chip(name),
            el("td", {},
                el("code", { text: `--${name}` }),
                c.alpha ? el("small", { text: ` α ${c.alpha}` }) : null
            ),
            td(shown(c)),
            td(num(c, lstar(c).toFixed(1)), "num"),
            td(`${r.toFixed(2)}:1`, `num ${cls(r, 1.5, 4.5)}`.trim())
        ));
    }
    ...
    for (const [label, build, selector] of PARTS) {
        ...
        partsBody.append(el("tr", {},
            el("td", {}, el("span", { class: "chip", style: `background-color:${hex(bg)}` })),
            el("td", {}, el("code", { text: label })),
            el("td", {},
                hex(bg),
                bg.alpha ? el("small", { text: ` α ${bg.alpha}` }) : null
            ),
            td(lstar(bg).toFixed(1), "num"),
            td(border ? hex(border) : "—"),
            td(border ? `${borderRatio.toFixed(2)}:1` : "—", `num ${border ? cls(borderRatio, 1.5, 3) : ""}`.trim()),
            td(hex(text)),
            td(`${textRatio.toFixed(2)}:1`, `num ${cls(textRatio, 3, 4.5)}`.trim())
        ));
    }
    host.remove();
}
...
function initPanel() {
    ...
    window.addEventListener("message", async (ev) => {
        // Same-origin by construction -- the panels are this page with
        // ?panel=1 -- so anything from elsewhere is not ours to act on.
        ...
        if (msg.type === "overrides") {
            ...
            parent.postMessage({ type: "applied", ...result }, location.origin);
            report();
        }
    });
...
function initShell() {
    ...
    function apply() {
        ...
        status.replaceChildren(...(bad.length
            ? [
                  el("span", { class: "bad", text: `${bad.length} line(s) ignored` }),
                  ": ",
                  // A rejected line is whatever was typed, markup included.
                  el("code", { text: bad[0] }),
              ]
            : ["Applying…"]));
        after.contentWindow.postMessage({ type: "overrides", values }, location.origin);
    }
    ...
    window.addEventListener("message", (ev) => {
        ...
        if (msg.type === "height") {
            ...
            status.replaceChildren(msg.pairs.length
                ? el("table", {}, el("tbody", {}, ...msg.pairs.map((pair) => el("tr", {},
                      td(pair.name),
                      td(`${pair.from} → ${pair.to}`),
                      td(
                          pair.shadowedBy
                              ? `same color as ${pair.shadowedBy}, already replaced`
                              : `${pair.count} occurrence${pair.count === 1 ? "" : "s"}`,
                          pair.shadowedBy ? "warn" : "num"
                      )
                  ))))
    ...
    async function copy() {
        ...
        if (!done) {
            // The whole editor content: the one place markup typed into the
            // page could reach the page.
            status.replaceChildren(
                el("pre", { style: "margin:.5rem 0 0;white-space:pre-wrap", text })
            );
        }
    }
...
function forwardKey(ev) {
    if (parent !== window && !ev.ctrlKey && !ev.metaKey && "dx".includes(ev.key)) {
        parent.postMessage({ type: "key", key: ev.key }, location.origin);
    }
}

// The page is the only caller. Guarded rather than trusted: the manifests that
// glob `web/static/src/webclient/**` have to exclude this file, and the day one
// of them forgets, running the shell against the webclient DOM takes the module
// loader down with it -- which is exactly how this guard came to be written.
if (document.body?.classList.contains("o_designer_tools")) {
    if (MODE === "panel") {
        initPanel();
