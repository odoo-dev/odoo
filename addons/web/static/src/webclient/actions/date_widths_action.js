...
import { strftimeToLuxonFormat, toLocaleDateTimeString } from "@web/core/l10n/dates";
...
class Stats extends Component {
    ...
    sanitizeStats(stats) {
        return Object.entries(stats).map(([locale, values]) => ({ locale, values }));
    }
...
class DateWidthsAction extends Component {
    ...
    getValues(format, locale) {
        ...
        for (let month = 1; month <= 12; month++) {
            values.push(
                toLocaleDateTimeString(luxon.DateTime.local(2017, month, 15, 10, 30, 45), {
                    showSeconds: true,
                })
            );
            values.push(
                toLocaleDateTimeString(luxon.DateTime.local(2017, month, 15, 22, 30, 45), {
                    showSeconds: true,
                })
            );
        }
