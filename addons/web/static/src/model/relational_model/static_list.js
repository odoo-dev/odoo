...
function copyRecordData(record, forcedFields = []) {
    const data = {};
    for (const [name, value] of Object.entries(record.data)) {
        if (
            ![...forcedFields, "display_name"].includes(name) &&
...
export class StaticList extends DataPoint {
    ...
    async _duplicateRecords(records, options) {
        ...
        const forcedFields = options.forcedFields || [];
        ...
        await Promise.all(
            records.map((record, index) =>
                newRecords[index]._update({
                    ...copyRecordData(record, forcedFields),
