...
export class DatetimePickerPlugin extends Plugin {
    // `bottom_sheet`/`popover`/`ui` aren't converted to plugins yet: they're only
    // added to `env.services` once the (later-sequenced) legacy service starter
    // runs, so they can't be resolved as eager fields here. `scope.run` gives
    // `useService` a valid scope to resolve them lazily, on first actual use.
    scope = useScope();

    get bottomSheetService() {
        return this.scope.run(() => useService("bottom_sheet"));
    }

    get popoverService() {
        return this.scope.run(() => useService("popover"));
    }

    get ui() {
        return this.scope.run(() => useService("ui"));
    }
    ...
    create(params, options) {
        ...
        async function apply() {
            ...
            if (stringValue === lastAppliedStringValue || stringValue === stringProps.value) {
                return;
            }
        ...
        function updateValueFromInputs() {
            ...
            const updated = params.onWillParseValues?.(inputs.map((input) => input && input.value));
            if (updated) {
                return true;
            }
        ...
        const createPopover = params.createPopover || this.defaultCreatePopover;
        const getInputs =
            params.getInputs ||
...
services.add(DatetimePickerPlugin);
...
export const datetimePickerService = {
    start() {
        return usePlugin(DatetimePickerPlugin);
    },
};
