...
QUnit.module("SwitchCompanyMenu", (hooks) => {
    hooks.beforeEach(() => {
        patchWithCleanup(sessionInfo.user_companies, {
            allowed_companies: {
                1: { id: 1, name: "Hermit" },
    ...
    QUnit.test("companies can be toggled: toggle a second company", async (assert) => {
        ...
        assert.containsOnce(scMenu.el, ".o_dropdown_menu", "dropdown is still opened");
    ...
    QUnit.test("single company mode: companies can be logged in", async (assert) => {
        ...
        assert.containsNone(scMenu.el, ".o_dropdown_menu", "dropdown is directly closed");
