...
test.tags("mobile");
test("close sidebar on window resize", async () => {
    defineMenus([
        {
            id: 1,
            children: [{ id: 10, name: "Menu 10" }],
        },
    ]);
    ...
    const env = await makeMockEnv();
    Object.defineProperty(env, "isSmall", { get: () => true });

    getService("menu").setCurrentMenu(1);
    const navbar = await mountWithCleanup(NavBar);

    // Open sidebar via menu toggle
    await contains("a.o_menu_toggle").click();
    expect(".o_app_menu_sidebar").toHaveCount(1);

    // Trigger resize - sidebar should be closed
    await resize({ width: 800 });
    await waitNavbarAdaptation();

    expect(".o_app_menu_sidebar").toHaveCount(0, {
        message: "sidebar should be closed after resize",
    });
    expect(navbar.state.isAppMenuSidebarOpened).toBe(false, {
        message: "sidebar state should be false after resize",
    });
    expect(navbar.state.isAllAppsMenuOpened).toBe(false, {
        message: "all apps menu state should be false after resize",
    });
});

test.tags("desktop");
test("navbar adapts to isSmall change on resize", async () => {
    defineMenus([
        {
            id: 1,
            children: [{ id: 10, name: "Menu 10" }],
    ...
    // Verify initial state - desktop mode
    expect(".o_menu_toggle").toHaveCount(0);

    // Resize to mobile - sidebar should close and adapt
    await resize({ width: 800 });
    await waitNavbarAdaptation();

    // After resize, sidebar should be closed
    expect(".o_app_menu_sidebar").toHaveCount(0);
});
