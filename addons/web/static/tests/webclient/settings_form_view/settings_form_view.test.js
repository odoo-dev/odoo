...
test("settings search does not escape characters when highlighting searched text", async () => {
    await mountView({
        type: "form",
        resModel: "res.config.settings",
        arch: /* xml */ `
            <form string="Settings" class="oe_form_configuration o_base_settings" js_class="base_settings">
                <app string="CRM" name="crm">
                    <block title="Research &amp; Development">
                        <setting>
                            <div>This test is to check wheather &amp; gets escaped during search or not</div>
                        </setting>
                    </block>
                </app>
            </form>
    ...
    await editSearch("a");
    expect(queryAllTexts(".highlighter")).toEqual(["a", "a", "a", "a"]);
    await editSearch("&");
