    ...
    const expected = /*xml*/ `
        <t t-translation="off">
            <div class="o_form_renderer"
                 ...
                 t-attf-class="{{__comp__.props.record.isInEdition ? 'o_form_editable' : 'o_form_readonly'}} d-flex d-print-block {{ __comp__.uiService.size &lt; 5 ? &quot;flex-column&quot; : &quot;flex-nowrap h-100&quot; }} {{ ${hasTouch()} ? &quot;o_field_highlight&quot; : &quot;&quot; }} {{ __comp__.props.record.dirty ? 'o_form_dirty' : !__comp__.props.record.isNew ? 'o_form_saved' : '' }}"
                 t-ref="{ set: (el) => __comp__.__owl__.__refs__.compiled_view_root = el }">
                <div t-on-scroll="__comp__.onScrollThrottled" class="o_form_sheet_bg">
                    <div class="o_form_sheet position-relative">
                    </div>
                </div>
...
test("invisible is correctly computed with another t-if", () => {
    ...
    const expected = `<t t-translation="off"><div class="myNode" t-if="( myCondition or myOtherCondition ) and !__comp__.evaluateBooleanExpr(&quot;field == 'value'&quot;,__comp__.props.record.evalContextWithVirtualIds)" t-ref="{ set: (el) => __comp__.__owl__.__refs__.compiled_view_root = el }"/></t>`;
    expect(compileTemplate(arch)).toHaveOuterHTML(expected);
});
