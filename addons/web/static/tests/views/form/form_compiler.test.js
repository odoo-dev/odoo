...
test("invisible is correctly computed with another t-if", () => {
    ...
    const expected = `<t t-translation="off"><div class="myNode" t-if="( myCondition or myOtherCondition ) and !__comp__.evaluateBooleanExpr(&quot;field == 'value'&quot;,__comp__.props.record.evalContextWithVirtualIds)" t-ref="__comp__.rootRef"/></t>`;
    expect(compileTemplate(arch)).toHaveOuterHTML(expected);
});
