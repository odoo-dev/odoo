...
test("PriorityField when not set", async () => {
    ...
    expect(".o_field_widget .o_priority a.o_priority_star[data-icon='star'].oi-filled").toHaveCount(0, {
        message: "should have no full star since there is no value",
    });

    expect(".o_field_widget .o_priority a.o_priority_star[data-icon='star']").toHaveCount(2, {
...
test("PriorityField in form view", async () => {
    ...
    expect(".o_field_widget .o_priority:not(.o_field_empty)").toHaveCount(1);
    expect(".o_field_widget .o_priority a.o_priority_star").toHaveCount(2);
    expect(".o_field_widget .o_priority a.o_priority_star[data-icon='star'].oi-filled").toHaveCount(1);
    expect(".o_field_widget .o_priority a.o_priority_star[data-icon='star']").toHaveCount(1);

    // click on the second star in edit mode
    await click(".o_field_widget .o_priority a.o_priority_star[data-icon='star']:last");
    await animationFrame();

    expect(".o_field_widget .o_priority a.o_priority_star").toHaveCount(2);
    expect(".o_field_widget .o_priority a.o_priority_star[data-icon='star'].oi-filled").toHaveCount(2);
    expect(".o_field_widget .o_priority a.o_priority_star[data-icon='star']").toHaveCount(0);
});
...
test("PriorityField hover a star in form view", async () => {
    ...
    // hover last star
    const star = ".o_field_widget .o_priority a.o_priority_star[data-icon='star']:last";
    await hover(star);
    await animationFrame();
    expect(".o_field_widget .o_priority a.o_priority_star").toHaveCount(2);
    expect(".o_field_widget .o_priority a.o_priority_star[data-icon='star'].oi-filled").toHaveCount(2, {
...
test("PriorityField can write after adding a record -- kanban", async () => {
    ...
    await click(".o_priority a.o_priority_star[data-icon='star']");
    // wait for web_save
    await animationFrame();
...
test("PriorityField in editable list view", async () => {
    ...
    expect(".o_data_row:first-child .o_priority a.o_priority_star[data-icon='star'].oi-filled").toHaveCount(1, {
        message: "should have one full star since the value is the second value",
    });
    expect(".o_data_row:first-child .o_priority a.o_priority_star[data-icon='star']").toHaveCount(1, {
    ...
    await click(".o_priority a.o_priority_star[data-icon='star'].oi-filled");
    await animationFrame();

    expect(".o_data_row:first-child .o_priority a.o_priority_star").toHaveCount(2, {
        message: "should still have two stars",
    });
    expect(".o_data_row:first-child .o_priority a.o_priority_star[data-icon='star'].oi-filled").toHaveCount(0, {
        message: "should now have no full star since the value is the first value",
    });
    expect(".o_data_row:first-child .o_priority a.o_priority_star[data-icon='star']").toHaveCount(2, {
    ...
    await click(".o_priority a.o_priority_star[data-icon='star']:last");
    await animationFrame();

    expect(".o_data_row:last-child .o_priority a.o_priority_star").toHaveCount(2, {
        message: "should still have two stars",
    });
    expect(".o_data_row:last-child .o_priority a.o_priority_star[data-icon='star'].oi-filled").toHaveCount(2, {
        message: "should now have two full stars since the value is the third value",
    });
    expect(".o_data_row:last-child .o_priority a.o_priority_star[data-icon='star']").toHaveCount(0, {
...
test("PriorityField hover in editable list view", async () => {
    ...
    // hover last star
    const star = ".o_data_row:first-child .o_priority a.o_priority_star[data-icon='star']:last";
    await hover(star);
    await animationFrame();

    expect(".o_data_row:first-child .o_priority a.o_priority_star").toHaveCount(2);
    expect(".o_data_row:first-child .o_priority a.o_priority_star[data-icon='star'].oi-filled").toHaveCount(2, {
        message: "should temporary have two full stars since we are hovering the third value",
    });
    expect(".o_data_row:first-child .o_priority a.o_priority_star[data-icon='star']").toHaveCount(0, {
        message: "should temporary have no empty star since we are hovering the third value",
    });
...
test("PriorityField with readonly attribute", async () => {
    ...
    expect("span.o_priority_star[data-icon='star'].o_disabled").toHaveCount(2, {
        message: "stars of priority widget should rendered with span tag if readonly",
    });
    await hover(".o_priority_star[data-icon='star']:last");
    await animationFrame();
    expect.step("hover");
    expect(".o_field_widget .o_priority a.o_priority_star[data-icon='star'].oi-filled").toHaveCount(0, {
        message: "should have no full stars on hover since the field is readonly",
    });
    await click(".o_priority_star[data-icon='star']:last");
    await animationFrame();
    expect.step("click");
    expect("span.o_priority_star[data-icon='star'].o_disabled").toHaveCount(2, {
        message: "should still have two stars",
...
test('PriorityField edited by the smart action "Set priority..."', async () => {
    ...
    expect("a[data-icon='star'].oi-filled").toHaveCount(1);
    ...
    await animationFrame();
    expect("a[data-icon='star'].oi-filled").toHaveCount(2);
});
