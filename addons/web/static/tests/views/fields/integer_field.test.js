...
test("with enable_formatting false only", async () => {
    Product._records = [{ id: 1, price: 1234567 }];
    await mountView({
        type: "form",
        resModel: "product",
        resId: 1,
        arch: `<form><field name="price" options="{'enable_formatting': false, 'skip_thousands_separator': true}"/></form>`,
    });

    expect(".o_field_widget input").toHaveValue("1234567", {
        message: "Should not format value when enable_formatting is false.",
    });
});
