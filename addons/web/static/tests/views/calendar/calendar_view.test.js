...
beforeEach(() => {
    ...
    patchWithCleanup(CalendarRenderer.prototype, {
        async getCalendarData(direction, fetchSurroundingCalendars = false) {
            if (fetchSurroundingCalendars) {
                // the flag must be set on true to avoid unnecessary search_read calls in tests when ActionSwiper is present
                return super.getCalendarData(direction);
            }
            return super.getCalendarData();
        }
    });
});
...
test(`three calendars are rendered in the ActionSwiper`, async () => {
    mockTouch(true);
    patchWithCleanup(CalendarRenderer.prototype, {
        async getCalendarData(direction) {
            // for this test, set the flag to true to use the default behavior
            return super.getCalendarData(direction, true);
        }
    });
    // if not given any option, default week start is on Sunday
    mockTimeZone(-7);
    ...
    expect.verifySteps(["event.search_read", "event.search_read", "event.search_read"]);
