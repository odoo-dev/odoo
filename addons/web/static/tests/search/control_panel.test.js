...
test("view switcher", async () => {
    await mountWithSearch(
        ...
        { resModel: "foo" },
        {
            viewSwitcherEntries: [
                ...
                { type: "kanban", icon: "view_kanban", name: "Kanban" },
            ],
        }
    );
    ...
    expect(`.o_switch_view:eq(1) [data-icon='view_kanban']`).toHaveCount(1);
...
test("view switcher on mobile", async () => {
    ...
    expect(queryAll(`[data-icon='view_kanban']`, { root: views[1] })).toHaveCount(1);
