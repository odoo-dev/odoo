    ...
    if (failedDetails) {
      for (const detail of failedDetails) {
        if (Markup.isMarkup(detail, "group")) {
          lines.push(
            `${number}.${detail.groupIndex}. (${formatHumanReadable(
              detail.content
            )})`
          );
          continue;
...
export class Runner {
  ...
  async dryRun(callback) {
    if (this.state.status !== "ready") {
      throw new HootError(
        "cannot run a dry run after the test runner started",
        {
          level: "global",
        }
      );
  ...
  _nextJob(jobs, job) {
    ...
    if (job) {
      ...
      if (
        parent &&
        (!jobs.length || jobs.some((j) => parent.path.includes(j)))
      ) {
        return parent;
      }
    }
  ...
  _handleErrorInTest(ev, error) {
    for (const callbackRegistry of this._getCallbackChain(
