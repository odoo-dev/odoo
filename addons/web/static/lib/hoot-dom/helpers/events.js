...
async function _fill(targetResolver, value, options) {
    ...
    if (options?.instantly) {
        ...
        await _press(initialTarget, { ctrlKey: true, key: "v" });
    } else {
        ...
        for (const char of String(value)) {
            ...
            await _press(initialTarget, { key, shiftKey: key !== char });
        }
        if (options?.composition) {
    ...
    await registerForChange(initialTarget, initialValue, options?.confirm);
}
...
export async function edit(value, options) {
    ...
    if (getNodeValue(activeElement)) {
        await _clear(activeElement);
    }
    await _fill(activeElement, value, options);
