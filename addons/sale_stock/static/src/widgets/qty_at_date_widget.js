    ...
    openForecast() {
        this.actionService.doAction("stock.stock_forecasted_product_product_action", {
            additionalContext: {
                ...
                warehouse_id:
                    this.props.record.data.warehouse_id && this.props.record.data.warehouse_id.id,
...
export class QtyAtDateWidget extends Component {
    ...
    initCalcData() {
        ...
        if (data.scheduled_date) {
            ...
            this.calcData.will_be_late =
                data.forecast_expected_date && data.forecast_expected_date > data.scheduled_date;
    ...
    async calcDataForDisplay() {
        ...
        if (data.product_uom_id?.[0]) {
            lineUom = (
                await this.orm.read("uom.uom", [data.product_uom_id[0]], ["factor", "rounding"])
            )[0];
        }
        let lineProduct;
    ...
    updateCalcData() {
        ...
        if (data.forecast_expected_date) {
            this.calcData.forecast_expected_date_str = formatDateTime(data.forecast_expected_date, {
                format: localization.dateFormat,
            });
        }
    }
