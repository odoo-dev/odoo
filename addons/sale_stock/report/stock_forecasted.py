...
class StockForecasted_Product_Product(models.AbstractModel):
    ...
    def _get_report_header(self, product_template_ids, product_ids, wh_location_ids):
        ...
        so_lines = self.env['sale.order.line'].sudo().search(domain).grouped(lambda l: (l.product_id, l.warehouse_id))
        out_qty = {
            (f'{k[0].id}_{k[1].id}'): sum(
                v.mapped(lambda line: line.product_uom_id._compute_quantity(line.product_uom_qty, line.product_id.uom_id))
        ...
        for (product, warehouse) in so_lines:
            res["product"][f"{product.id}_{warehouse.id}"]["draft_sale_orders"] = (
                so_lines[product, warehouse].mapped("order_id").sorted(key=lambda so: so.name).read(fields=["id", "name"])
            )
            res["product"][f"{product.id}_{warehouse.id}"]["draft_sale_orders_matched"] = (
                self.env.context.get("sale_line_to_match_id") in so_lines[product, warehouse].ids
            )
        return res
