    ...
    def _stock_account_get_last_step_stock_moves(self):
        ...
        for invoice in self:
            ...
            if (invoice.move_type == 'out_invoice' or (
                ...
                rslt += invoice.invoice_line_ids.sale_line_ids.move_ids.filtered(lambda x: x.state == 'done' and x.location_dest_id.usage == 'customer')
            else:
                rslt += invoice.reversed_entry_id.invoice_line_ids.sale_line_ids.move_ids.filtered(lambda x: x.state == 'done' and x.location_id.usage == 'customer')
                # Add refunds generated from the SO
                rslt += invoice.invoice_line_ids.sale_line_ids.move_ids.filtered(lambda x: x.state == 'done' and x.location_id.usage == 'customer')
        return rslt
