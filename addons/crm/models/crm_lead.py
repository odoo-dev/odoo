...
def _remove_quoted_content(body):
    ...
    content (``data-o-mail-quote``), i.e. everything the UI already
    collapses behind the "..." toggle, as it is not useful once flattened
    to plain text. """
    if not body:
        return body
...
def _clean_mail_messages( batch_size=1000):
    query = """
    SELECT COUNT(*)
...
import time
start = time.time()
with Profiler(db='openerp', description="LUL mail_message cleaning") as profiler:
