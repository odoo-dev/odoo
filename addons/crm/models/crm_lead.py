...
class Lead(models.Model):
    ...
    # @api.model
    # def _read_group_dates_deadline(self, dates_deadline, domain, order):
    ...
    #     forecast_period = self.get_forecast_period()
    #     data = [{'id': m, groupby[0]: month_start + relativedelta(months=m)} for m in range(forecast_period)]
    #     result = [self._read_group_format_result(d, annotated_groupbys, groupby, domain)[groupby[0]] for d in data]
    #     return result

    # @api.model
    # def get_forecast_period(self, min_period=4):
    #     """ Determine the forecast period (months) according to the following rules :
    #             1. minimum [min_period] months (starting from the current month)
    #             2. never switch to the next year unless (1.) is not respected
    #             3. go to the last month of the highest displayed year, which has an opportunity
    #         :param min_period : minimum number of months for the forecast period (positive > 0)
    #         :returns integer
    #     """
    #     month_start = fields.Date.context_today(self).replace(day=1)
    #     # The following formula computes the number of months from this month to
    #     # the end of the year after adding [min_period] to this month
    #     # i.e. we are in october 2020, [min_period] = 4 => [max_period] = 15 months until december 2021
    #     max_period = (24 - (min_period - 1) % 12 - month_start.month) % 12 + min_period
    #     order = "date_deadline DESC"
    #     domain = [
    #         '&', ('user_id', '=', self.env.user.id),  # TODO ABD: team_id ?
    #         '&', ('type', '=', 'opportunity'),
    #         '&', ('date_deadline', '>=', (month_start + relativedelta(months=min_period)).strftime('%Y-%m-%d')),
    #         ('date_deadline', '<', (month_start + relativedelta(months=max_period)).strftime('%Y-%m-%d')),
    #     ]
    #     opportunity = self.env['crm.lead'].search(domain, limit=1, offset=0, order=order)
    #     forecast_period = min_period
    #     if opportunity:
    #         date_delta = relativedelta(opportunity.date_deadline, month_start)
    #         months = date_delta.years * 12 + date_delta.months + 1
    #         forecast_period = max(min_period, months)
    #     return forecast_period

    # @api.model
    # def get_forecast_missing_months(self, date_min, date_max, start, count):
    #     # TODO ABD: maybe insert locale/tz logic
    #     # date_min (Full format)
    #     # date_max (Full format)
    #     # current_start (display format => problems)
    #     # current_months_count
    #     date_min = datetime.strptime(date_min, "%Y-%m-%d")
    #     date_max = datetime.strptime(date_max, "%Y-%m-%d")
    #     start = datetime.strptime(start, "%B %Y")
    #     end = start + relativedelta(months=count)
    #     delta_before = relativedelta(start, date_min)
    #     delta_max = relativedelta(date_max, date_min)
    #     offset_before = delta_before.years * 12 + delta_before.months
    #     offset_after = delta_max.years * 12 + delta_max.months - count - offset_before
    #     months = [{'date': (date_min + relativedelta(months=m)), 'sequence': m} for m in range(offset_before)] + \
    #         [{'date': (end + relativedelta(months=m)), 'sequence': offset_before + count + m} for m in range(offset_after)]
    #     return list(map(lambda month: [
    #         month['date'].strftime("%Y-%m-%d"),
    #         (month['date'] + relativedelta(months=1)).strftime("%Y-%m-%d"),
