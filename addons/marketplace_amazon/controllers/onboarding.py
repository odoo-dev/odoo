...
def compute_oauth_signature(account_id):
    ...
    :param int account_id: The account being authorized, as an `marketplace.account` id.
...
class AmazonController(http.Controller):
    ...
    @http.route(
        '/marketplace/amazon/return', type='http', methods=['GET'], auth='user', website=True, sitemap=False
    )
    def amazon_return_from_authorization(self, **data):
        ...
        amazon_seller_key = data['selling_partner_id']
        ...
        account = request.env['marketplace.account'].browse(account_id).exists()
        if not account:
            raise ValidationError(_("Could not find Marketplace account with id %s", account_id))
        account.amazon_seller_key = amazon_seller_key

        # Craft the URL of the Amazon account.

        account_url = f'/odoo/action-marketplace.action_marketplace_account/{account_id}'

        # Request and set the refresh token on the account and finalize the set up.
        try:
            amazon_utils.exchange_authorization_code(account, authorization_code)
            # account.action_update_available_marketplaces()
        except (UserError, ValidationError) as e:
            return request.render(
                'marketplace_amazon.authorization_error',
