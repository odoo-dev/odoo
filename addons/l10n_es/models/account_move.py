...
class AccountMove(models.Model):
    ...
    def _compute_l10n_es_invoice_type(self):
        ...
        for move in self:
            ...
            # Non-EU/no-country partners aren't subject to this no-identification heuristic: they
            # identify themselves via passport/foreign-ID instead of a VAT number, so lacking one
            # doesn't imply simplified. Same goes for an EU (including ES) partner who is
            # identified via one of those alternative documents instead of a VAT number: they
            # aren't an anonymous/undocumented customer either.
