...
class StripeIssuingController(Controller):
    ...
    def stripe_issuing_webhook(self):
        ...
        if not request.env['res.company'].sudo().search_count([('stripe_webhook_secret', '!=', False)], limit=1):
            raise ValidationError(_("No stripe issuing webhook configured on this database"))
    ...
    def _process_authorization_event(self, event):
        ...
        card = request.env['hr.expense.stripe.credit.card'].search(
            [('stripe_id', '=', auth_object['card']['id']), ('company_id', '=', request.env.company.id)],
            limit=1,
        )
        if not card:
            raise ValidationError(_("A card that doesn't exist on the database was used"))
        if card._can_pay_amount():
    ...
    def _process_transaction_event(self, event):
        ...
        if tr_object['type'] == 'capture':
            self._update_expense_from_transaction(tr_object)
    ...
    def _process_card_event(self, event):
        ...
        if event['type'] in {'issuing_card.created', 'issuing_card.updated'}:
            existing_card = request.env['hr.expense.stripe.credit.card'].search(
                [('stripe_id', '=', card_object['id']), ('company_id', '=', request.env.company.id)],
                limit=1,
            )
            existing_cardholder = request.env['hr.employee'].search(
                [('stripe_id', '=', card_object['cardholder']['id']), ('company_id', '=', request.env.company.id)],
                limit=1,
            )
            if existing_card:
                ...
                card_object['cardholder'].update({
                    ...
                    'first_name': cardholder.private_first_name or card_object['cardholder']['first_name'],
                    'last_name': cardholder.private_first_name or card_object['cardholder']['last_name'],
                })
                request.env['hr.expense.stripe.credit.card']._create_from_stripe([card_object])
    ...
    def _process_cardholder_event(self, event):
        ...
        if event['type'] in {'issuing_cardholder.created', 'issuing_cardholder.updated'}:
            employee = request.env['hr.employee']._create_from_stripe([{
    ...
    def _validate_signature(self, company_secret, signature_header, payload):
        ...
        if (time.time() - int(signature_data['t'])) > 180:  # 3 minutes
            return False
    ...
    @api.model
    def _create_expense_from_authorization(self, au_object):
        merchant_data = au_object['merchant_data']
        card = request.env['hr.expense.stripe.credit.card'].search(
            [('stripe_id', '=', au_object['card']), ('company_id', '=', request.env.company.id)],  # card is the stripe_id
            limit=1,
        )
        if not card:
            raise ValidationError(_("A card that doesn't exist on the database was used"))
        ...
        domain = [
            ('can_be_expensed', '=', True), ('company_id', '=', request.env.company.id),
        ...
        amount_company_currency = -format_amount_from_stripe(au_object['amount'], card.currency_id)
        create_dict = {
            ...
            'card_id': card.id,
            'manager_id': False,
            'stripe_authorization_id': au_object['authorization'],
            'stripe_transaction_id': au_object['id'],
        ...
        foreign_currency = request.env['res.currency'].with_context(active_test=False).search(
            [('name', '=', au_object['merchant_currency'].upper())],
            limit=1,
        )
        if foreign_currency:
            if not foreign_currency.active:
                foreign_currency.active = True
            amount_foreign_currency = -format_amount_from_stripe(au_object['merchant_amount'], foreign_currency)
    ...
    def _update_expense_from_transaction(self, event):
        pass

    @api.model
    def _cancel_expense_or_reverse_move(self, tr_object):
        expense = request.env['hr.expense'].search(
            [('stripe_transaction_id', '=', tr_object['id']), ('company_id', '=', request.env.company.id)],
            limit=1,
        )
        if not expense:
