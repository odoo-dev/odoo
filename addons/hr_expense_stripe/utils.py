...
def make_request_stripe_proxy(route, payload, method="POST", headers=None, test_mode=False):
    if not headers:
        headers = {}
    ...
    try:
        response = requests.request(method, url, data=json.dumps(payload), headers=headers, timeout=60)
