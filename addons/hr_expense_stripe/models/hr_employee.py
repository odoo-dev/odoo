        ...
        for record_data in vals:
            ...
            cardholder = self.env['hr.employee'].search(
                domain=[
                    ...
                    '|', ('stripe_id', '=', stripe_id),
                         '&', ('stripe_id', '=', False), '|', ('work_email', '=', record_data['email']), ('email', '=', record_data['email']),
                ],
                limit=1,
            )
            if not cardholder:
            ...
            if cardholder:
                ...
                if not cardholder.private_first_name:
                    new_vals['private_first_name'] = record_data.get('individual', {}).get('first_name', False)
                if not cardholder.private_last_name:
                    new_vals['private_last_name'] = record_data.get('individual', {}).get('last_name', False)
