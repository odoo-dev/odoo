...
class HrEmployee(models.Model):
    ...
    def _convert_stripe_data_to_odoo_vals(self, stripe_data):
        ...
        country = self.env['res.country'].search([('code', '=', stripe_data['billing']['address']['country'].lower())], limit=1)
        state = self.env['res.country.state'].search([('name', '=like', stripe_data['billing']['address']['state'])], limit=1)
        return res.update({
            'name': name or stripe_data['name'],
            'work_email': stripe_data['email'],
            'work_mobile': stripe_data['phone_number'],
    ...
    def _stripe_search_filters(self):
        # EXTENDS stripe.issuing
        return {**super()._stripe_search_filters(), 'email': self.work_email or self.email}