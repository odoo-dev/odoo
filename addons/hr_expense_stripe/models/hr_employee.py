...
class HrEmployee(models.Model):
    ...
    def unlink(self):  # TODO, archive (write) instead of unlink and disable on stripe
        unlink_result = super().unlink()
        for record in self:
            if record.stripe_id:
                make_request_stripe_proxy(f'/api/stripe_issuing/1/cardholders/{record.stripe_id}', { 'account': record.company_id.stripe_id }, method='DELETE')
