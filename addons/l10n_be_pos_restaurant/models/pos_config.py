...
class PosConfig(models.Model):
    ...
    def _create_takeaway_fiscal_position(self, config):
        ...
        if tax_21 and tax_12 and tax_6:
            fp = self.env.ref(f"l10n_be_pos_restaurant.{self.env.company.id}_fiscal_position_take_out", raise_if_not_found=False)
            if not fp:
                ...
                self.env['ir.model.data']._update_xmlids([
                    {
                        'xml_id': f"l10n_be_pos_restaurant.{self.env.company.id}_fiscal_position_take_out",
                        'record': fp,
                        'noupdate': True,
                    }
                ])
            if not self.env.ref(f"l10n_be_pos_restaurant.{self.env.company.id}_tax_6_take_out", raise_if_not_found=False):
                ...
                self.env['ir.model.data']._update_xmlids([
                    {
                        'xml_id': f"l10n_be_pos_restaurant.{self.env.company.id}_tax_6_take_out",
                        'record': tax_6_copy,
                        'noupdate': True,
                    }
                ])
