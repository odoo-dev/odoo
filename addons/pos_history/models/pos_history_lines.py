...
class PosHistoryLine(models.Model):
    ...
    """
    Lightweight backend model to track deleted POS order lines.
    - Stores only essential fields; avoid adding heavy or niche data.
    - Not intended for frontend (JS) usage. Any tracking in JS should be
    handled in real-time via API calls (e.g., removeOrderline, deleteOrder).

    Tracking Fields:
    ----------------
    - pos_line_uuid: Unique identifier of the original order line (used to detect removal).
    - order_id: Reference to the related order.

    Notes:
    ------
    - Add extra fields only in localization-specific modules if needed.
    - Keep this model lean; avoid loading in normal POS flows, to ensure no overhead on POS loading time.
    """
    ...
    pos_line_uuid = fields.Char(string='Uuid', readonly=True, copy=False)
    ...
    _unique_pos_line_uuid = models.Constraint('unique (pos_line_uuid)', 'An order line with this pos_line_uuid already exists')
    ...
    def _load_pos_data_fields(self, config):
        return ['product_id', 'qty', 'price_unit', 'price_subtotal', 'price_subtotal_incl', 'pos_line_uuid', 'combo_item_id', 'combo_parent_id', 'combo_line_ids', 'price_extra', 'attribute_value_ids', 'write_date', 'order_id', 'discount', 'tax_ids']
