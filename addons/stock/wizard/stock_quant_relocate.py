...
class StockQuantRelocate(models.TransientModel):
    ...
    dest_location_id = fields.Many2one('stock.location', ui_domain="[('usage', '=', 'internal'), ('company_id', '=', company_id)]")
    dest_package_id_domain = fields.Char(compute="_compute_dest_package_id_domain")
    dest_package_id = fields.Many2one('stock.package', ui_domain="dest_package_id_domain", compute="_compute_dest_package_id", store=True)
