...
class StockForecasted_Product_Product(models.AbstractModel):
    ...
    def _get_product_leadtime(self, res, product_template_ids, product_ids):
        ...
        location = warehouses[:1].lot_stock_id
        for product in products:
            if not location:
                leadtime = [{'total_delay': 0}, {}]
            else:
                rule = product._get_rules_from_location(location)
                leadtime = rule._get_lead_days(product) or [{'total_delay': 0}, {}]
    ...
    def _get_warehouses(self):
        ...
        if warehouse_id:
            ...
            return self.env['stock.warehouse'].search(domain)[:1]
    ...
    def _get_report_lines(self, product_template_ids, product_ids, wh_location_ids, wh_stock_locations, read=True):
        ...
        def _get_out_move_reserved_data(out, linked_moves, used_reserved_moves, currents, sub_location_to_stock, wh_stock_sub_location_ids):
            reserved_out = 0
        ...
        def _get_out_move_taken_from_stock_data(out, currents, reserved_data, sub_location_to_stock, wh_stock_sub_location_ids):
        ...
        all_sub_locations = self.env['stock.location'].search([
            ('id', 'child_of', wh_stock_locations.ids),
            ('id', 'not in', wh_stock_locations.ids)
        ])
        ...
        for out_moves in outs_per_product.values():
            ...
            for out in out_moves:
                moves_data[out] = _get_out_move_reserved_data(
                    out, linked_moves_per_out[out], used_reserved_moves, currents, sub_location_to_stock, wh_stock_sub_location_ids
                )
            # another loop to remove qty from current stock after reserved is counted for
            for out in out_moves:
                data = _get_out_move_taken_from_stock_data(out, currents, moves_data[out], sub_location_to_stock, wh_stock_sub_location_ids)
        ...
        lines = []
        wh_stock_location_ids = wh_stock_locations.ids
        for product in (ins | outs).product_id | self._get_products(product_template_ids, product_ids):
            ...
            unreconciled_outs = []
            # remaining stock
            free_stock = sum(currents[product.id, stock_id] for stock_id in wh_stock_location_ids)
