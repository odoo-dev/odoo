...
class StockMoveLine(models.Model):
    ...
    def _pre_put_in_pack_hook(self):
    ...
    def action_put_in_pack(self):
        move_lines_to_pack = self.filtered(lambda ml: ml.state not in ('done', 'cancel'))
        move_lines_to_pack = move_lines_to_pack._package_move_lines()
        if move_lines_to_pack:
            res = move_lines_to_pack._pre_put_in_pack_hook()
            if not res:
                package = move_lines_to_pack._put_in_pack()
                return move_lines_to_pack._post_put_in_pack_hook(package)
            return res
