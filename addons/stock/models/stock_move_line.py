...
class StockMoveLine(models.Model):
    ...
    def _onchange_result_package_id(self):
        ...
        if not (package and package.package_type_id and package != self._origin.result_package_id):
            return
        max_weight = package.package_type_id.max_weight
        if not max_weight:
            return
        current_package_weight = package._get_weight(picking_id=self.picking_id.id, include_quants=True).get(package, 0.0)
    ...
    def _post_put_in_pack_hook(self, package):
        if weight := self.env.context.get('weight'):
