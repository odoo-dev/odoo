...
class StockQuant(models.Model):
    ...
    def _search_last_count_date(self, operator, value):
        ...
        valid_location = ['internal', 'transit']
        domain = Domain([('state', '=', 'done'), ('is_inventory', '=', True), '|', ('location_id.usage', 'in', valid_location), ('location_dest_id.usage', 'in', valid_location)])
        groups = self.env['stock.move.line']._read_group(
            domain=domain,
        ...
        filtered = {}
        for row in groups:
            product, lot, package, owner, result_package, location, location_dest, max_date = row
            internal_loc = location if location.usage in valid_location else location_dest
            if not internal_loc:
                continue
            key = (product.id, lot.id, package.id, owner.id, result_package.id, internal_loc.id)
            if key not in filtered or filtered[key][-1] < max_date:
                filtered[key] = row
        ...
        for product, lot, package, owner, result_package, location, location_dest, max_date in result:
            test = location if location.usage in valid_location else location_dest
