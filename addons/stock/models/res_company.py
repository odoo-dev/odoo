...
class ResCompany(models.Model):
    ...
    def create_missing_inventory_loss_location(self):
        ...
        companies_having_property = self.env['ir.default'].sudo().search_fetch([('field_id', '=', inventory_loss_product_template_field.id)]).mapped('company_id')
    ...
    def create_missing_production_location(self):
        ...
        companies_having_property = self.env['ir.default'].sudo().search_fetch([('field_id', '=', production_product_template_field.id)]).mapped('company_id')
        company_without_property = company_ids - companies_having_property
    ...
    def create_missing_scrap_location(self):
        company_ids  = self.env['res.company'].search([])
        companies_having_scrap_loc = self.env['stock.location'].search_fetch([('usage', '=', 'inventory')]).mapped('company_id')
    ...
    def create_missing_scrap_sequence(self):
        company_ids  = self.env['res.company'].search([])
        company_has_scrap_seq = self.env['ir.sequence'].search_fetch([('code', '=', 'stock.scrap')]).mapped('company_id')
