...
class StockWarehouseOrderpoint(models.Model):
    ...
    def _get_orderpoint_action(self):
        ...
        for (product, warehouse), quantity in to_refill.items():
            ...
            virutal_available = product.with_context(
                warehouse=warehouse.id,
                to_date=fields.datetime.now() + relativedelta.relativedelta(days=lead_days)
            ).virtual_available
            if float_compare(virutal_available, 0, precision_rounding=product.uom_id.rounding) >= 0:
                key_to_remove.append((product.id, warehouse.id))
            else:
                to_refill[(product.id, warehouse.id)] = virutal_available
