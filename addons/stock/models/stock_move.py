...
class StockMove(models.Model):
    ...
    def _get_lot_domain(self):
        return """[('product_id', '=', product_id)] + (
            [('id', 'in', context.get('lot_ids'))] if context.get('lot_ids')
            else [('product_qty', '>', 0), ('quant_ids', 'any', [('location_id', 'child_of', location_id)])] if location_usage == 'internal'
            else []
        )"""
