...
class StockWarehouse(models.Model):
    ...
    def create(self, vals_list):
        ...
        # setup inter-warehouse/company transit locations
    ...
    def write(self, vals):
        ...
        if vals.get('partner_id'):
            self._update_inter_warehouse_resupply_routes_values(vals.get('partner_id'))

        res = super().write(vals)

        if vals.get('partner_id'):
            self._update_inter_warehouse_location_properties()
    ...
    def create_resupply_routes(self, supplier_warehouses):
        ...
        for supplier_wh in supplier_warehouses:
            ...
            pull_rules_list = supplier_wh._get_supply_pull_rules_values(
                [self.Routing(output_location, transit_location, supplier_wh.out_type_id, 'pull', 'make_to_stock')],
            ...
            if supplier_wh.delivery_steps != 'ship_only':
                ...
                pull_rules_list += supplier_wh._get_supply_pull_rules_values(
                    [self.Routing(supplier_wh.lot_stock_id, output_location, supplier_wh.pick_type_id, 'pull', 'make_to_stock')],
                    values={'route_id': inter_wh_route.id})
            pull_rules_list += self._get_supply_pull_rules_values(
                [self.Routing(transit_location, self.lot_stock_id, self.in_type_id, 'pull_push', 'make_to_stock')],
                values={'route_id': inter_wh_route.id, 'partner_address_id': supplier_wh.partner_id.id, 'push_domain': f"[('partner_id', '=', {self.partner_id.id}), ('location_id.warehouse_id', '=', {supplier_wh.id})]"})
    ...
    def _update_inter_warehouse_resupply_routes_values(self, partner_id):
        ...
        grouped_routes = self.env['stock.route'].sudo()._read_group([('supplier_wh_id', 'in', self.ids)], ['supplier_wh_id'], ['id:recordset'])
        ...
        for warehouse in self:
            for route in warehouse.resupply_route_ids:
                ...
                for rule in rules_to_update:
                    if f'("partner_id", "=", {warehouse.partner_id.id})' in rule.push_domain:
    ...
    def get_rules_dict(self):
        ...
        return {
            warehouse.id: {
                'one_step': [warehouse.Routing(supplier_loc, warehouse.lot_stock_id, warehouse.in_type_id, 'pull', 'make_to_stock')],
                'two_steps': [
                    warehouse.Routing(supplier_loc, warehouse.lot_stock_id, warehouse.in_type_id, 'pull', 'make_to_stock'),
                    warehouse.Routing(warehouse.wh_input_stock_loc_id, warehouse.lot_stock_id, warehouse.store_type_id, 'push', 'make_to_order')],
                'three_steps': [
                    warehouse.Routing(supplier_loc, warehouse.lot_stock_id, warehouse.in_type_id, 'pull', 'make_to_stock'),
                    warehouse.Routing(warehouse.wh_input_stock_loc_id, warehouse.wh_qc_stock_loc_id, warehouse.qc_type_id, 'push', 'make_to_order'),
                    warehouse.Routing(warehouse.wh_qc_stock_loc_id, warehouse.lot_stock_id, warehouse.store_type_id, 'push', 'make_to_order')],
                'ship_only': [
                    warehouse.Routing(warehouse.lot_stock_id, customer_loc, warehouse.out_type_id, 'pull', 'make_to_stock'),
                    warehouse.Routing(warehouse.lot_stock_id, intercompany_loc, warehouse.out_type_id, 'pull', 'make_to_stock'),
                    warehouse.Routing(warehouse.lot_stock_id, warehouse.company_id.internal_transit_location_id, warehouse.out_type_id, 'pull', 'make_to_stock')],
                'pick_ship': [
                    warehouse.Routing(warehouse.lot_stock_id, customer_loc, warehouse.pick_type_id, 'pull', 'make_to_stock'),
                    warehouse.Routing(warehouse.lot_stock_id, intercompany_loc, warehouse.pick_type_id, 'pull', 'make_to_stock'),
                    warehouse.Routing(warehouse.lot_stock_id, warehouse.company_id.internal_transit_location_id, warehouse.pick_type_id, 'pull', 'make_to_stock'),
                    warehouse.Routing(warehouse.wh_output_stock_loc_id, customer_loc, warehouse.out_type_id, 'push', 'make_to_order'),
                    warehouse.Routing(warehouse.wh_output_stock_loc_id, intercompany_loc, warehouse.out_type_id, 'push', 'make_to_order'),
                    warehouse.Routing(warehouse.wh_output_stock_loc_id, warehouse.company_id.internal_transit_location_id, warehouse.out_type_id, 'push', 'make_to_order')],
                'pick_pack_ship': [
                    warehouse.Routing(warehouse.lot_stock_id, customer_loc, warehouse.pick_type_id, 'pull', 'make_to_stock'),
                    warehouse.Routing(warehouse.lot_stock_id, intercompany_loc, warehouse.pick_type_id, 'pull', 'make_to_stock'),
                    warehouse.Routing(warehouse.lot_stock_id, warehouse.company_id.internal_transit_location_id, warehouse.pick_type_id, 'pull', 'make_to_stock'),
                    warehouse.Routing(warehouse.wh_pack_stock_loc_id, warehouse.wh_output_stock_loc_id, warehouse.pack_type_id, 'push', 'make_to_order'),
                    warehouse.Routing(warehouse.wh_output_stock_loc_id, customer_loc, warehouse.out_type_id, 'push', 'make_to_order'),
                    warehouse.Routing(warehouse.wh_output_stock_loc_id, intercompany_loc, warehouse.out_type_id, 'push', 'make_to_order'),
                    warehouse.Routing(warehouse.wh_output_stock_loc_id, warehouse.company_id.internal_transit_location_id, warehouse.out_type_id, 'push', 'make_to_order')],
