...
export class ForecastedDetails extends Component {
    ...
    async _reserve(move_id) {
        await this.orm.call("stock.forecasted_product_product", "action_reserve_linked_picks", [
            move_id,
        ]);
    ...
    async _unreserve(move_id) {
        await this.orm.call("stock.forecasted_product_product", "action_unreserve_linked_picks", [
            move_id,
        ]);
        this.props.reloadReport();
    }
    ...
    _onHandCondition(line) {
        return (
            !line.document_in && !line.in_transit && line.replenishment_filled && line.document_out
        );
    }
    ...
    isOnHand(line) {
        return (
            this.OnHandLinesPerProduct[line.product.id] &&
            this.OnHandLinesPerProduct[line.product.id].includes(this.lines[this.line_index])
        );
    }
    ...
    isReconciled(line) {
        return (
            this.ReconciledLinesPerProduct[line.product.id] &&
            this.ReconciledLinesPerProduct[line.product.id].includes(this.lines[this.line_index])
        );
    }
