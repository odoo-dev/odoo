...
export class GenerateDialog extends Component {
    ...
    async _onGenerateCustomSerial() {
        ...
        if (product.lot_sequence_id) {
            ...
            this.nextCustomSerialNumber = await this.orm.call("ir.sequence", "next_by_id", [
                this.sequence.id,
            ]);
    ...
    async _onGenerate() {
        ...
        if (this.sequence && this.nextSerial.el.value === this.nextCustomSerialNumber) {
            await this.orm.write("ir.sequence", [this.sequence.id], {
                number_next_actual: this.sequence.number_next_actual + newlines.length,
            });
        }
        this.props.close();
    }
