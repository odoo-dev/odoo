...
class TestReportStockQuantity(tests.TransactionCase):
    ...
    def test_past_date_quantity_with_multistep_delivery(self):
        ...
        """
        def get_inv_qty_at_date(product_id, warehouse, inv_datetime):
            product = self.env['product.product'].with_context(warehouse_id=warehouse.id, to_date=inv_datetime).browse(product_id)
        ...
        for date, expected_qties in (
            ...
            qty = get_inv_qty_at_date(product.id, warehouse, date)
