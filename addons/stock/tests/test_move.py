...
class TestStockMove(TestStockCommon):
    ...
    def test_modifying_lots_updates_move_lines_and_quantity(self):
        """Ensure that modifying the tracking (lot/serial) field on a move correctly
        updates its move lines and reserved quantities.
        """
        ...
        delivery.action_confirm()
        self.assertEqual(delivery.move_ids.lot_ids, lot1 | lot2)
        ...
        delivery.button_validate()
        self.assertRecordValues(delivery.move_ids, [{'lot_ids': lot3.ids, 'quantity': 20}])
