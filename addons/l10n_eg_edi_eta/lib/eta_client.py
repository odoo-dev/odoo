...
class ETAClient:
    ...
    def get_invoice_url(self, uuid, long_id):
        domain = self.invoice_domain.rstrip("/")

        return f'{domain}/documents/{uuid}/share/{long_id}'
    ...
    def get_receipt_url(self, uuid, date_order, amount_total, seller_rin):
        domain = self.receipt_domain.rstrip("/")
        return f'{domain}/receipts/search/{uuid}/share/{date_order}#Total:{amount_total},IssuerRIN:{seller_rin}'
    ...
    def _request(
        ...
        try:
            ...
            return False, self._parse_error(
