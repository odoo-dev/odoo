...
class MyInvoisDocument(models.Model):
    ...
    In later versions, we will also link them to invoices to reduce the duplicated ic.
    """
    ...
    def _compute_display_name(self):
        for document in self:
            document.display_name = document.name if document.name != '/' else self.env._('Draft')
    ...
    def _unlink_check(self):
        for document in self:
            if document.myinvois_state in ["in_progress", "valid", "rejected"]:
                raise UserError(self.env._('You cannot delete a document that is active on MyInvois.\nYou must cancel it first.'))
    ...
    def action_generate_xml_file(self, overwrite_file=True):
        ...
        for document in self:
            ...
            if errors:
                raise UserError(self.env._("Error when generating the documents' files:\n\n- %(errors)s", errors='\n- '.join(errors)))
    ...
    def _myinvois_submission_statuses_update(self, with_commit=True):
        ...
        for submission_uid, results in statuses.items():
            ...
            if results['error']:
                ...
                records._myinvois_log_message(bodies={move.id: message for move in self})
                continue
            ...
            for record, status in results['statuses'].items():
                ...
                if status.get('reason') or status['status'] == 'invalid':
                    if status.get('reason'):
                        message = self.env._('The MyInvois platform returned a "%(status)s" status for this document for reason: %(reason)s', status=status['reason'], reason=status['reason'])
                    else:
                        message = self.env._('The MyInvois platform returned an "%(status)s" status for this document.', status=status['reason'])
    ...
    def _myinvois_statuses_update_cron(self):
        ...
        for submission_uid, documents in grouped_documents:
            ...
            if self._can_commit():
                self.env['ir.cron']._notify_progress(done=processed_documents, remaining=document_count - processed_documents)
                self._cr.commit()
