...
class Quiz(models.Model):
    ...
    unlimited_tries = fields.Boolean("Unlimited Tries",
        help="Will allow attendees to reset the quiz and try again.")
...
class QuizQuestion(models.Model):
    ...
    correct_answer = fields.One2many('event.quiz.answer', compute='_compute_correct_answer')
    ...
    def _compute_correct_answer(self):
        for question in self:
            question.correct_answer = question.answer_ids.filtered(lambda e: e.is_correct)

    @api.constrains('answer_ids')
    def _check_answers_integrity(self):
        for question in self:
            if not len(question.correct_answer) == 1:
