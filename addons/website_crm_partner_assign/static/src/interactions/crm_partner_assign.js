...
export class CRMPartnerAssign extends Interaction {
    ...
    dynamicContent = {
        ".interested_partner_assign_confirm": {
            "t-on-click.prevent.stop": this.locked(this.onInterestedPartnerConfirmClick),
        },
        ".desinterested_partner_assign_confirm": {
            "t-on-click.prevent.stop": this.locked(this.onDesinterestedPartnerConfirmClick),
        },
    ...
    async onEditContactClick() {
        await this.services.orm.call("crm.lead", "update_contact_details_from_portal", [
            [parseInt(this.contactFormEl.querySelector(".opportunity_id").value)],
            {
                ...
                state_id: parseInt(
                    this.contactFormEl.querySelector(".state_id").selectedOptions[0].value
                ),
                country_id: parseInt(
                    this.contactFormEl.querySelector(".country_id").selectedOptions[0].value
                ),
            },
        ]);
        window.location.reload();
    ...
    async onInterestedPartnerConfirmClick() {
        ...
        const contacted =
            this.interestedPartnerFormEl.querySelector(".contacted_interested").checked;
