...
publicWidget.registry.RatingPopupComposer = publicWidget.Widget.extend({
    ...
    _reloadRatingPopupComposer: function () {
        if (this.options.hide_rating_avg) {
            ...
            const ratingAverage = qweb.render('portal_rating.rating_stars_static', {inline_mode: true, widget: this, 'val': this.rating_avg});
            this.$('.o_rating_popup_composer_stars').empty().html(ratingAverage);
        }

        // Append the modal
        const modal = qweb.render('portal_rating.PopupComposer', {inline_mode: true, widget: this, 'val': this.rating_avg});
...
return publicWidget.registry.RatingPopupComposer;
