...
def test_02_copy_ids_views_unlink_on_module_update(env):
    ...
    - Removing the theme.ir.ui.view B from the XML file and then updating the
      ...
      Note: Updating a theme module never applies the theme on a website, it
            only refreshes the theme templates. A template removed from the XML
            is the one case where the update still has to reach the websites,
            otherwise their copies would be left behind as orphans.
    """
    View = env['ir.ui.view']
