...
class TestImageUploadProgress(odoo.tests.HttpCase):
    ...
    def test_02_image_upload_progress_unsplash(self):
        ...
        Web_Unsplash.fetch_unsplash_images = http.route("/web_unsplash/fetch_images", type='rpc', auth="user")(fetch_unsplash_images)

        # disable undraw, no third party should be called in tests
        media_library_search.routing_type = 'json'
        Web_Editor.media_library_search = http.route(['/web_editor/media_library_search'], type='rpc', auth="user", website=True)(media_library_search)
