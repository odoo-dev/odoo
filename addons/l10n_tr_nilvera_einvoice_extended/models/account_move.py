...
class AccountMove(models.Model):
    ...
    l10n_tr_tax_office_id = fields.Many2one(related="partner_id.l10n_tr_tax_office_id")
    ...
    def _compute_l10n_tr_exemption_code_domain_list(self):
        for record in self:
            domain = []
            if (
                record.l10n_tr_gib_invoice_scenario
                in ["TEMELFATURA", "KAMU", "EARSIVFATURA"]
                and record.l10n_tr_gib_invoice_type == "ISTISNA"
            ):
            ...
            if (
                record.l10n_tr_gib_invoice_scenario
                in ["TEMELFATURA", "KAMU", "EARSIVFATURA"]
                and record.l10n_tr_gib_invoice_type == "IHRACKAYITLI"
            ):
                domain.append("export_registration")
            if record.l10n_tr_gib_invoice_scenario == "IHRACAT":
                domain.append("export_exception")
    ...
    @api.depends("l10n_tr_gib_invoice_scenario")
    ...
    def _get_partner_l10n_tr_nilvera_customer_alias_name(self):
        self.ensure_one()
        if self.l10n_tr_gib_invoice_scenario != "IHRACAT":
