        ...
        try:
            ...
            if id_record:
                ...
                if model_name == 'mail.mail':
                    website_form_signature = kwargs.get("website_form_signature") or ''
                    form_has_email_cc = {'email_cc', 'email_bcc'} & kwargs.keys() or \
                        'email_cc' in website_form_signature
                    # remove the email_cc information from the signature
                    website_form_signature = website_form_signature.split(':')[0]
                    if kwargs.get("email_to"):
                        ...
                        if not consteq(website_form_signature, hash_value):
