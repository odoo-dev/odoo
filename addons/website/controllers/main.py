...
class Website(Home):
    ...
    def robots(self, **kwargs):
        # Don't use `request.env['website']._get_current_website().domain` here, the template is in charge of
    ...
    def social(self, social, **kwargs):
        website = request.env['website']._get_current_website()
        url = getattr(website, 'social_%s' % social, False)
        if not url:
            raise werkzeug.exceptions.NotFound()
    ...
    def autocomplete(self, search_type=None, term=None, order=None, limit=5, max_nb_chars=999, options=None):
        ...
        options = options or {}
        website = request.env['website']._get_current_website()
        results_count, search_results, fuzzy_term = website._search_with_fuzzy(search_type, term, limit, order, options)
