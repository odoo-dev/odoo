...
class WebsiteVisitorTests(MockVisitor, HttpCaseWithUserDemo):
    ...
    def test_clean_inactive_visitors(self):
        ...
        self._test_clean_inactive_visitors(inactive_visitors, active_visitors)

    def _test_clean_inactive_visitors(self, inactive_visitors, active_visitors):
        ...
        self.env['website.visitor']._cron_clean_inactive_visitors()

        if inactive_visitor_ids:
            # all inactive visitors should be deleted
            self.assertFalse(bool(self.env['website.visitor'].search([('id', 'in', inactive_visitor_ids)])))
        if active_visitor_ids:
            # all active visitors should be kept
            self.assertEqual(active_visitors, self.env['website.visitor'].search([('id', 'in', active_visitor_ids)]))
