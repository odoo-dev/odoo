...
class Website(models.CachedModel):
    ...
    @api.model
    def _get_theme_configurator_snippets(self, theme_name):
        """
    ...
    def _configurator_get_footer_links(self):
        return [
            {'text': _("Privacy Policy"), 'href': '/privacy'},
        ]

    def _check_configurator_access(self):
        """Restrict the configurator entry points to website designers.

        Those methods are reachable over RPC but do not read or write any
        website record, so no access rule would be checked otherwise.
        """
        if not self.env.user.has_group('website.group_website_designer'):
            raise AccessError(_("Only website designers can use the website configurator."))
    ...
    def configurator_apply(self, **kwargs):
        ...
        website._configurator_addons_apply(**kwargs)

        # Update footers links after addon setup to go through module overrides
        # of `_configurator_get_footer_links`.
        footer_links = website._configurator_get_footer_links()
        footer_ids = [
        ...
        configurator_snippets = website._get_theme_configurator_snippets(theme_name)
