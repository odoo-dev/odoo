...
from odoo.tools import BinaryBytes, BinaryValue, file_open
...
class Website(models.CachedModel):
    ...
    def _default_favicon(self):
        with file_open('web/static/img/favicon.ico', 'rb') as f:
            return BinaryBytes(f.read(), file_name='favicon.ico')
    ...
    def _handle_favicon(self, vals):
        if icon := vals.get('favicon'):
            file_name = icon.file_name if isinstance(icon, BinaryValue) else ''
            vals['favicon'] = BinaryBytes(image_process(icon, size=(256, 256), crop='center', output_format='ICO'), file_name)
