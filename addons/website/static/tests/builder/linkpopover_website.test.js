...
test("should add rel='nofollow' when checkbox is selected and applied", async () => {
    ...
    expect(".o-popover-sliding-panel").toHaveCount(1);
    await contains(
        ".o_advance_options_grid .o-popover-sliding-panel-card:nth-of-type(1) > div:nth-of-type(1) input[type='checkbox']"
    ).click();
...
test("should add multipule relAttribute in anchor tag when checkbox is selected and applied", async () => {
    ...
    await contains(
        ".o_advance_options_grid .o-popover-sliding-panel-card:nth-of-type(1) > div:nth-of-type(2) input[type='checkbox']"
    ).click();
    await contains(
        ".o_advance_options_grid .o-popover-sliding-panel-card:nth-of-type(1) > div:nth-of-type(3) input[type='checkbox']"
    ).click();
    await contains(".o-popover-sliding-panel [data-icon='keyboard_arrow_left']").click();
    await waitFor(".o-we-linkpopover");
    await contains(".o_we_apply_link").click();
...
test("should add _blank attribute on open in a new window is checked", async () => {
    ...
    expect(".o-popover-sliding-panel").toHaveCount(1);
    await contains(
        ".o_advance_options_grid .o-popover-sliding-panel-card:nth-of-type(2) > div:nth-of-type(1) input[type='checkbox']"
    ).click();
    await contains(
        ".o_advance_options_grid .o-popover-sliding-panel-card:nth-of-type(2) > div:nth-of-type(2) input[type='checkbox']"
    ).click();
...
test("should add, edit and remove URL parameters from advanced options", async () => {
    ...
    await contains(".o-popover-sliding-panel-card-add").click();
    ...
    await contains(".o-popover-sliding-panel-card-remove:first").click();
    expect(".o_url_parameter_name").toHaveCount(1);
    await click(".o-popover-sliding-panel [data-icon='keyboard_arrow_left']");
    await waitFor(".o-we-linkpopover");
    await contains(".o_we_apply_link").click();
