...
test.skip("order of search results in add snippet dialog", async () => {
    const snippetsDescription = (withName = false) => [
        {
            name: "foo_bar",
            groupName: "a",
            content: getBasicSection("content 1", { name: "foo_bar", snippet: "s_foo_bar" }),
            keywords: [],
        },
        {
            name: "foo",
    ...
    // If the search starts with "s_" search among classes of child nodes
    ...
    // If the search is exactly equal to a snippet xmlid, show that snippet first
    await contains(".o_add_snippet_dialog aside input[type='search']").edit("s_foo");
    expect(
        queryFirst(".o_add_snippet_dialog .o_add_snippet_iframe:iframe .o_snippet_preview_wrap")
            .innerHTML
    ).toEqual(snippetsDescriptionProcessed[1].content);

    // If the search is a full class name used on the snippet root node, show that snippet first
    ...
    // Otherwise show a partial class match of a snippet first if it happens on the root node
    await contains(".o_add_snippet_dialog aside input[type='search']").edit("s_fo");
    expect(
        queryFirst(".o_add_snippet_dialog .o_add_snippet_iframe:iframe .o_snippet_preview_wrap")
            .innerHTML
