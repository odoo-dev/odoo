...
test("alert message added on edit and removed on save", async () => {
    const resultSave = [];
    ...
    await contains(":iframe .s_floating_blocks").click();
    expect(":iframe .s_floating_blocks_alert_empty").toHaveCount(1);
    await contains(".o-snippets-top-actions button:contains(Save)").click();
    expect(resultSave[0]).not.toInclude(".s_floating_blocks_alert_empty");
});

test("alert message displayed if floating blocks has no cards", async () => {
    await setupWebsiteBuilder(websiteContent);
    ...
    await contains(":iframe .s_floating_blocks_block").click();
    await contains(".options-container[data-container-title='Card'] .oe_snippet_remove").click();
    expect(":iframe .s_floating_blocks_alert_empty").not.toInclude("d-none");
});

test("floating blocks snippet removed on save if empty", async () => {
    const resultSave = [];
    onRpc("ir.ui.view", "save", ({ args }) => {
        resultSave.push(args[1]);
        return true;
