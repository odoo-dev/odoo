...
import wUtils from '@website/js/utils';
    ...
    publicWidget.registry.s_website_form = publicWidget.Widget.extend({
        ...
        send: async function (e) {
            ...
            self.el.querySelectorAll(".s_website_form_custom_error")?.forEach((error) => {
                error.remove();
            });
        ...
        update_status_inline(message, inputEl) {
            ...
            inputEl.parentElement.classList.contains("date") ? inputEl.parentElement.after(renderToElement("website.s_website_form_status_custom_error",
            {message: message})) : inputEl.parentElement.appendChild(renderToElement("website.s_website_form_status_custom_error", {message: message}));
        },
