...
const NavbarLinkPopoverWidget = weWidgets.LinkPopoverWidget.extend({
     ...
     */
    start() {
        // break link has no sense on navbar menu links, instead show edit menu
        const $anchor = $('<a/>', {
            href: '#', class: 'ml-2 js_edit_menu', title: _t('Edit Menu'),
            'data-placement': 'top', 'data-toggle': 'tooltip',
        }).append($('<i/>', {class: 'fa fa-hand-rock-o'}));
        this.$('.o_we_break_link').replaceWith($anchor);
...
NavbarLinkPopoverWidget.createFor = async function (parent, targetEl) {
    // Target might already have a popover, eg cart icon in navbar
    if ($(targetEl).data('bs.popover')) {
        return null;
    }
    const popoverWidget = new NavbarLinkPopoverWidget(parent, targetEl);
