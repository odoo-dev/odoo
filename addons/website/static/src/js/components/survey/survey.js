...
const actions = {
    selectWebsiteType({state}, id) {
        Object.values(state.features).forEach((feature) => {
            feature.selected = feature.website_type === WEBSITE_TYPES[id].name;
        });
    ...
    toggleFeature({state}, featureId) {
        ...
        const isSelectedType = WEBSITE_TYPES[state.selectedType] && state.features[featureId].website_type === WEBSITE_TYPES[state.selectedType].name;
...
async function getInitialState(wid) {
    ...
    const features = rpc.query({
        ...
        method: 'search_read',
        fields: ['title', 'description', 'type', 'icon', 'website_type'],
