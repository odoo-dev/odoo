...
async function getIndustryImages(orm, state, theme = "") {
    const industryId = state.selectedIndustry?.id;
...
export class DescriptionScreen extends Component {
    ...
    async fetchIndustryImages(industryId) {
        ...
        this.state.setImages({});
        const images = await getIndustryImages(this.orm, this.state);
        if (
...
export class PaletteSelectionScreen extends Component {
    ...
    setup() {
        ...
        this.orm = useService("orm");
        this.hoverState = useState({ palette: null });
...
- "reason": a short sentence in ${userLanguage} mentioning the business context, like: "For a family restaurant with a cozy positioning, I'd recommend warm colors to feel welcoming."`;
        ...
        try {
            ...
            const parsed = match && JSON.parse(match[0]);
            if (parsed) {
                const palette = palettes[parsed.id];
                if (parsed.reason) {
    ...
    async _removeAttachments(ids) {
        await rpc("/html_editor/attachment/remove", { ids: ids });
    }
}
...
export class ThemeSelectionScreen extends ApplyConfiguratorScreen {
    ...
    setup() {
        ...
        useLayoutEffect(
            ...
            () => [this.state.previewHeaders?.join("\n") || ""]
        );
        onMounted(async () => {
    ...
    scalePreviewIframe(iframe) {
        ...
        // `top` is animated in container pixels, so the distance must use the
        // scaled iframe height, not the raw document height.
    ...
    async getInitialState() {
        ...
        if (localState) {
            ...
            if (storedState.selectedIndustry?.id > 0) {
                images = await getIndustryImages(this.orm, storedState, themes[0]?.name || "");
            }
