...
class ImageSelector extends Component {
    ...
    setup() {
        ...
        let firstImageSrc;
        if (this.props.socialDefaultImageSrc) {
            firstImageSrc = this.props.socialDefaultImageSrc;
        } else {
            firstImageSrc = `/web/image/website/${encodeURIComponent(
        ...
        this.state = useState({
            images: [
                firstImage,
                ...this.props.pageImages.filter((src) => src !== firstImage.src).map((src) => {
                    return {
                        src,
    ...
    get isSocialDefaultImageActive() {
        return (
            this.props.hasSocialDefaultImage
            && this.props.socialDefaultImageSrc === this.state.images.find(({ active }) => active).src
        );
    }

    setDefaultCoverImage(ev) {
...
export class OptimizeSEODialog extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            this.socialDefaultImageSrc = this.data.social_default_image_src? this.data.social_default_image_src : '';
        ...
        if (seoContext.defaultImage) {
            rpcCalls.push(
                rpc("/website/set_default_social_image", {
                    website_id: this.website.currentWebsite.id,
                    img_url: seoContext.defaultImage,
                })
            );
        } else {
            rpcCalls.push(
                rpc("/website/unset_has_social_default_image", {
                    website_id: this.website.currentWebsite.id,
                })
            );
        }
