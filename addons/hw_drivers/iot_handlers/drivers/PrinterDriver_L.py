...
class PrinterDriver(Driver):
    ...
    def print_status_receipt(self):
        ...
        title = commands['title'] % (b'IoT Box Connected' if helpers.get_odoo_server_url() else b'IoT Box Status')
        self.print_raw(commands['center'] + title + b'\n' + pairing_code.encode() + wlan.encode() + mac.encode() + ip.encode() + homepage.encode() + commands['cut'])
