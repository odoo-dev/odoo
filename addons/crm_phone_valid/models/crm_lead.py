    ...
    class FormatPhoneMixin(object):
        def _check_contact_number(self, contact_number, country=None):
            ...
            try:
                ...
                raise UserError(_('Unable to parse the contact (phone,mobile,fax) number!'))
            if not phonenumbers.is_possible_number(nbr):
                raise UserError(_('Invalid contact (phone,mobile,fax) number, too few or too much digits!'))
            if not phonenumbers.is_valid_number(nbr):
                raise UserError(_('Invalid contact (phone,mobile,fax) number, the prefix does not belong to a valid NPA!'))
