...
class Web_Unsplash(http.Controller):
    ...
    # ------------------------------------------------------
    @http.route('/web_unsplash/attachment/add', type='rpc', auth='user', methods=['POST'])
    ...
    @http.route("/web_unsplash/fetch_images", type='rpc', auth="user")
    ...
    @http.route("/web_unsplash/get_app_id", type='rpc', auth="public")
    def get_unsplash_app_id(self, **post):
        return request.env['ir.config_parameter'].sudo().get_param('unsplash.app_id')

    @http.route("/web_unsplash/save_unsplash", type='rpc', auth="user")
