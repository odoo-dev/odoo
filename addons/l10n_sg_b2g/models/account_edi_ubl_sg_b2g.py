...
class AccountEdiXml_SgB2G(models.AbstractModel):
    ...
    def _add_invoice_header_nodes(self, document_node, vals):
        """
        OVERRIDE of `account.edi.xml.pint_sg` to add the SG B2G specific values.
        """
        ...
        document_node.update({
            'cbc:BuyerReference': {'_text': unit_code},
        })

        # Related Invoice ID (for Credit Notes)
    ...
    def _ubl_add_order_reference_node(self, vals):
        """
        OVERRIDE of `account.edi.xml.ubl_bis3`. If there is no II/PO, omit the OrderReference node.
        """
        invoice = vals['invoice']
        if not invoice.l10n_sg_is_from_po_invoice:
            return
        order_ref_node = vals['document_node']['cac:OrderReference']
        order_ref_node['cbc:ID']['_text'] = invoice.ref
    ...
    def _add_invoice_monetary_total_nodes(self, document_node, vals):
        """
        EXTENDS 'account.edi.xml.ubl_bis3'
        BIS3 sets TaxExclusiveAmount = LineExtensionAmount, which is only correct when
        document-level charges and allowances net to zero (as with early payment). Adjust when
        freight charges introduce a non-zero net.
        """
        super()._add_invoice_monetary_total_nodes(document_node, vals)
        net_charge = sum(
            ac['cbc:Amount']['_text'] * (1 if ac['cbc:ChargeIndicator']['_text'] == 'true' else -1)
            for ac in document_node['cac:AllowanceCharge']
        )
        if net_charge:
            monetary_total_tag = self._get_tags_for_document_type(vals)['monetary_total']
            monetary_total = document_node[monetary_total_tag]
            monetary_total['cbc:TaxExclusiveAmount']['_text'] = FloatFmt(
                float(monetary_total['cbc:TaxExclusiveAmount']['_text']) + net_charge,
                min_dp=vals['currency_dp'],
            )
            monetary_total['cbc:TaxInclusiveAmount']['_text'] = FloatFmt(
                float(monetary_total['cbc:TaxInclusiveAmount']['_text']) + net_charge,
                min_dp=vals['currency_dp'],
            )

    def _get_invoice_line_node(self, vals):
        """
        EXTENDS `account.edi.xml.ubl_20` to add 'OrderLineReference' node to the invoice line node.
        This field is mandatory for SG B2G invoices.
        """
        line_node = super()._get_invoice_line_node(vals)
        line_ref = vals['base_line']['record'].l10n_sg_b2g_invoice_line_no or line_node['cbc:ID']['_text']
        line_node['cac:OrderLineReference'] = {
            'cbc:LineID': {'_text': f"{line_ref}"},
        }

        # Swap the item quantity and gross price unit values if the item is service and the invoice is from a PO/II
        move = vals['invoice']
        line_product = vals['base_line']['product_id']
        if move.l10n_sg_is_from_po_invoice and line_product.type == 'service':
            if move.move_type == 'out_invoice':
                item_qty_node = line_node['cbc:InvoicedQuantity']
            elif move.move_type == 'out_refund':
                item_qty_node = line_node['cbc:CreditedQuantity']
            else:
                raise ValidationError(self.env._("Invalid invoice type: %s", move.move_type))

            item_qty = item_qty_node['_text']
            gross_price_unit_node = line_node['cac:Price']['cbc:PriceAmount']
            gross_price_unit = gross_price_unit_node['_text']

            item_qty_node['_text'] = gross_price_unit
            gross_price_unit_node['_text'] = item_qty
        ...
        if needs_separate_payee:
            document_node['cac:PayeeParty'] = {
                ...
                'cac:PartyName': {  # Mandatory field
                    'cbc:Name': {'_text': bank.acc_holder_name},
                },
            }
    ...
    def _export_invoice_constraints(self, invoice, vals):
        ...
        """
        invoice_description_nodes = vals['document_node']['cbc:Note']
        if not invoice_description_nodes:
        ...
        elif len(invoice_description_nodes[0]['_text']) > 254:
        ...
        """
        for line_node in vals['document_node']['cac:InvoiceLine']:
