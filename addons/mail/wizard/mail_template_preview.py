...
class MailTemplatePreview(models.TransientModel):
    ...
    def _compute_mail_template_fields(self):
        ...
        if not self.resource_ref or not self.resource_ref.id:
            ...
            try:
                embed = '_notify_get_recipients_groups' in dir(self.resource_ref)
                ...
                # Embed into notification layout
                if embed:
                    msg_vals = {
                        'body': mail_values['body_html'],
                        'partner_ids': mail_values.get('partner_ids', []),
                        'model': self.model_id.model,
                        'record_name': self.resource_ref.display_name,
                    }
                    mail_thread = self.env['mail.thread']
                    message = mail_thread._message_create([msg_vals])
                    recipients_data = mail_thread._notify_get_recipients(message, msg_vals)
                    if mail_values['email_to'] or not recipients_data:
                    ...
                    _lang, render_values, recipients_group = next(self.resource_ref._notify_get_classified_recipients_iterator(message, recipients_data, msg_vals=msg_vals))
                    render_values['email_add_signature'] = False
                    mail_values['body_html'] = mail_thread._notify_by_email_render_layout(
                        message,
                        recipients_group,
                        msg_vals=msg_vals,
                ...
                if mail_values['attachments']:
                    mail_values['attachment_ids'] += self.env['ir.attachment'].create([
                        {'name': attach_fname,
                         'datas': attach_datas,
