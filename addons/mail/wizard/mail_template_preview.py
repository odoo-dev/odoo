...
class MailTemplatePreview(models.TransientModel):
    ...
    recipient_names = fields.Char('Recipient Names', compute='_compute_mail_template_fields',
                                help="Names of the recipients")
    recipient_cc_names = fields.Char('Recipient Cc Names', compute='_compute_mail_template_fields',
                                     help="Names of the Cc recipients")
    # Extra fields info generated by _generate_template
    partner_ids = fields.Many2many('res.partner', string='Recipients To', compute='_compute_mail_template_fields')
    partner_cc_ids = fields.Many2many('res.partner', string='Recipients Cc', compute='_compute_mail_template_fields')
                ...
                try:
                    ...
                    if mail_template.email_layout_xmlid:
                        ...
                        msg_vals = {
                            ...
                            'email_layout_xmlid': mail_template.email_layout_xmlid,
                            ...
                            'partner_cc_ids': mail_values.get('partner_cc_ids', False),
    ...
    def _compute_has_several_languages_installed(self):
        for preview in self:
            preview.has_several_languages_installed = len(preview._fields['lang'].selection(preview)) > 1
    ...
    def _set_mail_attributes(self, values=None):
        for field in self._MAIL_TEMPLATE_FIELDS:
            if field in ('partner_to', 'partner_cc', 'report_template_ids'):
                # partner_to and partner_cc are used to generate respectively partner_ids and partner_cc_ids,
                # handled here below report_template_ids generates attachments, no usage here
                continue
        ...
        self.partner_cc_ids = values.get('partner_cc_ids', False) if values else False
        self.recipient_names = ", ".join(
            [partner.email_formatted or partner.name for partner in self.partner_ids] +
            ([values["email_to"]] if values and values.get("email_to") else [])
        )
        self.recipient_cc_names = ", ".join(
            [partner.email_formatted or partner.name for partner in self.partner_cc_ids] +
            ([values["email_cc"]] if values and values.get("email_cc") else [])
        )
