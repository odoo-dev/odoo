...
test("do not show message seen indicator on all the messages of the current user that are older than the last message seen by everyone", async () => {
    ...
    await contains(".o-mail-Message:has(:text('Message before last seen'))", {
        contains: [".o-mail-MessageSeenIndicator", { contains: ["[data-icon='check']", { count: 0 }] }],
    });
});
...
test("no seen indicator in 'channel' channels (with is_typing)", async () => {
    ...
    await contains(".o-mail-MessageSeenIndicator [data-icon='check']", { count: 1 }); // received in chat
