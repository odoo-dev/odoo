odoo.define("mail/static/src/widgets/discuss/discuss.js", function (require) {
    ...
    const DiscussWidget = AbstractAction.extend({
        ...
        init(parent, action, options = {}) {
            ...
            this.$buttons.on("click", ".o_widget_Discuss_controlPanelButtonMarkAllRead", (ev) => this._onClickMarkAllAsRead(ev));
        ...
        async willStart() {
            ...
            const initActiveId = this.options.active_id || (this.action.context && this.action.context.active_id) || (this.action.params && this.action.params.default_active_id) || "mail.box_inbox";
        ...
        _updateControlPanel() {
            // Invite
            if (this.discuss.thread && this.discuss.thread.channel_type === "channel") {
                this.$buttons.find(".o_invite").removeClass("o_hidden");
            } else {
                this.$buttons.find(".o_invite").addClass("o_hidden");
            }
            // Mark All Read
            if (this.discuss.thread && this.discuss.thread === this.env.messaging.inbox) {
                this.$buttons
            ...
            // Unstar All
            if (this.discuss.thread && this.discuss.thread === this.env.messaging.starred) {
                this.$buttons
                    .find(".o_unstar_all")
                    .removeClass("o_hidden")
                    .prop("disabled", this.discuss.threadViewer.messages.length === 0);
            ...
            // Mobile: Add channel
            if (this.env.messaging.device.isMobile && this.discuss.activeMobileNavbarTabId === "channel") {
                this.$buttons.find(".o_mobile_new_channel").removeClass("o_hidden");
            } else {
                this.$buttons.find(".o_mobile_new_channel").addClass("o_hidden");
            }
            // Mobile: Add message
            if (this.env.messaging.device.isMobile && this.discuss.activeMobileNavbarTabId === "chat") {
            ...
            if (this.discuss.threadViewer.checkedMessages.length > 0 || this.discuss.threadViewer.uncheckedMessages.length > 0) {
            ...
            if (this.discuss.threadViewer.checkedMessages.length > 0 && this.discuss.threadViewer.checkedMessages.filter((message) => !message.isModeratedByCurrentPartner).length === 0) {
            ...
            if (this.env.messaging.device.isMobile || !this.discuss.thread) {
                ...
                const prefix = this.discuss.thread.channel_type === "channel" && this.discuss.thread.public !== "private" ? "#" : "";
        ...
        _onClickModerationAccept() {
            this.env.models["mail.message"].moderate(this.discuss.threadViewer.checkedMessages, "accept");
        },
        /**
         * @private
         */
         ...
         */
        _onClickSelectAll() {
            this.env.models["mail.message"].checkAll(this.discuss.thread, this.discuss.threadStringifiedDomain);
        },
        /**
         * @private
         */
        _onClickUnselectAll() {
            this.env.models["mail.message"].uncheckAll(this.discuss.thread, this.discuss.threadStringifiedDomain);
        },
        /**
         * @private
         */
