/** @odoo-module alias=mail.widgets.FormRenderer **/

import ChatterContainer from 'mail.components.ChatterContainer';
...
FormRenderer.include({
    ...
    _makeChatterContainerComponent() {
        ...
        if (this.chatterFields.hasRecordReloadOnAttachmentsChanged) {
            this.on(
                'o_attachments_changed',
                this,
                ev => this.trigger_up('reload', { keepChanges: true })
            );
        }
        if (this.chatterFields.hasRecordReloadOnFollowersUpdate) {
            owl.Component.env.bus.on(
    ...
    _makeChatterContainerTarget() {
        const $el = $('<div class="o-FormRenderer-chatterContainer"/>');
