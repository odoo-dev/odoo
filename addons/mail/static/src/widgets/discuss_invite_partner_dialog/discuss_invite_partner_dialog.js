odoo.define("mail/static/src/widgets/discuss_invite_partner_dialog/discuss_invite_partner_dialog.js", function (require) {
    ...
    const PartnerInviteDialog = Dialog.extend({
            ...
            this.$input.select2({
                ...
                formatResult: (item) => {
                    ...
                    if (item.id === "odoobot") {
                        ...
                        const partner = this.env.models["mail.partner"].find((partner) => partner.id === item.id);
        ...
        async _invite() {
            ...
            const notification = _.str.sprintf(this.env._t("You added <b>%s</b> to the conversation."), names);
