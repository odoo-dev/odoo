/** @odoo-module alias=mail.viewAddons.Basic **/
...
BasicView.include({
    init() {
        ...
        const post_refresh = this._getFieldOption(
            'message_ids',
            'post_refresh',
            false,
        );
        ...
        for (const fieldName in fieldsInfo) {
            ...
            if (_.contains(['kanban_activity'], fieldInfo.widget)) {
    ...
    _getFieldOption(fieldName, optionName, defaultValue) {
        ...
        if (field?.options?.[optionName] !== undefined) {
