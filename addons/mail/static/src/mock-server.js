/** @odoo-module alias=mail.tests.MockServer **/
...
MockServer.include({
    ...
    async _performRpc(route, args) {
        ...
        if (args.model === 'mail.activity' && args.method === 'activity_format') {
            ...
            res = res.map(function (record) {
                if (record.mail_template_ids) {
                    record.mail_template_ids = record.mail_template_ids.map(
                        template_id => {
                            return {
                                id: template_id,
        ...
        if (args.model === 'mail.channel' && args.method === 'channel_get') {
            ...
            const pin = args.args[1] ?? args.kwargs.pin;
    ...
    _mockRouteMailInitMessaging() {
        ...
        const channelInfos = this._mockMailChannelChannelInfo(
        ...
        const directMessageInfos = this._mockMailChannelChannelInfo(
            directMessages.map(channel => channel.id),
        );
        ...
        const privateGroupInfos = this._mockMailChannelChannelInfo(
            privateGroups.map(channel => channel.id),
        );

        const moderation_channel_ids = this._getRecords(
            'mail.channel',
            [['is_moderator', '=', true]],
    ...
    async _mockRouteMailReadFollowers(args) {
        ...
        const followers = this._getRecords(
            'mail.followers',
    ...
    _mockMailActivityGetActivityData(res_model, domain) {
        ...
        return {
            ...
            activity_types: activityTypes.map(
                type => {
                    let mailTemplates = [];
                    if (type.mail_template_ids) {
                        mailTemplates = type.mail_template_ids.map(
                            id => {
    ...
    _mockMailChannelChannelFetched(ids) {
        ...
        for (const channel of channels) {
            const channelMessages = this._getRecords(
                'mail.message',
    ...
    _mockMailChannelChannelInfo(ids, extra_info) {
        ...
        const all_partners = [
            ...new Set(
                channels.reduce(
                    (all_partners, channel) => [...all_partners, ...channel.members],
                    [],
                ),
            ),
        ];
        ...
        return channels.map(channel => {
            ...
            const lastMessageId = messages.reduce(
    ...
    _mockMailMessageMarkAllAsRead(domain) {
        ...
        const notificationData = {
            ...
            needaction_inbox_counter: this._mockResPartnerGetNeedactionCount(),
            type: 'mark_as_read',
        };
    ...
    _mockMailMessageMessageFormat(ids) {
        ...
        return messages.map(
            message => {
                const thread = message.model && this._getRecords(
                    message.model,
                    [['id', '=', message.res_id]],
                )[0];
                let formattedAuthor;
                ...
                const allNotifications = this._getRecords(
                    'mail.notification',
                    [['mail_message_id', '=', message.id]],
                );
                ...
                const trackingValueIds = this._getRecords(
                    'mail.tracking.value',
                ...
                if (message.subtype_id) {
                    const subtype = this._getRecords(
    ...
    _mockMailMessageModerate(args) {
        ...
        if (decision === 'reject' || decision === 'discard') {
            model.records = _.reject(
                model.records,
                rec => _.contains(messageIDs, rec.id),
            );
            ...
            for (const message of messages) {
                this._mockWrite(
                    'mail.message',
                    [[message.id], { moderation_status: 'accepted' }],
                );
    ...
    _mockMailMessage_MessageNotificationFormat(ids) {
        ...
        return messages.map(
            message => {
                let notifications = this._getRecords(
                    'mail.notification',
                    [['mail_message_id', '=', message.id]],
                );
