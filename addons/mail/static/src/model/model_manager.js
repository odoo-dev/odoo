odoo.define("mail/static/src/model/model_manager.js", function (require) {
    ...
    class ModelManager {
        ...
        all(Model, filterFunc) {
            const allRecords = Object.values(this._records).filter((e) => e instanceof Model);
            if (filterFunc) {
        ...
        delete(record) {
            ...
            const data = {};
            const recordRelations = Object.values(Model.fields).filter((field) => field.fieldType === "relation");
        ...
        get(Model, recordOrLocalId) {
            ...
            const record = this._records[recordOrLocalId instanceof this.env.models["mail.model"] ? recordOrLocalId.localId : recordOrLocalId];
        ...
        update(record, data) {
            if (!this._isInUpdateCycle) {
                ...
                while (this._toComputeFields.size > 0 || this._toUpdateAfters.length > 0) {
            ...
            for (const Model of Object.values(Models)) {
                for (const fieldName in Model.fields) {
                    ...
                    if (field.fieldType === "attribute") {
                        const invalidKeys = Object.keys(field).filter((key) => !["autocreate", "compute", "default", "dependencies", "fieldType", "related"].includes(key));
                        if (invalidKeys.length > 0) {
                            throw new Error(`Field "${Model.modelName}/${fieldName}" contains some invalid keys: "${invalidKeys.join(", ")}".`);
                        }
                    }
                    if (field.fieldType === "relation") {
                        const invalidKeys = Object.keys(field).filter((key) => !["autocreate", "compute", "dependencies", "fieldType", "inverse", "isCausal", "related", "relationType", "to"].includes(key));
                        if (invalidKeys.length > 0) {
                            throw new Error(`Field "${Model.modelName}/${fieldName}" contains some invalid keys: "${invalidKeys.join(", ")}".`);
                        }
                        if (!Models[field.to]) {
                    ...
                    if (field.dependencies && !field.compute && !parentedMatchingFields.some((field) => field.compute)) {
                    ...
                    if ((field.compute || parentedMatchingFields.some((field) => field.compute)) && (field.dependencies || parentedMatchingFields.some((field) => field.dependencies))) {
            ...
            for (const Model of Object.values(Models)) {
                for (const fieldName in Model.fields) {
                    ...
                    if (!field.relationType) {
                        throw new Error(`Field "${Model.modelName}/${fieldName}" must define a relation type in "relationType".`);
                    }
                    if (!["one2one", "one2many", "many2one", "many2many"].includes(field.relationType)) {
                        throw new Error(`Field "${Model.modelName}/${fieldName}" has invalid relation type "${field.relationType}".`);
                    }
                    if (!field.inverse) {
                        throw new Error(`Field "${Model.modelName}/${fieldName}" must define an inverse relation name in "inverse".`);
                    }
                    if (!field.to) {
                        throw new Error(`Relation "${Model.modelNames}/${fieldName}" must define a model name in "to" (1st positional parameter of relation field helpers).`);
                    }
                    const RelatedModel = Models[field.to];
                    if (!RelatedModel) {
                        throw new Error(`Model name of relation "${Model.modelName}/${fieldName}" does not exist.`);
                    }
                    const inverseField = RelatedModel.fields[field.inverse];
                    if (!inverseField) {
                        throw new Error(`Relation "${Model.modelName}/${fieldName}" has no inverse field "${RelatedModel.modelName}/${field.inverse}".`);
                    }
                    if (inverseField.inverse !== fieldName) {
                        throw new Error(`Inverse field name of relation "${Model.modelName}/${fieldName}" does not match with field name of relation "${RelatedModel.modelName}/${inverseField.inverse}".`);
                    }
                    ...
                    if (!allSelfAndParentNames.includes(inverseField.to)) {
                        throw new Error(`Relation "${Model.modelName}/${fieldName}" has inverse relation "${RelatedModel.modelName}/${field.inverse}" misconfigured (currently "${inverseField.to}", should instead refer to this model or parented models: ${allSelfAndParentNames.map((name) => `"${name}"`).join(", ")}?)`);
                    }
                    if ((field.relationType === "many2many" && inverseField.relationType !== "many2many") || (field.relationType === "one2one" && inverseField.relationType !== "one2one") || (field.relationType === "one2many" && inverseField.relationType !== "many2one") || (field.relationType === "many2one" && inverseField.relationType !== "one2many")) {
                        throw new Error(`Mismatch relations types "${Model.modelName}/${fieldName}" (${field.relationType}) and "${RelatedModel.modelName}/${field.inverse}" (${inverseField.relationType}).`);
                    }
                }
            }
        }
        ...
        _makeInverseRelationField(Model, field) {
            const relFunc = field.relationType === "many2many" ? ModelField.many2many : field.relationType === "many2one" ? ModelField.one2many : field.relationType === "one2many" ? ModelField.many2one : field.relationType === "one2one" ? ModelField.one2one : undefined;
            if (!relFunc) {
                throw new Error(`Cannot compute inverse Relation of "${Model.modelName}/${field.fieldName}".`);
            }
            const inverseField = new ModelField(
                Object.assign({}, relFunc(Model.modelName, { inverse: field.fieldName }), {
            ...
            for (const Model of Object.values(Models)) {
                for (const field of Object.values(Model.fields)) {
                    for (const dependencyFieldName of field.dependencies) {
                        ...
                        dependencyField.dependents = [...new Set(dependencyField.dependents.concat([dependent]))];
                    }
                    if (field.related) {
                        ...
                        relationField.dependents = [...new Set(relationField.dependents.concat([relationFieldDependent]))];
                        ...
                        const relatedFieldDependent = [field.id, relationField.inverse, field.fieldName].join(DEPENDENT_INNER_SEPARATOR);
                        relatedField.dependents = [...new Set(relatedField.dependents.concat([relatedFieldDependent]))];
                    }
                }
            }
            /**
