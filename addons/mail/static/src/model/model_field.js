odoo.define("mail/static/src/model/model_field.js", function (require) {
    ...
    class ModelField {
        ...
        constructor({ autocreate = false, compute, default: def, dependencies = [], dependents = [], fieldName, fieldType, hashes: extraHashes = [], inverse, isCausal = false, modelManager, related, relationType, to } = {}) {
        ...
        write(record, newVal, { registerDependents = true } = {}) {
            ...
            for (const dependent of this.dependents) {
                const [hash, currentFieldName, relatedFieldName] = dependent.split(this.modelManager.DEPENDENT_INNER_SEPARATOR);
                if (relatedFieldName) {
        ...
        _setRelationUnlinkMany2Many(record, newValue) {
            ...
            const otherLocalIds = newValue === null ? [...this.read(record)] : this._setRelationConvertX2ManyValue(newValue);
            const OtherModel = this.env.models[this.to];
            ...
            for (const otherLocalId of otherLocalIds) {
                ...
                this.write(
                    record,
                    prevLocalIds.filter((localId) => localId !== otherLocalId)
                );
