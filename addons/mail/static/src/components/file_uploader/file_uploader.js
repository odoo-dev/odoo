odoo.define("mail/static/src/components/file_uploader/file_uploader.js", function (require) {
    ...
    class FileUploader extends Component {
        ...
        _createAttachment(fileData) {
            return this.env.models["mail.attachment"].create(Object.assign({}, fileData, this.props.newAttachmentExtraData));
        }
        ...
        async _unlinkExistingAttachments(files) {
            for (const file of files) {
                const attachment = this.props.attachmentLocalIds.map((attachmentLocalId) => this.env.models["mail.attachment"].get(attachmentLocalId)).find((attachment) => attachment.name === file.name && attachment.size === file.size);
        ...
        async _onAttachmentUploaded(ev, ...filesData) {
            for (const fileData of filesData) {
                ...
                if (error || !id) {
                    ...
                    const relatedTemporaryAttachments = this.env.models["mail.attachment"].find((attachment) => attachment.filename === filename && attachment.isTemporary);
