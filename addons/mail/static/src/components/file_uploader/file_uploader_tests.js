odoo.define("mail/static/src/components/file_uploader/file_uploader_tests.js", function (require) {
    ...
    const { afterEach: utilsAfterEach, beforeEach: utilsBeforeEach, inputFiles, nextAnimationFrame, start: utilsStart } = require("mail/static/src/utils/test_utils.js");
        ...
        QUnit.module("components", {}, function () {
            QUnit.module("file_uploader", {}, function () {
                QUnit.module("file_uploader_tests.js", {
                    beforeEach() {
                        ...
                        this.createFileUploaderComponent = async (props) => {
                            ...
                            const fileUploader = new FileUploaderComponent(null, Object.assign({ attachmentLocalIds: [] }, props));
                ...
                QUnit.test("no conflicts between file uploaders", async function (assert) {
                    ...
                    inputFiles(fileUploader1.el.querySelector(".o_FileUploader_input"), [file1]);
                    await nextAnimationFrame(); // we can't use afterNextRender as fileInput are display:none
                    assert.strictEqual(this.env.models["mail.attachment"].all().length, 1, "Uploaded file should be the only attachment created");
                    ...
                    inputFiles(fileUploader2.el.querySelector(".o_FileUploader_input"), [file2]);
                    await nextAnimationFrame();
                    assert.strictEqual(this.env.models["mail.attachment"].all().length, 2, "Uploaded file should be the only attachment added");
                });
            });
        });
    });
