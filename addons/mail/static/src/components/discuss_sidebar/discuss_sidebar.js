odoo.define("mail/static/src/components/discuss_sidebar/discuss_sidebar.js", function (require) {
    ...
    class DiscussSidebar extends Component {
        ...
        constructor(...args) {
            super(...args);
            useStore((...args) => this._useStoreSelector(...args), { compareDepth: () => this._useStoreCompareDepth() });
        ...
        get quickSearchPinnedAndOrderedChats() {
            const allOrderedAndPinnedChats = this.env.models["mail.thread"].all((thread) => thread.channel_type === "chat" && thread.isPinned && thread.model === "mail.channel").sort((c1, c2) => (c1.displayName < c2.displayName ? -1 : 1));
        ...
        get quickSearchOrderedAndPinnedMultiUserChannels() {
            const allOrderedAndPinnedMultiUserChannels = this.env.models["mail.thread"].all((thread) => thread.channel_type === "channel" && thread.isPinned && thread.model === "mail.channel").sort((c1, c2) => (c1.displayName < c2.displayName ? -1 : 1));
            if (!this.discuss.sidebarQuickSearchValue) {
        ...
        _useStoreSelector(props) {
            return {
                allOrderedAndPinnedChats: this.env.models["mail.thread"]
                    .all((thread) => thread.channel_type === "chat" && thread.isPinned && thread.model === "mail.channel")
                ...
                allOrderedAndPinnedMultiUserChannels: this.env.models["mail.thread"]
                    .all((thread) => thread.channel_type === "channel" && thread.isPinned && thread.model === "mail.channel")
                    .sort((c1, c2) => (c1.displayName < c2.displayName ? -1 : 1))
                    .map((channel) => channel.__state),
                allPinnedChannelAmount: this.env.models["mail.thread"].all((thread) => thread.isPinned && thread.model === "mail.channel").length,
