    ...
    class MessageSeenIndicator extends Component {
        ...
        constructor(...args) {
            ...
            useStore((props) => {
                ...
                const messageSeenIndicator = this.env.models["mail.message_seen_indicator"].find((messageSeenIndicator) => messageSeenIndicator.message === message && messageSeenIndicator.thread === thread);
                return {
        ...
        get indicatorTitle() {
            ...
            if (this.messageSeenIndicator.hasSomeoneSeen) {
                const partnersThatHaveSeen = this.messageSeenIndicator.partnersThatHaveSeen.map((partner) => partner.name);
                if (partnersThatHaveSeen.length === 1) {
                    return _.str.sprintf(this.env._t("Seen by %s"), partnersThatHaveSeen[0]);
                }
                if (partnersThatHaveSeen.length === 2) {
                    return _.str.sprintf(this.env._t("Seen by %s and %s"), partnersThatHaveSeen[0], partnersThatHaveSeen[1]);
                }
            ...
            if (this.messageSeenIndicator.hasSomeoneFetched) {
                const partnersThatHaveFetched = this.messageSeenIndicator.partnersThatHaveFetched.map((partner) => partner.name);
                if (partnersThatHaveFetched.length === 1) {
                    return _.str.sprintf(this.env._t("Received by %s"), partnersThatHaveFetched[0]);
                }
                if (partnersThatHaveFetched.length === 2) {
                    return _.str.sprintf(this.env._t("Received by %s and %s"), partnersThatHaveFetched[0], partnersThatHaveFetched[1]);
                }
        ...
        get messageSeenIndicator() {
            return this.env.models["mail.message_seen_indicator"].find((messageSeenIndicator) => messageSeenIndicator.message === this.message && messageSeenIndicator.thread === this.thread);
        }
