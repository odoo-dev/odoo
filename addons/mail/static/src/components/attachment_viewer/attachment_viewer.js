    ...
    class AttachmentViewer extends Component {
        ...
        constructor(...args) {
            ...
            useStore((props) => {
                ...
                return {
                    attachment: attachmentViewer && attachmentViewer.attachment ? attachmentViewer.attachment.__state : undefined,
                    attachments: attachmentViewer ? attachmentViewer.attachments.map((attachment) => attachment.__state) : [],
         ...
         */
        _handleImageLoad() {
            if (this.attachmentViewer.attachment.fileType === "image" && this._renderedAttachment !== this.attachmentViewer.attachment) {
        ...
        _next() {
            const attachmentViewer = this.attachmentViewer;
            const index = attachmentViewer.attachments.findIndex((attachment) => attachment === attachmentViewer.attachment);
        ...
        _previous() {
            const attachmentViewer = this.attachmentViewer;
            const index = attachmentViewer.attachments.findIndex((attachment) => attachment === attachmentViewer.attachment);
            const nextIndex = index === 0 ? attachmentViewer.attachments.length - 1 : index - 1;
            attachmentViewer.update({
                attachment: [["link", attachmentViewer.attachments[nextIndex]]],
            });
        }
        ...
        _updateZoomerStyle() {
            const attachmentViewer = this.attachmentViewer;
            const tx = this._imageRef.el.offsetWidth * attachmentViewer.scale > this._zoomerRef.el.offsetWidth ? this._translate.x + this._translate.dx : 0;
            const ty = this._imageRef.el.offsetHeight * attachmentViewer.scale > this._zoomerRef.el.offsetHeight ? this._translate.y + this._translate.dy : 0;
            if (tx === 0) {
                this._translate.x = 0;
            }
            if (ty === 0) {
        ...
        _zoomOut({ scroll = false } = {}) {
            ...
            const unflooredAdaptedScale = this.attachmentViewer.scale - (scroll ? SCROLL_ZOOM_STEP : ZOOM_STEP);
            this.attachmentViewer.update({
