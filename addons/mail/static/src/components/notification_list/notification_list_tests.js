        ...
        QUnit.module("components", {}, function () {
            QUnit.module("notification_list", {}, function () {
                ...
                QUnit.test("marked as read thread notifications are ordered by last message date", async function (assert) {
                    ...
                    assert.containsN(document.body, ".o_ThreadPreview", 2, "there should be two thread previews");
                    const threadPreviewElList = document.querySelectorAll(".o_ThreadPreview");
                    assert.strictEqual(threadPreviewElList[0].querySelector(":scope .o_ThreadPreview_name").textContent, "Channel 2020", "First channel in the list should be the channel of 2020 (more recent last message)");
                    assert.strictEqual(threadPreviewElList[1].querySelector(":scope .o_ThreadPreview_name").textContent, "Channel 2019", "Second channel in the list should be the channel of 2019 (least recent last message)");
                });
                ...
                QUnit.test("thread notifications are re-ordered on receiving a new message", async function (assert) {
                    ...
                    await afterNextRender(() => {
                        ...
                        this.widget.call("bus_service", "trigger", "notification", [[["my-db", "mail.channel", 100], messageData]]);
                    });
                    assert.containsN(document.body, ".o_ThreadPreview", 2, "there should still be two thread previews");
                    const threadPreviewElList = document.querySelectorAll(".o_ThreadPreview");
                    assert.strictEqual(threadPreviewElList[0].querySelector(":scope .o_ThreadPreview_name").textContent, "Channel 2019", "First channel in the list should now be 'Channel 2019'");
                    assert.strictEqual(threadPreviewElList[1].querySelector(":scope .o_ThreadPreview_name").textContent, "Channel 2020", "Second channel in the list should now be 'Channel 2020'");
                });
            });
        });
    });
