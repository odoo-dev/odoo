odoo.define("mail/static/src/components/notification_list/notification_list.js", function (require) {
    ...
    class NotificationList extends Component {
         ...
         */
        async _loadPreviews() {
            const threads = this.notifications.filter((notification) => notification.thread).map((notification) => this.env.models["mail.thread"].get(notification.thread));
        ...
        _useStoreSelector(props) {
            ...
            if (props.filter === "all") {
                ...
                notifications = Object.values(notificationGroups)
                    .sort((group1, group2) => (group1.date.isAfter(group2.date) ? -1 : 1))
        ...
        _useStoreSelectorThreads(props) {
            if (props.filter === "mailbox") {
                ...
                return this.env.models["mail.thread"].all((thread) => thread.channel_type === "channel" && thread.isPinned && thread.model === "mail.channel").sort((c1, c2) => (c1.displayName < c2.displayName ? -1 : 1));
            } else if (props.filter === "chat") {
                return this.env.models["mail.thread"].all((thread) => thread.channel_type === "chat" && thread.isPinned && thread.model === "mail.channel").sort((c1, c2) => (c1.displayName < c2.displayName ? -1 : 1));
            } else if (props.filter === "all") {
                // "All" filter is for channels and chats
                return this.env.models["mail.thread"].all((thread) => thread.isPinned && thread.model === "mail.channel").sort((c1, c2) => (c1.displayName < c2.displayName ? -1 : 1));
            } else {
