/** @odoo-module alias=mail.components.AttachmentViewer **/
...
class AttachmentViewer extends usingModels(Component) {
    ...
    constructor(...args) {
        ...
        /**
         * Used to ensure that the ref is always up to date, which seems to be
         * needed if the element has a t-key, which was added to force the
         * rendering of a new element when the src of the image changes.
         * This was made to remove the display of the previous image as soon as
         * the src changes.
         */
     ...
     */
    get imageStyle() {
        const attachmentViewer = this.record;
        let style = `transform: ` +
            `scale3d(${
                attachmentViewer.scale(this)
            }, ${
                attachmentViewer.scale(this)
            }, 1) ` +
            `rotate(${attachmentViewer.angle(this)}deg);`;

        if (attachmentViewer.angle(this) % 180 !== 0) {
            style += `` +
     ...
     */
    _download() {
        const id = this.record.attachment(this).id(this);
    ...
    _handleImageLoad() {
        if (
            !this.record ||
            !this.record.attachment(this)
        ) {
            return;
        }
        const refs = this._getRefs();
        const image = refs[`image_${this.record.attachment(this).id(this)}`];
        if (
            this.record.attachment(this).fileType(this) === 'image' &&
            !image.complete
        ) {
            this.env.services.action.dispatch(
                'Record/update',
    ...
    _next() {
        const attachmentViewer = this.record;
        const index = attachmentViewer.attachments(this).findIndex(
            attachment => attachment === attachmentViewer.attachment(this),
        );
        const nextIndex = (index + 1) % attachmentViewer.attachments(this).length;
        this.env.services.action.dispatch(
            'Record/update',
            attachmentViewer,
            {
                attachment: this.env.services.action.dispatch(
                    'RecordFieldCommand/link',
                    attachmentViewer.attachments(this)[nextIndex],
                ),
            },
        );
    }
    ...
    _previous() {
        ...
        const nextIndex = index === 0
            ? attachmentViewer.attachments(this).length - 1
            : index - 1;
        this.env.services.action.dispatch(
            'Record/update',
            attachmentViewer,
        ...
        printWindow.document.write(`
            <html>
                ...
                <body onload='onloadImage()'>
                    <img src="${
                        this.record.attachment(this).defaultSource(this)
                    }" alt=""/>
                </body>
            </html>`);
    ...
    _rotate() {
        this.env.services.action.dispatch(
            'Record/update',
            this.record,
            { angle: this.record.angle(this) + 90 },
        );
    }
    ...
    _updateZoomerStyle() {
        const attachmentViewer = this.record;
        const refs = this._getRefs();
        const image = refs[`image_${attachmentViewer.attachment(this).id(this)}`];
        const tx = (
            (
                image.offsetWidth * attachmentViewer.scale(this) >
                this._zoomerRef.el.offsetWidth
            )
            ? this._translate.x + this._translate.dx
            : 0
        );
        const ty = (
            (
                image.offsetHeight * attachmentViewer.scale(this) >
    ...
    _zoomOut(
        ...
        if (this.record.scale(this) === MIN_SCALE) {
            return;
        }
...
Object.assign(AttachmentViewer, {
    props: {
        record: {
            type: Object,
            validate(p) {
                if (p.constructor.modelName !== 'AttachmentViewer') {
                    return false;
                }
                return true;
            },
