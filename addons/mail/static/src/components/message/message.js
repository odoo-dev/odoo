odoo.define("mail/static/src/components/message/message.js", function (require) {
    ...
    class Message extends Component {
        ...
        constructor(...args) {
            ...
            useStore(
                (props) => {
                    ...
                    const threadStringifiedDomain = threadViewer ? threadViewer.stringifiedDomain : undefined;
                    return {
                        attachments: message ? message.attachments.map((attachment) => attachment.__state) : undefined,
                        author: author ? author.__state : undefined,
                        hasMessageCheckbox: message ? message.hasCheckbox : false,
                        isDeviceMobile: this.env.messaging.device.isMobile,
                        isMessageChecked: message && threadViewer ? message.isChecked(thread, threadStringifiedDomain) : false,
        ...
        isBottomVisible({ offset = 0 } = {}) {
            ...
            return elRect.bottom < parentRect.bottom + offset && parentRect.top < elRect.bottom + offset;
        }
        ...
        get trackingValues() {
            return this.message.tracking_value_ids.map((trackingValue) => {
                ...
                if (value.field_type === "datetime") {
                    if (value.old_value) {
                        value.old_value = moment.utc(value.old_value).local().format("LLL");
                    }
                    if (value.new_value) {
                        value.new_value = moment.utc(value.new_value).local().format("LLL");
                    }
                } else if (value.field_type === "date") {
                    if (value.old_value) {
                        value.old_value = moment(value.old_value).local().format("LL");
                    }
                    if (value.new_value) {
                        value.new_value = moment(value.new_value).local().format("LL");
                    }
                }
                return value;
            });
        ...
        _insertReadMoreLess($element) {
            ...
            const $children = $element.contents().filter((index, content) => content.nodeType === 1 || (content.nodeType === 3 && content.nodeValue.trim()));
