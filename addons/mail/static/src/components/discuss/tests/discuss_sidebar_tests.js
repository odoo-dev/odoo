...
QUnit.test('sidebar channels should ordered alphabetically', async function (assert) {
    assert.expect(2);

    this.data['mail.channel'].records.push({ id: 19, name: "Xyz" }, { id: 20, name: "abc" },
                                        { id: 21, name: "Abc" }, { id: 22, name: "Xyz" });
    await this.start();
    ...
    const threadResults = document.querySelectorAll('.o_DiscussSidebar_groupChannel .o_DiscussSidebar_item');
    assert.deepEqual(
        [threadResults[2].getAttribute('data-thread-local-id'), threadResults[3].getAttribute('data-thread-local-id')],
        ["mail.thread_mail.channel_19", "mail.thread_mail.channel_22"],
        "Channels having the same name should be ordered based on the Id"
    );
});
