    ...
    function factory(dependencies) {
        class Thread extends dependencies["mail.model"] {
            ...
            init(...args) {
                 ...
                 */
                this._currentPartnerInactiveTypingTimer = new Timer(this.env, () => this.async(() => this._onCurrentPartnerInactiveTypingTimeout()), 5 * 1000);
                /**
                 ...
                 */
                this._currentPartnerLongTypingTimer = new Timer(this.env, () => this.async(() => this._onCurrentPartnerLongTypingTimeout()), 50 * 1000);
                /**
                 ...
                 */
                this._throttleNotifyCurrentPartnerTypingStatus = throttle(this.env, ({ isTyping }) => this.async(() => this._notifyCurrentPartnerTypingStatus({ isTyping })), 2.5 * 1000);
            }
            ...
            static convertData(data) {
                ...
                if ("direct_partner" in data) {
                    if (!data.direct_partner) {
                        ...
                        data2.correspondent = [["insert", this.env.models["mail.partner"].convertData(data.direct_partner[0])]];
                    }
                }
                if ("members" in data) {
                    if (!data.members) {
                        ...
                        data2.members = [["insert-and-replace", data.members.map((memberData) => this.env.models["mail.partner"].convertData(memberData))]];
                    }
                }
                if ("seen_partners_info" in data) {
                    if (!data.seen_partners_info) {
                        ...
                        data2.partnerSeenInfos = [
                            [
                                "insert-and-replace",
                                data.seen_partners_info.map(({ fetched_message_id, id, partner_id, seen_message_id }) => {
                                    return {
                                        id,
             ...
             */
            static async createChannel({ autoselect = false, autoselectChatWindowMode, name, partnerId, public: publicStatus, type }) {
                ...
                const thread = this.insert(Object.assign({ isPinned: true }, this.convertData(data)));
                if (autoselect) {
            ...
            static async joinChannel(channelId, { autoselect = false } = {}) {
                const channel = this.find((thread) => thread.id === channelId && thread.model === "mail.channel");
            ...
            static async loadPreviews(threads) {
                ...
                for (const preview of messagePreviews) {
                    ...
                    if (messageData) {
                        this.env.models["mail.message"].insert(this.env.models["mail.message"].convertData(messageData));
                    }
                }
            }
            ...
            async fetchAttachments() {
                const attachmentsData = await this.async(() =>
                ...
                for (const attachmentData of attachmentsData) {
                    this.env.models["mail.attachment"].insert(
                        Object.assign(
                            {
            ...
            async markAsSeen() {
                ...
                if (this.model === "mail.channel") {
                    const seen_message_id = await this.async(() =>
                        this.env.services.rpc(
                            {
                                model: "mail.channel",
                                method: "channel_seen",
            ...
            async markNeedactionMessagesAsRead() {
                await this.async(() => this.env.models["mail.message"].markAsRead(this.needactionMessages));
            }
            ...
            open({ chatWindowMode = "last_visible", resetDiscussDomain = false } = {}) {
                ...
                if ((!device.isMobile && discuss.isOpen) || (device.isMobile && this.model === "mail.box")) {
            ...
            async refreshFollowers() {
                ...
                const [{ message_follower_ids: followerIds }] = await this.async(() =>
                    this.env.services.rpc({
                        model: this.model,
                        method: "read",
                ...
                if (followerIds && followerIds.length > 0) {
                    const { followers } = await this.async(() =>
                        this.env.services.rpc({
                    ...
                    this.update({
                        followers: [["insert-and-replace", followers.map((data) => this.env.models["mail.follower"].convertData(data))]],
                    });
                } else {
                    this.update({
            ...
            async registerCurrentPartnerIsTyping() {
                ...
                const newOrderedTypingMemberLocalIds = this.orderedTypingMemberLocalIds.filter((localId) => localId !== currentPartner.localId);
            ...
            registerOtherMemberTypingMember(partner) {
                const timer = new Timer(this.env, () => this.async(() => this._onOtherMemberLongTypingTimeout(partner)), 60 * 1000);
                this._otherMembersLongTypingTimers.set(partner, timer);
                timer.start();
                const newOrderedTypingMemberLocalIds = this.orderedTypingMemberLocalIds.filter((localId) => localId !== partner.localId);
             ...
             */
            async unfollow() {
                const currentPartnerFollower = this.followers.find((follower) => follower.partner === this.env.messaging.currentPartner);
            ...
            _computeAllAttachments() {
                const allAttachments = [...new Set(this.originThreadAttachments.concat(this.attachments))].sort((a1, a2) => (a1.id < a2.id ? 1 : -1));
            ...
            _computeIsCurrentPartnerFollowing() {
                return this.followers.some((follower) => follower.partner && follower.partner === this.env.messaging.currentPartner);
            }
            ...
            _computeLastCurrentPartnerMessageSeenByEveryone() {
                ...
                const otherPartnerSeenInfos = this.partnerSeenInfos.filter((partnerSeenInfo) => partnerSeenInfo.partner !== this.messagingCurrentPartner);
                if (otherPartnerSeenInfos.length === 0) {
                    return [["unlink-all"]];
                }

                const otherPartnersLastSeenMessageIds = otherPartnerSeenInfos.map((partnerSeenInfo) => (partnerSeenInfo.lastSeenMessage ? partnerSeenInfo.lastSeenMessage.id : 0));
                if (otherPartnersLastSeenMessageIds.length === 0) {
                    return [["unlink-all"]];
                }
                const lastMessageSeenByAllId = Math.min(...otherPartnersLastSeenMessageIds);
                const currentPartnerOrderedSeenMessages = this.mainCacheOrderedMessages.filter((message) => message.author === this.messagingCurrentPartner && message.id <= lastMessageSeenByAllId);
            ...
            _computeLastNeedactionMessage() {
                const orderedNeedactionMessages = this.needactionMessages.sort((m1, m2) => (m1.id < m2.id ? -1 : 1));
                const { length: l, [l - 1]: lastNeedactionMessage } = orderedNeedactionMessages;
            ...
            _computeOrderedOtherTypingMembers() {
                return [["replace", this.orderedTypingMembers.filter((member) => member !== this.env.messaging.currentPartner)]];
            }
            ...
            _computeOrderedTypingMembers() {
                return [["replace", this.orderedTypingMemberLocalIds.map((localId) => this.env.models["mail.partner"].get(localId)).filter((member) => !!member)]];
            }
            ...
            _computeTypingStatusText() {
                ...
                if (this.orderedOtherTypingMembers.length === 1) {
                    return _.str.sprintf(this.env._t("%s is typing..."), this.orderedOtherTypingMembers[0].nameOrDisplayName);
                }
                if (this.orderedOtherTypingMembers.length === 2) {
                    return _.str.sprintf(this.env._t("%s and %s are typing..."), this.orderedOtherTypingMembers[0].nameOrDisplayName, this.orderedOtherTypingMembers[1].nameOrDisplayName);
                }
            ...
            async _notifyCurrentPartnerTypingStatus({ isTyping }) {
                if (this._forceNotifyNextCurrentPartnerTypingStatus || isTyping !== this._currentPartnerLastNotifiedIsTyping) {
                    if (this.model === "mail.channel") {
                        await this.async(() =>
                            this.env.services.rpc(
                                {
            ...
            _updateAfter(previous) {
                ...
                if (this.pendingFoldState && previous.pendingFoldState !== this.pendingFoldState) {
                    this.notifyFoldStateToServer();
                }
                if (this.serverFoldState === this.pendingFoldState) {
            ...
            async _onCurrentPartnerLongTypingTimeout() {
                ...
                this._throttleNotifyCurrentPartnerTypingStatus.clear();
                await this.async(() => this._throttleNotifyCurrentPartnerTypingStatus({ isTyping: true }));
            }
        ...
        Thread.fields = {
            allAttachments: many2many("mail.attachment", {
                ...
                dependencies: ["attachments", "originThreadAttachments"],
            }),
            ...
            displayName: attr({
                ...
                dependencies: ["channel_type", "correspondent", "correspondentNameOrDisplayName", "custom_channel_name", "name"],
            }),
            /**
            ...
            foldState: attr({
                ...
                dependencies: ["pendingFoldState", "serverFoldState"],
            }),
            ...
            isCurrentPartnerFollowing: attr({
                ...
                default: false,
                dependencies: ["followersPartner", "messagingCurrentPartner"],
            }),
            isModeratedByCurrentPartner: attr({
                compute: "_computeIsModeratedByCurrentPartner",
                dependencies: ["model", "messagingCurrentPartner"],
            }),
            isPinned: attr({
                default: false,
            }),
            ...
            lastCurrentPartnerMessageSeenByEveryone: many2one("mail.message", {
                ...
                dependencies: ["partnerSeenInfos", "mainCacheOrderedMessages", "messagingCurrentPartner"],
            }),
            ...
            messages: many2many("mail.message", {
                ...
                dependencies: ["mainCacheMessages", "originThreadMessages"],
            }),
            ...
            orderedTypingMembers: many2many("mail.partner", {
                ...
                dependencies: ["orderedTypingMemberLocalIds", "typingMembers"],
            }),
            /**
