    ...
    function factory(dependencies) {
        class Discuss extends dependencies["mail.model"] {
            ...
            async handleAddChannelAutocompleteSource(req, res) {
                ...
                items.push(
                    {
                        label: _.str.sprintf(`<strong>${this.env._t("Create %s")}</strong>`, `<em><span class="fa fa-hashtag"/>${escapedValue}</em>`),
                        escapedValue,
                        special: "public",
                    },
                    {
                        label: _.str.sprintf(`<strong>${this.env._t("Create %s")}</strong>`, `<em><span class="fa fa-lock"/>${escapedValue}</em>`),
                        escapedValue,
                        special: "private",
                    }
                );
            ...
            openInitThread() {
                ...
                const thread = this.env.models["mail.thread"].find((thread) => thread.id === (model !== "mail.box" ? Number(id) : id) && thread.model === model);
                if (!thread) {
                    return;
                }
        ...
        Discuss.fields = {
            activeId: attr({
                ...
                dependencies: ["thread", "threadId", "threadModel"],
            }),
            /**
            ...
            initActiveId: attr({
                ...
                default: "mail.box_inbox",
                dependencies: ["isOpen", "thread", "threadId", "threadModel"],
            }),
            /**
