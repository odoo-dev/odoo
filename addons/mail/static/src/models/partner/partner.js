    ...
    function factory(dependencies) {
        ...
        class Partner extends dependencies["mail.model"] {
            ...
            static async imSearch({ callback, keyword, limit = 10 }) {
                // prefetched partners
                let partners = [];
                const searchRegexp = new RegExp(_.str.escapeRegExp(utils.unaccent(keyword)), "i");
                const currentPartner = this.env.messaging.currentPartner;
                for (const partner of this.all((partner) => partner.active)) {
                    if (partners.length < limit) {
                        if (partner !== currentPartner && searchRegexp.test(partner.name)) {
                ...
                const dataList = await this.env.services.rpc(
                    {
                        ...
                        params: {
                            partner_ids: toFetchPartnersLocalIds.map((partnerLocalId) => this.get(partnerLocalId).id),
                        },
                    },
                    { shadow: true }
                );
        ...
        Partner.fields = {
            ...
            nameOrDisplayName: attr({
                ...
                dependencies: ["display_name", "name"],
            }),
