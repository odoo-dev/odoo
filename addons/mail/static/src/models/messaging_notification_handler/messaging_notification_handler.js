    ...
    function factory(dependencies) {
        class MessagingNotificationHandler extends dependencies["mail.model"] {
            ...
            _filterNotificationsOnUnsubscribe(notifications) {
                const unsubscribedNotif = notifications.find((notif) => notif[1].info === "unsubscribe");
                if (unsubscribedNotif) {
                    notifications = notifications.filter((notif) => notif[0][1] !== "mail.channel" || notif[0][2] !== unsubscribedNotif[1].id);
                }
            ...
            async _handleNotifications(notifications) {
                ...
                const proms = filteredNotifications.map((notification) => {
                    ...
                    switch (model) {
                        ...
                        case "mail.channel":
                            return this._handleNotificationChannel(
                                Object.assign(
                                    {
                                        channelId: id,
                                    },
            ...
            async _handleNotificationChannel(data) {
                const { channelId, id, info, is_typing, last_message_id, partner_id } = data;
                switch (info) {
             ...
             */
            async _handleNotificationChannelFetched({ id, channelId, last_message_id, partner_id }) {
                const channel = this.env.models["mail.thread"].find((thread) => thread.id === channelId && thread.model === "mail.channel");
                if (!channel) {
            ...
            async _handleNotificationChannelMessage(param0) {
                ...
                const oldMessage = this.env.models["mail.message"].find((message) => message.id === convertedData.id);
                // locally save old value, as insert would overwrite them
                const oldMessageModerationStatus = oldMessage && oldMessage.moderation_status;
                const oldMessageWasModeratedByCurrentPartner = oldMessage && oldMessage.isModeratedByCurrentPartner;
                ...
                if (oldMessage) {
                    if (oldMessageModerationStatus === "pending_moderation" && message.moderation_status !== "pending_moderation" && oldMessageWasModeratedByCurrentPartner) {
                ...
                // unread counter.
                if (channel && channel.mass_mailing && this.env.session.notification_type === "email") {
                    return;
                }
             ...
             * @param {integer} param0.partner_id
             */
            async _handleNotificationChannelSeen({ channelId, id, last_message_id, partner_id }) {
                const channel = this.env.models["mail.thread"].find((thread) => thread.id === channelId && thread.model === "mail.channel");
                if (!channel) {
                    // for example seen from another browser, the current one has no
                    // knowledge of the channel
            ...
            _handleNotificationNeedaction(data) {
                ...
                for (const thread of message.allThreads) {
                    if (thread.channel_type === "channel" && message.allThreads.includes(inboxMailbox)) {
            ...
            _handleNotificationPartnerAuthor(data) {
                this.env.models["mail.message"].insert(this.env.models["mail.message"].convertData(data.message));
            }
            ...
            _handleNotificationPartnerChannel(data) {
                ...
                let channel = this.env.models["mail.thread"].find((thread) => thread.id === convertedData.id && thread.model === "mail.channel");
                const wasCurrentPartnerMember = channel && channel.members.includes(this.env.messaging.currentPartner);

                channel = this.env.models["mail.thread"].insert(Object.assign({ isPinned: true }, convertedData));

                if (channel.channel_type === "channel" && data.info !== "creation" && !wasCurrentPartnerMember) {
                    this.env.services["notification"].notify({
                        message: _.str.sprintf(this.env._t("You have been invited to: %s"), owl.utils.escape(channel.name)),
            ...
            _handleNotificationPartnerDeletion({ message_ids }) {
                ...
                for (const id of message_ids) {
                    ...
                    if (message) {
                        if (message.moderation_status === "pending_moderation" && message.originThread.isModeratedByCurrentPartner) {
            ...
            _handleNotificationPartnerMarkAsRead({ channel_ids, message_ids = [] }) {
                ...
                for (const cache of inboxMailbox.caches) {
                    const messages = message_ids.map((id) => this.env.models["mail.message"].find((message) => message.id === id)).filter((message) => !!message);
                ...
                let channels;
                if (channel_ids) {
                    channels = channel_ids.map((id) => this.env.models["mail.thread"].find((thread) => thread.id === id && thread.model === "mail.channel")).filter((thread) => !!thread);
                } else {
                    // flux specific: channel_ids unset means "mark all as read"
                    channels = this.env.models["mail.thread"].all((thread) => thread.model === "mail.channel");
                }
            ...
            _handleNotificationPartnerModerator({ message: data }) {
                this.env.models["mail.message"].insert(this.env.models["mail.message"].convertData(data));
                const moderationMailbox = this.env.messaging.moderation;
            ...
            _handleNotificationPartnerToggleStar({ message_ids = [], starred }) {
                ...
                for (const messageId of message_ids) {
                    const message = this.env.models["mail.message"].find((message) => message.id === messageId);
                    if (!message) {
                        continue;
                    }
                    if (starred) {
            ...
            _handleNotificationPartnerUnsubscribe(channelId) {
                ...
                if (channel.correspondent) {
                    ...
                    message = _.str.sprintf(this.env._t("You unpinned your conversation with <b>%s</b>."), owl.utils.escape(correspondent.name));
                } else {
                    message = _.str.sprintf(this.env._t("You unsubscribed from <b>%s</b>."), owl.utils.escape(channel.name));
                }
                this.env.services["notification"].notify({
                    message,
            ...
            _notifyNewChannelMessageWhileOutOfFocus({ channel, message }) {
                ...
                if (!author) {
                    ...
                    if (channel.channel_type === "channel") {
                        ...
                        notificationTitle = _.str.sprintf(this.env._t("%s from %s"), owl.utils.escape(authorName), channelNameWithIcon);
                    } else {
                ...
                const titlePattern = messaging.outOfFocusUnreadMessageCounter === 1 ? this.env._t("%d Message") : this.env._t("%d Messages");
