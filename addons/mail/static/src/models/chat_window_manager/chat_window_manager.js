    ...
    function factory(dependencies) {
        ...
        class ChatWindowManager extends dependencies["mail.model"] {
            ...
            openThread(thread, { mode } = {}) {
                let chatWindow = this.env.models["mail.chat_window"].find((chatWindow) => chatWindow.thread === thread);
                if (!chatWindow) {
            ...
            _compute_ordered() {
                ...
                const _ordered = this._ordered.filter((chatWindowLocalId) => this.chatWindows.includes(this.env.models["mail.chat_window"].get(chatWindowLocalId)));
             ...
             */
            _computeAllOrdered() {
                return [["replace", this._ordered.map((chatWindowLocalId) => this.env.models["mail.chat_window"].get(chatWindowLocalId))]];
            }
            ...
            _computeAllOrderedHidden() {
                return [["replace", this.visual.hidden.chatWindowLocalIds.map((chatWindowLocalId) => this.env.models["mail.chat_window"].get(chatWindowLocalId))]];
            }
            ...
            _computeAllOrderedVisible() {
                return [["replace", this.visual.visible.map(({ chatWindowLocalId }) => this.env.models["mail.chat_window"].get(chatWindowLocalId))]];
            }
            ...
            _computeUnreadHiddenConversationAmount() {
                const allHiddenWithThread = this.allOrderedHidden.filter((chatWindow) => chatWindow.thread);
                let amount = 0;
            ...
            _computeVisual() {
                ...
                let maxAmountWithoutHidden = Math.floor(relativeGlobalWindowWidth / (CHAT_WINDOW_WIDTH + BETWEEN_GAP_WIDTH));
                let maxAmountWithHidden = Math.floor((relativeGlobalWindowWidth - HIDDEN_MENU_WIDTH - BETWEEN_GAP_WIDTH) / (CHAT_WINDOW_WIDTH + BETWEEN_GAP_WIDTH));
                if (device.isMobile) {
                ...
                if (chatWindows.length <= maxAmountWithoutHidden) {
                    ...
                    if (chatWindows.length > maxAmountWithHidden) {
                        ...
                        visual.hidden.offset = visual.visible[maxAmountWithHidden - 1].offset + CHAT_WINDOW_WIDTH + BETWEEN_GAP_WIDTH;
                    }
        ...
        ChatWindowManager.fields = {
            ...
            visual: attr({
                ...
                default: BASE_VISUAL,
                dependencies: ["allOrdered", "deviceGlobalWindowInnerWidth", "deviceIsMobile", "discussIsOpen"],
            }),
        };
