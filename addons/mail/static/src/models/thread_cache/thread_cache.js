    ...
    function factory(dependencies) {
        class ThreadCache extends dependencies["mail.model"] {
            ...
            async loadMoreMessages() {
                ...
                const minMessageId = Math.min(...this.messages.map((message) => message.id));
                domain = [["id", "<", minMessageId]].concat(domain);
                const messagesData = await this.async(() =>
                    this.env.services.rpc(
                        {
                            model: "mail.message",
                            method: "message_fetch",
            ...
            _computeUncheckedMessages() {
                return [["replace", this.messages.filter((message) => message.hasCheckbox && !this.checkedMessages.includes(message))]];
            }
            ...
            _handleMessagesLoaded(messagesData) {
                this.update({
                    ...
                    isLoading: false,
                    isLoadingMore: false,
                    messages: [["insert", messagesData.map((data) => this.env.models["mail.message"].convertData(data))]],
                });
                if (!this.thread) {
                    return;
                }
        ...
        ThreadCache.fields = {
            ...
            uncheckedMessages: many2many("mail.message", {
                ...
                dependencies: ["checkedMessages", "messagesCheckboxes", "messages"],
            }),
        };
