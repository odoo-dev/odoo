    ...
    function factory(dependencies) {
        class Message extends dependencies["mail.model"] {
            ...
            static convertData(data) {
                ...
                if ("attachment_ids" in data) {
                    if (!data.attachment_ids) {
                        ...
                        data2.attachments = [["insert-and-replace", data.attachment_ids.map((attachmentData) => this.env.models["mail.attachment"].convertData(attachmentData))]];
                    }
                }
                ...
                if ("notifications" in data) {
                    data2.notifications = [["insert", data.notifications.map((notificationData) => this.env.models["mail.notification"].convertData(notificationData))]];
                }
            ...
            isChecked(thread, threadStringifiedDomain) {
                // aku todo
                const relatedCheckedThreadCache = this.checkedThreadCaches.find((threadCache) => threadCache.thread === thread && threadCache.stringifiedDomain === threadStringifiedDomain);
            ...
            _computeFailureNotifications() {
                return [["replace", this.notifications.filter((notifications) => ["exception", "bounce"].includes(notifications.notification_status))]];
            }
            ...
            _computeIsCurrentPartnerAuthor() {
                return this.author && this.messagingCurrentPartner && this.messagingCurrentPartner === this.author;
            }
            ...
            _computeIsModeratedByCurrentPartner() {
                return this.moderation_status === "pending_moderation" && this.originThread && this.originThread.isModeratedByCurrentPartner;
            }
            ...
            _computePrettyBody() {
                ...
                for (const emoji of emojis) {
                    const { unicode } = emoji;
                    const regexp = new RegExp(`(?:^|\\s|<[a-z]*>)(${unicode})(?=\\s|$|</[a-z]*>)`, "g");
                    const originalBody = this.body;
                    prettyBody = this.body.replace(regexp, ` <span class="o_mail_emoji">${unicode}</span> `);
        ...
        Message.fields = {
            allThreads: many2many("mail.thread", {
                ...
                dependencies: ["originThread", "threadCachesThread"],
            }),
            ...
            isCurrentPartnerAuthor: attr({
                ...
                default: false,
                dependencies: ["author", "messagingCurrentPartner"],
            }),
            isModeratedByCurrentPartner: attr({
                compute: "_computeIsModeratedByCurrentPartner",
                default: false,
                dependencies: ["moderation_status", "originThread", "originThreadIsModeratedByCurrentPartner"],
            }),
            isNeedaction: attr({
                compute: "_computeIsNeedaction",
                dependencies: ["allThreads", "messagingInbox"],
                default: false,
            }),
            isTemporary: attr({
                default: false,
