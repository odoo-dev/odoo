            ...
            QUnit.module("message", {}, function () {
                ...
                QUnit.test("create", async function (assert) {
                    ...
                    const message = this.env.models["mail.message"].create({
                        ...
                        threadCaches: [["link", [thread.mainCache, this.env.messaging.inbox.mainCache, this.env.messaging.starred.mainCache]]],
                    });
                    ...
                    assert.ok(message);
                    assert.strictEqual(
                        this.env.models["mail.message"].find((message) => message.id === 4000),
                        message
                    );
                    assert.strictEqual(message.body, "<p>Test</p>");
                    assert.ok(message.date instanceof moment);
                    assert.strictEqual(moment(message.date).utc().format("YYYY-MM-DD hh:mm:ss"), "2019-05-05 10:00:00");
                    ...
                    assert.ok(message.allThreads.includes(this.env.models["mail.thread"].find((thread) => thread.id === 100 && thread.model === "mail.channel")));
                    ...
                    const channel = this.env.models["mail.thread"].find((thread) => thread.id === 100 && thread.model === "mail.channel");
                    assert.ok(channel);
