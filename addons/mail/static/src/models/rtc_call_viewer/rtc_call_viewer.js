...
function factory(dependencies) {
    ...
    class RtcCallViewer extends dependencies['mail.model'] {
        ...
        _computeIsMinimized() {
            ...
            if (this.env.messaging.userSetting.isRtcCallViewerFullScreen || this.threadView.compact) {
                return false;
            }
        ...
        _computeLayout() {
            ...
            if (!this.threadView.thread.videoCount || !this.env.messaging.focusedRtcSession) {
                return 'tiled';
            }
            if (this.threadView.thread.rtcSessions.length < 2) {
                return 'tiled';
            }
            if (this.threadView.compact && this.env.messaging.userSetting.rtcLayout === 'sidebar') {
                return 'spotlight';
            }
