odoo.define("mail/static/src/models/messaging_initializer/messaging_initializer.js", function (require) {
    ...
    function factory(dependencies) {
        class MessagingInitializer extends dependencies["mail.model"] {
             ...
             */
            _init({ channel_slots, commands = [], is_moderator = false, mail_failures = {}, mention_partner_suggestions = [], menu_id, moderation_channel_ids = [], moderation_counter = 0, needaction_inbox_counter = 0, partner_root, public_partner, shortcodes = [], starred_counter = 0 }) {
                const discuss = this.messaging.discuss;
                this._initPartners({
             ...
             */
            _initChannels({ channel_channel = [], channel_direct_message = [], channel_private_group = [] } = {}) {
                for (const data of channel_channel.concat(channel_direct_message, channel_private_group)) {
                    this.env.models["mail.thread"].insert(Object.assign({}, this.env.models["mail.thread"].convertData(data), { isPinned: true }));
                }
            }
             ...
             */
            _initMailboxes({ is_moderator, moderation_counter, needaction_inbox_counter, starred_counter }) {
            ...
            _initMailFailures(mailFailuresData) {
                for (const messageData of mailFailuresData) {
                    const message = this.env.models["mail.message"].insert(this.env.models["mail.message"].convertData(messageData));
             ...
             */
            _initPartners({ moderation_channel_ids = [], partner_root: { active: partnerRootIsActive, display_name: partnerRootDisplayName, id: partnerRootId }, public_partner: { active: publicPartnerIsActive, display_name: publicPartnerDisplayName, id: publicPartnerId } }) {
                this.messaging.update({
                    currentPartner: [
                        [
                            "insert",
