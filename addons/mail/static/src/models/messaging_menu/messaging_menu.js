    ...
    function factory(dependencies) {
        class MessagingMenu extends dependencies["mail.model"] {
            ...
            _updateCounter() {
                ...
                const unreadChannels = this.env.models["mail.thread"].all((thread) => thread.message_unread_counter > 0 && thread.model === "mail.channel");
                ...
                if (this.messaging.notificationGroupManager) {
                    counter += this.messaging.notificationGroupManager.groups.reduce((total, group) => total + group.notifications.length, 0);
                }
                return counter;
            }
