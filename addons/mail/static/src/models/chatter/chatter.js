    ...
    function factory(dependencies) {
        ...
        class Chatter extends dependencies["mail.model"] {
            ...
            async refreshActivities() {
                ...
                const [{ activity_ids: newActivityIds }] = await this.async(() =>
                ...
                const activitiesData = await this.async(() =>
                    this.env.services.rpc({
                        model: "mail.activity",
                ...
                for (const activityData of activitiesData) {
                    const activity = this.env.models["mail.activity"].insert(this.env.models["mail.activity"].convertData(activityData));
            ...
            _updateAfter(previous) {
                // thread
                if (this.threadModel !== previous.threadModel || this.threadId !== previous.threadId) {
                    // change of thread
                ...
                if (previous.followerIds.join(",") !== this.followerIds.join(",") && !this.thread.isTemporary) {
                    this.thread.refreshFollowers();
                }
                if (previous.thread !== this.thread || (this.thread && this.messageIds.join(",") !== previous.messageIds.join(","))) {
                    this.refresh();
                }
            }
