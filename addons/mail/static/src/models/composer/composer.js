odoo.define("mail/static/src/models/composer/composer.js", function (require) {
    ...
    const { addLink, escapeAndCompactTextContent, parseAndTransform } = require("mail.utils");
    ...
    function factory(dependencies) {
        class Composer extends dependencies["mail.model"] {
            ...
            discard() {
                ...
                if (!discuss.isOpen || discuss.thread !== thread || !discuss.isReplyingToMessage) {
                    return;
                }
            ...
            insertIntoTextInput(content) {
                ...
                const partB = this.textInputContent.slice(this.textInputCursorEnd, this.textInputContent.length);
                this.update({
            ...
            insertMentionedPartner(partner) {
                ...
                const textLeft = this.textInputContent.substring(0, this.textInputContent.substring(0, cursorPosition).lastIndexOf("@") + 1);
            ...
            async postMessage(data, options) {
                ...
                if (thread.model === "mail.box") {
                    ...
                    const otherThread = this.env.models["mail.thread"].find((thread) => thread.id === res_id && thread.model === res_model);
                ...
                if (thread.model === "mail.channel") {
                    ...
                    const [messageData] = await this.async(() =>
                        this.env.services.rpc({
                            model: "mail.message",
                    ...
                    this.env.models["mail.message"].insert(
                        Object.assign({}, this.env.models["mail.message"].convertData(messageData), {
                            originThread: [
                                [
                                    "insert",
                                    {
            ...
            setLastSuggestedPartnerActive() {
                ...
                this.update({
                    activeSuggestedPartner: [["link", this.allSuggestedPartners[this.allSuggestedPartners.length - 1]]],
                });
            }
            ...
            setNextSuggestedPartnerActive() {
                ...
                const activeSuggestedPartnerIndex = this.allSuggestedPartners.findIndex((suggestedPartner) => suggestedPartner === this.activeSuggestedPartner);
                if (activeSuggestedPartnerIndex !== this.allSuggestedPartners.length - 1) {
                    this.update({
                        activeSuggestedPartner: [["link", this.allSuggestedPartners[activeSuggestedPartnerIndex + 1]]],
                    });
                } else {
                    this.update({
                        activeSuggestedPartner: [["link", this.allSuggestedPartners[0]]],
            ...
            _computeMentionedPartners() {
                const inputMentions = this.textInputContent.match(new RegExp("@[^ ]+(?= |&nbsp;|$)", "g")) || [];
            ...
            _generateMentionsLinks(body) {
                ...
                for (const i in inputMentions) {
                    ...
                    const partner = this.mentionedPartners.find((partner) => partner.name === matchName);
            ...
            async _getSuggestedPartners(mentionKeyword) {
                const mentions = await this.async(() =>
                    this.env.services.rpc(
                        {
                            model: "res.partner",
                ...
                this.update({
                    extraSuggestedPartners: [["insert-and-replace", extraSuggestedPartners.map((data) => this.env.models["mail.partner"].convertData(data))]],
                    mainSuggestedPartners: [["insert-and-replace", mainSuggestedPartners.map((data) => this.env.models["mail.partner"].convertData(data))]],
                });
            ...
            _validateMentionKeyword(delimiter, beginningOnly) {
                ...
                let searchStr = this.textInputContent.substring(beforeDelimiterPosition, this.textInputCursorStart);
        ...
        Composer.fields = {
            ...
            allSuggestedPartners: many2many("mail.partner", {
                ...
                dependencies: ["extraSuggestedPartners", "mainSuggestedPartners"],
            }),
            ...
            canPostMessage: attr({
                ...
                dependencies: ["attachments", "textInputContent"],
                default: false,
            }),
            ...
            hasSuggestedPartners: attr({
                ...
                dependencies: ["allSuggestedPartners"],
                default: false,
            }),
            isDoFocus: attr({
                default: false,
