/** @odoo-module alias=mail.utils.timer.tests **/
...
import nextTick from 'mail.utils.test.nextTick';
import Timer, { TimerClearedError } from 'mail.utils.Timer';
...
QUnit.test('[with cancelation intercept] timer start then immediate clear (duration: 0ms)', async function (assert) {
    ...
    assert.verifySteps(
        [],
        "should not have observed cleared timer (timer not yet cleared)",
    );
...
QUnit.test('[with cancelation intercept] timer start then immediate reset (duration: 0ms)', async function (assert) {
    ...
    this.timers[0].reset()
        .then(
            () => assert.step('timer_reset_timeout'),
        );
    await nextTick();
    assert.verifySteps(
        ['timer_cleared'],
