odoo.define("mail/static/src/utils/throttle/throttle.js", function (require) {
    ...
    class Throttle {
        ...
        async do(...args) {
            ...
            try {
                ...
                if (!(error instanceof _ThrottleFlushedError) || error.throttleId !== this.id) {
                    throw error;
                }
            } finally {
                this._pendingInvokeDeferred = undefined;
        ...
        async _startCoolingDown() {
            ...
            const cooldownTimeoutId = this.env.browser.setTimeout(() => coolingDownDeferred.resolve(), this._duration);
            let unexpectedError;
            try {
     ...
     */
    function throttle(env, func, duration, { silentCancelationErrors = true } = {}) {
        ...
        const callable = async (...args) => {
            try {
                ...
                const isSelfReinvokedError = error instanceof ThrottleReinvokedError && error.throttleId === throttleObj.id;
                const isSelfCanceledError = error instanceof ThrottleCanceledError && error.throttleId === throttleObj.id;
