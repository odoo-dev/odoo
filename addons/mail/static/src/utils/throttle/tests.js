/** @odoo-module alias=mail.utils.throttle.tests **/
...
import throttle, { ThrottleReinvokedError, ThrottleCanceledError } from 'mail.utils.throttle';
...
QUnit.test('flush throttled call', async function (assert) {
    ...
    throttledFunc().then(
        () => assert.step('throttle_observed_invoke_3'),
    );
    await nextTick();
    await env.services.action.dispatch(
        'Time/advance',
