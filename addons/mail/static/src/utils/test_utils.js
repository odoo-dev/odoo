odoo.define("mail/static/src/utils/test_utils.js", function (require) {
    ...
    const { addMessagingToEnv, addTimeControlToEnv } = require("mail/static/src/env/test_env.js");
    ...
    const {
        ...
        createView,
        makeTestPromise,
        mock: { addMockEnvironment, patch: legacyPatch, unpatch: legacyUnpatch },
    ...
    function _useChatWindow(callbacks) {
        const { mount: prevMount, destroy: prevDestroy } = callbacks;
    ...
    function _useDiscuss(callbacks) {
        const { init: prevInit, mount: prevMount, return: prevReturn } = callbacks;
        let discussWidget;
        const state = {
        ...
        return Object.assign({}, callbacks, {
            init: prevInit.concat((params) => {
                const { autoOpenDiscuss = state.autoOpenDiscuss, discuss: discussData = state.discussData } = params;
    ...
    function _useMessagingMenu(callbacks) {
        const { mount: prevMount, return: prevReturn } = callbacks;
    ...
    async function start(param0 = {}) {
        ...
        const { hasActionManager = false, hasChatWindow = false, hasDialog = false, hasDiscuss = false, hasMessagingMenu = false, hasTimeControl = false, hasView = false, waitUntilMessagingInitialized = true } = param0;
        ...
        const { init: initCallbacks, mount: mountCallbacks, destroy: destroyCallbacks, return: returnCallbacks } = callbacks;
         ...
         * kinds of buses.
         */
        testEnv.bus.on("hide_home_menu", null, () => testEnv.messagingBus.trigger("hide_home_menu"));
        testEnv.bus.on("show_home_menu", null, () => testEnv.messagingBus.trigger("show_home_menu"));
        testEnv.bus.on("will_hide_home_menu", null, () => testEnv.messagingBus.trigger("will_hide_home_menu"));
        testEnv.bus.on("will_show_home_menu", null, () => testEnv.messagingBus.trigger("will_show_home_menu"));
        ...
        testEnv.messaging.start().then(() => testEnv.messagingInitializedDeferred.resolve());
