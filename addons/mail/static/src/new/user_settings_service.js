...
class UserSettings {
    ...
    updateFromCommands(settings) {
        ...
        for (const volumeRecord of volumeRecordSet) {
            if (typeof volumeRecord.guest_id?.id === "number") {
    ...
    async saveVolumeSetting({ guestId, partnerId, volume }) {
        ...
        this.volumeSettingsTimeouts.set(
            key,
            browser.setTimeout(
                this._onSaveVolumeSettingTimeout.bind(this, { key, guestId, partnerId, volume }),
                5000
            )
        );
    }
     ...
     * @param {number} param0.volume
     */
    async _onSaveVolumeSettingTimeout({ key, guestId, partnerId, volume }) {
