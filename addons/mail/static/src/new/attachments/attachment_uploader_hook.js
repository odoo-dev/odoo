...
export function useAttachmentUploader(pThread, message, isPending = false) {
    ...
    const state = useState({
        attachments: pThread ? [...pThread.composer.attachments] : [],
    ...
    useBus(bus, "FILE_UPLOAD_LOADED", ({ detail: { upload } }) => {
        ...
        const indexComposer = originThread.composer.attachments.findIndex(({ id }) => id === tmpId);
        if (indexComposer >= 0) {
            originThread.composer.attachments[indexComposer] = attachment;
        } else {
            originThread.composer.attachments.push(attachment);
        }
