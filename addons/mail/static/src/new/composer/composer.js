...
export class Composer extends Component {
    ...
    get attachments() {
        return (
            this.props.messageToReplyTo?.message?.originThread.pendingAttachments ??
            this.thread?.pendingAttachments ??
            []
        );
    }
    ...
    get isSendButtonDisabled() {
        const attachments = this.thread?.pendingAttachments ?? [];
        return (
    ...
    async onClickFullComposer(ev) {
        const attachmentIds = this.thread?.pendingAttachments.map((attachment) => attachment.id);
        const context = {
    ...
    async sendMessage() {
        return this.processMessage(async (value) => {
            ...
            const postData = {
                attachments: this.thread?.pendingAttachments ?? [],
                isNote:
