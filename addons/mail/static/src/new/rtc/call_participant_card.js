...
import { CallParticipantVideoView } from "@mail/new/rtc/call_participant_video_view";

export class CallParticipantCard extends Component {
    static props = ["rtcSession", "className"];
    static components = { CallParticipantVideoView };
    ...
    get name() {
        return this.state.rtcSession?.channelMember?.partner?.name; // TODO guest
    }
    get avatarUrl() {
        return this.state.rtcSession?.channelMember?.partner?.avatarUrl; // TODO guest
    }
    get hasVideo() {
