...
class Rtc {
    ...
    endCall(channelId = this.channel.id) {
        this.messaging.state.threads[
            Thread.createLocalId({ id: channelId, model: "mail.channel" })
        ].rtcInvitingSession = undefined;
    ...
    _createPeerConnection(rtcSessionId) {
        ...
        peerConnection.ontrack = ({ transceiver, track }) => {
            ...
            const rtcSession = this.channel.rtcSessions.get(rtcSessionId);
            const volume = rtcSession.partner
                ? this.userSettings.partnerVolumes.get(rtcSession.channelMember.partnerId)
                : this.userSettings.guestVolumes.get(rtcSession.channelMember.guestId);
    ...
    async _initSession({
        ...
        const channelProxy = reactive(this.channel, () => {
            ...
            this.channel && this._onUpdateRtcSessions(channelProxy.rtcSessions);
            channelProxy.rtcSessions.size;
        });
        channelProxy.rtcSessions.size;
    ...
    async _joinCall(channelId, { startWithAudio = true, startWithVideo = false, videoType } = {}) {
        ...
        const { rtcSessions, iceServers, sessionId, invitedPartners, invitedGuests } =
            await this.rpc(
    ...
    async _notifyPeers(rtcSessionIds, { event, payload, type = "server" }) {
        ...
        if (type === "server") {
            this._peerNotificationsToSend.set(getRTCPeerNotificationNextTemporaryId(), {
                    ...
                    case "mail.channel.rtc.session/ended":
                        {
                            ...
                            if (rtc.currentRtcSession?.id === sessionId) {
                                rtc.endCall();
                                notification.notify({
                                    message: env._t("Disconnected from the RTC call by the server"),
                                    type: "warning",
                                });
                            }
                        }
