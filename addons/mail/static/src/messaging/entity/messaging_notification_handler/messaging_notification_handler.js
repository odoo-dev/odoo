...
function MessagingNotificationHandlerFactory({ Entity }) {
    ...
    class MessagingNotificationHandler extends Entity {
        ...
        async _handleNotificationChannelMessage(param0) {
            ...
            const currentPartner = this.env.entities.Partner.current;
        ...
        _handleNotificationPartnerChannel(data) {
            ...
            if (
                ...
                (
                    !channel ||
                    !channel.members.includes(this.env.entities.Partner.current)
                )
            ) {
                this.env.do_notify(
...
registerNewEntity('MessagingNotificationHandler', MessagingNotificationHandlerFactory, ['Entity']);
