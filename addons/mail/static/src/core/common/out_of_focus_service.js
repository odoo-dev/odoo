...
export class OutOfFocusService {
    ...
    async notify(message, thread) {
        ...
        let notificationTitle = message.thread?.displayName || _t("New message");
        let icon = message.thread?.avatarUrl || "/mail/static/src/img/odoobot_transparent.png";
        ...
        if (author) {
            if (message.thread?.model === "discuss.channel") {
                ...
                if (message.thread.channel_type === "channel") {
                    notificationTitle = `#${message.thread.displayName}`;
                }
            }
