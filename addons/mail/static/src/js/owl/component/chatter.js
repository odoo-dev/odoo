...
class Chatter extends owl.store.ConnectedComponent {
    ...
    constructor(...args) {
        ...
        if (threadField) {
            const fieldsInfo = this.props.record.fieldsInfo[this.props.fieldOptions.viewType || this.props.parent.viewType];
            const nodeOptions = fieldsInfo[threadField].options || {};
    ...
    _insertThread() {
        const threadField = this.props.mailFields.mail_thread;
        this.dispatch('insertThread', {
            _model: this.props.record.model,
            id: this.props.record.res_id,
            messageIds: this.props.record.data[threadField].res_ids,
        });
    }
    ...
    _onClickSend() {
        ...
        if (!this.storeProps.suggestedRecipients) {
            this.dispatch('fetchSuggestedRecipientsOnThread',
 ...
 * @return {Object}
 */
Chatter.mapStoreToProps = function (state, ownProps, getters) {
