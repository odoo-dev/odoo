...
class StoreFactory:
    ...
    def to(
        ...
        *,
        ...
        """
        assert payload is None or isinstance(payload, dict), "Notification payload should be a dict"
        ...
        if key not in stores:
            store = Store(target=(bus_channel, bus_subchannel))
            stores[key] = store
            to_send = store if payload is None else {**payload, "store_data": store}
            bus_channel._bus_send(notification_type, to_send, subchannel=bus_subchannel)
            if bus_channel._name == "discuss.channel" and bus_subchannel != "internal_users":
                store._internal_store = cls.to(bus_channel, "internal_users")
        return stores[key]
...
class Store(StoreFactory):
    ...
    def __init__(self, *, target=None):
        ...
        elif isinstance(target, Store.Target):
            self.target = target
