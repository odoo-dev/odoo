...
class Store:
        ...
        if cache is not None and isinstance(fields, str):
            ...
            if candidates := cache.get(key):
                ...
                for target_reads, field_list, identity in candidates:
                    if all(getattr(probe, name)() == value for name, value in target_reads):
    ...
    def _record_target_read(self, name, value):
        """Remember that a field list read `name` from the target while being built, so that
        `_format_fields_with_identity` knows what to compare before reusing it."""
        if self._target_reads is not None:
            self._target_reads[name] = value
        return value
    ...
    class FieldList(UserList):
        ...
        def is_for_current_user(self):
            ...
            return self.store._record_target_read(
                "is_for_current_user", self._is_for_current_user(),
            )
        ...
        def is_for_internal_users(self):
            ...
            user of the env."""
            return self.store._record_target_read(
                "is_for_internal_users", self._is_for_internal_users(),
            )
        ...
        def target_guest(self):
            ...
            """
            return self.store._record_target_read("target_guest", self._target_guest())
        ...
        def target_user(self):
            ...
            return self.store._record_target_read("target_user", self._target_user())
