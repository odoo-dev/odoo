...
class Channel(models.Model):
    ...
    def _constraint_parent_channel_id(self):
        ...
        if failing_channels := self.sudo().filtered(
            lambda c: c.parent_channel_id and c.parent_channel_id.parent_channel_id
        ):
            raise ValidationError(
                _(
                    "You cannot create a sub channel from a sub-channel (%(channels)s).",
    ...
    def _create_sub_channel(self, from_message_id=None, name=None):
        ...
        store = Store(sub_channel).add(sub_channel, {"forceOpen": True})
