...
class DiscussChannel(models.Model):
    ...
    def write(self, vals):
        ...
        if old_names:
            for channel in self:
                if channel.name != old_names[channel] and channel.channel_type in channel._types_notified_on_rename():
                    body = Markup('<div data-oe-type="channel_rename" class="o_mail_notification">%s</div>') % (channel.name or channel.display_name)
                    channel.message_post(body=body, message_type="notification", subtype_xmlid="mail.mt_comment")
        return result

    def _types_notified_on_rename(self):
        """Channel types that post a channel rename notification message when renamed."""
