...
class ChannelMember(models.Model):
    ...
    def unlink(self):
        ...
        for (p_id, g_id), rec in self.grouped(
                lambda m: (m.partner_id.id, m.guest_id.id)
        ).items():
            domain = expression.OR(
                [
                    domain,
                    [
                        ("partner_id" if p_id else "guest_id", "=", p_id or g_id),
                        ("channel_id", "in", rec.channel_id.sub_channel_ids.ids),
                    ],
                ]
            )
        for member in self.env["discuss.channel.member"].search(domain):
