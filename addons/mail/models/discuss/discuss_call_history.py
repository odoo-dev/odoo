...
class DiscussCallHistory(models.Model):
    ...
    def _broadcast_recording_availability(self):
        self._update_activity_done_message()
        for call_history in self:
    ...
    def _compute_duration_human_readable(self):
        for record in self:
            end_dt = record.end_dt or fields.Datetime.now()
            record.duration_human_readable = format_call_duration(
    ...
    def _update_activity_done_message(self):
        """ Re-render the chatter message logging this call. A recording and a transcript
        only land once the call is over, hence after that message was first rendered:
        without this, the icons announcing them would never show up on it.

        Runs in the elevated context its callers already establish: the SFU and the
        transcription callbacks are authenticated by their own means, not by a user. """
