...
class DiscussPoll(models.Model):
    ...
    channel_id = fields.Many2one("discuss.channel", related="start_message_id.channel_id")
    ...
    option_ids = fields.One2many("discuss.poll.option", "poll_id", required=True)
    ...
    winning_option_id = fields.Many2one("discuss.poll.option", compute="_compute_winning_option_ids")
    ...
    def _end_and_notify(self):
        for poll in self:
            poll.end_message_id = poll.channel_id.message_post(
    ...
    def _end_expired_polls(self):
        ...
        """)
        self.env["discuss.poll"].browse(
            [pid[0] for pid in self.env.cr.fetchall()]
        )._end_and_notify()
