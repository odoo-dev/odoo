        ...
        *   s (stage id):
        ...
        for record in self:
            ...
            stage_durations = defaultdict(int, {
                k: v for k, v in tracking.items()
                if k.isdigit()
            })

            if tracking.get('s') and tracking.get('d'):
                prev_dt = fields.Datetime.from_string(tracking['d'])
                stage_durations[str(tracking['s'])] += int((now - prev_dt).total_seconds())
            record.duration_tracking = {
                **(record.duration_tracking or {}),
                **dict(stage_durations),
                's': record[record._track_duration_field].id,
                'd': fields.Datetime.to_string(now),
            }
