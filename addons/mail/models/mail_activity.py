    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for model, activity_data in ongoing_activities.filtered('res_model')._classify_by_model().items():
            ...
            for record_sudo, activity in zip(records_sudo, activity_data['activities']):
                ...
                if activity_message:
                    # sudo: discuss.call.history: the message logging a call is kept to be
                    # re-rendered when the recording or the transcript of that call lands,
                    # which whoever marks the activity done need not be allowed to write.
