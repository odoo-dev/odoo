...
class MailTrackMixin(models.AbstractModel):
    ...
    def _create_mail_tracking_values(
        ...
        if col_info['type'] in {'integer', 'float', 'char', 'text'}:
            ...
            values.update({
                ...
                'old_value': format_amount(self.env, initial_value or 0, self[col_info['currency_field']]),
                'new_value': format_amount(self.env, new_value or 0, self[col_info['currency_field']]),
            })
    ...
    def _create_mail_tracking_values_property(
        ...
        return {**tracking_values, 'field_info': field_info, 'field_label': field_info.get('desc')}
