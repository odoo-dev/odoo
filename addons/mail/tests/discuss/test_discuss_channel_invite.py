...
class TestDiscussChannelInvite(HttpCase, MailCommon):
    ...
    def test_14_escaped_comma_in_search_for_channel_invite(self):
        exact = new_test_user(
            self.env, "exact", groups="base.group_user", email="exact@test.com", name="Doe, John"
        )
        decoy = new_test_user(
            self.env, "decoy", groups="base.group_user", email="decoy@test.com", name="Doe"
        )
        channel = self.env["discuss.channel"].create(
            {"name": "public community", "group_public_id": False},
        )
        res = self.env["res.partner"].search_for_channel_invite(
            "Doe\\, John", channel_id=channel.id
        )
        self.assertEqual(res["partner_ids"], exact.partner_id.ids)
        self.assertNotIn(decoy.partner_id.id, res["partner_ids"])
