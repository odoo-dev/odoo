...
class TestChannelInternals(MailCommon, HttpCase):
    ...
    def test_channel_write_should_send_notification(self):
        ...
        self.assertEqual(
            ...
            Markup('<div data-oe-type="channel_rename" class="o_mail_notification">test test</div>'),
        )

    def test_group_chat_name_reset_posts_rename_message_with_display_name(self):
        group_chat = self.env["discuss.channel"]._create_group(users_to=self.user_employee, name="Test Group")
        self.assertEqual(group_chat.name, "Test Group")
        group_chat.name = ""
        self.assertEqual(group_chat.name, "")
        self.assertEqual(
