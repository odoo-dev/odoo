...
class TestCallHistoryLog(MailCommon):
    ...
    def _log_call_on_a_new_partner(self):
        """Mark done an activity logging the call, as ending the call does.

        :return: the ``mail.message`` that logging it posted in the chatter"""
    ...
    def _make_the_recording_available(self):
        """Upload the recording of the call, the way the SFU callback does once the
        call is over (see `cloud_storage`'s `complete_recording`)."""
    ...
    def _pushed_message_updates(self, message):
        """The bus notifications sent so far that carry a new state of ``message``.

        :return: a list of the ``mail.message`` payloads pushed for it"""
