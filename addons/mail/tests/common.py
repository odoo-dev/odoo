...
class MailCase(MockEmail):
    ...
    def assertMailNotifications(self, messages, recipients_info):
        ...
        for message_info in recipients_info:
            ...
            bus_notifications = message.notification_ids._filtered_for_message_list().filtered(lambda n: n.notification_status == 'exception')
            if bus_notifications:
                self.assertMailBusNotifications(message)
    ...
    def assertMailBusNotifications(self, message):
        self.assertBusNotifications(
            [(self.cr.dbname, 'res.partner', message.author_id.id)],
            [{
                'type': 'mail_notification_update',
                'elements': message._format_mail_notifications(),
            }],
            check_unique=False
        )
