...
class Account_Edi_Proxy_ClientUser(models.Model):
    ...
    def _peppol_import_order_balance(self, attachment, peppol_state, uuid):
        ...
        if order:
            ...
            order_id = tree.findtext('./{*}ID')
            note_prefix = f'[{order_id}] ' if order_id else ''
            ...
            msg = Markup("<b>Received PEPPOL order balance (ID: {order_id}, Customer Reference: {customer_ref})</b>")
            if log:
