...
class TestSubcontractingDropshippingValuation(ValuationReconciliationTestCommon):
    ...
    def test_account_line_entry_kit_bom_dropship(self):
        ...
        self.assertRecordValues(bill.line_ids.sorted('balance'), [
            {'name': False,          'account_name': 'Accounts Payable',   'debit': 0.0,      'credit': 2184.0},
        ...
        self.assertRecordValues(invoice.line_ids.sorted('balance'), [
            ...
            {'name': f'{sale_order.name} - {invoice.name} installment #1',   'account_name': 'Accounts Receivable (copy)',   'debit': 621.0,    'credit': 0.0},
            {'name': f'{sale_order.name} - {invoice.name} installment #2',   'account_name': 'Accounts Receivable (copy)',   'debit': 1449.0,   'credit': 0.0},
        ])
