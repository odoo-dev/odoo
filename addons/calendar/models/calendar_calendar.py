...
class CalendarCalendar(models.Model):
    ...
    calendar_default_privacy = fields.Selection(
        [('public', 'Public by default'),
         ...
         ('members_only', 'Shared Calendar Members Only')],
        default=lambda self: self.env['ir.config_parameter'].sudo().get_str('calendar.default_privacy', 'private'),
        required=True,
    )
    ...
    share_user_ids = fields.Many2many('res.users', string='Shared with', compute='_compute_share_user_ids',
        inverse='_inverse_share_user_ids', ui_domain="[('id', '!=', owner_id), ('share', '=', False)]")
    ...
    def write(self, vals):
        ...
        blacklisted_fields = [k for k in vals if k not in self._get_public_writable_fields()]
        if blacklisted_fields and not self.env.su and any(self.env.user != calendar.owner_id for calendar in self):
            raise AccessError(_("Only the calendar owner can modify these fields: %(fields)s", fields=", ".join(blacklisted_fields)))
    ...
    def _search_calendar_user_id(self, operator, value):
        """ Search is needed to allow using the computed calendar_user_id field for related fields """
        return [('id', operator, self.env['calendar.user']._search([('calendar_id', 'in', self.ids), ('user_id', '=', self.env.user.id)]))]

    @api.depends('calendar_user_ids.user_id', 'calendar_user_ids.access_role')
    def _compute_share_user_ids(self):
        for calendar in self:
            # Do not include the owner in this list, as we do not want the user to be able to remove themselves.
    ...
    def _compute_owner_id(self):
        owner_records = self.env['calendar.user'].search([
            ('access_role', '=', 'owner'),
            ('calendar_id', 'in', self.ids),
        ])
        for calendar in self:
            calendar.owner_id = owner_records.filtered(lambda l: l.calendar_id.id == calendar.id).user_id.id
    ...
    def unlink(self):
        ...
        events._before_calendar_cascade_unlink()
        recurrences._before_calendar_cascade_unlink()
        events.unlink()
        recurrences.unlink()
        return super().unlink()

    def _get_public_writable_fields(self):
        """Sync modules can override this to allow write access to some fields."""
        return set()
