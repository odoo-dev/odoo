...
class TestRepair(TestRepairCommon):
    ...
    def test_purchase_price_so_create_from_repair(self):
        ...
        """
        if not self.env['ir.module.module'].search([('name', '=', 'sale_management'), ('state', '=', 'installed')]):
            self.skipTest("sale_management is not installed, so there is no purchase price to test")
