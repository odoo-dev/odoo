...
class RepairServiceLine(models.Model):
    ...
    @api.depends('qty_invoiced', 'quantity', 'uom_id', 'invoice_line_ids')
    ...
    def _update_repair_sale_order_line(self):
        lines_to_update = self.filtered(lambda rsl: not rsl.should_skip_line_update())
    ...
    def should_skip_line_update(self):
        if self.repair_id.sale_order_id:
            sale_line = self.sale_line_id
            return self.quantity == sale_line.product_uom_qty and self.uom_id == sale_line.product_uom_id
    ...
    # method should be moved somewhere else to be reused instead of rewritten?
