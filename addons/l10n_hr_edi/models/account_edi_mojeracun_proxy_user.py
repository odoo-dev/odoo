...
class AccountEdiProxyClientUser(models.Model):
    ...
    def _call_mer_proxy(self, endpoint, params=None):
        ...
        if response.json().get('File'):
            error_message = response.json().get('File')
    ...
    # Functionally identical to _get_new_documents in intention?
    def _cron_mer_get_document_status_inbox(self):
        edi_users = self.search([('proxy_type', '=', 'mojeracun'), ('company_id.l10n_hr_mer_proxy_state', '=', 'receiver')])
        edi_users._mer_api_query_document_process_status_inbox()

    def _cron_mer_update_outbox_document_status(self):
        edi_users = self.search([('proxy_type', '=', 'mojeracun'), ('company_id.l10n_hr_mer_proxy_state', '=', 'receiver')])
        for user in edi_users:
            moves = self.env['account.move'].search([
                ("company_id", "=", user.company_id.id),
                ("l10n_hr_mer_document_status", "=", '20'),
                ("l10n_hr_mer_document_id", "!=", False),
            ])
            for move in moves:
                response = user._mer_api_query_document_process_status_inbox(electronic_id = move.l10n_hr_mer_document_id)
                move.l10n_hr_mer_document_status = response[0]['StatusId']

    # Check ID API does not work yet
    def _cron_mer_get_participant_status(self):
        edi_users = self.search([('proxy_type', '=', 'mojeracun')])
        edi_users._mer_get_participant_status()

    # Doesn't seem to be of any use for MER
    ...
    def _mer_api_send(self, xml_file):
        ...
        """
        #print("--- DEBUG: api_send: file:", xml_file.replace("'", '"')[:600], "---")
        #print("--- DEBUG: api_send: dumps:", json.dumps(xml_file.replace("'", '"'))[:600], "---")
        self.ensure_one() # Calling this should only be done for one user, and there is no point in bundling things here
        params={
            'File': json.dumps(xml_file.replace("'", '"')), # Needs to be JSON-escaped
        }
    ...
    def _mer_api_report_operation(self, invoice_xml, delivery_date, is_copy, invoice_type):
        ...
        self.ensure_one()
        params={
            'xmlInvoice': json.dumps(invoice_xml), # Needs to be JSON-escaped
    ...
    def _mer_deregister_participant(self):
        ...
        if self.company_id.l10n_hr_mer_proxy_state == 'receiver':
            ...
            self._cron_mer_get_new_documents(undelivered=False)
            #self._cron_mer_get_document_status_inbox() # Ideally, this has to get all documents in MER and check them one by one
            #self._cron_mer_get_document_status_outbox()
    ...
    def _mer_get_new_documents(self, undelivered=True, notify=True):
        ...
        for edi_user in self:
            ...
            print("--- DEBUG: _mer_get_new_documents() response.json():", response.json(), "---")
            documents = [
                {
