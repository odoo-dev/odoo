...
@tagged('external', 'external_l10n', 'post_install', '-post_install_l10n', '-at_install', '-standard', 'l10n_hr_edi_live')
class TestL10nHrEdiMerApi(TestL10nHrEdiCommon, TestAccountMoveSendCommon):
    ...
    def test_mer_api_ping(self):
        """
        Test MER server responding correctly to the basic ping API.
        """
        self.env['l10n_hr_edi.mojeracun_connection']._mer_api_ping(self.env.company)
    ...
    def test_mer_api_query_inbox(self):
        """
        Test querying MER inbox for documents present, without a filter.
        Response depends on the documents sent to the server up to this point,
        so the only thing that can be really evaluated is not getting an error.
        """
        response = self.env['l10n_hr_edi.mojeracun_connection']._mer_api_query_inbox(
            self.env.company,
            filter=None,
            electronic_id=None,
            status_id=None,
    ...
    def test_mer_api_query_outbox(self):
        """
        Test querying MER outbox for documents present, without a filter.
        Response depends on the documents sent to the server up to this point,
        so the only thing that can be really evaluated is not getting an error.
        """
        response = self.env['l10n_hr_edi.mojeracun_connection']._mer_api_query_outbox(
            self.env.company,
            filter=None,
            electronic_id=None,
            status_id=None,
    ...
    def test_mer_api_query_document_process_status_outbox(self):
        ...
        response = self.env['l10n_hr_edi.mojeracun_connection']._mer_api_query_document_process_status_outbox(
            self.env.company,
    ...
    def _test_mer_api_check_identifier(self):
        """
        Test checking a partner (own) endpoint being present in the MojEracun system.
        """
        self.env['l10n_hr_edi.mojeracun_connection']._mer_api_check_id(
            self.env.company,
            id_type='1',
            id_value=self.env.company.l10n_hr_mer_company_ident
        )
    ...
    def test_mer_api_receive(self):
        ...
        """
        electronic_id = '3120260'
        response = self.env['l10n_hr_edi.mojeracun_connection']._mer_api_receive_document(
            self.env.company,
            electronic_id=electronic_id
        )
        ...
        move = self.env['l10n_hr_edi.mojeracun_connection']._mer_import_invoice(
            self.env.company,
            attachment,
            {
    ...
    def test_mer_flow_get_new_documents(self):
        ...
        """
        self.env['l10n_hr_edi.mojeracun_connection']._mer_get_new_documents(
            self.env.company,
