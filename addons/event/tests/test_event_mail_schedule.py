    ...
    def test_event_mail_schedule_on_slot(self):
        ...
        with self.mock_datetime_and_now(self.reference_now):
            registrations = self.env['event.registration'].with_user(self.user_eventuser).create([
                ...
                for slot in [test_event.event_slot_ids[0], test_event.event_slot_ids[1], self.env['event.slot']]
                for idx in range(5)
            ])
        ...
        _registrations_slot_2 = registrations.filtered(lambda r: r.event_slot_id == test_event.event_slot_ids[1])
        _registrations_noslot = registrations.filtered(lambda r: not r.event_slot_id)
