...
class TestEventSlotsCommon(EventCase):
    ...
    def setUpClass(cls):
        ...
        with cls.mock_datetime_and_now(cls, cls.reference_now):
            ...
            first_slot = cls.test_event.slot_ids.filtered(lambda s: s.start_hour == 9)
            second_slot = cls.test_event.slot_ids.filtered(lambda s: s.start_hour == 13)
...
class TestEventSlotSeats(TestEventSlotsCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.test_event_slot_noticket = cls.test_event.copy()

        first_slot = cls.test_event_slot_noticket.slot_ids.filtered(lambda s: s.start_hour == 9)
        second_slot = cls.test_event_slot_noticket.slot_ids.filtered(lambda s: s.start_hour == 13)

        # already existing registrations: 3 on first slot (1 archived), 1 on second slot (2 archived)
        with cls.mock_datetime_and_now(cls, cls.reference_now):
            cls._create_registrations_for_slot_and_ticket(cls.test_event_slot_noticket, first_slot, False, 3)
    ...
    def test_assert_initial_values(self):
        ...
        first_slot = test_event.slot_ids.filtered(lambda s: s.start_hour == 9)
        second_slot = test_event.slot_ids.filtered(lambda s: s.start_hour == 13)
        ...
        first_slot = test_event_nt.slot_ids.filtered(lambda s: s.start_hour == 9)
        second_slot = test_event_nt.slot_ids.filtered(lambda s: s.start_hour == 13)
        self.assertTrue(first_slot)
        self.assertTrue(second_slot)
        self.assertEqual(first_slot.seats_available, 2)
        self.assertEqual(first_slot.seats_reserved, 3)
    ...
    def test_seats_slots_notickets(self):
        ...
        for create_input, should_crash in [
            # ok for event max seats for both tickets
            (((first_slot, 2), (second_slot, 2)), False),
            # not enough seats on first ticket
            (((first_slot, 3),), True),
            # not enough seats on second ticket
            (((second_slot, 5),), True),
        ]:
            with self.subTest(create_input):
                create_values = []
                for slot, count in create_input:
                    create_values += [
                        {
                            'email': f'{slot.name}.{idx}@test.example.com',
                            'event_id': test_event.id,
                            'event_slot_id': slot.id,
                            'name': f'{slot.name} {idx}',
        ...
        self.assertEqual(len(all_slot2), 3, 'Test setup data: 1 active, 2 inactive')
    ...
    def test_seats_slots_tickets(self):
        ...
        # first slot: maximum 2 seats, and VIP ticket has 1 seat anyway
        # second slot: maximum 4 seats, Better has 3 max and 1 taken and VIP 1 max
        ...
        for create_input, should_crash in [
            ...
            with self.subTest(create_input):
                ...
                for slot, ticket, count in create_input:
                    create_values += [
                        {
                            'email': f'{slot.name}.{ticket.name}.{idx}@test.example.com',
                            'event_id': test_event.id,
                            'event_slot_id': slot.id,
                            'event_ticket_id': ticket.id,
                            'name': f'{slot.name} {ticket.name} {idx}',
                        } for idx in range(count)
                    ]
                if should_crash:
                    with self.assertRaises(exceptions.ValidationError):
