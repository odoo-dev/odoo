...
class EventEvent(models.Model):
    ...
    start_sale_date_located = fields.Char(
        'Start sale date located', compute='_compute_start_sale_date_located'
    )
    # Date fields
    ...
    @api.depends('start_sale_datetime', 'date_tz')
    def _compute_start_sale_date_located(self):
        for event in self:
            if event.start_sale_datetime:
                event.start_sale_date_located = format_datetime(
                    self.env, event.start_sale_datetime, tz=event.date_tz, dt_format='short')
