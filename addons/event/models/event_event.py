...
class EventEvent(models.Model):
    ...
    def _compute_event_slot_count(self):
        for event in self:
            event.event_slot_count = len(event.event_slot_ids)
    ...
    def _check_slots_dates(self):
        for event in self:
            if not event.is_multi_slots:
                continue
            slots_outside_event_bounds = event.event_slot_ids.filtered(lambda slot:
    ...
    def _get_date_range_str(self, slot_datetime=False, lang_code=False):
        self.ensure_one()
        datetime = slot_datetime or self.date_begin
