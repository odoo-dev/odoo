/** @odoo-module alias=website_livechat.MockServer **/

import 'im_livechat.MockServer'; // ensure mail overrides are applied first
...
MockServer.include({
    ...
    _mockMailChannelChannelInfo(ids, extra_info) {
        ...
        for (const channelInfo of channelInfos) {
            ...
            if (channel.channel_type === 'livechat' && channelInfo.livechat_visitor_id) {
                const visitor = this._getRecords(
                    'website.visitor',
                    [['id', '=', channelInfo.livechat_visitor_id]],
                )[0];
    ...
    _mockWebsiteVisitorActionSendChatRequest(ids) {
        ...
        for (const visitor of visitors) {
            ...
            const notification = [
                [false, 'res.partner', this.currentPartnerId],
                channelInfo,
            ];
