...
class WebsiteSaleL10nEsEcommerce(WebsiteSale):
    ...
    def _validate_address_values(
        ...
        if not has_identification:
            # Don't flag a specific field: the active one (whichever the ID-type selector is
            # currently set to) isn't reliably known server-side when it was left empty (an
            # empty additional-identifier value isn't even forwarded, see `_parse_form_data`),
            # and flagging a field name with no matching form input crashes the JS error
            # highlighting (`this.addressForm[fieldName]` would be undefined). The message alone
            # already blocks the submission (see the `error_messages` check in the caller).
            error_messages.append(self.env._(
                "Since you're ordering from outside Spain's VAT territory, please select an ID"
                " type (VAT, foreign ID, passport, residence certificate...) and fill it in,"
