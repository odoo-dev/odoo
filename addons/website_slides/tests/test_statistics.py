...
class TestChannelStatistics(common.SlidesCase):
    ...
    def test_channel_new_content(self):
        (self.slide | self.slide_2).write({'published_date': self.env.now + relativedelta(days=-6)})
        self.slide_3.write({'published_date': self.env.now + relativedelta(days=-8)})
        ...
        (self.slide | self.slide_2).write({'published_date': self.env.now + relativedelta(days=-8)})
