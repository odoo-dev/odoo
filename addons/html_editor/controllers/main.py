...
class HTML_Editor(Controller):
    ...
    @route(['/web_editor/image_shape_url/<module>/<path:filename>', '/html_editor/image_shape_url/<module>/<path:filename>'], type='http', auth="user", website=True)
    def image_shape_url(self, module, filename, image_url=None, **kwargs):
        ...
        try:
            ...
            if (
                ...
                or head_response.headers.get('content-type') not in SUPPORTED_IMAGE_MIMETYPES
            ):
                raise werkzeug.exceptions.BadRequest()
