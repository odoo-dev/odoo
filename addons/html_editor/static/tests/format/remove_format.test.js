...
describe("Toolbar", () => {
    async function removeFormatClick() {
        ...
        expect(".btn[name='remove_format']:not(.disabled)").toHaveCount(1); // remove format button should not be disabled

        click(".btn[name='remove_format']");
        await animationFrame();
        expect(".o-we-toolbar").toHaveCount(1); // toolbar still open
        expect(".btn[name='remove_format'].disabled").toHaveCount(1); // remove format button should be disabled
    }
    ...
    test("Should do nothing when no format in selection", async () => {
        ...
        expect(".o-we-toolbar").toHaveCount(1); // toolbar open
        expect(".btn[name='remove_format']").toHaveCount(1); // remove format
        expect(".btn[name='remove_format'].disabled").toHaveCount(1); // remove format button should be disabled when no format

        click(".btn[name='remove_format']");
        await animationFrame();
        expect(".o-we-toolbar").toHaveCount(1); // toolbar still open
        expect(".btn[name='remove_format'].disabled").toHaveCount(1); // remove format button should still be disabled
        expect(getContent(el)).toBe(
            `<p>this <span class="random-class">is[ a ]UX</span> test.</p>`
        );
    });
