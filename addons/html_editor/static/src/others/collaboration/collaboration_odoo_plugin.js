...
export class CollaborationOdooPlugin extends Plugin {
    ...
    setupCollaboration(collaborationChannel) {
        ...
        const loadCollabInfos = async () => {
            ...
            this.iceServers = infos.ice_servers;
            if (this.iceServers && !this.iceServers.length) {
                this.iceServers = [
                    {
            ...
            this.busConfig = {
                broadcastAll: getBroadcastAll(),
            };
        };
        function getBroadcastAll() {
            const rpcMutex = new Mutex();
            return async (rpcData) => {
        ...
        this.remoteLoading = loadCollabInfos().then(() => {
            const { requestHandlers, notificationHandlers } = getRemoteHandlers(this);
            const config = {
                peerId: this.config.collaboration.peerId,
                handleNotification: (notificationName, fromPeerId, notificationPayload) => {
                    ...
                    notificationHandlers?.[notificationName]?.(fromPeerId, notificationPayload);
                },
            ...
            if (this.sfuConfig) {
                ...
                this.remoteType = "bus.bus";
                config.busConfig = this.busConfig;
            }
    ...
    onServerLastIdUpdate(last_step_id) {
        ...
        if (this.isDocumentStale && (this.sfuConfig || this.busConfig || this.ptpJoined)) {
