...
/**
 * @typedef {((record: HistoryMutationRecord) => void)[]} on_attribute_change_handlers
 * @typedef {(() => void)[]} on_before_add_step_handlers
 * @typedef {((records: HistoryMutationRecord[]) => void)[]} on_before_filter_mutation_record_handlers
 * @typedef {((root: HTMLElement) => void)[]} on_content_updated_handlers
 * @typedef {(() => void)[]} on_external_step_added_handlers
 * @typedef {((records: HistoryMutationRecord[], currentOperation: "original"|"undo"|"redo"|"restore") => void)[]} on_handle_new_records_handlers
 ...
 * @typedef {((revertedStep: HistoryStep) => void)[]} on_after_redo_handlers
 * @typedef {((revertedStep: HistoryStep) => void)[]} on_after_undo_handlers
 * @typedef {(() => void)[]} on_restore_savepoint_handlers
...
export class HistoryPlugin extends Plugin {
    ...
    processNewRecords(mutationRecords) {
        ...
        records
            ...
            .forEach((record) => this.trigger("on_attribute_change_handlers", record));
        return records;
    }
    ...
    handleNewRecords(records, dispatch = true) {
        ...
        if (processedRecords.length) {
            ...
            if (dispatch) {
                ...
                this.trigger("on_handle_new_records_handlers", processedRecords, stepType);
            }
            // Process potential new records adds by on_handle_new_records_handlers.
    ...
    filterAndAdjustHistoryMutationRecords(records) {
        this.trigger("on_before_filter_mutation_record_handlers", records);
    ...
    _addStep({
        ...
        this.trigger("on_normalize_handlers", stepCommonAncestor, type);
        ...
        // this._setLinkZws();
        this.trigger("on_before_add_step_handlers");
        if (extraStepInfos) {
    ...
    undo() {
        ...
        this.trigger("on_after_undo_handlers", revertedStep);
    }
    redo() {
        this.handleObserverRecords();
        ...
        this.trigger("on_after_redo_handlers", revertedStep);
    }
    /**
    ...
    makeSavePoint() {
        ...
        return () => {
            ...
            this.trigger("on_restore_savepoint_handlers");
        };
    }
    /**
