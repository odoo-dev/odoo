 ...
 *
 * @typedef {Object} ChildListSingleMutation
 * @property { "childList" } type
...
export class HistoryPlugin extends Plugin {
    ...
    makeSnapshotStep() {
        this.observedNodes.add(this.editable);
        descendants(this.editable).forEach((node) => this.observedNodes.add(node));
        return {
            selection: {
    ...
    isValidRecord(record) {
        ...
        if (record.previousSibling === undefined && record.nextSibling === undefined) {
            // No valid position for node insertion, due to unobserved mutations
            return false;
        }
        return true;
    }
    ...
    handleUnobservedMutations(records) {
        ...
        for (const record of records) {
            if (!this.observedNodes.has(record.target)) {
     ...
     * @see storeOldValue
     *
     * @param {HistoryMutationRecord} record of type "attributes", "classList" or "characterData"
     * @returns {HistoryMutationRecord}
     */
    ...
    trackObservedNodes(record) {
        [...record.addedNodes]
            ...
            .forEach((node) => this.observedNodes.add(node));
        [...record.removedNodes]
            .flatMap((node) => [node, ...descendants(node)])
    ...
    updateChildListRecord(record) {
        let { previousSibling, nextSibling, removedNodes } = record;

        // Adjust sibling references
        const isValidReference = (node) => node === null || this.observedNodes.has(node);
        ...
        const filteredRemovedNodes = [...removedNodes].filter((n) => this.observedNodes.has(n));
    ...
    applyRemoveMutation(mutation) {
        ...
        toremove.remove();
        this.observedNodes.delete(toremove);
    }
    ...
    applyAddMutation(mutation) {
        ...
        for (const [siblingId, position] of [
            [previousId, "after"],
            [nextId, "before"],
        ]) {
    ...
    reverseMutation(mutation) {
        const swapProperties = (a, b) => ({ ...mutation, [a]: mutation[b], [b]: mutation[a] });
        switch (mutation.type) {
            case "custom":
