...
export class HtmlViewer extends Component {
    ...
    getEmbedding(host) {
        return this.embeddedComponents(this.config.embeddedComponents)[host.dataset.embedded];
    }

    mountComponent(host, { Component, getProps }) {
        const props = getProps?.(host) || {};
        const { dev, translateFn, getRawTemplate } = this.__owl__.app;
