    ...
    static createElements(selectedVideos, { document = window.document } = {}) {
        return selectedVideos.map((videoData) => {
            ...
            if (videoData.platform === VideoFile.id) {
                // A video file has no id, and its options are not encoded in
                // its url: the container has to carry them, being the only
                // thing the `<video>` element can be rebuilt from.
                div.dataset.videoOptions = JSON.stringify(videoData.options || {});
            } else {
