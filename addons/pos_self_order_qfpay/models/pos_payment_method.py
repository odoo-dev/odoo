...
class PosPaymentMethod(models.Model):
    ...
    def _load_pos_self_data_domain(self, data, config):
        ...
        if config.self_ordering_mode == 'kiosk':
            domain = Domain.OR([[('use_payment_terminal', '=', 'qfpay'), ('id', 'in', data['pos.config'][0]['payment_method_ids'])], domain])
        return domain
    ...
    def _qfpay_handle_webhook(self, config, data, uuid):
        ...
        if order:
            ...
            order._send_payment_result("Success" if data['status'] == "1" else "fail")
