...
test("with lot tracking and without lot creation orderlines will merge", async () => {
...
test("combo price unchanged with lot tracked product", async () => {
    const store = await setupPosEnv();
    const order = store.addNewOrder();
...
test("order with existing serial keeps serial lines", async () => {
    const store = await setupPosEnv();
    const order = store.addNewOrder();
