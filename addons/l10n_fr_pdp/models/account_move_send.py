...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_action_what_is_peppol_activate_peppol(self, company, moves):
        action = super()._get_action_what_is_peppol_activate_peppol(company, moves)
        # used by the 'l10n_fr_pdp.what_is_pdp' js action
        action['context']['res_config_settings_id'] = self.env['res.config.settings'].create({'company_id': company.id}).id
        return action
    ...
    def action_what_is_peppol_activate(self, moves):
        ...
        if (
            ...
            config = self.env['res.config.settings'].create({'company_id': companies.id})
            action = {
