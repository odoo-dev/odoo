...
class AccountEdiProxyClientUser(models.Model):
    ...
    def _pdp_process_messages_status(self, messages, uuid_to_record):
        ...
        for uuid, content in messages.items():
            ...
            if record_model == 'pdp.response':
                ...
                # TODO: remove later? attach the XML to the move and log a message
                origin_move = pdp_response.move_id
                ...
                origin_move._message_log(
                    body=self.env._(
                        ...
                        issue_date=pdp_response.issue_date,
                    ),
    ...
    def _pdp_send_response(self, reference_moves, status, additional_info=None):
        ...
        try:
            ...
            log_message = Markup(self.env._(
                "An error occurred with the PDP proxy while sending a response.<br/>Status: %(status)s - %(error)s",
        ...
        if response.get('error'):
            log_message = Markup(self.env._(
                "An error occurred with the PDP server while sending a response.<br/>Status: %(status)s - %(error)s",
                status=status_string,
            ...
            log_message = self.env._(
                "A PDP response with Response Code '%(status)s' was sent to the PDP Access Point.",
                status=status_string,
            )
    ...
    def _pdp_import_invoice(self, uuid, content, journal):
        ...
        decoded_document = self._decrypt_data(document_content, enc_key)
        filetype = 'pdf' if content['document_type'] == 'Factur-X' else 'xml'
        attachment = self.env["ir.attachment"].create(
            {
                "name": f"{filename}.{filetype}",
                "raw": decoded_document,
                "type": "binary",
                "mimetype": f"application/{filetype}",
            }
        )
