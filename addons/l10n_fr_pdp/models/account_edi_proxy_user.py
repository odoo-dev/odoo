...
class AccountEdiProxyClientUser(models.Model):
    ...
    def _pdp_import_incoming_response(self, uuid, content, origin_move):
        ...
        if not response_code_description or not issue_date or (flow_number == '6' and not ref_status_code_description):
            ...
            self._pdp_log_einvoicing_chatter(
                ...
                error_source=_("PA Lifecycle") if flow_number == '2' else _("PPF Lifecycle"),
            )
            return response
        ...
        if content['state'] == 'done':
            self._pdp_log_einvoicing_chatter(
                ...
                error_source=(
                    _("PA Lifecycle") if flow_number == '2' else _("PPF Lifecycle")
    ...
    def _pdp_proxy_error_message(self, error):
        return error.get('data', {}).get('message') or error.get('message') or error.get('subject') or _("Unknown error")
    ...
    def _pdp_format_einvoicing_message(
        ...
        items = []
        if pa_status:
            items.append(Markup('<li><strong>%s</strong> %s</li>') % (_("PA Status:"), pa_status))
        if ppf_status:
            items.append(Markup('<li><strong>%s</strong> %s</li>') % (_("PPF Status:"), ppf_status))
        if details:
            detail_items = Markup('').join(
                Markup('<li>%s</li>') % self._pdp_format_multiline_value(detail)
                for detail in details
            )
            items.append(Markup('<li><strong>%s</strong><ul>%s</ul></li>') % (_("Details:"), detail_items))
        if errors is not None:
            error_items = Markup('').join(
                Markup('<li>%s</li>') % self._pdp_format_multiline_value(error)
                for error in errors
            )
            error_label = _("Errors from %(error_source)s:", error_source=error_source) if error_source else _("Errors:")
            items.append(Markup('<li><strong>%s</strong><ul>%s</ul></li>') % (error_label, error_items))
        if not items:
            return Markup()
        return Markup('<strong>%s</strong><ul class="mb-0">%s</ul>') % (
            _("E-Invoicing Status Update"),
            Markup('').join(items),
        )
