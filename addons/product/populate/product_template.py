...
class ProductAttribute(models.Model):
    ...
    def _populate_factories(self):
        return [
            ("name", populate.constant('product_attribute_{counter}')),
...
class ProductAttributeValue(models.Model):
    ...
    def _populate_factories(self):
        ...
        return [
            ("name", populate.constant('product_attribute_value_{counter}')),
            ("sequence", populate.randomize([False] + [i for i in range(1, 101)])),
...
class ProductTemplate(models.Model):
    ...
    def _populate(self, size):
        ...
        product_variants = res.product_variant_ids
        _logger.info('Set barcode on product variants (%s)', len(product_variants))
        for product in product_variants:
            product.barcode = "BARCODE-%s" % product.id
    ...
    def _populate_factories(self):
        ...
        def get_rand_float(values, counter, random):
            return random.randrange(1500) * random.random()

        # VFE TODO variants number limit check ???
        attribute_ids = self.env.registry.populated_models["product.attribute"]

        def get_attributes(values, counter, random):
            # TODO ryv: something doesn't use dynamic and no_variant attribute to get more product with a lot of variant
            if random.random() < 0.25:  # 25 % chance to have no attributes
                return False
            ...
            values_count = 0
            no_variant = False
            for __ in range(attributes_qty):
                ...
                for __ in range(vals_qty):
                    ...
                    if not no_variant and values_count + 1 > 1000:
                        break
