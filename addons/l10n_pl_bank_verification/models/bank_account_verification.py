...
PL_PROXY_ERROR_CODES = {
    ...
    'invalid_date': _lt("Can only perform bank verification for today, please try again in a few minutes."),
...
class BankAccountVerification(models.Model):
    ...
    verification_status = fields.Selection(
        selection=[
            ...
            ('error', 'An error occurred during check with IAP'),  # IAP called -> error
        ],
        ...
        help="Flag the payment verification status with one of the following:\n"
            ...
            "- Error: An error occurred during check with IAP.\n"
    )
    # stored in UTC
    ...
    def _l10n_pl_get_verification(self, partner_bank_data, date):
        ...
        # exclude verifications in 'error' state to check them again
        ...
        for endpoint, banks in endpoints.items():
            ...
            try:
                ...
                _logger.exception(self.env._("Error while making request with endpoint %s, and date %s", endpoint, date))
                if isinstance(e, requests.exceptions.ConnectionError):
                    _logger.exception(self.env._("Cannot reach iap server"))
                    continue
                msg = response and PL_PROXY_ERROR_CODES.get(json.loads(response.content.decode())['error'])
                _logger.exception(msg or self.env._("Unknown error"))
                continue
            ...
            try:
                ...
                banks_to_create_verification_for = [bank for bank in banks if not verifications_in_error_map.get((bank.partner_id, bank))]
                create_vals += self._get_creation_vals('error', partner_banks=banks_to_create_verification_for)
                _logger.exception(self.env._("Error while decoding response for %s, with date %s", endpoint, date))
                continue
