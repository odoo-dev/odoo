...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _l10n_ec_setup_location_accounts(self, companies):
        loss_locs = dict(self.env['stock.location']._read_group(domain=[('usage', '=', 'inventory'), ('scrap_location', '=', False)], groupby=['company_id', 'id']))
        prod_locs = dict(self.env['stock.location']._read_group(domain=[('usage', '=', 'production'), ('scrap_location', '=', False)], groupby=['company_id', 'id']))
