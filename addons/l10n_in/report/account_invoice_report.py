        ...
        sub_select_str = """
            SELECT aml.id AS id,
                ...
                CASE WHEN tag_rep_ln.account_tax_report_line_id IN
                    ...
                    ELSE 0
                    END AS sgst_amount,
                (WITH account_tax_temp_table AS (SELECT account_account_tag_id FROM account_tax_report_line_tags_rel
                        WHERE account_tax_report_line_id IN (SELECT res_id FROM ir_model_data WHERE module='l10n_in' AND name='tax_report_line_cess'))
