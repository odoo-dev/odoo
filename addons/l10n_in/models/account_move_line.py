...
class AccountMoveLine(models.Model):
    ...
    def _set_l10n_in_gstr_section(self):
        ...
        def get_sales_section(line, tax_tags_dict):
            ...
            if gst_treatment in ('unregistered', 'consumer') and tags_have_categ(line_tags, 'gst') and not (is_reverse_charge_tax(line) or is_lut_tax(line)):
                if transaction_type == 'intra_state' or (
                    transaction_type == "inter_state" and (
            ...
            if is_inv:
                ...
                if gst_treatment in ('regular', 'composition', 'uin_holders') and tags_have_categ(line_tags, 'gst') and not is_lut_tax(line):
                ...
                if not is_reverse_charge_tax(line):
                    ...
                    if gst_treatment == 'special_economic_zone' and tags_have_categ(line_tags, 'export_sez'):
                        ...
                        return 'sale_sez_wp'
                    # Deemed export
                    if gst_treatment == 'deemed_export' and tags_have_categ(line_tags, 'export_sez') and not is_lut_tax(line):
