...
class PaymentTransaction(models.Model):
    ...
    def _generate_hdfc_upi_qr_code(self):
        ...
        expiry_minutes = provider.hdfc_upi_qr_expiry_time or 5
        ...
        expiry_formatted = self.hdfc_upi_qr_expiry.strftime("%Y-%m-%dT%H:%M:%S+05:30")
    ...
    def _process_notification_data(self, notification_data):
        ...
        if status.upper() == 'SUCCESS':
            ...
            self._set_canceled("HDFC UPI: " + (notification_data.get('response_message') or _("Payment failed.")))
            self.write(vals)
            ...
            _logger.warning("Received unrecognized payment status for transaction %s: %s", self.reference, status)
            self.write(vals)
