...
class L10n_InWithholdWizard(models.TransientModel):
    ...
    def _check_amounts(self):
        for wizard in self:
            ...
            if float_compare(wizard.amount, 0.0, precision_digits=precision) <= 0:
                raise ValidationError(_("Negative or zero values are not allowed in amount for withhold"))
            if float_compare(wizard.base, 0.0, precision_digits=precision) <= 0:
                raise ValidationError(_("Negative or zero values are not allowed in base for withhold"))
