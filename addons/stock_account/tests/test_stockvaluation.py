...
class TestStockValuation(TestStockValuationCommon):
    _test_user_groups = (
        ...
        'point_of_sale.group_pos_manager',  # FIXME: remove this group
    )
    ...
    def test_forecast_report_value(self):
        ...
        currency_1 = self.env['res.currency'].sudo().create({
            'name': 'UNF',
            'symbol': 'U',
            'rounding': 0.01,
        ...
        currency_2 = self.env['res.currency'].sudo().create({
            'name': 'DBL',
            'symbol': 'DD',
            'rounding': 0.01,
    ...
    def test_stock_report_avco_warehouse_dependency(self):
        ...
        warehouse_3 = self.env['stock.warehouse'].sudo().create({'code': 'WH-neg'})
    ...
    def test_stock_report_avco_lot_valuation_warehouse_dependency(self):
        ...
        warehouse_3 = self.env['stock.warehouse'].sudo().create([
    ...
    def test_avco_report_multiple_page(self):
        ...
        recs[-2:].sudo()._compute_cumulative_fields()
    ...
    def test_cron_post_stock_valuation_domain(self):
        ...
        def fake_close(records, auto_post=False, raise_error_if_closed=True):
    ...
    def test_multi_company_valuation(self):
        ...
        self.env = self.env(context=dict(self.env.context, allowed_company_ids=[self.env.company.id, self.branch.id]))
    ...
    def test_generate_entry_branch_correct_account_on_closing(self):
        ...
        """
        self.env.company.sudo().inventory_valuation = 'periodic'
        self.env.company.sudo().inventory_period = 'daily'
        self.branch.sudo().inventory_valuation = 'periodic'
        self.branch.sudo().inventory_period = 'daily'
        company_ids = [self.env.company.id, self.branch.id]
    ...
    def test_cron_generate_entry_multi_company(self):
        ...
        self.env.company.sudo().inventory_valuation = 'periodic'
        self.env.company.sudo().inventory_period = 'daily'
        self.other_company.sudo().inventory_valuation = 'periodic'
        self.other_company.sudo().inventory_period = 'daily'
        ...
        self.env.user.company_ids = [Command.link(no_move_company.id)]
        self.product_standard.with_company(self.other_company).standard_price = 30

        company_ids = [self.env.company.id, self.other_company.id, no_move_company.id]
        self.env = self.env(context=dict(self.env.context, allowed_company_ids=company_ids))
    ...
    def test_report_includes_account_with_zero_qty_products_but_non_zero_balance(self):
        ...
        account_a = self.env['account.account'].sudo().create({
            'name': 'Valuation Account A',
            'code': 'VAL.A',
            'account_type': 'asset_current',
            'reconcile': True,
        })
        account_b = self.env['account.account'].sudo().create({
