...
class TestLotValuation(TestStockValuationCommon):
    ...
    def test_avco_lot_multi_in_out(self):
        ...
        self._make_in_move(product, 10, 30, lot_ids=[lot2])
        self._make_in_move(product, 10, 20, lot_ids=[lot1])
        out1 = self._make_out_move(product, 5, lot_ids=[lot1])
        out2 = self._make_out_move(product, 5, lot_ids=[lot2])
        self._make_in_move(product, 5, 10, lot_ids=[lot2])
    ...
    def test_avco_lot_replay_multi(self):
        ...
        out = self._make_out_move(product, 5, lot_ids=[lot2])
        ...
        self.assertEqual(lot2.total_value, 2250)
        self.assertEqual(lot2.standard_price, 150)
        self.assertEqual(lot1.total_value, 100)
    ...
    def test_fifo_lot_multi_in_out(self):
        ...
        self._make_in_move(product, 10, 10, lot_ids=[lot1])
        self._make_in_move(product, 10, 20, lot_ids=[lot1])
        self._make_in_move(product, 10, 50, lot_ids=[lot2])
        out1 = self._make_out_move(product, 15, lot_ids=[lot1])
        out2 = self._make_out_move(product, 4, lot_ids=[lot2])
        ...
        self.assertEqual(lot2.total_value, 300)
    ...
    def test_fifo_lot_replay_single(self):
        ...
        out = self._make_out_move(product, 5, lot_ids=[lot1])
        move_in_2 = self._make_in_move(product, 10, 20, lot_ids=[lot1])
        ...
        self.assertEqual(lot1.product_qty, 15)
        self.assertEqual(lot1.total_value, 1100)
    ...
    def test_fifo_lot_replay_multi(self):
        ...
        self._make_in_move(product, 10, 1, lot_ids=[lot1])
        self._make_in_move(product, 10, 100, lot_ids=[lot2])
        out = self._make_out_move(product, 5, lot_ids=[lot2])
        ...
        self.assertEqual(lot2.total_value, 500)
    ...
    def test_standard_lot_replay(self):
        ...
        self.assertEqual(lot1.product_qty, 6)
        self.assertEqual(lot1.total_value, 90)
    ...
    def test_fifo_remaining_qty_single_receipt_multiple_lots(self):
        """
        A single receipt can carry several lots (one move line per lot). Its
        remaining_qty must be the sum of what's left of every lot it brought in,
        not just the last lot processed.
        Receive 10 (5 of lot1 + 5 of lot2) in one move, then deliver 2 from lot1
        and 3 from lot2: 10 - 2 - 3 = 5 must remain on the receipt.
        """
        self.product.categ_id = self.category_fifo
        in_move = self._make_in_move(self.product, 10, 5, lot_ids=[self.lot1, self.lot2])
        self._make_out_move(self.product, 2, lot_ids=[self.lot1])
        self._make_out_move(self.product, 3, lot_ids=[self.lot2])

        self.assertEqual(in_move.remaining_qty, 5)
