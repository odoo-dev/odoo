...
class TestLotValuation(TestStockValuationCommon):
    ...
    def _set_move_value(self, move):
        move.value_manual = move.price_unit * move.quantity

    def _set_move_lines_quantity(self, move, quantity=None):
    ...
    def test_real_time_valuation(self):
        ...
        self._make_in_move(self.product1, 10, 7, lot_ids=[self.lot3])
        self._make_out_move(self.product1, 2, lot_ids=[self.lot1])
        amls = self.env['account.move'].browse(self.env.company.action_close_stock_valuation()['res_id']).line_ids
        self.assertRecordValues(amls, [
            {'account_id': self.account_expense.id, 'debit': 0.0, 'credit': 110.0},  # getting closing on variation instead of expense
            {'account_id': self.account_stock_valuation.id, 'debit': 110.0, 'credit': 0.0},
        ])
    ...
    def test_enable_lot_valuation_variant(self):
        ...
        self.assertEqual(productA.total_value, 156.67)  # 156.67 because of lot valuation
        self.assertEqual(productA.qty_available, 24)
        self.assertEqual(productB.total_value, 146.67)  # 144.67 because of lot valuation
        self.assertEqual(productB.qty_available, 22)
        self.assertEqual(lotA_1.total_value, 44.0)
        self.assertEqual(lotA_1.product_qty, 7)
        self.assertEqual(lotA_2.total_value, 112.67)
        self.assertEqual(lotA_2.product_qty, 17)
        self.assertEqual(lotB_1.total_value, 36.0)
    ...
    def test_change_standard_price(self):  # works well in ui
    ...
    def test_value_multicompanies(self):
        ...
        move1.picked = True
        move1._action_done()
        self._set_move_value(move1)
        self._set_move_lines_quantity(move1, 9)
    ...
    def test_average_manual_lot_revaluation(self):
        ...
        move = self._make_in_move(self.product1, 6, 7, lot_ids=[self.lot1])
        ...
        self._set_move_lines_quantity(move, 5)
    ...
    def test_average_manual_product_revaluation_with_lots(self):
        ...
        self._set_move_lines_quantity(move1, 10)
        self._set_move_lines_quantity(move2, 5)
    ...
    def test_lot_change_lot_after_done(self):
        ...
        move = self._make_out_move(self.product1, 3, create_picking=True, lot_ids=[self.lot1], force_assign=True)
        self.assertEqual(self.product1.standard_price, 4.67)
        ...
        self.assertRecordValues(move.value, [{'value': 13.5, 'quantity': 3}])

    def test_lot_fifo_vacuum(self):
        """ Test lot fifo vacuum"""
        self.product1.standard_price = 9
        m1 = self._make_out_move(self.product1, 2, lot_ids=[self.lot1], force_assign=True)
        m2 = self._make_out_move(self.product1, 3, lot_ids=[self.lot2], force_assign=True)
        self._make_in_move(self.product1, 10, 7, lot_ids=[self.lot3])
        self._set_move_lines_quantity(m1, 2)
        self._set_move_lines_quantity(m2, 3)
    ...
    def test_lot_valuation_product1_price_diff(self):
        """
    ...
    def test_deliveries_with_minimal_access_rights(self):
        ...
        self.env['stock.quant']._update_available_quantity(product_lot, warehouse.lot_stock_id, 10.0, lot_id=self.lot1)
