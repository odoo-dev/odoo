...
class TestAccountMove(TestStockValuationCommon):
    ...
    def test_storno_accounting(self):
        ...
        product = self.product_standard_auto
        # SETUP: company accounting configuration (master-data) -> sudo
    ...
    def test_standard_manual_tax_edit(self):
        ...
        move_form = Form(self.env["account.move"].with_context(default_move_type="out_invoice"))
        move_form.partner_id = self.partner
        # SETUP: account.account tax configuration (master-data) -> sudo
        self.company.income_account_id.sudo().write({
    ...
    def test_cogs_analytic_accounting(self):
        ...
        becomes unbalanced."""
        product = self.product_standard_auto
        # SETUP: analytic master-data -> sudo
        default_plan = self.env['account.analytic.plan'].sudo().create({
            'name': 'Default',
        })
    ...
    def test_stock_picking_applies_analytic_distribution_to_journal_entry(self):
        ...
        """
        self.env.user.sudo().group_ids += self.env.ref('analytic.group_analytic_accounting')
        product = self.product_standard_auto
        cost_of_production = self.env['account.account'].sudo().create({
    ...
    def test_cogs_account_branch_company(self):
        ...
        product = self.product_standard_auto
        branch = self.branch
        test_account = self.env['account.account'].sudo().with_company(branch.id).create({
            'name': 'STCK Test Account',
            'code': '100119',
            'account_type': 'asset_current',
        })
