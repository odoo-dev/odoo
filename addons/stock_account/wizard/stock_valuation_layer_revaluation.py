...
class StockValuationLayerRevaluation(models.TransientModel):
    ...
    def default_get(self, default_fields):
        ...
        if context.get('active_model') == 'stock.lot' or res.get('lot_ids'):
            layers = self.env['stock.valuation.layer'].browse(context.get('active_ids')).exists()
            res['adjusted_layer_ids'] = layers.ids
            res['is_valuation_from_groupby_lot'] = True
            res['product_id'] = layers.product_id.id
            res['lot_ids'] = [(6, 0, layers.mapped('lot_id').ids)]

        if context.get('active_model') == 'stock.valuation.layer':
            # coming from action button "Adjust Valuation" in valuation layer list view or when lot is group_by in valuation layer list view
    ...
    is_valuation_from_groupby_lot = fields.Boolean('valuation from group by lot')
    ...
    def action_validate_revaluation(self):
        ...
        if self.adjusted_layer_ids and not self.is_valuation_from_groupby_lot:
