...
DEVICE_ACTIVITY_UPDATE_FREQUENCY = 3600  # seconds
...
class Session(collections.abc.MutableMapping):
    ...
    def update_fingerprint(self, request):
        """
        ...
        suspicious = None
        fingerprint = request.httprequest.json.get('fingerprint', '')

        if not fingerprint:
            if self['_fingerprint']:
                suspicious = True
            return fingerprint, suspicious  # suspect ? None or True

        if not self['_fingerprint']:
            self['_fingerprint'] = fingerprint
            return fingerprint, suspicious  # suspect ? None

        if consteq(fingerprint, self['_fingerprint']):
            suspicious = False
        else:
            suspicious = True

        return fingerprint, suspicious  # suspect ? True or False
    ...
    def update_trace(self, request):
        ...
        try:
            device, activity = self['_trace'][device_key]
            first_activity, last_activity = activity

            if bool(now - last_activity < DEVICE_ACTIVITY_UPDATE_FREQUENCY):
                suspicious = False
                # If it is suspicious, it has been notified during creation of the new trace
                return (device, activity), suspicious  # suspect ? False

            new_trace = (device, (first_activity, now))
        ...
        for (tuser_agent, tip_address), (_, tlast_activity) in self['_trace'].values():
        ...
        self.is_dirty = True
        return new_trace, suspicious  # suspect ? None or True
