...
def _verify_target(target: bytes):
    ...
    try:
        # use urllib3's parser to parse example.com/ as host=example.com, path=/
        url = parse_url(target.decode('ascii'))
...
class HTTPServer:
    ...
    def _log(self, level, msg, *args, extra=None, **kwargs):
        ...
        req = self.h11request
        res = self.h11response

        extra = {
            'cursor_mode': current_thread.cursor_mode or '-',
            'http_headers': (res or req).headers if req or res else (),
            'http_request_line': (
                (b'%s %s HTTP/%s' % (req.method, req.target, req.http_version)).decode()
                if req else
                '- - HTTP/?'
            ),
            'http_response_status': res.status_code if res else '-',
            'ident': current_thread.sess_id or '-',
        ...
        # http date "Thu, 30 Apr 2026 16:18:06 GMT" => log date "30/Apr/2026 16:18:06"
        http_date = (
            next(v for k, v in res.headers if k == b'date').decode()
            if res else
            format_date_time(time.time())
        )
        extra.setdefault('date', http_date[5:-4].replace(' ', '/', 2))

        if 'http_response_body' not in extra:
            if not res:
                extra['http_response_body'] = '-'
            else:
                content_length = next(
                    (v for k, v in res.headers if k == b'content-length'),
                    None,
                )
                extra['http_response_body'] = (
                    int(content_length) if content_length is not None
                    else 0 if self.h11response.status_code in _NO_BODY_STATUS
                    else 'stream'
                )

        http_log(level, msg, *args, extra=extra, **kwargs)
    ...
    def _make_environ(self):
        ...
        url = parse_url(self.h11request.target.decode('ascii'))
        ...
        environ.update({
            ...
            if b'_' not in header  # like gunicorn
        })
    ...
    def process_request(self):
        ...
        if self.conn.client_is_waiting_for_100_continue:
            ...
            self._log(logging.DEBUG, '[100] ', extra={
    ...
    def start_response(
        ...
        if current_thread.rpc_model_method:
            self.h11request = h11.Request(
                method=self.h11request.method,
                target=self.h11request.target + b'#' + current_thread.rpc_model_method.encode(),
                headers=self.h11request.headers,
                http_version=self.h11request.http_version,
                _parsed=True,
            )
