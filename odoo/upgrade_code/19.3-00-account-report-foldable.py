...
def _get_foldable_xml_value(foldable_node):
    ...
    New data uses Selection keys: 'foldable', 'always_unfolded', 'never_unfoldable'.
    ...
    if text in ('False', '0', ''):
        return 'false_bool'
    if text in ('foldable', 'always_unfolded', 'never_unfoldable'):
        return text
...
def _foldable_matches_compute(xml_value, computed_value):
    ...
    return xml_value == 'false_bool' and computed_value in ('never_unfoldable', 'always_unfolded')
    ...
    for fileno, file in enumerate(files, start=1):
        ...
        def process(root, _file=file):
            for record in root.iter('record'):
                ...
                if xml_value in ('foldable', 'always_unfolded', 'never_unfoldable'):
                    continue
