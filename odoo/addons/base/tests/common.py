    ...
    def setUpClass(cls):
        ...
        if cls._test_user_groups:
            cls._test_user = new_test_user(
                ...
                # regressions, see the # FIXME notes on some _test_user_groups). Such a
                # group is simply ignored when its module is absent.
                group_ids=[
                    group.id
                    for xmlid in cls._test_user_groups
    ...
    def _create_company(cls, company_xmlid='base.test_company_template', **create_values):
        ...
        if cls._force_new_company or template_company in cls.env.user.company_ids:
            if not cls._force_new_company:
                _logger.warning("Cannot use %s, company is already in the companies of user %s", company_xmlid, cls.env.user.name)
