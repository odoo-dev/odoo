...
class Mixin:
    ...
    def test_download_remote_certificate(self):
        ...
        invalid certificate.
        """
        if TestCliMailServerSMTPD in type(self).mro():
            self.skipTest("cannot import certificate with the cli mail server")
        ...
        for encryption in ('ssl', 'starttls'):
            with (self.subTest(encryption=encryption),
                ...
                with RecordCapturer(self.env['base.x509.wizard'], []) as capture:
                    action = self.mail_server.download_remote_certificate()
                [x509] = capture.records  # the downloaded certificate
                self.assertEqual(action['res_id'], x509.id)
                self.assertEqual(action['context'].get('mail_server_id'), self.mail_server.id)
                ...
                x509.with_context(action['context']).save()
    ...
    def test_download_remote_certificate_failure(self):
        ...
        with self.start_smtpd('none', auth_required=False):
            with self.assertRaises(UserError) as error_capture:
                self.mail_server.download_remote_certificate()
