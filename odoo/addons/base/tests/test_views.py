...
class TestViews(ViewCase):
    ...
    def test_valid_accessibility_icon_text(self):
        self.assertWarning(
            ...
            'A <span> with a warning icon must have title in its tag, parents, descendants or have text'
        )
        self.assertWarning(
            '<form><button icon="warning"/></form>',
            'A button with a warning icon must have title in its tag, parents, descendants or have text'
        )
        self.assertWarning(
