...
class IrActionsReport(models.Model):
    ...
    def _run_wkhtmltopdf(
        ...
        if request and request.db and request.session.sid:
            ...
            sid = root.session_store.generate_key()
            temp_data = {
                **request.session,
                '_trace_disable': True,
                'session_token': self.env.user._compute_session_token(sid),
            }
            temp_session = root.session_store.session_class(temp_data, sid, False)
