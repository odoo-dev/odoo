    ...
    def get_asset_paths(self, bundle, addons=None, css=False, js=False, xml=False, asset_paths=None, circular_path=None):
        ...
        def process_path(directive, target, path_def):
            ...
            if directive == APPEND_DIRECTIVE:
                add_paths(addon, paths)
            elif directive == PREPEND_DIRECTIVE:
                add_paths(addon, paths, bundle_start_index)
            elif directive == AFTER_DIRECTIVE:
                add_paths(addon, paths, target_index + 1)
                ...
                # add all paths found, then remove the target paths
                add_paths(addon, paths, target_index)
        ...
        assets = self.sudo().search(self._get_asset_domain(bundle), order='sequence')
        ...
        for addon in addons:
            ...
            for command in manifest_assets.get(bundle, []):
                ...
                if isinstance(command, str):
                    ...
                    directive, path_def = APPEND_DIRECTIVE, command
