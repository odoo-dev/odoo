    ...
    def _generate_speedscope(self, params):
        ...
        for record in self:
            if record.init_stack_trace != init_stack_trace:
                raise UserError('All profiles must have the same initial stack trace to be displayed together.')
        sp = Speedscope(init_stack_trace=json.loads(init_stack_trace))
        for profile in self:
            if params['sql_no_gap_profile'] or params['sql_density_profile'] or params['combined_profile']:
                sp.add(f'sql {profile.id}', json.loads(profile.sql), params['aggregate_sql'])
