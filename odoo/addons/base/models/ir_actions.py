    ...
    def run(self):
        ...
        for action in self.sudo():
            ...
            if action_groups:
                ...
                model_name = action.model_id.model # TOCHECK with ORM team: why needed, is it because _inherits ? also does this needs sudo() ?
                try:
    ...
    def _compute_update_related_model_id(self):
        for action in self:
            if action.evaluation_type == 'value' and action.update_field_id and action.update_field_id.relation:
                ...
                action.update_related_model_id = action.env["ir.model"].search([("model", "=", relation)]).id  # check if simplification possible
            else:
