...
from odoo.tools.binary import EMPTY_BINARY, BinaryBytes, BinaryValue, RenamedBinaryValue
    ...
    def convert_to_cache(self, value, record, validate=True) -> BinaryValue | None:
        ...
        if isinstance(value, dict):
            ...
            if 'content' not in value:
                if not file_name:
                    raise ValidationError(f"{self}: missing both 'content' and 'file_name' when writing")
                existing_val = record[self.name]
                # if not existing_val: # TODO check what todo when computing name of unexesting binary field
                #     raise ValidationError(f"{self}: cannot set 'file_name' without 'content' on an empty field")
                return RenamedBinaryValue(existing_val, new_name=file_name)
        ...
        if self.store and any(records._ids):
            ...
            if cache_value:
                if isinstance(value, dict) and 'content' not in value:
                    atts.write({'name': cache_value.file_name})
                else:
