...
class Date(BaseDate[date]):
    ...
    def today(record=None, /) -> date:
        ...
        # TODO _logger.warning("give today a record", stack_info=True)
        return date.today()
...
class Datetime(BaseDate[datetime]):
    ...
    def now(record=None, /) -> datetime:
        ...
        # TODO _logger.warning("give now a record", stack_info=True)
        return datetime.now().replace(microsecond=0)

    @staticmethod
    def today(record=None, /) -> datetime:
