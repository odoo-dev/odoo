...
from .utils import (
    OriginIds, expand_ids,
    check_pg_name, check_object_name, check_property_field_value_name,
    get_field_name_from_expression, origin_ids,
    PREFETCH_MAX,
...
if typing.TYPE_CHECKING:
    ...
    from .types import DomainType, IdType, Self, ValuesType
...
class BaseModel(metaclass=MetaModel):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    @api.model
    @api.readonly
    def search_count(self, domain: DomainType, limit=None) -> int:
    ...
    @api.readonly
    @api.returns('self')
    def search_fetch(self, domain: DomainType, field_names, offset=0, limit=None, order=None) -> Self:
    ...
    def _read_group_groupby(self, groupby_spec: str, query: Query) -> SQL:
        ...
        if field.type in ('datetime', 'date') or (field.type == 'properties' and granularity):
            ...
            if granularity in READ_GROUP_NUMBER_GRANULARITY:
                sql_expr = Date.sql_expression_for_property(self, sql_expr, granularity, field.type == 'datetime')
            elif field.type == 'datetime':
                # set the timezone only
                sql_expr = Date.sql_expression_for_property(self, sql_expr, '', is_datetime=True)
            if granularity == 'week':
