...
class MetaModel(type):
    ...
    def __new__(meta, name, bases, attrs):
        ...
        if attrs.get('_register', True):
            ...
            # when _log_access is set to False, inherit from SimpleModel instead of Model
            # TODO deprecate and use directly SimpleModel
            # XXX mode to model_classes.py
            keep_model = attrs.get('_log_access', attrs.get('_auto'))
            ...
            if not keep_model:
                from .models_simple import SimpleModel, Model  # noqa: PLC0415
                bases = tuple(SimpleModel if base is Model else base for base in bases)
