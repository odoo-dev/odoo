...
class _FlushingSavepoint(Savepoint):
    def __init__(self, cr: Cursor):
        ...
        if cr.transaction is not None:
            cr.transaction._state.push_state()
        super().__init__(cr)
    ...
    def rollback(self):
        ...
        if cr.transaction is not None:
            cr.transaction._state.rollback()
        super().rollback()
    ...
    def _close(self, rollback: bool):
        ...
        try:
            ...
            if cr.transaction is not None:
                cr.transaction._state.pop_state()
...
class Cursor(_CursorProtocol):
    ...
    def commit(self) -> None:
        ...
        self.flush()
        committing = self.transaction._state.committing() if self.transaction is not None else nullcontext()
        with committing:
            self._cnx.commit()
            self._now = None
    ...
    def rollback(self) -> None:
        ...
        self._now = None
        if self.transaction is not None:
            if self.transaction._state.parent is not None:
                _logger.warning("Pending savepoints not released!", stack_info=True)
            self.transaction._state.rollback()
