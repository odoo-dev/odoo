...
from .exceptions import AccessError, CacheMiss, UserError
from .tools import SQL, OrderedSet, Query, clean_context, frozendict, lazy_property
from .tools.misc import StackMap, SENTINEL
from .tools.translate import LazyGettext, get_translated_module, get_translation
...
DomainType = list[typing.Literal["&", "|", "!"] | tuple[str, str, typing.Any]]
...
class Params:
    def __init__(self, args: Iterable, kwargs: Mapping):
        self.args = args
        self.kwargs = kwargs
    ...
    def __str__(self) -> str:
        return ', '.join(itertools.chain(
            ...
            (f"{key}={value!r}" for key, value in sorted(self.kwargs.items())),
        ))
...
class Meta(type):
    ...
    def __new__(meta, name, bases, attrs):
        ...
        parent: Meta = type.__new__(meta, name, bases, {})
...
def _downgrade(method, value, self, args, kwargs):
...
def model(method: T) -> T:
    ...
    """
    if method.__name__ == 'create':  # type: ignore
...
def call_kw(model: BaseModel, name: str, args: Sequence, kwargs: Mapping) -> typing.Any:
    ...
    if api == "model_create":
        ...
        result = _downgrade(method, result, recs, args, kwargs)

    return result


# Cannot map to BaseModel to avoid circular dependencies
class Environment(Mapping[str, typing.Any]):
    ...
    registry: Registry
    cache: Cache
    _cache_key: dict[Field, typing.Any]
    _protected: StackMap
    ...
    def __new__(cls, cr: BaseCursor, uid: int, context: dict, su: bool = False, uid_origin: int | None = None):
        if uid == SUPERUSER_ID:
            su = True
    ...
    def __getitem__(self, model_name: typing.Any) -> BaseModel:
    ...
    @typing.overload
    def protecting(self, what: Collection[tuple[Field, BaseModel]]) -> typing.ContextManager[None]:
        ...

    @contextmanager
    def protecting(self, what, records: BaseModel | None = None) -> Iterator[None]:
        ...
        try:
            protected.pushmap()
            ids_by_field: Mapping[Field, Sequence[IdType]]
            if records is not None:  # Handle first signature
                ...
                for fields, what_records in what:
                    for field in fields:
                        ids_by_field[field].extend(what_records._ids)  # type: ignore
    ...
    def flush_query(self, query: SQL) -> None:
        ...
        fnames_to_flush: defaultdict[str, OrderedSet[str]] = defaultdict(OrderedSet)
...
class Transaction:
    ...
    def __init__(self, registry: Registry):
        ...
        # make the weakset OrderedWeakSet
        self.envs.data = OrderedSet()  # type: ignore
        # cache for all records
        self.cache = Cache()
        # fields to protect {field: ids}
        self.protected: StackMap[Field, OrderedSet[IdType]] = StackMap()
        # pending computations {field: ids}
        self.tocompute: defaultdict[Field, OrderedSet[IdType]] = defaultdict(OrderedSet)
...
class Cache:
    ...
    def __init__(self):
        ...
        self._data: defaultdict[Field, dict[IdType, typing.Any] | dict[typing.Any, dict[IdType, typing.Any]]] = defaultdict(dict)
        ...
        # in `_data`
        self._dirty: defaultdict[Field, set[IdType]] = defaultdict(OrderedSet)

        # {field: {record_id: ids}} record ids to be added to the values of
        # x2many fields if they are not in cache yet
        self._patches: defaultdict[Field, defaultdict[IdType, list[IdType]]] = defaultdict(lambda: defaultdict(list))
    ...
    def _get_field_cache(self, model: BaseModel, field: Field) -> Mapping:
    ...
    def get_records_different_from(self, records: BaseModel, field: Field, value: typing.Any) -> BaseModel:
