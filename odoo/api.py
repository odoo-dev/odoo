...
def call_kw(model, name, args, kwargs):
    ...
    if not method:
        raise AttributeError(
            f"The method '{name}' does not exist on the model '{model._name}'"
        )
...
class Environment(Mapping):
    ...
    def __new__(cls, cr, uid, context, su=False, uid_origin=None):
        ...
        for env in transaction.envs:
            if (env.cr, env.uid, env.su, env.uid_origin, env.context) == (
                cr,
                uid,
                su,
                uid_origin,
    ...
    def _(self, source: str | LazyGettext, *args, **kwargs) -> str:
        ...
        try:
            ...
            _logger.debug(
                'translation went wrong for "%r", skipped', source, exc_info=True
            )
        return source
...
class Cache:
    ...
    def contains(self, record, field):
        ...
        if field.translate:
            ...
            if cache_value is None:
                return True
            lang = (
                (record.env.lang or "en_US")
                if field.translate is True
                else record.env._lang
            )
    ...
    def update_raw(self, records, field, values, dirty=False, check_dirty=True):
        ...
        if dirty:
            ...
            if (
                not field.company_dependent
                and field in records.pool.field_depends_context
            ):
                # put the values under conventional context key values {'context_key': None},
                # in order to ease the retrieval of those values to flush them
    ...
    def insert_missing(self, records, field, values):
        ...
        if field.translate:
            if env.context.get("prefetch_langs"):
                ...
                _langs = (
                    [f"_{l}" for l in langs]
                    if field.translate is not True and env._lang.startswith("_")
                    else []
                )
