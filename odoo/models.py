...
class BaseModel(metaclass=MetaModel):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    _sql_objects: dict[str, DatabaseObject] = frozendict()  #: SQL constraints and indexes
    ...
    def _build_model(cls, pool, cr):
        ...
        if getattr(cls, '_sql_constraints', None):
        ...
        if constraint_name := exc.diag.constraint_name:
            for cons in self._sql_objects.values():
                if constraint_name == cons.full_name(self):
                    cons_rec = self.env['ir.model.constraint'].search([
    ...
    def _setup_base(self):
        ...
        sql_objects.update(cls._local_sql_objects)
        cls._sql_objects = frozendict(sql_objects)
...
class DatabaseObject:
    ...
    def get_error_message(self, model: BaseModel, diagnostics=None) -> str:
        ...
        if callable(message):
            ...
            # do not add diagnostics.message_detail because it could contain too much information
        return message
