...
class BaseModel(metaclass=MetaModel):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def action_archive(self):
        ...
        active = self.filtered(lambda record: record[field_name])
        active[field_name] = False
    ...
    def action_unarchive(self):
        ...
        field_name = self._active_name
        assert field_name, f"No 'active' field on model {self._name}"
        inactive = self.filtered(lambda record: not record[field_name])
        inactive[field_name] = True
