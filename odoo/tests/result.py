...
class OdooTestResult(object):
    ...
    def __repr__(self):
        return ("<%s.%s run=%i errors=%i failures=%i>" %
                (self.__class__.__module__, self.__class__.__qualname__, self.testsRun, len(self.errors), len(self.failures)))

    def __str__(self):
        return f'{self.failures} failed, {self.errors} error(s) of {self.testsRun} tests'
