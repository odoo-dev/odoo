...
- [🦉OWL Release Notes](#owl-release-notes)
  - [Overview](#overview)
  ...
  - [Other ideas](#other-ideas)
    - [Make it easy to output a simple reactive value](#make-it-easy-to-output-a-simple-reactive-value)
    - [Simplified way to define a prop](#simplified-way-to-define-a-prop)
  - [Examples](#examples)
...
considerably, as each change woul bring bugs, especially since it is very difficult
...
some "soft limits": 
- the reactivity simple, albeit simple in surface, is actually quite subtle, and
...
- no good support for typing/autocompletion in props/env. So lots of work has been done to write
  or generate .d.ts files, docstring, and tooling. 
- difficulty to write performant code when we have to make computation based on
...
For example, if we push an element in a list. In that case, we need to explicitely
...
triple();       // returns 9
tiple.set(6);   // signal s is set to 2!
s();            // returns 2
triple();       // returns 6
```
...
is executed immediately, and whenever the value it depends on change. It also returns a callback function to clean the effect (stop it). 
...
- a function `providePlugins` that allow a component to define a set of plugins
...
unless we want to explicitely use it.
...
We have to sometimes use explicitely `this` to avoid issues. For example:
...
Instead of now defining a type
with a mini dsl using objects, we now uses functions, that makes it easier to
compose and manipulate types, and allow IDEs to know the correct type, so
autocompletion will work much better. Here is what it looks like:
...
framework.  One of the main issue is that it is somewhat imperative (we have to
explicitely tell the component what to do in order to maintain a coherent state),
...
want a registry of registries, we have to define each subregistry explicitely.
...
*external* was intended to indicate listening to events outside the component's
...
listener in `mounted` and removing it when `unmounted`, it now adds immediately
the event listener, and remove it when `destroyed`. The reason for that change
is that it is usually an error to wait for `mounted` before starting listening
to change. Also, it makes the hook useful for plugins, since plugins have a
...
In Owl 2.X, an owl `App` has a main root component. We had to introduce the concept of *sub roots* to allow different roots to
...
class ContentPlugin extends Plugin {
    el = config( { editable: t.signal()}).el;
}
