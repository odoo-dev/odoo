...
| 3  | `useEffect` semantics changed | 596 calls | [Note](#useeffect-semantic-change) |
| 4  | `this.props` removed | | [Note](#thisprops-removed) |
| 5  | static `props` / `defaultprops` ignored (use the props function) | 281 default props | [Note](#static-props--defaultprops-ignored) |
| 6  | `this.env` removed | 161 `useSubEnv` | [Note](#thisenv-removed) |
| 7  | rendering context changes (reading from component through `this`) | | [Note](#rendering-context-changes) |
| 8  | `onWillUpdateProps` removed | 183 calls | [Note](#onwillupdateprops-removed) |
| 9  | `t-esc` removed | | [Note](#t-esc-removed) |
|10  | `t-ref` changes: takes a signal (or resource) | 1022 calls | [Note](#t-ref-changes-takes-a-signal-or-resource) |
|11  | `t-model` changes: takes a signal | 197 calls | [Note](#t-model-changes-takes-a-signal) |
|12  | `onWillRender` removed | 70 calls | [Note](#onwillrender-removed) |
|13  | `onRendered` removed | 20 calls | [Note](#onrendered-removed) |
|14  | `this.render` removed | 130 calls | [Note](#thisrender-removed) |
|15  | `t-portal` removed | 18 calls | [Note](#t-portal-removed) |
|16  | `useExternalListener` renamed to `useListener` (and changed) | 210 calls | [Note](#useexternallistener-renamed-to-uselistener-and-changed) |
|17  | `App` has only sub roots | 20 new `App` calls | [Note](#app-has-only-sub-roots) |
|18  | `loadFile` removed | | [Note](#loadfile-removed) |
|19  | `t-call` not allowed on tags `!== t` | | [Note](#t-call-not-allowed-on-tags-!==-t) |
|20  | `t-call` body evaluated lazily, variables passed as parameters | | [Note](#t-call-body-evaluated-lazily-variables-passed-as-parameters) |
|21  | `useComponent` removed | 93 calls | [Note](#usecomponent-removed) |
|22  | `t-slot` renamed to `t-call-slot` | 224 calls | [Note](#t-slot-renamed-to-t-call-slot) |
|23  | `validateType` changed | 1 call | [Note](#validatetype-changed) |
|24  | `validate` removed | 4 calls | [Note](#validate-removed) |
...
### static `props` / `defaultprops` ignored
...
### `t-ref` changes: takes a signal (or resource)
...
### `t-model` changes: takes a signal
...
for this breaking change is to simply uses signals/reactive values.
...
### useExternalListener renamed to useListener (and changed)

todo 

### App has only sub roots
...
Validation functions changed in Owl 3.x, there are now two functions `validateType` and `assertType`.

- `validateType(value, type)`: check that the value matches the type and returns a list of issues if any
- `assertType(value, type)`: check that the value matches the type and throws an error if there is an issue
...
- add a compabitility layer in `addons/web/static/lib/owl/odoo_module.js`
...
| `onRendered` removed | remove some uses of onWillRender | remove all uses of onRendered manually | |
...
| `t-slot` renamed to `t-call-slot` | | rename all occurences |
| `validateType` changed | | adapt with owl3.validateType or owl3.assertType if throws an error |
| `validate` removed | implement `owl.validate` in owl2/utils | adapt with owl3.assertType |
